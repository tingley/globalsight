...
public class VendorPOReport
{
    ...
    public void generateReport()
    {
        ...
        selenium.waitForPopUp(VendorPOReportWebForm.POPUP_WINDOW_NAME,
                CommonFuncs.SHORT_WAIT);
        selenium.selectWindow("name="
                + VendorPOReportWebForm.POPUP_WINDOW_NAME);
    ...
    private void initOptions()
    {
        ...
        selenium.removeSelection(VendorPOReportWebForm.PROJECTS_SELECTOR,
                "label=<ALL>");
        selenium.removeSelection(VendorPOReportWebForm.JOBSTATUS_SELECTOR,
                "label=<ALL>");
        selenium.removeSelection(VendorPOReportWebForm.TARGETLOCALE_SELECTOR,
                "label=<ALL>");

        // Projects
        for (int i = 0; i < projects.length; i++)
        {
            selenium.addSelection(VendorPOReportWebForm.PROJECTS_SELECTOR,
                    "label=" + projects[i]);
        }
        // Job Status
        for (int i = 0; i < jobStatus.length; i++)
        {
            selenium.addSelection(VendorPOReportWebForm.JOBSTATUS_SELECTOR,
                    "label=" + jobStatus[i]);
        }
        // Target Locales
        for (int i = 0; i < targetLocales.length; i++)
        {
            selenium.addSelection(VendorPOReportWebForm.TARGETLOCALE_SELECTOR,
                    "label=" + targetLocales[i]);
        }
        // Date Range
        selenium.type(VendorPOReportWebForm.STARTSTIME, startTime);
        selenium.select(VendorPOReportWebForm.STARTSTIMEUNITS, "label="
                + startTimeUnits);
        selenium.type(VendorPOReportWebForm.ENDSTIME, endsTime);
        selenium.select(VendorPOReportWebForm.ENDSTIMEUNITS, "label="
                + endsTimeUnits);
        // Currency
