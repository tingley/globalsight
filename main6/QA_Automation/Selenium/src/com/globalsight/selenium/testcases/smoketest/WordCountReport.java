...
import com.globalsight.selenium.pages.WordCountReportWebForm;
...
public class WordCountReport
{
    ...
    public void generateReport()
    {
        ...
        selenium.click(WordCountReportWebForm.REPORT_LINK);

        selenium.waitForPopUp(WordCountReportWebForm.POPUP_WINDOW_NAME,
                CommonFuncs.SHORT_WAIT);
        selenium.selectWindow("name="
                + WordCountReportWebForm.POPUP_WINDOW_NAME);

        initOptions();

        selenium.click(WordCountReportWebForm.SUBMIT_BUTTON);
        ...
        File file = fileRead.getFile(WordCountReportWebForm.REPORT_FILE_NAME);
    ...
    private void initOptions()
    {
        ...
        selenium.removeSelection(WordCountReportWebForm.JOBSTATUS_SELECTOR,
                "label=<ALL>");
        selenium.removeSelection(WordCountReportWebForm.TARGETLOCALE_SELECTOR,
                "label=<ALL>");

        // Job Status
        for (int i = 0; i < jobStatus.length; i++)
        {
            selenium.addSelection(WordCountReportWebForm.JOBSTATUS_SELECTOR,
                    "label=" + jobStatus[i]);
        }
        // Target Locales
        for (int i = 0; i < targetLocales.length; i++)
        {
            selenium.addSelection(WordCountReportWebForm.TARGETLOCALE_SELECTOR,
                    "label=" + targetLocales[i]);
        }
        // Date Range
        selenium.type(WordCountReportWebForm.STARTSTIME, startTime);
        selenium.select(WordCountReportWebForm.STARTSTIMEUNITS, "label="
                + startTimeUnits);
        selenium.type(WordCountReportWebForm.ENDSTIME, endsTime);
        selenium.select(WordCountReportWebForm.ENDSTIMEUNITS, "label="
                + endsTimeUnits);
        // Time Format
        selenium.select(WordCountReportWebForm.DATEFORMAT, "label="
                + displayFormat);
    }
}
