...
public class JavaScriptFilter extends BaseTestCase
{
    ...
    String wJavaScriptName = ConfigUtil.getDataInCase(getClassName(),
            "FilterName");
    String sJSFuncText = ConfigUtil.getDataInCase(getClassName(),
            "JSFunctionText");
    String[] UnicodeEsp = ConfigUtil.getDataInCase(getClassName(),
            "enableUnicodeEsp").split(",");
    ...
    public void initJSFilter() throws Exception
    {
        ...
        if (selenium.isElementPresent("link=" + wJavaScriptName))
        {
            Reporter.log("The javaproperties filter " + wJavaScriptName
                    + " has already exists!");
        }
        else
        {
            ...
            for (int i = 0; i < 2; i++)
            {
                ...
                if (bUniCodeEscape)
                {
                    ...
                    selenium.uncheck(FilterConfiguration.JAVASCRIPT_FILTER_UNICODE_ESCAPE_CHECKBOX);

                selenium.click(FilterConfiguration.JAVASCRIPT_FILTER_SAVE_BUTTON);

                Assert.assertEquals(
                        selenium.isElementPresent("link=" + wJavaScriptName
                                + id), true);
                ...
                tmp.createJob(filterJob + i, dir, fp[3], targetLocales);
                Thread.sleep(20000);
                ...
                // Open the MainEditor
                selenium.click(MyActivities.TARGET_FILES_TABLE
                        + "/tr/td[2]/div/a");
                ...
                System.out
                        .println("The job is exported. Please check the unicode escape manually");
