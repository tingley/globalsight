...
namespace PowerPointConverter
{
	public class Service1 : System.ServiceProcess.ServiceBase
	{
		...
		private void InitializeComponent()
		{
			...
			// Service1
			// 
			this.ServiceName = "GlobalSight Converter - PowerPoint";
		...
		protected override void OnStart(string[] args)
		{
			...
			try 
			{
				m_watchDirName = RegistryUtil.GetGlobalSightRegistryValue("MsOfficeConvDir") + @"\powerpoint";
				DirectoryInfo watchDir = new DirectoryInfo(m_watchDirName);
				watchDir.Create();
				Logger.Initialize(m_watchDirName + @"\powerpointConverter.log");
				m_log = Logger.GetLogger();
				m_log.Log("GlobalSight PowerPoint Converter starting up.");
				m_log.Log("Creating and starting threads to watch directory " + m_watchDirName);
				
				m_importConverter = new PowerPointConverterImpl(PowerPointConverterImpl.ConversionType.IMPORT);
				m_exportConverter = new PowerPointConverterImpl(PowerPointConverterImpl.ConversionType.EXPORT);
			...
			{
				string msg = "PowerPointConverter failed to initialize because of: " + e.Message + "\r\n" + e.StackTrace;
		...
		protected override void OnStop()
		{
			...
			m_log.Log("PowerPointConverter shutting down.");
