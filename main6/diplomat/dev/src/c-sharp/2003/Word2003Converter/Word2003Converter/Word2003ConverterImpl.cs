...
// NOTE: You must load the Microsoft Office 11 Object Library
// and the Microsoft Word 11 Object Library (Project->Add Reference,COM)
// Primary Interop Assemblies for Office 2003 must be installed
// from the Office 2003 installation CD through the setup program.
// 
...
namespace GlobalSight.Word2003Converter
{
	...
	/// HTML->RTF
	/// </summary>
	public class Word2003ConverterImpl : GlobalSight.Common.Converter
	{
		...
		/// <summary>
		/// Creates a Word2003ConverterImpl to be used for import or export.
		/// </summary>
		/// <param name="p_conversionType"> the type of conversion 
		/// (import or export)</param>
		public Word2003ConverterImpl(ConversionType p_conversionType)
		{
		...
		public void Convert(string p_fileName, string p_language)
		{
			...
			{
				Logger.LogError("Word 2003 Conversion Failed", e);
		...
		private void SaveDocument()
		{
			...
			object bidimarks = false; // TODO: find out what this does.
		...
		private void CreateWordAppClass()
		{
			...
			m_wordApp.DefaultWebOptions().BrowserLevel =
				Word.WdBrowserLevel.wdBrowserLevelMicrosoftInternetExplorer5;
		...
		private void OpenWordDoc()
		{
			...
			m_wordDoc.Activate();
			m_wordDoc.WebOptions.Encoding = Microsoft.Office.Core.MsoEncoding.msoEncodingUTF8;
		...
		private void QuitWord()
		{
			if (m_wordApp != null)
			{
				...
				try
				{
					m_wordApp.Application.Quit(ref saveChanges, ref origFormat,
						ref routeDoc);
				}
				catch (Exception e)
				{
