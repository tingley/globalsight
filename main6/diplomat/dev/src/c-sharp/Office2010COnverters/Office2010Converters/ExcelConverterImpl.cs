...
using Excel_Class = Microsoft.Office.Interop.Excel;
using GlobalSight.Common;

namespace GlobalSight.Office2010Converters
{
	/// <summary>
	/// ExcelConverterImpl is responsible for converting a MS Excel file to HTML and
	/// vice versa.
	/// </summary>
	public class ExcelConverterImpl : GlobalSight.Common.Converter
	{
		private Logger m_log = null;
		private Excel_Class.Application m_excelApp = null;
		private Excel_Class.Workbook m_workbook = null;
		...
		public void Convert(string p_fileName, string p_language)
		{
			...
			{
				Logger.LogError(m_log, "Excel Conversion Failed", e);
		...
		private void CreateExcelAppClass()
		{
			if (m_excelApp == null)
			{
				...
				m_excelApp = new Excel_Class.Application();
			}

			m_excelApp.Visible = false;
			m_excelApp.DisplayAlerts = false;
			m_excelApp.Interactive = false;
			m_excelApp.DefaultWebOptions.Encoding = Office_Core.MsoEncoding.msoEncodingUTF8;
			...
			string p_fileName)
		{
			...
			{
				m_newFormatType = Excel_Class.XlFileFormat.xlHtml;
				m_newFileName = baseFileName + HTML;
			}
			else if (p_convertTo.Equals(XLS))
			{
                m_newFormatType = Excel_Class.XlFileFormat.xlExcel8;
				m_newFileName = baseFileName + XLS;
			}
            else if (p_convertTo.Equals(XLSX))
            {
