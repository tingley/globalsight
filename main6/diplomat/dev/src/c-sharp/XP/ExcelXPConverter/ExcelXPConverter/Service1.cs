...
namespace ExcelXPConverter
{
	public class Service1 : System.ServiceProcess.ServiceBase
	{
		...
		private ExcelXPConverterImpl m_importConverter = null;
		private ExcelXPConverterImpl m_exportConverter = null;
		...
		private void InitializeComponent()
		{
			...
			// Service1
			// 
			this.ServiceName = "GlobalSight Converter - Excel XP";
		...
		protected override void OnStart(string[] args)
		{
			...
			try 
			{
				m_watchDirName = RegistryUtil.GetGlobalSightRegistryValue(
					"MsOfficeConvDir") + @"\excel";
				DirectoryInfo watchDir = new DirectoryInfo(m_watchDirName);
				watchDir.Create();
				Logger.Initialize(m_watchDirName + @"\excelxpConverter.log");
				m_log = Logger.GetLogger();
				m_log.Log("GlobalSight Excel XP Converter starting up.");
				m_log.Log("Creating and starting threads to watch directory " + 
					m_watchDirName);
				
				m_importConverter = new ExcelXPConverterImpl(
					ExcelXPConverterImpl.ConversionType.IMPORT);
				m_importConverterRunner = new ConverterRunner(
					m_importConverter, m_watchDirName);
				m_exportConverter = new ExcelXPConverterImpl(
					ExcelXPConverterImpl.ConversionType.EXPORT);
			...
			{
				string msg = "Excel XP Converter failed to initialize because of: " + 
					e.Message + "\r\n" + e.StackTrace;
		...
		protected override void OnStop()
		{
			...
			m_log.Log("Excel XP Converter shutting down.");
