...
/**
 * The CapAdapter provides functionality for receiving messages from CAP and
 * processing them, as well as preparing messages for sending to CAP.
 */
public class CapAdapter extends BaseAdapter
{
     ...
     * involves receiving messages from CAP for export, secondary target file
     * creation, etc. And it involves sending messages to CAP for import.
     *
     * This adapter handles events like GXML_CREATED_EVENT,
     * GXML_LOCALIZED_EVENT,etc.
     *
     * @param p_cxeMessage
     ...
     * @exception GeneralException
     */
    public AdapterResult[] handleMessage(CxeMessage p_cxeMessage) throws GeneralException
    {
        switch (p_cxeMessage.getMessageType().getValue())
        {
            ...
            default:
                Exception e = new Exception(
                        "Unhandled message type: " + p_cxeMessage.getMessageType().getName());
    ...
    public AdapterResult[] handleMessage(AdapterResult ars) throws Exception
    {
        ...
        for (CxeMessage msg : msgs)
        {
            ...
            Map<String, Object> data = capImporter.getContent();
            dataList.add(data);
        }
        ...
        {
            // GBS-4400
            RequestHandlerActivatorUtil.handleRequestWithThread(dataList);
        }
    ...
    /**
     * Prepares the content for sending to CAP, and then sends it. This involves
     * creating the l10nxml as well. This method will return null as there is no
     ...
     * @exception GeneralException
     */
    private AdapterResult[] performImport(CxeMessage p_cxeMessage) throws GeneralException
    {
        try
        {
            int requestType = Request.EXTRACTED_LOCALIZATION_REQUEST;
    ...
    /**
     * Handles the export case. If the content is extracted, then the next event
     * is the "pre-merge" event so the content should get sent to the merger. If
     * the content is unextracted, then the next event will be the "post-merge"
     * event to send the content directly to the target adapter
     *
     * NOTE: This is the only case that may return an export error CxeMessage
     * object because it was not in the act of sending something to CAP. The
     * other methods send something to CAP, so returning an error message that
     * could not be sent to CAP wouldn't help.
     * 
     * @param p_cxeMessage
     *            CxeMessage of type XXX_LOCALIZED_EVENT
     * @param p_eventType
     *            Mapper.PRE_MERGE_EVENT or Mapper.POST_MERGE_EVENT
     * @return AdapterResult[]
     * @exception GeneralException
     */
    ...
    /**
     * Handles notifying CAP of export status (errors and successes) This also
     * handles the case of Secondary Target File Creation
     * 
     * @param p_cxeMessage
     *            CxeMessage of type CXE_EXPORT_STATUS_EVENT
     * @return AdapterResult[] -- (null or contains results if for dynamic
     *         preview)
     * @exception GeneralException
     */
     ...
     * Also notifies of successful export completion.
     * 
     * @param p_cxeMessage
