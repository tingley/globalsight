...
public class ExportDatabaseOperationHelper
{
    ...
    */
    ...
    /*Sets the EventFlowXml used by the helper
    * Also parses the XML to get the root element
    * @throws DatabaseAdapterException
    */
    ...
    /*Sets the PrsXml used by the helper
    *Also parses the XML to get the root element
    * @throws DatabaseAdapterException
    */
	...
	try {
	    ...
	    m_prsXmlElement = XmlUtil.parseForRootElement(m_prsXml);
            ...
            //write back to the DB, we'll have GLOBALSIGHT_UNKOWN_SEGMENT in the data
            if (getDatabaseMode().equals("preview"))
            {
             setFilledRecord();

             //now go through the PrsXmlElement and remove records that do not have the
             //same sequence number
             String sequenceNumber = m_filledRecordElement.getAttribute("sequenceNumber");
             String s = null;
	...
	try {
		...
		{
		String quotedId = Utility.quote(m_prsXmlElement.getAttribute("id")); //get the ID of the PrsXml file
		theLogger.println(Logger.DEBUG_C,"Deleting PrsXml id " + quotedId + " from the DB.");
	...
	{
	    theLogger.printStackTrace(Logger.ERROR,"Unable to get database connection: " , cpe);
	    throw new DatabaseAdapterException ("DbConnectionEx",m_errorArgs,cpe);}
	catch (SQLException sqle)
	{
	    theLogger.printStackTrace(Logger.ERROR,"Unable to delete PrsXml from DB: " , sqle);
	    rollback(connection);
	    throw new DatabaseAdapterException ("SqlExceptionEx",m_errorArgs,sqle);
	}
	catch (Exception ioe)
	{
	    theLogger.printStackTrace(Logger.ERROR,"Unable to parse PrsXml for PrsXml id: " , ioe);
	    throw new DatabaseAdapterException ("CxeInternalEx",m_errorArgs,ioe);
	...
	try {
	    ...
	    theLogger.println(Logger.INFO,"Now writing content back to the database.");
	...
	catch (DatabaseWriteException db_we)
	{
		...
		theLogger.println(Logger.ERROR, niceMsg);
            ...
            theLogger.printStackTrace(Logger.ERROR, "Problem writing to the customer db.", db_we);
	    throw new DatabaseAdapterException("DbExceptionEx",m_errorArgs,db_we);
	}
    }


    /* This function extracts the PreviewUrlXml from the database based on the
     * record profile ID. It gets the original and target PrsXml snippets and then uses
     * the content in them to do the argument substitution in the PreviewUrlXml.
     */
     public String findPreviewUrlXml()
     throws DatabaseAdapterException
	 {
	 ...
	 String subbedPreviewUrlXml = ""; //the complete after-substitution preview URL XML
	 try {
	     //first get the record profile Id from the PrsXml snippet
             String recordProfileId = m_filledRecordElement.getAttribute("recordProfileId");

	     //now get the previewUrlXml out of the DB using the record profile id
	     connection = ConnectionPool.getConnection();
	     String sql = new String ("SELECT XML_URL FROM URL_LIST UL, CUSTOMER_DB_ACCESS_PROFILE CP" +
			      " WHERE CP.ID=?" +
	    ...
	    if (previewUrlXml != null)
	    {
		  ...
		  previewUrlXml, originalPrsXmlSnippet, m_filledRecord);
	       ...
	       Logger.writeDebugFile("subbedPreviewUrl.xml", subbedPreviewUrlXml);
	    }
	    else
	    {
		...
		theLogger.println(Logger.WARNING,"No preview url xml in database to preview.");
	 }
	catch (ConnectionPoolException cnfe)
	{
	    theLogger.printStackTrace(Logger.ERROR, "Could connect to GlobalSight database", cnfe);
	    throw new DatabaseAdapterException("DbConnectionEx",m_errorArgs,cnfe);
	}
	catch (SQLException sqle)
	{
	    theLogger.printStackTrace(Logger.ERROR,"SQL error while processing Preview URL XML.",
				      sqle);
	    throw new DatabaseAdapterException("SqlExceptionEx",m_errorArgs,sqle);
	}
	catch (PreviewUrlXmlException puxe)
	{
	    theLogger.printStackTrace(Logger.ERROR,"Parsing error while processing Preview Url Xml",puxe);
        ...
        {
            theLogger.printStackTrace(Logger.ERROR,"Error while preparing prs xml snippets for write back",e);
	...
	try {
	    ...
	    Element charsetElement = (Element)targetElement.getElementsByTagName("charset").item(0);
	    targetEncoding = charsetElement.getFirstChild().getNodeValue();
	    theLogger.println(Logger.DEBUG_C,"target charset = " + targetEncoding);
	}
	catch (Exception e) {
	    theLogger.printStackTrace (Logger.WARNING,
				       "Extracting target charset from eventFlowXml. Using default of UTF-8", e);
	    throw new DatabaseAdapterException("CxeInternalEx",m_errorArgs,e);
	}
   ...
   throws DatabaseAdapterException
   {
       ...
       {
	   theLogger.printStackTrace(Logger.ERROR,"Converting content to target charset",te);
   ...
   private String getDatabaseMode()
   throws DatabaseAdapterException
   {
	   ...
	   try
	       ...
	       theLogger.println(Logger.DEBUG_C,"Read databaseMode: " + m_databaseMode);
	   }
	   catch (Exception e)
	   {
	       theLogger.printStackTrace(Logger.ERROR,"Could not extract target's databaseMode from EventFlowXml: " , e);
	       throw new DatabaseAdapterException ("CxeInternalEx",m_errorArgs,e);
	   }
       }
   ...
   //retrieves the original PrsXmlSnippet from the database that corresponds to the snippet
   //received by the DB Target Adapter for preview export
   private String findOriginalPrsXmlSnippet()
   throws DatabaseAdapterException
       {
       ...
       try {
	   //if anything goes wrong at all just return the empty string -- no preview 
	   Element elem = m_prsXmlElement;
	   String id = elem.getAttribute("id"); //get the ID of the PrsXml file

	   String sequenceNumber = m_filledRecordElement.getAttribute("sequenceNumber"); //get the row id of this row
	   String recordProfileId = m_filledRecordElement.getAttribute("recordProfileId");	//get the row id of this row

	   //now find the original PrsXml file with the same ID
	   String originalPrsXml = null;
	   theLogger.println(Logger.DEBUG_C,"Now getting the original PrsXml for id " + id);
	   String quotedId = Utility.quote(id);

	   String sql = new String ("SELECT PRSXML FROM PRSXML_STORAGE WHERE NAME like ?");
	   connection = ConnectionPool.getConnection();
	   PreparedStatement query = connection.prepareStatement(sql);
	   ...
	   if (originalPrsXml == null)
	       {
	       theLogger.println(Logger.ERROR,"No PrsXml in DB for id: " + quotedId);
	       throw new DatabaseAdapterException("CxeInternalEx",m_errorArgs,null);
	   }

	   query.close();

	   //now get the snippet from the original
	   theLogger.println(Logger.DEBUG_C,"Now getting the original PrsXml row snippet");
	   ...
	   for (int i=0; i < nl.getLength(); i++)
	   {
		 ...
		 originalSnippet = new String ("<?xml version=\"1.0\"?>\n<paginatedResultSetXml>\n" +
       ...
       {
	   theLogger.printStackTrace(Logger.ERROR,"SQL error while processing PrsXml snippet.",
				     sqle);
	   throw new DatabaseAdapterException("SqlExceptionEx",m_errorArgs,sqle);
       }
       catch (SAXException saxe)
       {
	   theLogger.printStackTrace(Logger.ERROR,"Parsing error while processing PrsXml snippet.",saxe);
	   throw new DatabaseAdapterException("CxeInternalEx",m_errorArgs,saxe);
       }
       catch (IOException ioe)
       {
	   theLogger.printStackTrace(Logger.ERROR,"Parsing error while processing PrsXml snippet.",ioe);
	   throw new DatabaseAdapterException("CxeInternalEx",m_errorArgs,ioe);
       }
       finally {
	   returnConnection(connection);
   ...
   private void rollback (Connection p_connection)
   {
	   ...
	   {
	       theLogger.printStackTrace(Logger.ERROR,"Could not rollback: " , e);
	   }
       }
   }
   ...
   private void returnConnection (Connection p_connection)
   {
       if (p_connection != null)
	   {
	   try {ConnectionPool.returnConnection(p_connection);}
	   catch (Exception e)
	   {}
       }
   }
   ...
   //to get just whichever element was filled. The others have a constant in them
   ...
   private void setFilledRecord() throws Exception
   {
	   ...
	   StringWriter  stringWriter  = new StringWriter(bufsiz);
           ...
           if (s.indexOf(ExportConstants.DEFAULT_SEGMENT) == -1)
           {
               ...
               m_filledRecord = s;
               theLogger.println(Logger.DEBUG_D, "Found filled record: " + m_filledRecord);
           }
       }
       
       if (!foundRecord)
       {
           theLogger.println(Logger.ERROR, "Could not find filled record for substitution.");
       }
   }

   private byte[] readBytesFromMessageData(MessageData p_messageData) throws IOException
   {
