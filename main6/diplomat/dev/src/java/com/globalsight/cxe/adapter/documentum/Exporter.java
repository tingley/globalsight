...
public class Exporter
{
    ...
    CxeMessage export()
    {
        ...
        try
        {
            Logger.writeDebugFile("dctmta_ef.xml", m_cxeMessage.getEventFlowXml());
            parseEventFlowXml();
            m_logger.info("Updating object " + m_oid + " for target locale " +
                          m_targetLocale);
            documentumExport();
        ...
        catch (Exception e)
        {
            ...
            DocumentumAdapterException fsae = new DocumentumAdapterException("CxeInternalEx",errorArgs,e);
            exportStatusMsg = makeExportErrorMessage(fsae);
        }
    ...
    private void parseEventFlowXml() throws Exception
    {
        ...
        Element msCategory = m_parser.getCategory(DocumentumOperator.DCTM_CATEGORY);
    ...
    /** Does the actual write back to DCTM.
    ** It creates the target file if it does not exist,
    ** otherwise it checks it out and overwrites it.
    */
    private void documentumExport() throws Exception
    {
        ...
        String newObjId = (String)m_cxeMessage.getParameters().get(DocumentumOperator.DCTM_NEWOBJECTID);
        String workflowId = (String)m_cxeMessage.getParameters().get(DocumentumOperator.DCTM_WORKFLOWID);
        Boolean isJobDone = (Boolean)m_cxeMessage.getParameters().get(DocumentumOperator.DCTM_ISJOBDONE);
        Element msCategory = m_parser.getCategory(DocumentumOperator.DCTM_CATEGORY);
        Boolean isAttrFile = Boolean.valueOf(m_parser.getCategoryDaValue(msCategory,
                DocumentumOperator.DCTM_ISATTRFILE)[0]);
        String userId = m_parser.getCategoryDaValue(msCategory, DocumentumOperator.DCTM_USERID)[0];
        ...
        m_logger.debug("oldObjectId, newObjectId, workflowId, isJobDone, isAttrFile : " + 
                m_oid + "," + newObjId + "," + workflowId + "," + isJobDone + "," + isAttrFile);
        DocumentumOperator.getInstance().writeBackToDCTM(userId,newObjId, destFilePath, m_targetLocale, 
                isAttrFile.booleanValue(), m_oid, workflowId, isJobDone.booleanValue());
