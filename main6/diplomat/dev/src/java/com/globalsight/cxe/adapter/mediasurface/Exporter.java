...
public class Exporter
{
    ...
    private Exporter()
    {
        ...
        m_username = "msadmin";
        m_password = "password";
        m_contentServer = new ContentServer("iiop://tempest/Mediasurface","Staging Server",8080);
    }
    ...
    CxeMessage export()
    {
        ...
        catch (Exception e)
        {
            ...
            MediasurfaceAdapterException fsae = new MediasurfaceAdapterException("CxeInternalEx",errorArgs,e);
            exportStatusMsg = makeExportErrorMessage(fsae);
        }
    ...
    /**
     * Prepares the export success status message for sending to the next adapter
     * 
    ...
    private CxeMessage makeExportSuccessMessage() throws Exception
    {
        ...
        {
            newParams.put("AbsoluteExportPath", absoluteExportPath ); // copy to new params
        }
     ...
     * @return CxeMessage
     */
    private CxeMessage makeExportErrorMessage(MediasurfaceAdapterException p_fsae)
    {
        CxeMessageType type = CxeMessageType.getCxeMessageType(CxeMessageType.CXE_EXPORT_STATUS_EVENT);
        CxeMessage newCxeMessage = new CxeMessage(type);
        newCxeMessage.setEventFlowXml(m_cxeMessage.getEventFlowXml());
    ...
    private void parseEventFlowXml() throws Exception
    {
        ...
        String values[] = m_parser.getCategoryDaValue(msCategory,"OriginalItemKey");
        m_itemKey = Integer.parseInt(values[0]);

        String url = m_parser.getCategoryDaValue(msCategory,
                                                 "ContentServerUrl")[0];
        ...
        m_username = m_parser.getCategoryDaValue(msCategory,
                                                 "MediasurfaceUser")[0];
    ...
    {
        //get the sourceitem
        m_item = m_mediaSurface.getItem(m_securityContextHandle, new ItemKey(m_itemKey));
        ...
        else
        {
            ...
            else
            {
                ...
                m_targetItem.edit();
                m_targetItem.setBinaryContent(
                    m_cxeMessage.getMessageData().getInputStream(),
                    size);
            }
    ...
    /**
     * Nicer method to lookup items from Mediasurface
     * since it throws Exceptions instead of returning
     * null
     * 
     * @param p_url  url
    ...
    /**
     * Creates a new item to be a copy of the
     * source item.
     * 
    ...
    private void makeNewTargetItem(ArrayList p_sourceParents) throws Exception
    {
        ...
        m_targetItem = m_mediaSurface.createItem(
                                                m_securityContextHandle, type.getKey(), name,
                                                host.getKey(), site.getKey());
        ...
        if (m_isFinalExport == true)
        {
            ...
            {
                m_logger.warn("Could not sign off item '" + m_targetItem.getUrl() + "' : " +
                              e.getMessage());
            }
        }
        else
    ...
    /**
     * Returns an arraylist of the item's parents
     * as IItem objects. parents[0] is the top level
     * parent.
     * 
     * @param p_item -- an IItem
     * @return ArrayList
     * @exception Exception
     */
    ...
    /**
     * Looks up the Mediasurface ILocale that corresponds to
     * the target locale and sets m_targetILocale
     */
    private void setTargetILocale() throws Exception
    {
        ILocale locales[] = m_mediaSurface.getLocales(m_securityContextHandle);
        Locale targetLocale = GlobalSightLocale.makeLocaleFromString(
                                                                    m_targetLocale);
    ...
    private void fillTargetItemFields() throws Exception
    {
        ...
        String msXml = MessageDataReader.readString(m_cxeMessage.getMessageData());
        ...
        for (int i=0; i < nl.getLength(); i++)
        {
            ...
            {
                //for now just re-use the item's date instead of trying to parse
                //the date from the XML
                m_targetItem.setDateFieldValue(
                                              fieldName, m_item.getDateFieldValue(fieldName));
            }
            else
            {
    ...
    /**
     * Creates the top level target item and binds it to
     * the host, but not as the root.
     * 
     * @param p_sourceTopLevelItem
     *               the source language top level item
     */
    private IItem makeTargetTopLevelItem(IItem p_sourceTopLevelItem) throws Exception
    {
        IItem targetTopLevelItem = m_mediaSurface.createItem(
                                                            m_securityContextHandle,
                                                            p_sourceTopLevelItem.getType().getKey(),
                                                            m_targetLocale,
                                                            p_sourceTopLevelItem.getHost().getKey(),
                                                            p_sourceTopLevelItem.getSite().getKey());
    ...
    /**
     * Copy fields and metadata from source to target item. This assumes the target item
     * is already being edited.
     * 
     * @param p_source source item
     * @param p_target target item
     */
    private void copyItem(IItem p_source, IItem p_target) throws Exception
    {
        ...
        for (int i=0; i < fieldNames.length; i++)
        {
            ...
            if (fieldName.toLowerCase().indexOf("date") > -1)
            {
                p_target.setDateFieldValue(fieldName,p_source.getDateFieldValue(fieldName));
            }
            else
            {
    ...
    private IItem makeStructureForItem(ArrayList p_sourceParents) throws Exception
    {
    ...
    private IItem makeMirror(IItem a, IItem parent) throws Exception
    {
        ...
        IItem newChild = m_mediaSurface.createItem(
                                                  m_securityContextHandle, type.getKey(), a.getFullName(),
                                                  host.getKey(), site.getKey());
    ...
    /**
     * Makes a preview URL XML that can be used to preview
     * this item from MediaSurface. Many views can
     * be shown this way.
     * 
    ...
    private String makePreviewUrlXml()
    {
        try
        {
            ...
            xml.append("<locale name=\"").append(m_sourceLocale).append("\">\r\n");
            ...
            xml.append("<locale name=\"").append(m_targetLocale).append("\">\r\n");
