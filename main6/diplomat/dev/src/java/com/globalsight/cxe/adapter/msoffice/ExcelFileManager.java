...
public class ExcelFileManager 
{
	...
	{
        List<File> fs = FileUtil.getAllFiles(new File(dir,"xl/worksheets"), new FileFilter()
        {
            ...
            public boolean accept(File pathname)
            {
                ...
                return name.startsWith("sortsheet") && name.endsWith(".xml");
            }
        });
        ...
        for (File f : fs)
        {
            ...
            try
            {
                ...
                for (Node n : ns)
                {
                    ...
                    if ("si".equals(name))
                    {
                        String index = util.getAttribute(n, "siIndex").getNodeValue();
                        siNodes.put(index, n);
                    }
	...
	private void updateSi(Map<String, Node> siNodes, String dir) throws Exception
    {
        if (siNodes.size() == 0)
            return;
    ...
    public void sortSegments(String dir, List<String> hIds, String excelOrder, HashMap<String, String> hideCellMap) 
	{
		...
		for (File f : fs)
		{
		    String fileName = FileUtils.getPrefix(FileUtils
