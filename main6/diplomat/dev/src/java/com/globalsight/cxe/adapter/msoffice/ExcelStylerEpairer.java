...
public class ExcelStylerEpairer {
    ...
    /**
     * Get a map that contain the pair of shared string and style.
     * The content of v is shared string index, and the s attribute is style index.
     * 
     * @return
     * @throws Exception
     */
	...
	private HashMap<String, Node> getFonts() throws Exception
	{
		if (style2font == null)
		{
            ...
            if (affectedNodes != null && affectedNodes.getLength() > 0)
            {
                ...
                for (int i = 0; i < len; i++)
                {
                	org.w3c.dom.Element node = (org.w3c.dom.Element) affectedNodes.item(i);
		...
		if (style != null)
		{
			...
			if (r != null)
			{
				...
				while (f != null)
				{
					...
					{
						Node fc = c.getOwnerDocument().createElement(f.getNodeName());
						NamedNodeMap nm = f.getAttributes();
						
						for (int j = 0; j < nm.getLength(); j++)
						{
							Node attNode = nm.item(j);
							Attr att = c.getOwnerDocument().createAttribute(attNode.getNodeName());
