...
/**
 * The MicrosoftAppHelper is an abstract base class intended to be
 * subclassed to handle converting native format files (Word, Quark,
 * Frame, etc.) to XML and back.
 */
public class MicrosoftWordHelper implements IConverterHelper
{
    //
    // CONSTANTS
    //
     ...
     * specific to the MicrosoftApplicationAdapter.
     */
    static public final String EFXML_DA_CATEGORY_NAME = "MicrosoftOffice";

    static private final String POST_NATIVE_TO_HTML_CONVERSION_EVENT =
        "HTML_IMPORTED_EVENT";
    static private final String POST_MERGE_EVENT =
        "MSOFFICE_LOCALIZED_EVENT";
     ...
     *
     * @param p_cxeMessage cxeMessage
     * @param p_logger logger to use
     * @param p_msOfficeProperties - The properties of MS Office
    ...
    /**
     * Performs the conversion of MS Office to HTML.
     * Returns an array of events that should be published.
     * They correspond to html import events for the resulting
     * HTML files (from the conversion).
     *
     * @return CxeMessage[]
     * @exception MsOfficeAdapterException When conversion to HTML fails.
     */
        ...
        throws MsOfficeAdapterException
    {
        ...
        try
        {
            ...
            String expectedHtmlFileName = fullSafeName.substring(
                0, fullSafeName.lastIndexOf(".")) + ".html";
            ...
            String expectedSubDirName = fullSafeName.substring(
                0, fullSafeName.lastIndexOf(".")) + m_tempDirSuffix;

            File expectedSubDir = new File (expectedSubDirName);
            if (!expectedSubDir.exists())
            {
                m_tempDirSuffix = TEMP_DIR_SUFFIX_2000;
                expectedSubDirName = fullSafeName.substring(
                    0, fullSafeName.lastIndexOf(".")) + m_tempDirSuffix;
            ...
            if (expectedSubDir.exists() && expectedSubDir.isDirectory())
            {
                ...
                m_logger.debug("Expected sub dir " +
                    expectedSubDirName + " has " + files.length + " files.");
    ...
    /**
     * Performs the conversion of HTML to MS Office format type.
     * This only does the conversion if the HTML file is the last page
     * in the batch, since all the pages for a MS Office file are in a
     * job together.
     *
     * @throws MsOfficeAdapterException - When conversion from HTML to
     * MS Office fails.
     */
        ...
        throws MsOfficeAdapterException
    {
        ...
        try
        {
            ...
            String relSafeName = m_parser.getCategoryDaValue(
                categoryElement, "relSafeName")[0];
            ...
            if (isExportFileComplete(key, docPageCount.intValue()))
            {
                ...
                String htmlName = safeBaseFileName.substring(
                    0, safeBaseFileName.lastIndexOf(".")) + ".html";
                String fullHtmlName = m_saveDir +  File.separator +
                    htmlName;
            ...
            else
            {
                ...
                CxeMessageType type = CxeMessageType.getCxeMessageType(
                    CxeMessageType.CXE_EXPORT_STATUS_EVENT);
                CxeMessage outputMsg = new CxeMessage(type);
                outputMsg.setEventFlowXml(m_eventFlowXml);
    ...
    private void copyGeneratedFilesToTargetLocales(String p_expectedSubDirName,
        String p_expectedHtmlFileName)
        throws Exception
    {
        RecursiveCopy rc = new RecursiveCopy();

        String l10nProfileId = m_parser.getL10nProfileId();
        ArrayList targetLocales = findTargetLocales(l10nProfileId);
        String baseDirName = p_expectedSubDirName.substring(
            lastSeparatorIndex(p_expectedSubDirName)+1);
        String baseFileName = p_expectedHtmlFileName.substring(
            lastSeparatorIndex(p_expectedHtmlFileName)+1);
        ...
        for (int i = 0; i < targetLocales.size(); i++)
        {
            ...
            if (expectedSubDir.exists() && expectedSubDir.isDirectory())
            {
                m_logger.debug("Copying files from " +
                    p_expectedSubDirName + " to " + targetDir);
     ...
     * and both will be written to the same directory <br>
    ...
    private String createBaseFileNameToUseForConversion()
    {
        ...
        // is at the end.
        String baseFileName = displayName.substring(
            lastSeparatorIndex(displayName) + 1);
        
        //In Multi-Company, different companies may have files whose displayName
        //are same, so we append timestamp at the head of the displayName to
        //distinguish them. 
        //baseFileName = displayName.hashCode() + baseFileName;
        baseFileName = Calendar.getInstance().getTimeInMillis() + baseFileName;
        
        m_logger.debug("basename: "+ baseFileName);

        // Make sure that the extension is lower case for conversion purposes.
        String extension = baseFileName.substring(
            baseFileName.length() - 3).toLowerCase();
        ...
        baseFileName = baseFileName.substring(
            0, baseFileName.length() -3) + extension;
        m_logger.debug("basename: "+ baseFileName);
    ...
    /**
     * Find and return a list of target locales for the given
     * l10n profile id.
     */
    ...
    /**
     * Import the files named in the arraylist by modifying an
     * eventflowxml for each one, and then returning an array of
     * CxeMessage objects that correspond to HTML_IMPORTED_EVENTs so
     * that the rest of CXE imports these files.
     *
     * @param filesToImport arrayList of filenames
     * @return CxeMessage[]
     * @exception Exception
     */
    private CxeMessage[] importFiles(ArrayList filesToImport)
        throws Exception
    {
        // For PPT issue
        ...
        if (m_type == MS_PPT)
        {
            ...
            slideSequence = 
                generateSlideSequence((File)specialFileMap.get("list"));
            bulletCss = 
                generateBulletCssList((ArrayList)specialFileMap.get("css"));
        ...
        CxeMessageType msgType = CxeMessageType.getCxeMessageType(
            CxeMessageType.HTML_IMPORTED_EVENT);
        ...
        for (int i = 0; i < x.length; i++)
        {
            ...
            String modifiedEventFlowXml =
                modifyEventFlowXmlForImport(f, i + 1, x.length, 
                                            slideSequence, bulletCss);
     ...
     * EFXML.  This restores the EventFlowXml to (almost) the way it
     * was before coming to the MicrosoftApplication source adapter.
     */
        ...
        throws Exception
    {
        ...
        String originalBaseHref =  m_parser.getCategoryDaValue(
            categoryElement, "originalBaseHref")[0];
        ...
        throws Exception
    {
        ...
        String originalPostMergeEvent =
            parser.setPostMergeEvent(POST_MERGE_EVENT);
        ...
        if (m_type == MS_PPT)
        {
            ...
            else
            {
                ...
                String slideId = 
                    slideSequence.get(p_htmlFile.getName()).toString();
                if (slideId.length() == 1) tempBaseName.append("000");
                else if (slideId.length() == 2) tempBaseName.append("00");
                else if (slideId.length() == 3) tempBaseName.append("0");
        ...
        else if (m_type == MS_XLS)
        {
                    ...
                    p_htmlFile.getName().trim().startsWith("tabstrip") )
            {
                  ...
                  baseName = p_htmlFile.getName().substring(0, p_htmlFile.getName().lastIndexOf("."));
        ...
        String originalBaseHref = parser.setBaseHref(
            determineNewBaseHref(htmlFileName));

        //<cateogory name="MicrosoftApplicationAdapter">
        Element categoryElement = parser.addCategory(
            EventFlowXmlParser.EFXML_DA_CATEGORY_NAME);
        ...
        values[0] = htmlFileName.substring(m_saveDir.length(),htmlFileName.length());
        ...
        Logger.writeDebugFile("efxml_" + Integer.toString(p_docPageNum) + ".xml",
            modifiedEventFlowXml);
    ...
    {
        File root = new File(m_saveDir + File.separator + name + m_tempDirSuffix);
        
        if (!root.exists())
        {
            root = new File(m_saveDir + File.separator + name + TEMP_DIR_SUFFIX_2000);
        }
        ...
        throws Exception
    {
        ...
        try
        {
            ...
            String baseName = m_safeBaseFileName.substring(
                0, m_safeBaseFileName.lastIndexOf("."));
            ...
            String maxTTW = m_msOfficeProperties.getProperty(
                MsOfficeConfiguration.MAX_TIME_TO_WAIT);
    ...
    private void writeCommandFile(String p_commandFileName) throws Exception
    {
        ...
        if (m_isImport)
        {
            ...
            try
            {
                ...
                {
                    acceptChanges += m_msOfficeProperties.getProperty(
                        MsOfficeConfiguration.WORD_TRACKING_CHANGES);
                }
                else if (m_type == MS_XLS)
                {
                    acceptChanges += m_msOfficeProperties.getProperty(
                        MsOfficeConfiguration.EXCEL_TRACKING_CHANGES);
                }
                else
                {
                    acceptChanges += "NA";
    ...
    /**
     * Add the html/htm files to the list files to be imported.  Just
     * make sure not to add the automatically generated error file by
     * PowerPoint to be included in the list, as it's only a generic
     * error message that's not part of the ppt file at all.
     */
    private void validateForImport(File p_file, ArrayList p_filesToImport)
    {
        ...
        {            
            if (m_type == MS_PPT)
            {
                ...
                String isMasterTranslate = SystemConfiguration.getInstance().getStringParameter(
                ...
                if (fileName.indexOf("slide") == 0)
                {
                    p_filesToImport.add(p_file);
                        ...
                        isMasterTranslate.equalsIgnoreCase("true"))
                {
                    p_filesToImport.add(p_file);
                }
            }
            else
            {
                ...
                try 
                {
                    fileProfile = ServerProxy.getFileProfilePersistenceManager().readFileProfile(fileProfileId);
                } 
                catch (Exception e) 
                {
                    m_logger.info("Load fileProfile failed.");
                }
                
                // GBS-789 : Can not create aligner package with MS Office file formats
                // There is no fileProfileId when create Aligner Package, and we assign it as 0.
                if("header".equalsIgnoreCase(fileName) && m_type == MS_DOC && fileProfileId != 0)
                {
        ...
        else if (name.endsWith(".xml"))
        {
            if ((m_type == MS_PPT) && "filelist".equalsIgnoreCase(fileName))
            {
                ...
                if ((p_filesToImport == null) || 
                    (p_filesToImport.size() < 1) ||
                ...
                try
                {
                    ...
                    while (line != null)
                    {
                        if (line.startsWith(".style"))
                        {
                            ...
                            {
                                // Keep looping until entered next style definiton
                                // or found style name.
                                line = br.readLine();
                                continue;
                            }
                        ...
                        if (line.startsWith(".x"))
                        {
                            ...
                            {
                                // Keep looping until entered next class (html class element)
                            ...
                            else if ( (line.indexOf("mso-style-parent") != -1) )
                            {
                                ...
                                int endIndex = line.indexOf(";", beginIndex);
                                styleName = (String)tempMap.get(line.substring(beginIndex, endIndex));
                                // Map class (html class element) to style parent name.
                                excelStyle2Name.put(style.substring(1), styleName);
                            }
                        }
                        line = br.readLine();
                    }
    ...
    /**
     * For PPT issue
     * Get the slide sequence from filelist.xml
     */
    private HashMap generateSlideSequence(File p_file) throws Exception
    {
        ...
        while ((line = reader.readLine()) != null)
        {
            ...
            {
                slideSequence.put(line.substring(line.indexOf("slide"), 
                                                 line.lastIndexOf("\"")),
    ...
    private String generateBulletCssList(ArrayList cssFiles) throws Exception
    {
        ...
        for (int i = 0; i < cssFiles.size(); i++)
        {
            ...
            BufferedReader reader = 
                new BufferedReader(new FileReader((File)cssFiles.get(i)));
            String line = null;
            while ((line = reader.readLine()) != null)
            {
     ...
     */
    private String writeFileToTempDir()
        throws Exception
    {
        m_safeBaseFileName = createBaseFileNameToUseForConversion();
        String fullSafeName =
            m_saveDir + File.separator + m_safeBaseFileName;
    ...
    /**
     * Returns the event to publish after doing a conversion
     * from XML to Native. This should be an event that
    ...
    /**
     * Access the Hashtable s_exportBatches to see what the current
     * page count of this export batch is. This call decrements the
     * value in the table.
     */
    private static boolean isExportFileComplete(
        String p_fileId, int p_pageCount)
    {
        ...
        synchronized (s_exportBatches)
        {
            ...
            else
            {
                ...
                if (curPageCnt == 0)
                {
                    // The batch is complete, remove the value from the hashtable.
                    result = true;
        ...
        throws Exception
    {
        ...
        saveDir.append(m_isImport ?
            m_parser.getSourceLocale() :
     ...
     * The baseHref will look something like:
     * http://blah:7001/word/en_US
     */
    private String determineNewBaseHref(String p_htmlFileName)
        throws Exception
    {
        ...
        String capLoginUrl = SystemConfiguration.getInstance().getStringParameter(
            SystemConfigParamNames.CAP_LOGIN_URL);
        StringBuffer newBaseHref = new StringBuffer();
        newBaseHref.append(capLoginUrl.substring(0,capLoginUrl.lastIndexOf("/")));
        newBaseHref.append("/");
        ...
        if (m_type == MS_PPT)
        {
            ...
            String relPath= p_htmlFileName.substring(
                convDir.length() + 1, lastSeparatorIndex(p_htmlFileName));
        ...
        else
        {
            ...
            if ( idx > -1 )
            {
                newBaseHref.append(relPath.substring(0,lastSeparatorIndex(relPath)));
            }
            else
            {
                newBaseHref.append(relPath);
        ...
        throws MsOfficeAdapterException
    {
        ...
        {
            throw new MsOfficeAdapterException("PowerPointNotInstalled", null, null);
        }
    }
}
