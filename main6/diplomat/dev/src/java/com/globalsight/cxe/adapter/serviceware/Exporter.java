...
import com.globalsight.cxe.adapter.serviceware.ServiceWareAPI;
...
public class Exporter
{
    ...
    CxeMessage export()
    {
        ...
        try
        {
            ...
            parseEventFlowXml();
            m_logger.info("Updating knowledge object " + m_koId + " for target locale " +
                          m_targetLocale);
            String sessionId = ServiceWareAPI.connect();
            String localizedXml = MessageDataReader.readString(
                m_cxeMessage.getMessageData());
            ServiceWareAPI.updateKnowledgeAndConceptObjects(
                sessionId, localizedXml, m_targetLocale);
        ...
        catch (Exception e)
        {
            ...
            ServiceWareAdapterException fsae = new ServiceWareAdapterException("CxeInternalEx",errorArgs,e);
            exportStatusMsg = makeExportErrorMessage(fsae);
        }
    ...
    private void parseEventFlowXml() throws Exception
    {
        ...
        m_koId = m_parser.getCategoryDaValue(msCategory,
                                             "KOID")[0];
