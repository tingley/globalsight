...
{
    private static final Logger s_logger = Logger.getLogger(FileSystemUtil.class);
     ...
     * @param p_locales
     *            Vector<String>
     * @return result Vector, include Vector<File> files, Vector
     *         <FileProfile> fileProfiles, Vector<String> locales, Vector
     *         <Integer> exitValueser.
     * @throws WebServiceException
     */
    public static Vector execScript(Vector p_files, Vector p_fps, Vector p_locales, long p_jobId,
            String p_jobName)
    {
        if (p_files.size() != p_fps.size())
        {
            throw new IllegalArgumentException("The size of files is " + p_files.size()
                    + "but the file profile size is " + p_fps.size());
        }
        ...
        for (int i = 0; i < p_files.size(); i++)
        {
            ...
            if (scriptOnImport != null && scriptOnImport.length() > 0)
            {
                ...
                int index = AmbFileStoragePathUtils.getCxeDocDirPath(fp.getCompanyId()).length()
                        + 1;
                ...
                if (!scriptedFolder.exists())
                {
                    ...
                    try
                    {
                        ...
                        s_logger.info("Script on Import " + scriptOnImport + " is called to handle "
                                + filePath);
                    }
                    catch (Exception e)
                    {
