...
public class BatchInfo implements Serializable {
	...
	public static class ProcessMode implements Serializable {
		...
		public static final ProcessMode MANUAL = new ProcessMode(0, MANUAL_STRING);
		public static final ProcessMode AUTOMATIC = new ProcessMode(1, AUTOMATIC_STRING);
	...
	protected BatchInfo(Element p_elem) {
		...
		m_pageCount = Integer.parseInt(XmlUtils.getDirectChildElementValue(p_elem, PAGE_COUNT));
		m_pageNumber = Integer.parseInt(XmlUtils.getDirectChildElementValue(p_elem, PAGE_NUMBER));
		m_docPageCount = Integer.parseInt(XmlUtils.getDirectChildElementValue(p_elem, DOC_PAGE_COUNT));
		m_docPageNumber = Integer.parseInt(XmlUtils.getDirectChildElementValue(p_elem, DOC_PAGE_NUMBER));
		m_displayName = XmlUtils.getDirectChildElementValue(p_elem, DISPLAY_NAME);
		Element baseHrefElement = XmlUtils.getDirectChildElement(p_elem, BASE_HREF);
		if (baseHrefElement != null)
			m_baseHref = XmlUtils.getElementValue(baseHrefElement);
		Element jobNameElement = XmlUtils.getDirectChildElement(p_elem, JOB_NAME);
		if (jobNameElement != null)
			m_jobName = XmlUtils.getElementValue(jobNameElement);
		Element masterTranslatedElement = XmlUtils.getDirectChildElement(p_elem, MASTER_TRANSLATED);
        if (masterTranslatedElement != null)
            m_masterTranslated = XmlUtils.getElementValue(masterTranslatedElement);
        Element inddHiddenTranslatedElement = XmlUtils.getDirectChildElement(p_elem, INDD_HIDDEN_TRANSLATED);
        if (inddHiddenTranslatedElement != null)
            m_inddHiddenTranslated = XmlUtils.getElementValue(inddHiddenTranslatedElement);
	...
	public String serializeToXml() {		
		...
		XmlUtils.appendElement(sb, BATCH_ID, EditUtil.encodeXmlEntities(m_batchId));
		...
		XmlUtils.appendElement(sb, DOC_PAGE_COUNT, String.valueOf(m_docPageCount));
		XmlUtils.appendElement(sb, DOC_PAGE_NUMBER, String.valueOf(m_docPageNumber));
		XmlUtils.appendElement(sb, DISPLAY_NAME, EditUtil.encodeXmlEntities(m_displayName));
		XmlUtils.appendElement(sb, BASE_HREF, EditUtil.encodeXmlEntities(m_baseHref));
