...
//                                    format, false, processedChars);
    ...
    private static String newHandleString4Export(String ccc, List<Escaping> es,
            boolean doDecode, String format, String escapingChars,String contenType)
    {
        StringBuffer sub = new StringBuffer();
        ccc = doDecode ? m_xmlEncoder.decodeStringBasic(ccc) : ccc;
			...
			String format, String escapingChars, String contentType)
	{
				...
				.equalsIgnoreCase("startFinishes"))
		{
			...
			if (startIsRegex)
			{
				startRegex = escaping.getStartPattern();
				if (startRegex != null)
				{
			...
			if (finishIsRegex)
			{
				finishRegex = escaping.getFinishPattern();
				if (finishRegex != null)
				{
			...
			String format, String escapingChars, String contentType)
	{
				...
				.equalsIgnoreCase("startFinishes"))
		{
			...
			String startRegex = null, finishRegex = null;
			if (startIsRegex)
			{
				startRegex = escaping.getStartPattern();
				if (startRegex != null)
			...
			String contentType)
	{
		...
		{
			//es xun huan fang zai wai mian shi wei le kao lv "start/finish regex"
			for (Escaping escaping : es)
			{
				if (escaping.isCheckActive())
				{
	...
	private static String checkActiveHandleChar4Import(String ccc, Escaping escaping,
