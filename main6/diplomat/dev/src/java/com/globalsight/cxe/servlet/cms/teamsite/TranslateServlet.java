...
public class TranslateServlet extends HttpServlet implements SingleThreadModel
{
    private static final Logger s_logger = Logger
            .getLogger(TranslateServlet.class);

    // error message displayed when there are selected files that are part of
    // on-going jobs
    ...
    public TranslateServlet() throws ServletException
    {
    }
            ...
            IOException
    {
        ...
        if (userName == null)
        {
            ...
            JSPCallingUtility.invokeMessageJSP(getServletContext(), theRequest,
                    theResponse, message);
            return;
        }
        ...
        try
        {
            isSuperPm = ServerProxy.getUserManager().containsPermissionGroup(
                    aUser.getUserId(), WebAppConstants.SUPER_PM_NAME);
        }
        catch (Exception e)
        {
        ...
        /*
         * Removing this for now Needs to be rewritten // verify access rights
         * if(!theUser.hasTranslateAccessRights()) { String message =
         * "You have insufficient access privileges to translate files";
         * reportFailure(theRequest, theResponse, message);
         * JSPCallingUtility.invokeMessageJSP(context, theRequest, theResponse,
         * message); return; }
         */
        ...
        if (stage == null)
        {
            ...
            catch (EnvoyServletException ese)
            {
                String message = "Failed in selecting file profiles\n"
                        + ese.getMessage();
                reportFailure(theRequest, theResponse, message);
                JSPCallingUtility.invokeMessageJSP(getServletContext(),
                        theRequest, theResponse, message);
            }
        }
        // the stage is "import"
    ...
    /**
     * creates a page that allows the user to select a template file as part of
     * the process of starting a job
     * 
     * @param theRequest
     *            : the servlet request -- used for the invokation of the JSP
     *            used to return error messages
     * @param theResponse
     *            : the servlet response -- used for the invokation of the JSP
     *            used to return error messages
     * 
     ...
     *             as a pass-through
     */
    private void selectTemplate(HttpServletRequest theRequest,
            HttpServletResponse theResponse, boolean isSuperPm)
            throws ServletException, IOException, EnvoyServletException
    {
        String areaPath = theRequest.getParameter("area_path");
        if (areaPath == null || areaPath.indexOf("WORKAREA") < 0)
        {
            // print error message if user selected "Translate" outside working
            // area.
        ...
        // convert TeamSite's file paths to our paths and put them in the
        // convertedFilePaths Vector
        // first, extract the original Teamsite file paths
        Vector originalFilePaths = extractFileProperties(theRequest,
                theResponse);
        ...
        while (en.hasMoreElements())
        {
            ...
            convertedFilePaths.add(convertOriginalTSPath(
                    originalFilePath.substring(0, start), theRequest));
        }
        ...
        /*
         * try { fileSystemProfiles = new Vector
         * (ServerProxy.getFileProfilePersistenceManager
         * ().getAllFileProfiles()); } catch (Exception ne) { throw new
        ...
        try
        {
            ...
            while (it.hasNext())
            {
                ...
                L10nProfile l10nProfile = ServerProxy.getProjectHandler()
                        .getL10nProfile(fileProfile.getL10nProfileId());
                DispatchCriteria criteria = l10nProfile.getDispatchCriteria();
                jobNamePrompt
                        .add(criteria.getCondition() == DispatchCriteria.BATCH_CONDITION ? Boolean.TRUE
                                : Boolean.FALSE);
            }
        }
        catch (Exception e)
        ...
        JSPCallingUtility.invokeImportJobJSP(getServletContext(), theRequest,
                theResponse, fileSystemProfiles, hiddenFieldsBean,
                jobNamePrompt);

    }

    private Vector getFileProfiles() throws ServletException,
            EnvoyServletException
    {
        try
        {
            ...
            while (it.hasNext())
            {
                long fpCompanyId = ((FileProfile) it.next()).getCompanyId();
    ...
    /**
     * Creates a JavaBean (SortedNameValuePairsBean) with all the name and
     * values that will go in the import page as hidden fields.
     */
    private SortedNameValuePairsBean makeListOfHiddenFields(
            HttpServletRequest theRequest) throws ServletException
    {
        ...
        // add a last hidden field with a stage tag to indicate that we are done
        // with the template
    ...
    /**
     * This method extracts all the IW file paths in the HttpServletRequest. If
     * a one of the selections was a directory, the method recursively collects
     ...
     */
    private Vector extractFileProperties(HttpServletRequest theRequest,
            HttpServletResponse theResponse) throws ServletException
    {
        ...
        {
            String parameterName = (String) e.nextElement();
            String parameterValue = (String) theRequest
            ...
            if (result)
            {
                ...
                // post-pend index to end file after "-_-_-" to use for
                // recovering filesize
     ...
     * 
     * @param tsFileName
     *            the absolute path of a teamsite file
     * @param theRequest
     *            the original HttpServletRequest received by this servlet
     * 
     * @return the converted file path of the Teamsite file into the GlobalSight
     *         file path
     */
    private String convertOriginalTSPath(String tsFileName,
            HttpServletRequest theRequest) throws EnvoyServletException
    {
        ...
        try
        {
            ...
            String iwDocRoot = decodeSpacesInPath(theRequest
                    .getParameter("area_path"));
            ...
            String partialTSFilePath = tsFileName.substring(archivePath
                    .length());
     ...
     */
    private void importFiles(HttpServletRequest theRequest,
            HttpServletResponse theResponse) throws ServletException,
            EnvoyServletException
    {
        ...
        String userName = theRequest.getParameter("user_name");
        s_logger.debug("Username from theRequest.getparameter() call: "
                + userName);

        // determine the job name from the Http request
        String jobName = EditUtil.utf8ToUnicode(theRequest
                .getParameter("jobName"));
        ...
        try
        {
            ...
            while (en.hasMoreElements())
            {
                ...
                FileExtension fext = ServerProxy
                        .getFileProfilePersistenceManager().readFileExtension(
                                fextId);
        ...
        try
        {
            // count the number of files with the proper extensions <=>
            // pageCount
            ...
            while (en.hasMoreElements())
            {
                ...
                String convertedFilePath = this.convertOriginalTSPath(
                        originalTSFilePath.substring(0, start), theRequest);

                boolean properFileExtension = false;
                properFileExtension = compareToExtensions(
                        originalTSFilePath.substring(0, start),
                        extensions.elements());
            ...
            int pageCount = pageNumber; // the total number of files being sent
                                        // to CAP
            ...
            while (en.hasMoreElements())
            {
                ...
                String originalTSFilePath = (String) en.nextElement();
                int start = originalTSFilePath.indexOf("-_-_-");
                String fileSize = theRequest.getParameter("size_"
                        + originalTSFilePath.substring(start + 5));
                originalTSFilePath = decodeSpacesInPath(originalTSFilePath);
                // remove index
                ...
                String overwriteSource = theRequest
                        .getParameter("overwriteSource");
                String callbackImmediately = theRequest
                        .getParameter("callbackImmediately");
                ...
                if (properFileExtension)
                {
                    ...
                    publishEventToCxe(originalTSFilePath.substring(0, start),
                            convertedFilePath,
                            new Integer(fileSize).intValue(), locale,
                            characterSet, batchId, pageNumber, pageCount,
                            fileProfileId, localizationProfileID, jobName,
                            userName, serverName, storeName, taskId,
    ...
    private boolean compareToExtensions(String filePath, Enumeration enp)
    {
        if (enp.hasMoreElements() == false)
            return true; // return true if there are no extensions for the file
                         // profile

        boolean properFileExtension = false;

        // check that file extension is in File system profile list of
        // extensions
        String extension;
        String extensionU;
    ...
    private void publishEventToCxe(String p_originalFilename,
            ...
            String p_fileProfileId, String p_localizationProfile,
            String p_jobName, String p_userName, String p_serverName,
            String p_storeName, String p_taskId, String p_overwriteSource,
            String p_callbackImmediately) throws Exception
    {
        s_logger.info("Requesting file from teamsite" + p_originalFilename);
        CxeProxy.importFromTeamSite(p_originalFilename, p_convertedFileName,
                p_fileSize, p_locale, p_charset, p_batchId, p_pageNum,
                p_pageCount, 1, 1, p_fileProfileId, p_localizationProfile,
                p_jobName, p_userName, p_serverName, p_storeName, p_taskId,
                p_overwriteSource, p_callbackImmediately,
    ...
    private void reportFailure(HttpServletRequest theRequest,
            HttpServletResponse theResponse, String p_message)
    {
        try
        {
                ...
                if (tid.intValue() > 0)
                {
                    ...
                    if (ts.getOS().equalsIgnoreCase("nt"))
                    {
                        tsURL = new URL(TeamSiteExchange.TS_HTTP + ts.getName()
                                + ":" + ts.getExportPort()
                                + TeamSiteExchange.TS_IMPORT_NT);
                    }
                    else if (ts.getOS().equalsIgnoreCase("unix"))
                    {
                        tsURL = new URL(TeamSiteExchange.TS_HTTP + ts.getName()
                                + ":" + ts.getExportPort()
                                + TeamSiteExchange.TS_IMPORT_UNIX);
                    }
                    ...
                    PrintWriter out = new PrintWriter(new OutputStreamWriter(
                            tsConnection.getOutputStream()));
                    if (taskId != null && !taskId.equals("null"))
                    {
                        out.print("taskId=" + URLEncoder.encode(taskId) + "&");
                    }
                    if (callbackImmediately != null
                            && !callbackImmediately.equals("null"))
                    {
                        out.print("callbackImmediately="
                                + URLEncoder.encode(callbackImmediately) + "&");
                    }
                    out.print("teamsiteJobState="
                            + URLEncoder.encode("IMPORT_FAILED") + "&");
                    ...
                    BufferedReader in = new BufferedReader(
                            new InputStreamReader(tsConnection.getInputStream()));
                    ...
                    // now check the LastLine to see what the returned status
                    // from the
