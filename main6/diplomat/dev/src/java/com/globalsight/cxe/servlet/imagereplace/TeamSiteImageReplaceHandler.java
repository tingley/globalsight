...
public class TeamSiteImageReplaceHandler extends HttpServlet
{
    ...
    public TeamSiteImageReplaceHandler() throws ServletException
    {
        try
        {
    ...
    /**
     * This method processing the POST request made to upload image back to Team
     * Site.
     * <p>
    ...
    /**
     * <p>
     * The real method for processing the POST request for uploading image back
     * to TeamSite.
     * </p>
     * <p>
     * The is a rather long method, which could potentially be broken down to
     * two methods, just for ease of readability only. The first part of the
     * method spends time reading the EventFlow XML to determine the source and
     * target information. Using the information, it determines the work area
     * path and relative path of the image file. Using the work area path, along
     * with source and target information, it checks if there are any TeamSite
     * branches defined in system.
     * </p>
     * <p>
     * Finally, it calls another method that posts an HTTP request to TeamSite
     * with file and it's contents.
     * </p>
     * <p>
     * Majority of code has been bootlegged from TSImportFileOperation and it is
     * a cadidate for some refactoring.
     * </p>
     * 
     * @param p_request
     *            Http Request object
            ...
            IOException
    {
        ...
        String imageSrcLocation = p_request
                .getParameter(ExportConstants.IMAGE_FILENAME);
        String imageContents = p_request
                .getParameter(ExportConstants.IMAGE_DATA);

        theLogger.println(Logger.DEBUG_A, ExportConstants.LOCALE_SUBDIR + "="
                + localeSubDir);
        theLogger.println(Logger.DEBUG_A, ExportConstants.IMAGE_FILENAME + "="
                + imageSrcLocation);
        theLogger.println(Logger.DEBUG_A, ExportConstants.EVENT_FLOW_XML + "="
                + eventFlowXml);
        ...
        try
        {
            ...
            hostName = baseHref.substring(baseHref.indexOf("://") + 3,
                    baseHref.lastIndexOf(":"));
            storeName = baseHref.substring(baseHref
                    .indexOf(Exporter.TS_BASEHREF_MOUNT)
                    + Exporter.TS_BASEHREF_MOUNT.length() + 1);
            ...
            try
            {
                ts = (TeamSiteServer) ServerProxy
                        ...
                        .getTeamSiteServerByName(hostName);
                ...
                {
                    BackingStore bs = (BackingStore) tsBackingStores
                            .elementAt(i);
            ...
            theLogger.println(Logger.INFO, "The teamsite mode is "
                    + teamsiteMode);
            localeElement = (Element) targetElement.getElementsByTagName(
                    "locale").item(0);
            ...
            for (int l = 0; l < categoryList.getLength(); l++)
            {
                ...
                if (name != null && name.equals("TeamSite"))
                {
                    ...
                    for (int m = 0; m < attributeList.getLength(); m++)
                    {
                        ...
                        {
                            NodeList values = attrElement
                                    .getElementsByTagName("dv");
                            Element valElement = (Element) values.item(0);
                            sourceFilename = valElement.getFirstChild()
                                    .getNodeValue();
                        }
                    }
                }
            }
            theLogger.println(Logger.DEBUG_A, "sourceFilename = "
                    + sourceFilename);
            //
            ...
            theLogger.println(Logger.DEBUG_A, "(Pass1)workAreaPath = "
                    + workAreaPath);
            ...
            //
            // if (( sourceBranch == DBsourceBranch) && (target language == DB
            // target language))
            // write translated file in DBtargetBranch
            // else
            // write translated file in sourceBranch with relPath having
            // "locale" prefix

            // Find target language
            String langStr = targetLocale.substring(0,
                    targetLocale.indexOf("_"));
            String countryStr = targetLocale.substring(targetLocale
                    .indexOf("_") + 1);
            TableLangs theTargetLanguage = TableLangs.findLanguageByLocale(
                    langStr, countryStr);
            theLogger.println(Logger.DEBUG_A, "TargetLanguage id = "
                    + theTargetLanguage.getID() + " langStr = " + langStr
            ...
            sourceBranch = sourceBranch.substring(0, startIndex - 1); // do not
                                                                      // include
                                                                      // ending
                                                                      // "/"
            ...
            theLogger.println(
                    ...
                    "sourceBranch = "
                            + sourceBranch
                            + " TargetLanguage id = "
                            + theTargetLanguage.getID()
                            + " branchLanguage = "
                            + ((branchLanguage != null) ? branchLanguage
                                    .getSourceBranch() : "null"));
            if (branchLanguage != null)
            {
                // (( sourceBranch == DBsourceBranch) && (target language == DB
                // target language))
                ...
                String prefixPath = workAreaPath.substring(0, startIndex); // e.g.
                                                                           // "/iw4/iwmnt/default/"
                String endPath = workAreaPath.substring(prefixPath.length()
                        + sourceBranch.length());
                workAreaPath = prefixPath + DBtargetBranch + endPath; // fixed
                                                                      // "DIR"
                                                                      // for the
                                                                      // multiple
                                                                      // branch
                                                                      // case
                theLogger.println(Logger.DEBUG_A, "DBtargetBranch = "
                        + DBtargetBranch);
                theLogger.println(Logger.DEBUG_A, "sourceBranch = "
                        + sourceBranch);
                theLogger.println(Logger.DEBUG_A, "prefixPath = " + prefixPath);
                theLogger.println(Logger.DEBUG_A, "endPath = " + endPath);
            }
            else
            // single branch case -- prepend the target locale
            {
                relPath = "/" + targetLocale + relPath;
            }

            theLogger.println(Logger.DEBUG_A, "(Pass2)workAreaPath = "
                    + workAreaPath);
            theLogger.println(Logger.DEBUG_A, "(Pass2)relPath = " + relPath);

            // Now write the file out
            //
            theLogger.println(Logger.DEBUG_A,
                    "Writing replaced image file out.");
            if (!writeTargetFile(relPath, workAreaPath, imageContents,
                    teamsiteMode, sourceLocale))
            {
        ...
        catch (SAXException se)
        {
            theLogger
                    .printStackTrace(
                            Logger.ERROR,
                            "Unable to parse EventFlowXml. Cannot determine where to export the file.",
                            se);
    ...
    /**
     * <p>
     * Post the target file (in this case, the replaced image) back to the
     * TeamSite target branch.
     * </p>
     * 
     * @param relPath
     *            the path of the replaced image
     * @param dir
     *            the work area path (effectively document root)
     * @param content
     *            the file contents. In this case (for image) the content is in
     *            Base64 encoding.
     *
     * @return true if it succeeds, false if not
     */
    private boolean writeTargetFile(String relPath, String dir, String content,
            String teamsiteMode, String sourceLocale)
    {
        ...
        {
            theLogger.println(Logger.DEBUG_A,
                    "caught RESyntaxException " + exc.getMessage());
            return false;
        }
        ...
        if (result)
        {
            ...
            theLogger.println(Logger.DEBUG_A, "targetFileName = "
                    + targetFileName);
        }
        else
        {
            // did not work
            theLogger.println(Logger.DEBUG_A,
                    "no regular expression match to relPath");
            return false;
        }
        ...
        try
        {
            ...
            out.print("session="
                    + URLEncoder.encode("teamsiteSession", "UTF-8") + "&");
            out.print("teamsiteMode="
                    + URLEncoder.encode(teamsiteMode, "UTF-8") + "&");
            out.print("sourceLocale="
                    + URLEncoder.encode(sourceLocale, "UTF-8") + "\n\n");
            out.close();
            ...
            theLogger.println(Logger.DEBUG_A,
                    "Status returned from Teamsite CGI: " + status);
        ...
        catch (Exception exc)
        {
            ...
            theLogger.println(Logger.DEBUG_A,
                    "Caught exception message " + exc.getMessage());
            return false;
        }
    }
     ...
     *
     * @param the
     *            absolute path to a file name
     *
     * @return the partial file name up to the work area including the trailing
     *         slash
     */
    private String findWorkAreaPath(String sourceFileName)
    {
        int workAreaIndex = sourceFileName.indexOf("WORKAREA/");
        String waPath = sourceFileName.substring(0,
                workAreaIndex + "WORKAREA/".length());
        String wbPath = sourceFileName.substring(workAreaIndex
                + "WORKAREA/".length());
        String workAreaName = wbPath.substring(0, wbPath.indexOf("/")); // include
                                                                        // the
                                                                        // slash
