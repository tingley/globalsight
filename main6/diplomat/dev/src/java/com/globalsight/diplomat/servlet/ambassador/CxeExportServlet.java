...
public class CxeExportServlet extends HttpServlet
{
            ...
            HttpServletResponse p_response) throws ServletException, IOException
    {
                ...
                && p_request.getParameter(ExportConstants.IMAGE_DATA) != null)
        {
            ...
            getServletContext().getNamedDispatcher("ImageReplace").forward(p_request, p_response);
    ...
    private void showPreview(HttpServletRequest p_request, HttpServletResponse p_response,
            String p_eventFlowXml, String p_previewUrlXml) throws ServletException
    {
        HttpSession theSession = p_request.getSession();
        try
        {
            ...
            srcLocale = ((Element) srcElement.getElementsByTagName("locale").item(0))
                    .getFirstChild().getNodeValue();
            tgtLocale = ((Element) tgtElement.getElementsByTagName("locale").item(0))
                    .getFirstChild().getNodeValue();
            ...
            s_logger.debug("CES: There are " + srcUrls.length + " source URLs and " + tgtUrls.length
                    + " target Urls.");
            ...
            theSession.setAttribute("srcEncoding", p.getSourceUrlListEncoding());
            theSession.setAttribute("tgtEncoding", p.getTargetUrlListEncoding());
        ...
        catch (Exception e)
        {
            ...
            try
            {
                showErrorPage(p_request, p_response,
                        new CapAdapterException("PreviewProblems", null, e));
            }
            catch (Exception ex)
            {
    ...
    private void showErrorPage(HttpServletRequest p_request, HttpServletResponse p_response,
            GeneralException p_generalException) throws Exception
    {
        EnvoyServletException ese = new EnvoyServletException(EnvoyServletException.EX_GENERAL,
                p_generalException);
        ErrorBean errorBean = new ErrorBean(0, p_generalException.getMessage(), ese);

        p_request.setAttribute(WebAppConstants.ERROR_BEAN_NAME, errorBean);
        getServletContext().getRequestDispatcher(WebAppConstants.ERROR_PAGE).forward(p_request,
                p_response);
    }
    ...
    private void exportForDynamicPreview(HttpServletRequest p_request) throws Exception
    {
        ...
        CxeProxy.exportForDynamicPreview(eventFlowXml, fmd, cxeRequestType, targetLocale,
                targetCharset, bomType, messageId, exportLocation, localeSubDir, exportBatchId,
                pageCount, pageNum, sessionId);
    }

    private PreviewInfoHolder waitForPreviewMessage(HttpServletRequest p_request) throws Exception
    {
        // GBS-4400, remove the jms stuff. Not sure what this function is about.
