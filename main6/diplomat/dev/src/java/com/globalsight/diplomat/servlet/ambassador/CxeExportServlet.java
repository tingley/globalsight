...
/**
 * The CxeExportServlet receives POSTs of GXML files
 * It handles export for image replace and dynamic preview.
 */
public class CxeExportServlet extends HttpServlet
{
     ...
     * Holds the information needed to do a preview.
     */
    private class PreviewInfoHolder
    {
        String previewUrlXml; //the preview url xml
        String modEventFlowXml; //the modified event flow xml (after going through cxe)
    }

    public CxeExportServlet()
    ...
    */
    ...
    throws IOException, ServletException
    {
        ...
        try
        {
            CompanyThreadLocal.getInstance().setValue(UserUtil.getCurrentCompanyName(p_request));
            ...
            if (previewUrlXml !=null && previewUrlXml.length() > 1)
            {
                ...
                showPreview(p_request, p_response, modEventFlowXml, previewUrlXml);
            }
            else
            {
                ...
                GeneralException ge = new GeneralException("Could not get dynamic preview.");
        ...
        catch (Exception ex)
        {
            ...
            CapAdapterException cae = new CapAdapterException("ExportServletEx",new String[0],ex);
    ...
    private boolean isImageReplaceRequest(HttpServletRequest p_request, HttpServletResponse p_response)
    throws ServletException, IOException
    {
            ...
            p_request.getParameter(ExportConstants.IMAGE_DATA) != null)
        {
            ...
            getServletContext()
            .getNamedDispatcher("ImageReplace")
    ...
    private void showPreview (HttpServletRequest p_request, HttpServletResponse p_response,
                              String p_eventFlowXml, String p_previewUrlXml)
    throws ServletException
    {
        HttpSession theSession = p_request.getSession();
        try
        {
            ...
            Element srcElement = (Element) elem.getElementsByTagName("source").item(0);
            Element tgtElement = (Element) elem.getElementsByTagName("target").item(0);
            srcLocale = ((Element) srcElement.getElementsByTagName("locale").item(0)).getFirstChild().getNodeValue();
            tgtLocale = ((Element) tgtElement.getElementsByTagName("locale").item(0)).getFirstChild().getNodeValue();
            s_logger.debug("SrcLocale is " + srcLocale);
            s_logger.debug("TgtLocale is " + tgtLocale);

            //spit out HTML for a page with two frames. One for source URLs and one for Target URLs
            PreviewUrlXmlParser p = new PreviewUrlXmlParser(p_previewUrlXml, srcLocale, tgtLocale);
            p.parse();
            UrlElement[] srcUrls = p.getSourceUrls();
            UrlElement[] tgtUrls = p.getTargetUrls();
	    s_logger.debug( "CES: There are " + srcUrls.length + " source URLs and " +
			      tgtUrls.length + " target Urls.");
	    ...
	    //this used to put two objects into the stash because each frame would
	    //pull out a separate copy. we now cannot use the session to
	    //pass information between the frames used for preview...so the singleton
	    ...
	    //this is a cleanup problem.
            ...
            theSession.setAttribute("srcEncoding",
                                    p.getSourceUrlListEncoding());
            theSession.setAttribute("tgtEncoding",
                                    p.getTargetUrlListEncoding());
            ...
            if (useSSL.equals("true"))
                cxeServer = "https://" + p_request.getServerName() + ":" + SSLPort;
            else
                cxeServer = "http://" + p_request.getServerName() + ":" + nonSSLPort;
        ...
        catch (Exception e)
        {
            ...
            try {
                showErrorPage(p_request,p_response, new CapAdapterException ("PreviewProblems",null,e));
            }
            catch (Exception ex)
            {
     ...
     * @exception Exception
     */
    private void exportForDynamicPreview(HttpServletRequest p_request) throws Exception
    {
        ...
        Integer pageCount = Integer.valueOf(p_request.getParameter(ExportConstants.PAGE_COUNT));
        Integer pageNum = Integer.valueOf(p_request.getParameter(ExportConstants.PAGE_NUM));
        String sessionId = p_request.getSession().getId();

        if (exportLocation == null)
        {
//            SystemConfiguration config = SystemConfiguration.getInstance();
//            exportLocation = config.getStringParameter(SystemConfigParamNames.CXE_DOCS_DIR);
            exportLocation = AmbFileStoragePathUtils.getCxeDocDirPath();
            s_logger.warn("ExportLocation is null, using cxe docs dir " + exportLocation);
        }

        if (localeSubDir == null)
        {
            localeSubDir = targetLocale;
            s_logger.warn("LocaleSubDir is null, using target locale " + localeSubDir);
        }
    ...
    /**
     * Waits for the JMS return message to come. That message
     * should have the sessionId as a property and should
     * contain the PreviewUrlXml
     * 
     * @param p_request
     * @return PreviewInfoHolder
     * @throws Exception
     */
    private PreviewInfoHolder waitForPreviewMessage(HttpServletRequest p_request) throws Exception
    {
        String sessionId = p_request.getSession().getId();
        s_logger.info("Waiting up to 5 minutes for dynamic preview reply for sessionId " + sessionId);
        //now wait for the reply if we're doing a preview
        InitialContext context = new InitialContext();
        TopicConnectionFactory cf = (TopicConnectionFactory) context.lookup(CxeProxy.JMS_FACTORY);
        TopicConnection topicConnection = cf.createTopicConnection();
        topicConnection.start();
        TopicSession session = topicConnection.createTopicSession(false, Session.AUTO_ACKNOWLEDGE);
        String jmsTopicName = EventTopicMap.JMS_PREFIX + EventTopicMap.FOR_DYNAMIC_PREVIEW;
        ...
        TopicSubscriber subscriber = session.createSubscriber(topic,messageSelector,false);
        long timeout = 5L * 60L * 1000L; //five minute timeout
        Message jmsMessage = subscriber.receive(timeout);
        if (jmsMessage == null)
            throw new Exception("Timeout waiting for JMS message with preview information. Waited 5 minutes.");
        ...
        info.previewUrlXml =  (String) cxeMessage.getParameters().get("PreviewUrlXml");
