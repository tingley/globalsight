...
{
    private static final long serialVersionUID = -5058390986451759501L;

    public DynamicPreviewProxyServlet() throws ServletException
    {
        try
        {
        ...
        try
        {
            ...
            System.setProperty("ja    vax.net.debug", "ssl,hands     hake,data,trustmanager");
        ...
        catch (SecurityException se)
        {
            theLogger
                    .printStackTrace(
                            Logger.ERROR,
                            "Cannot add Java Secure Sockets Extension (JSSE) security provider to handle SSL in java.",
                            se);
        ...
        catch (NoClassDefFoundError ncdfe)
        {
            theLogger
                    .printStackTrace(
                            Logger.ERROR,
                            "Cannot find Java Secure Sockets Extension (JSSE) security provider class to handle SSL in java.",
                            ncdfe);
      ...
      throws IOException, ServletException
   {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response)
            throws IOException, ServletException
    {
        theLogger.println(Logger.DEBUG_D, "DynamicPreviewProxyServlet: proxying.");
        ...
        try
        {
            ...
            theLogger.println(Logger.DEBUG_D,
                    "DynamicPreviewProxyServlet: url is " + prsexportUrl.toString());
            ...
            String line = null;
            theLogger.println(Logger.DEBUG_D,
                    "DynamicPreviewProxyServlet: reading export servlet output.");
        ...
        {
            theLogger.printStackTrace(Logger.ERROR, "Cannot proxy CapExportServlet:", e);
            out.println("<html>\n<body><p> ERROR: Cannot call CapExportServlet: " + e.toString()
                    + "</p>");
   ...
   //Always trust CAP
    class MyX509TrustManager implements X509TrustManager
    {
        ...
        {
            theLogger.println(Logger.DEBUG_D, "There are " + chain.length + " X509 certificates.");
            ...
            for (int i = 0; i < chain.length; i++)
            {
                ...
                try
                {
                    ...
                    theLogger.println(Logger.DEBUG_D, "Certificate " + i + " is valid.");
                }
                catch (CertificateExpiredException cee)
                {
                    theLogger.println(Logger.ERROR, "Certificate  " + i + " is expired.");
                }
                catch (CertificateNotYetValidException cnyve)
                {
                    theLogger.println(Logger.ERROR, "Certificate  " + i + "is not yet valid.");
                }

                theLogger.println(Logger.DEBUG_D,
                        "Certificate number is: " + theCert.getSerialNumber());
                theLogger
                        .println(Logger.DEBUG_D, "Certificate version is: " + theCert.getVersion());
                theLogger.println(Logger.DEBUG_D, "Issuer is " + theCert.getIssuerDN());
                theLogger.println(Logger.DEBUG_D, "Subject is " + theCert.getSubjectDN());
            }
        }
    }
}
