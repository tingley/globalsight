...
public class DJ_ConnectionProfile
{
    ...
    public DJ_ConnectionProfile (long p_id)
    {
		...
		String sql = "SELECT NAME, DRIVER, CONNECTION, USER_NAME, PASSWORD FROM " + CONNECTION_PROFILE + " WHERE " + CONNECTION_PROFILE_ID +
		    " = " + m_id;
	    ...
	    theLogger.printStackTrace(Logger.ERROR, "DJ_ConnectionProfile", cpe);
	}
    ...
    /////////////////////////////////////////////////
    public DJ_ConnectionProfile (long p_id, String p_profileName, String p_driver,
				 String p_connectionString, String p_userName, String p_password)
    {
        m_id = p_id;
    ...
    public boolean equals (DJ_ConnectionProfile p_profile)
    {
        if ( (m_id == p_profile.getID()) && 
	     ...
	     (0 == (m_connectionString.compareTo(p_profile.getConnectionString()))) &&
    ...
    public void setProfileName (String p_profileName) { m_profileName = p_profileName; }
    public void setDriver (String p_driver) { m_driver = p_driver; }
    public void setConnectionString (String p_connectionString) { m_connectionString = p_connectionString; }
	...
	try {
	    ...
	    if (m_id > 0)
		{
		    String sql = "DELETE FROM " + CONNECTION_PROFILE + " WHERE " + CONNECTION_PROFILE_ID + " = " + m_id;
	...
	try {
	    connection = ConnectionPool.getConnection();
	    String sql = "UPDATE " + CONNECTION_PROFILE + " SET " + NAME + "=" + Utility.quote(m_profileName) + ", " + 
		DRIVER + "=" + Utility.quote(m_driver) + ", " + CONNECTION + "=" + Utility.quote(m_connectionString) + ", " + 
		USER_NAME + "=" + Utility.quote(m_userName) + ", " + 
		PASSWORD + "=" + Utility.quote(m_password) + " WHERE " + CONNECTION_PROFILE_ID + " = " + m_id;
	    Statement s = connection.createStatement();
	    s.executeUpdate(sql);
	}
	catch (ConnectionPoolException cpe) {
	...
	try {
	    connection = ConnectionPool.getConnection();
	    String sql = "INSERT INTO " + CONNECTION_PROFILE + " VALUES(" + INCREMENT_CONNECTION_ID + ".NEXTVAL ," +
		Utility.quote(m_profileName) + "," + Utility.quote(m_driver) + "," +
		Utility.quote(m_connectionString) + "," + Utility.quote(m_userName) + "," + Utility.quote(m_password) + ")";
	    Statement s = connection.createStatement();
	    s.executeUpdate(sql);
	}
	catch (ConnectionPoolException cpe) {
	...
	for (int i=0; i<records.size(); ++i)
		...
		if ( (profile.getAcquisitionConnectID() == m_id) ||
			...
			dependency = new Dependency(profile.getID(),
						    profile.getName(), RECORD_PROFILE);
			break;
		    }
	    }
