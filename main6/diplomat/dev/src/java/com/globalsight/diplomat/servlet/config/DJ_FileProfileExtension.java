...
{    
    ...
    /////////////////////////////////////////////////
    public DJ_FileProfileExtension(WrapFileProfileExtensionTable p_fileProfileExtension)
    {
    ...
    public DJ_FileProfileExtension(long p_id, boolean p_extensionBased)
    {
        ...
        try
        {
            ...
            if (p_extensionBased)
            {
                sql = "SELECT FILE_PROFILE_ID, EXTENSION_ID FROM " + FILE_PROFILE_EXTENSION + " WHERE " + 
                    EXTENSION_ID + " = " + p_id;
            }
            else
            {
                sql = "SELECT FILE_PROFILE_ID, EXTENSION_ID FROM " + FILE_PROFILE_EXTENSION + " WHERE " + 
                    FILE_PROFILE_ID + " = " + p_id;
            }
            ...
            while (results.next())
            {
                ...
                m_fileProfileExtensionList.add(new WrapFileProfileExtensionTable(fileProfileID, extensionID));
            }
            query.close();
        }
	catch (ConnectionPoolException cpe) {
	    theLogger.printStackTrace(Logger.ERROR, "DJ_FileProfileExtension", cpe);
	}
	catch (SQLException e) {
	    theLogger.printStackTrace(Logger.ERROR, "DJ_FileProfileExtension", e);
	}
	finally {
    ...
    /////////////////////////////////////////////////
    public Vector getList() { return m_fileProfileExtensionList; }
    ...
    {
        Connection connection = null;
        String sql = "INSERT INTO " + FILE_PROFILE_EXTENSION + " VALUES ( ?, ?)";
	    ...
	    connection = ConnectionPool.getConnection();
            ...
            {
                WrapFileProfileExtensionTable entity = (WrapFileProfileExtensionTable)m_fileProfileExtensionList.elementAt(i);
    ...
    public void delete()
    {
        ...
        for (int i=0; i<m_fileProfileExtensionList.size(); ++i)
        {
            long id = ((WrapFileProfileExtensionTable) m_fileProfileExtensionList.elementAt(i)).getFileProfileID();    
            boolean found = false;
	...
	try {
	    ...
	    if (idList.size() > 0)
		{
		    ...
		    String whereClause = FILE_PROFILE_ID + " = " + ((Long)idList.elementAt(0)).longValue();
		    for (int i=1; i<idList.size(); ++i)
			whereClause += " OR " + FILE_PROFILE_ID + " = " + ((Long)idList.elementAt(i)).longValue();
		    String sql = "DELETE FROM " + FILE_PROFILE_EXTENSION + " WHERE " + whereClause;
