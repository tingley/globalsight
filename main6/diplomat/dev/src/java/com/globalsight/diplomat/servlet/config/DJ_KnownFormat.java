...
public class DJ_KnownFormat
{
    ...
    public DJ_KnownFormat(long p_id)
    {
	    ...
	    connection = ConnectionPool.getConnection();
            ...
            String sql = "SELECT NAME, FORMAT_TYPE, PRE_EXTRACT_EVENT, PRE_MERGE_EVENT FROM " + KNOWN_FORMAT_TYPE + " WHERE " + ID + " = " + m_id;
    ...
    public String getFormatType() { return m_formatType; }
    public String getPreExtractEvent() { return m_preExtractEvent; }
    public String getPreMergeEvent() { return m_preMergeEvent; }
    
    public void setID(long p_id) { m_id = p_id; }
    public void setName(String p_name) { m_name = p_name; }
    public void setFormatType(String p_formatType) { m_formatType = p_formatType; }
    public void setPreExtractEvent(String p_preExtractEvent) { m_preExtractEvent = p_preExtractEvent; }
    public void setPreMergeEvent(String p_preMergeEvent) { m_preMergeEvent = p_preMergeEvent; }
	...
	public void deleteEntry()
	{
		...
		if (m_id > 0)
		{
		    String sql = "DELETE FROM " + KNOWN_FORMAT_TYPE + " WHERE " + ID + " = " + m_id;	
	...
	public void update()
	{
	    ...
	    try {
		connection = ConnectionPool.getConnection();
		String sql = "UPDATE " + KNOWN_FORMAT_TYPE + " SET " + NAME + "=" + Utility.quote(m_name) + ", " +
	        FORMAT_TYPE + "=" + Utility.quote(m_formatType) + ", " + 
	        PRE_EXTRACT_EVENT + "=" + Utility.quote(m_preExtractEvent) + ", " + 
	...
	try {
	    connection = ConnectionPool.getConnection();
	    String sql = "INSERT INTO " + KNOWN_FORMAT_TYPE + " VALUES(" + INCREMENT_KNOWN_FORMAT_ID + ".NEXTVAL," +
		Utility.quote(m_name) + "," + Utility.quote(m_formatType) + "," + Utility.quote(m_preExtractEvent) + 
