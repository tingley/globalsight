...
public class ServletXMLProfile extends HttpServlet
{
    protected final String DEFAULT_PAGE = "/jsp/XMLProfile.jsp";
    protected final String MANAGE_PAGE = "/jsp/XMLProfileManage.jsp";
    ...
    protected final String XML_NAME = "xmlName";
    protected final String UPDATE_ME = "updateMe";

    public ServletXMLProfile() throws ServletException
    {
        try
        {
            ...
            IOException
    {
        ...
        else
        {
            theLogger.println(Logger.DEBUG_D,
                    "ServletXmlProfile: using old session");
        }
        if (manageProfile)
        ...
        theLogger.println(Logger.DEBUG_D, "ServletXmlProfile: nextPage is "
                + nextPage);
        ...
        else
        {
            theLogger
                    .println(
                            Logger.ERROR,
                            "ServletXmlProfile: the request displatcher is null. The CXE webserver must be restarted.");
        }
    }
    ...
    public void newProfile(HttpServletRequest p_request)
    {
        ...
        String emptyString = "";
        session.removeAttribute(XML_NAME);
    ...
    public void modifyProfile(HttpServletRequest p_request)
    {
        ...
        Long id = new Long(p_request.getParameter(XML_NAME));
        DJ_XML_Rule profile = new DJ_XML_Rule(id.longValue());
        session.setAttribute(XML_NAME, id);
    ...
    public void saveProfile(HttpServletRequest p_request)
    {
        ...
        {
            Long id = new Long(0);
            if (session.getAttribute(XML_NAME) != null)
                id = (Long) session.getAttribute(XML_NAME);
            ...
            DJ_XML_Rule profile = new DJ_XML_Rule(id.longValue(), profileName,
                    xml);
            profile.save();
        }
    }
