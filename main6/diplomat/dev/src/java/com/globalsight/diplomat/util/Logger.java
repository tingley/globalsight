...
public class Logger
{
	...
	/** Creates a file with the given filename in the debug file directory
	* <br>
	* @param filename The basename of the file. A fullpath will be created.
	* @param content The file content to write out.
	*/
	public static void writeDebugFile (String p_filename, String p_content)
	{
		...
		try
		{
			String fullname = theLogger.m_debugFileDirectory + File.separator + p_filename;
			theLogger.println(Logger.DEBUG_B, "Writing debug file " + p_filename + " in UTF8.");
		...
		{
			theLogger.println(Logger.ERROR, "Error writing debug file " + p_filename + ": " + fnfe);
		}
		catch (IOException e)
		{
			theLogger.println(Logger.ERROR, "Error writing debug file " + p_filename + ": " + e);
		}
                catch (Throwable t)
                {
                   theLogger.println(Logger.ERROR, "Error writing debug file " + p_filename + ": " + t);
                }
	}
	...
	* <br>
	* @param filename The basename of the file. A fullpath will be created.
	* @param content The message data containing the content
	*/
	public static void writeDebugFile (String p_filename, MessageData p_content)
	{
           ...
           {
               Logger.getLogger().println(Logger.ERROR, "Error writing debug file " + p_filename + ": " + t);
           }
	}

	/////////////////////////////////////////////////
	/** Creates a file with the given filename in $HOME/debug
	* <br>
	* @param filename The basename of the file. A fullpath will be created.
	* @param content The file content to write out.
	*/
	public static void writeDebugFile (String filename, byte[] content)
	{
		...
		try
		{
			String file = theLogger.m_debugFileDirectory + File.separator + filename;
		...
		{
			theLogger.println(Logger.ERROR, "Error writing debug file " + filename + ": " + e);
		}
                catch (Throwable t)
                {
                   theLogger.println(Logger.ERROR, "Error writing debug file " + filename + ": " + t);
                }
	}
	...
	* @throws IOException if the file cannot be created. Does nothing if setLogname()
	...
	*/
	...
	/** gets the current debug level. The debug level is set in the properties file.*/
	...
	/** Writes the string to the log file if p_messageDebugLevel is less than or equal to
	** the current debug level
	...
	/** Writes the exception's stack trace to the log file if p_messageDebugLevel is less than or equal to
	** the current debug level
        ** @deprecated do not use this class        
        */
	public void printStackTrace(int p_messageDebugLevel, String p_msg, Throwable p_throwable)
	{
	...
	public Logger()
	{
            ...
            InputStream is = Logger.class.getResourceAsStream("/properties/Logger.properties");
            props.load(is);
            //get the directories
            ...
            {
                throw new IllegalStateException("Could not initialize Logger " + e.getMessage());
            }
	}

   /**
    * Returns true if the Logger's writeDebugFiles
    * property is true
    * 
    * @return true | false
    */
