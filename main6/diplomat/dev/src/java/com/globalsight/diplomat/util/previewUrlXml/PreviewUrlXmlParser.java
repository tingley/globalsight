 ...
 * PreviewUrlXmlParser
 * <p>
 * This class can parse a PreviewUrlXml document and store its
 * contents.
 */
public class PreviewUrlXmlParser
{
   ...
   private String m_srcUrlListEncoding = "UTF-8"; //the IANA character encoding
   private String m_tgtUrlListEncoding = "UTF-8"; //the IANA character encoding

	/**
	 * Return a PreviewUrlXmlParser object that knows how to parse
	 * the given previewUrlXml for the given srcLocale and tgtLocale
	 * 
	 * @param a string containing the Preview Url Xml
	 * @param the source locale
	 * @param the target locale
	 */
	public PreviewUrlXmlParser (String p_previewUrlXml, String p_srcLocale, String p_tgtLocale)
	{
	 ...
	 * the given previewUrlXml for the given srcLocale and tgtLocale
	 * 
	 * @param a URL to a file of PreviewUrlXml
	 * @param the source locale
	 * @param the target locale
	 * @throws PreviewUrlXmlException
	 * @throws IOException
	 */
	public PreviewUrlXmlParser (URL p_previewUrlXml, String p_srcLocale, String p_tgtLocale)
	throws PreviewUrlXmlException, IOException
	{
		m_srcLocale = p_srcLocale;
		m_tgtLocale = p_tgtLocale;
		m_srcUrls = null;
	...
	* contents.
	*
	* @throws PreviewUrlXmlException if expected elements are not found.
	*/
	public void parse()
	throws PreviewUrlXmlException
	{
		...
		try
		{
			...
			// System.out.println("Got nodelist of size " + nl.getLength());
			int n;

			//first find the elements corresponding to the src and tgt locales
			for (n=0; n<nl.getLength() && (srcLocaleElement==null || tgtLocaleElement==null); n++)
			{
	...
	/**
	 * Returns an array of UrlElement objects corresponding to the
	 * source URLs in the XML.
	 *
   ...
   { return m_tgtUrlListEncoding; }
	...
	{ 
		...
		try
		{
			...
			Element urlElementParent = (Element) nl.item(0);
                        ...
                        theLogger.println(Logger.DEBUG_D,
                                          "PreviewUrlXmlParser: read encoding "
                                          + encoding + " for " + p_choice);
			...
			if (description.equals("source"))
			{
				m_srcUrls = new UrlElement[nl.getLength()];
				theUrlArray = m_srcUrls;
			}
			else
			{
				m_tgtUrls = new UrlElement[nl.getLength()];
			...
			for (n=0; n<nl.getLength(); n++)
			{
				...
				href = urlElement.getElementsByTagName("href").item(0).getFirstChild().getNodeValue();
				// System.out.println("\tHREF = " + href);
				label = urlElement.getElementsByTagName("label").item(0).getFirstChild().getNodeValue();

				//now print out args	
				NodeList urlArgs = urlElement.getElementsByTagName("arg");
				ArgElement[] theArgs = new ArgElement[urlArgs.getLength()];
				for (int j=0; j<urlArgs.getLength();j++)
				{
					...
					parameterNode = (Element)argNode.getElementsByTagName("parameter").item(0);
					parameter = parameterNode.getFirstChild().getNodeValue();
					par_subSource = parameterNode.getAttribute("substitution_source");
					...
					if (valueList.getLength() > 0)
					{
						...
						val_subSource = valueNode.getAttribute("substitution_source");
					...
					theArgs[j] = new ArgElement(parameter, par_subSource,
												value, val_subSource);
					/*System.out.println(theArgs[j].getParameter() +"="+ theArgs[j].getValue()
		...
		{
		    Logger.getLogger().println(Logger.ERROR, "There are no " + description + " URLs for locale " + theLocale);
		    throw new PreviewUrlXmlException (e.getMessage());
		}
	}
	...
	throws Exception
	{
		if (args.length < 3)
		{
			System.out.println("USAGE: PreviewUrlXmlParser <url> <srcLocale> <tgtLocale>");
			System.exit(0);
		}
		...
		PreviewUrlXmlParser p = new PreviewUrlXmlParser(new URL(url),srcLocale,tgtLocale);
		p.parse();
		...
		{
			u = srcUrls[i];
			System.out.println(u.getLabel() + " " + u.getType() + " " + u.getHref());
