/* http://www1.chapman.edu/~jipsen/mathml/ASCIIMathML143LGPL.js */
/*
ASCIIMathML.js
==============
...
Version 1.4.3 Mar 24, 2005, (c) Peter Jipsen http://www.chapman.edu/~jipsen
...
Lesser General Public License (at http://www.gnu.org/copyleft/lesser.html)
for more details.
*/

var checkForMathML = true;    // check if browser can display MathML
var notifyIfNoMathML = true;  // put note at top of page if no MathML capability
var AMmathcolor = "red";      // change it to Black or any other preferred color
var AMdisplaystyle = true;    // puts limits above and below large operators
var AMseparatetokens = false; // if true, letter tokens must be separated by nonletters
var AMdoubleblankmathdelimiter = false; // if true,  x+1  is equal to `x+1`
                                        // for IE this works only in <!--   -->
var AMisIE = document.createElementNS==null;
...
function AMnotAvailableMessage()
{
    var nd = AMcreateElementXHTML("span");
    nd.appendChild(document.createTextNode(
		"To view the MathML rendering, install "));
...
function AMisMathMLavailable()
{
    if (navigator.appName.slice(0,8)=="Netscape")
    {
        ...
        else return AMnotAvailableMessage();
    }
...
function AMparseMath(str) 
{
	...
	if (AMmathcolor) node.setAttribute("mathcolor", AMmathcolor);
	if (AMdisplaystyle) node.setAttribute("displaystyle", "true");
	AMnestingDepth = 0;
			...
			if (arr.length>1 || mtch) 
			{
				if (checkForMathML) 
				{
					...
					if (AMnoMathML && notifyIfNoMathML)
					{
						...
						var el = document.getElementById('idWarning');
						if (el) el.appendChild(nd);
					}
				}
				if (!AMnoMathML)
				{
					n.parentNode.replaceChild(
						AMstrarr2docFrag(arr,n.nodeType==8),n);
				}
			}
		}
	}
...
function AMunzoomMathPlayer(node)
{
    // Removes spans that the MathPlayer inserts to zoom
    // <m:math> expressions. The DHTML structure is this:
    // <span><span zoom><m:math></span><m:math orig></span>
    ...
    while (found)
    {
        ...
        for (var i = 0; i < spans.length; i++)
        {
            ...
            if (span.childNodes.length == 2 &&
                span.childNodes[0].tagName == "SPAN" &&
                span.childNodes[1].tagName == "math")
            {
                var math = span.childNodes[1];
                span.parentElement.insertBefore(math, span);
