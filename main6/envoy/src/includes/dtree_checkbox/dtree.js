...
function dTree(objName, iPath, formId) {
	...
	this.icon = {
		root : iPath + '/includes/dtree_checkbox/img/base.gif',
		folder : iPath + '/includes/dtree_checkbox/img/folder.gif',
		folderOpen : iPath + '/includes/dtree_checkbox/img/folderopen.gif',
		node : iPath + '/includes/dtree_checkbox/img/page.gif',
		empty : iPath + '/includes/dtree_checkbox/img/empty.gif',
		line : iPath + '/includes/dtree_checkbox/img/line.gif',
		join : iPath + '/includes/dtree_checkbox/img/join.gif',
		joinBottom : iPath + '/includes/dtree_checkbox/img/joinbottom.gif',
		plus : iPath + '/includes/dtree_checkbox/img/plus.gif',
		plusBottom : iPath + '/includes/dtree_checkbox/img/plusbottom.gif',
		minus : iPath + '/includes/dtree_checkbox/img/minus.gif',
		minusBottom : iPath + '/includes/dtree_checkbox/img/minusbottom.gif',
		nlPlus : iPath + '/includes/dtree_checkbox/img/nolines_plus.gif',
		nlMinus : iPath + '/includes/dtree_checkbox/img/nolines_minus.gif',
		expandAll : iPath + '/includes/dtree_checkbox/img/expand_all.gif',
		collapseAll : iPath + '/includes/dtree_checkbox/img/collapse_all.gif',
		remove: iPath + '/includes/dtree_checkbox/img/trash.png'
	};
	this.parEnum = "";
	this.obj = objName;
	this.aNodes = [];
...
dTree.prototype.add = function(id, pid, name, value, url, title, target, icon,
		iconOpen, open) {
	this.aNodes[this.aNodes.length] = new Node(id, pid, name, value, url, title,
...
dTree.prototype.toString = function() {
	var str = '<div class="dtree"><span id="' + this.formId + '" >\n';
...
dTree.prototype.node = function(node, nodeId) {
	var str = '<div class="dTreeNode" style="';
	if (nodeId == 0){
		str += 'height:0px;';
	} else {
		str += 'height:18px;line-height:18px';
	}
	...
	if (this.config.useCheckBox) {
		if (nodeId != 0) {
			...
			str += '<input type="checkbox" name="fileId" id="c' + this.parEnum
...
function caBox(formId, regx) {
	var checkboxes = document.getElementsByName("fileId");
	if (document.getElementById(regx).checked) {
		...
		// checked prev node
		for (var j = 0; j < regxArray.length; j++) {
			var pDiv = regx.substring(0, regx
					.indexOf(regxArray[regxArray.length - j - 1] + "-"));
		...
		for (var j = 0; j < regxArray.length - 1; j++) {
			if (!isCheckedByRec(checked, regx.substring(0, regx
...
function isCheckedByRec(checkboxes, regx) {
	for ( var i = 0; i < checkboxes.length; i++) {
		var element = checkboxes[i];
		// isChecked
