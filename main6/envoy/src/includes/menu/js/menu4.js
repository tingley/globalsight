...
Menu.prototype.createPopup = function () {
	...
	else
	{
		if ( pm == null )
			...
			w = pm.getDocument().defaultView;
		this.popup = pm.getDocument().defaultView;
	}
...
MenuItem.prototype.setEnabled = function (arg) {
    this.disabled = !arg;
}
...
MenuBar.prototype.detachEvents = function () {
	...
	{		
		this._htmlElement.addEventListener( "onmouseover",this.eventListeners.onmouseover, false);
		this._htmlElement.addEventListener( "onmouseout", this.eventListeners.onmouseout, false);
		this._htmlElement.addEventListener( "onmousedown", this.eventListeners.onmousedown, false);
		document.addEventListener( "onkeydown", this.eventListeners.onkeydown, false);
		window.removeEventListener("onunload", this.eventListeners.onunload, false);
	}
}
...
MenuBar.prototype.hookupMenu = function ( element ) {
	...
	{
		element.addEventListener( "onmouseover",this.eventListeners.onmouseover, false);
		element.addEventListener( "onmouseout", this.eventListeners.onmouseout, false);
		element.addEventListener( "onmousedown", this.eventListeners.onmousedown, false);
		document.addEventListener( "onkeydown", this.eventListeners.onkeydown, false);
		...
		onmousedown:	function ( id ) {
			...
			if ( el != null ) {
				...
				if ( mb.subMenu ) {
					...
					if ( new Date() - mb.subMenu._closedAt > 100 ) {	// longer than the time to											// do the hide
						mb.dispatchAction();
					}
					else {
						mb._hover = true;
