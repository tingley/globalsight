...
/**
 * The AlignerMDB is a JMS Message Driven Bean that is used to send
 * asynchronous messages to back-end Aligner services. This class is
 * also used to receive the final GXML for aligner import requests.
 */
public class AlignerMDB extends GenericQueueMDB
{
    ...
    /**
     * This is the JMS service activator for whatever the Aligner
     * needs to do.
     *
     * @param p_cxeRequest The JMS message containing the request for
     * alignment or alignment services
     */
    ...
    public void onMessage(Message p_cxeRequest)
    {
        try
        {
            ...
            CompanyThreadLocal.getInstance().setIdValue((String) hm.get(CompanyWrapper.CURRENT_COMPANY_ID));
            
            String contentFileName = (String)hm.get(CxeToCapRequest.CONTENT);
            String eventFlowXml = (String)hm.get(CxeToCapRequest.EVENT_FLOW_XML);
            ...
            AlignerExtractorResult aeResult = new AlignerExtractorResult(
                gxml, eventFlowXml, exception);
            AlignerExtractor ae = AlignerExtractor.getAlignerExtractor(
                alignerExtractorName);

            ae.addToResults(aeResult,
                (String)hm.get("PageCount"),
