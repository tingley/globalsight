...
public class CVSUtil {
	...
	public static int copyFiles(String p_source, String p_target) {
		logger.info("copyFiles(). Source file==" + p_source + ", Target file==" + p_target);
		...
		try {
			...
			if (files.length > 0) {
				...
				for (int j = 0; j < subFiles.size(); j++) {
					...
					//results.add(sFile);
					fileName = sFile.substring(sFile.lastIndexOf(File.separator));
	...
	private static void saveFileInfo(String p_jobName, String p_notes, ArrayList<File> p_files,
			CVSModule p_module) {
	...
	public static HashMap<String, String> seperateFileInfo(String p_file, String p_exportLocation) {
        ...
        if (jobName.length() + 1 < tmp.lastIndexOf(File.separator)) {
        	moduleName = tmp.substring(jobName.length() + 1, tmp.lastIndexOf(File.separator));
        } else {
        	moduleName = "";
        }
	...
	public static void saveCVSFile(HashMap<String, String> infos, String p_srcLocale) {
		try {
			if (infos == null)
				return;
			...
			if(CompanyWrapper.getCurrentCompanyId().equals("1"))
			{
				...
				companyIdStr = job.getCompanyId();
				companyId = new Long(companyIdStr).longValue();
				CompanyWrapper companyWrapper = new CompanyWrapper();
				Company company = companyWrapper.getCompanyById(job.getCompanyId());
			...
			HashMap result = helper.getTargetModuleMapping(companyId, sourceLocale, targetLocale, moduleId, sourceModule);
			String targetFile = "";
			String workDir = "";
			...
			if (result != null && result.get("type") != null) {
				...
				if ("0".equals(type)) {
					...
					if (t.isDirectory())
						targetFile += File.separator + getMappingRename(mm, filename, "0");
				} else {
					targetFile = targetFile.concat(mm.getTargetModule());
					File t = new File(targetFile);
					if (t.isDirectory())
						targetFile += File.separator + getMappingRename(mm, filename, mm.getSubFolderMapped());
					else {
						filename = getMappingRename(mm, filename, "1");
						targetFile = targetFile.concat(mm.getTargetModule()).concat(File.separator).concat(filename);
					}
				}
				isNoMapping = false;
				filename = targetFile.substring(targetFile.lastIndexOf(File.separator) + 1);
				if(CompanyWrapper.getCurrentCompanyId().equals("1"))
				{
					workDir = getBaseDocRootByCompany(companyIdStr).concat(mm.getTargetModule());
				}
				else
				{
				...
				targetFile = targetFile.concat(sourceModulePath).concat(File.separator).concat(targetLocale).concat(File.separator).concat(filename);
				if(CompanyWrapper.getCurrentCompanyId().equals("1"))
				{
					workDir = getBaseDocRootByCompany(companyIdStr).concat(sourceModulePath);
				}
				else
				{
			...
			//When commit back files, cvsroot can't be specified,otherwise there will be 
			...
			if (isNoMapping) {
				...
				cmd = new String[]{"cvs", "-d", cvsRoot, "add", targetLocale};
				exeCVSCmd(cmd, workDir);
				cmd = new String[]{"cvs", "ci", "-m", "\"" + jobNotes + "\"", targetLocale};
				exeCVSCmd(cmd, workDir);
				workDir += File.separator + targetLocale;
				
				cmd = new String[]{"cvs", "-d", cvsRoot, "add", filename};
				exeCVSCmd(cmd, workDir);
				cmd = new String[]{"cvs", "ci", "-m", "\"" + jobNotes + "\"", filename};
				exeCVSCmd(cmd, workDir);
			} else {
				workDir = targetFile.substring(0, targetFile.lastIndexOf(File.separator));
	...
	private static String getMappingRename(ModuleMapping p_mm, String p_filename, String p_flag) {
		if (p_mm == null)
			return "";
		String result = "";
		try {
			ModuleMappingManagerLocal manager = new ModuleMappingManagerLocal();
			ModuleMapping kmm = manager.getModuleMapping(p_mm.getSourceLocale(), p_mm.getTargetLocale(), p_mm.getSourceModule(), p_mm.getTargetModule(), String.valueOf(p_mm.getCompanyId()), p_flag);
			if (kmm != null) { 
				...
				for (ModuleMappingRename r : renames) {
					...
					if (source.indexOf("*") != -1 && target.indexOf("*") != -1) {
						...
						result = p_filename.replaceAll(r.getSourceName(), r.getTargetName());
				}
			}
			if ("".equals(result))
				return p_filename;
	...
	private static String rename(String p_filename, String p_source, String p_target){
		...
		try {
			...
			for (int i=0;i<tmpA.length;i++) {
				...
				if (l != -1) {
					tmp += p_filename.substring(k, l).concat(tmpB[i].equals("")?tmpA[i]:tmpB[i]);
					l = l + tmpA[i].length();
					k = l;
				} else {
					tmp = p_filename;
	...
	public static boolean isCVSEnabled() {
		...
		try {
			ProcessBuilder builder = new ProcessBuilder(new String[]{"cvs"});
			// set working directory
			if (workDirectory != null) {
				builder.directory(new File(workDirectory));
			}
	...
	public static ArrayList<String[]> getModuleList(String[] cmd, String workDirectory)	throws IOException, InterruptedException {
	...
	public static boolean isCheckedOut(String p_serverName, String p_moduleName) {
		...
		try {
			File f = new File(getBaseDocRoot() + p_serverName + File.separator + p_moduleName);
