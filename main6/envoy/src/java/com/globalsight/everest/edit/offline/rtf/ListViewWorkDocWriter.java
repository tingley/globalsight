...
/**
 * <p>Reads a p-tag input file and generates an RTF LIst-View file
 * that can be translated.</p>
 *
 ...
 *
 * <p>The input is read as characters from a InputStream object, the
...
public class ListViewWorkDocWriter extends RTFWriterUnicode
{
     ...
     * This string is used in regular expressions to recognize our
     * files programatically.  See UploadPageHandler where document
     * recognition is performed.
     */
    static public final String WORK_DOC_TITLE =
        "GlobalSight Extracted List-View Export";
        ...
        throws IOException, AmbassadorDwUpException
    {
        ...
        if (m_isTradosRtf && m_TagEditorDownload &&
            osd != null && osd.isCopyOfSource())
        {
            writeTradosTU("External Tools Helper Segment - DO NOT TRANSLATE",
                "External Tools Helper Segment - DO NOT TRANSLATE",
                0, false, false, osd.getDisplaySegmentFormat(),
                osd.getSegmentIssue());
        }
     ...
     *
     * TRADOS Note:
     * We had a problem in TagEditor where the tw4winexternal style
     * bled into the TRADOS OPEN segment box when opening a segment
     * and typing at the first character position. To fix this, we
     * moved "\\par\\pard" outside of the group in all possible
     * paragraphs that may proceed our segment paragraph. This
     * prevented the style from bleeding.
     */
        ...
        throws IOException, RegExException
    {
        ...
        //else
        //{
        srcSegment = p_osd.getDisplaySourceTextWithNewLinebreaks(
        ...
        //else
        //{
        trgSegment = p_osd.getDisplayTargetTextWithNewLineBreaks(
            String.valueOf(NORMALIZED_LINEBREAK));
        //}
        ...
        if (m_isTradosRtf)
        {
            writeTradosTU(srcSegment, trgSegment, matchValue,
                p_osd.isWriteAsProtectedSegment(),
                p_osd.isCopyOfSource(), p_osd.getDisplaySegmentFormat(),
                p_osd.getSegmentIssue());
        }
        else
        {
    ...
    private void writeNormalTU(String p_srcSegment, String p_trgSegment,
        ...
        String p_segNativeFormat, Issue p_issue)
        throws IOException
    {
//        boolean protect = false;
        ...
        else
        {
            ...
            else
            {
                ...
                writeRTF_docSegment(null, p_trgSegment,
                    m_strTargetTextStyle, m_strInternalStyle,
                    m_sourceIsRtlLang, false, p_segNativeFormat, false,
                    p_issue, m_user);
            }
        }
        ...
        throws IOException
    {
        ...
        //    m_sourceIsRtlLang);
        writeRTF_docSegment(null, p_srcSegment,
            m_strSourceTextStyle + HIDDEN + " ", m_strInternalStyle,
            m_sourceIsRtlLang, false, p_segNativeFormat, false,
            p_issue, m_user);
        //m_outputStream.write("}");
        ...
        if (p_isCopyOfSource)
        {
            ...
            //m_outputStream.write("}");
            writeRTF_docSegment(null, p_trgSegment, m_strSourceTextStyle,
                m_strInternalStyle, m_sourceIsRtlLang, false, p_segNativeFormat,
                false, null, null);
        }
        else
        {
     ...
     * prevented the style from bleeding.
     */
    private void writeResources(OfflineSegmentData p_osd)
        throws IOException
    {
        ...
        boolean atnEnabled =
            (m_segmentCounter <= m_page.getAnnotationThreshold());
        ...
        if (atnEnabled && m_resInsOpt == AmbassadorDwUpConstants.MAKE_RES_ATNS)
        {
            ...
            if (!m_isTradosRtf)
            {
                src_atn = makeSourceAnnotation(
                    ...
                    AmbassadorDwUpConstants.LABEL_ANNOTATION_ID_SOURCE, p_osd);
            ...
            if (!p_osd.isWriteAsProtectedSegment())
            {
                ...
                if (p_osd.hasTerminology())
                {
                    term_atn = makeTermAnnotation(
                        ...
                        AmbassadorDwUpConstants.LABEL_ANNOTATION_ID_TERM, p_osd);
                    has_term_atn = (term_atn.length() > 0);
                }

                if (has_tm_atn)
                {
                    m_outputStream.write((has_src_atn ? ", " + tm_atn : tm_atn));
                    m_outputStream.write(m_strEOL);
                }
                ...
                {
                    m_outputStream.write(
                        (has_src_atn || has_tm_atn ? ", " + term_atn : term_atn));
                    m_outputStream.write(m_strEOL);
                }
            }
        ...
        else // insert single hyperlink to resource page
        {
            StringBuffer sb = new StringBuffer();
            ...
            // escaped for rtf
            String url =
                DownloadHelper.makeMSWordResParentPath() + m_page.getPageId() +
                "\\\\\\\\" + m_page.getPageId() + ".html";

            m_outputStream.write(
                makeMsWordHyperLink(url, p_osd.getDisplaySegmentID(),
                    AmbassadorDwUpConstants.LINK_TIP_RESPAGE,
                    sb.toString(),
        ...
        throws IOException, AmbassadorDwUpException
    {
        ...
        m_outputStream.write(
            makeMsWordHyperLink(AmbassadorDwUpConstants.FILE_MAIN_HELP,
                "Anchor", AmbassadorDwUpConstants.LINK_TIP_MAIN_HELP,
    ...
    private String makeMsWordHyperLink(String p_url, String p_anchorName,
        String p_screenTip, String p_linkText, String p_style, String p_color)
    {
        StringBuffer sb = new StringBuffer();
        ...
        OfflineSegmentData p_segData)
    {
            ...
            (p_segData.getTermLeverageMatchList().size() > 0))
        {
            ...
            if (it.hasNext())
            {
                ...
                while (it.hasNext())
                {
                    sb.append(formatTermAtnEntry(
                        (TermLeverageMatchResult)it.next()));
                    sb.append("\\par");
                }
    ...
    /**
     * Helper function: returns segment details formatted for a
     * segment details annotation.
     */
    private String formatSegmentDetailsAtnEntry(OfflineSegmentData p_osd)
    {
        ...
        String [] keys =
            HtmlTableWriter.getSortedPtagKeys(p_osd.getPTag2NativeMap());
        if (keys.length > 0)
        {
    ...
    /**
     * Helper function: returns a TM entry formatted to fit within a
     * TM annotation.
     *
    ...
    private String formatTMAtnEntry(OfflineSegmentData p_osd)
    {
        ...
        for (int i = 0; i < l2.size(); i++)
        {
            ...
            sb.append((lm == null) ? "\\i [??] \\i0   " :
                "\\i [" + StringUtil.formatPercent(lm.getScoreNum(), 2) + "%]\\i0   ");

            sb.append(formatSegmentText((String)l2.get(i),
                m_strInternalStyle, m_targetIsRtlLang, false,
                p_osd.getDisplaySegmentFormat(), false));
    ...
    /**
     * Helper function: returns a term entry formatted to fit within a
     * term annotation.
     */
