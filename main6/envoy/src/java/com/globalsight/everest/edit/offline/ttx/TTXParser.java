...
//	private String latestPosition = null;
	...
	private void parseToolSettings(Element p_root)
	{
		...
		try 
		{
	        ...
	        Attribute creationDateAtt = 
	            ele.attribute(TTXConstants.TOOLSETTINGS_ATT_CREATIONDATE);
	        creationDate = 
	            (creationDateAtt==null?"":creationDateAtt.getValue());
	        
	        Attribute creationToolAtt = 
	            ele.attribute(TTXConstants.TOOLSETTINGS_ATT_CREATIONTOOL);
	        creationTool = 
	            (creationToolAtt==null?"":creationToolAtt.getValue());
	        
	        Attribute creationToolVersionAtt = 
	            ele.attribute(TTXConstants.TOOLSETTINGS_ATT_CREATIONTOOLVERSION);
	        creationToolVersion = 
	            (creationToolVersionAtt==null?"":creationToolVersionAtt.getValue());
		}
		catch (Exception e) 
		{
	...
	private void parseUserSettings(Element p_root)
	{
		...
		try 
		{
	        ...
	        Attribute dataTypeAtt = 
	            ele.attribute(TTXConstants.USERSETTINGS_DATA_TYPE);
	        dataType = (dataTypeAtt==null?"":dataTypeAtt.getValue());
	        
	        Attribute o_encodingAtt = 
	            ele.attribute(TTXConstants.USERSETTINGS_O_ENCODING);
	        o_encoding = (o_encodingAtt==null?"":o_encodingAtt.getValue());
	        
	        Attribute trgLangAtt = 
	            ele.attribute(TTXConstants.USERSETTINGS_TARGET_LANGUAGE);
	        targetLanguage = (trgLangAtt==null?"":trgLangAtt.getValue());
	        
	        Attribute srcDocumentPathAtt = 
	            ele.attribute(TTXConstants.USERSETTINGS_SOURCE_DOCUMENT_PATH);
	        sourceDocumentPath = 
	            (srcDocumentPathAtt==null?"":srcDocumentPathAtt.getValue());
	        
	        Attribute userIdAtt = 
	            ele.attribute(TTXConstants.USERSETTINGS_USERID);
	        userId = (userIdAtt==null?"":userIdAtt.getValue());
	        
	        Attribute dataTypeVersionAtt = 
	            ele.attribute(TTXConstants.USERSETTINGS_DATA_TYPE_VERSION);
	        dataTypeVersion = 
	            (dataTypeVersionAtt==null?"":dataTypeVersionAtt.getValue());
	        
	        Attribute settingsPathAtt = 
	            ele.attribute(TTXConstants.USERSETTINGS_SETTINGS_PATH);
	        settingsPath = 
	            (settingsPathAtt==null?"":settingsPathAtt.getValue());
	        
	        Attribute settingsNameAtt = 
	            ele.attribute(TTXConstants.USERSETTINGS_SETTINGS_NAME);
	        settingsname = 
	            (settingsNameAtt==null?"":settingsNameAtt.getValue());
	        
	        Attribute trgDefaultFontAtt = 
	            ele.attribute(TTXConstants.USERSETTINGS_TARGET_DEFAULT_FONT);
	        targetDefaultFont = 
	            (trgDefaultFontAtt==null?"":trgDefaultFontAtt.getValue());
		}
		catch (Exception e) 
		{
	...
	private void appendHeaderInfo()
	{
        ...
        results.append("# Encoding: ").append(this.gs_Encoding).append(
                TTXConstants.NEW_LINE);
        results.append("# Document Format: ").append(this.gs_DocumentFormat)
                .append(TTXConstants.NEW_LINE);
        results.append("# Placeholder Format: ").append(
                this.gs_PlaceholderFormat).append(TTXConstants.NEW_LINE);
        results.append("# Source Locale: ").append(this.gs_SourceLocale)
                .append(TTXConstants.NEW_LINE);
        results.append("# Target Locale: ").append(this.gs_TargetLocale)
                .append(TTXConstants.NEW_LINE);
        results.append("# Page ID: ").append(this.gs_PageID).append(
                TTXConstants.NEW_LINE);
        results.append("# Workflow ID: ").append(this.gs_WorkflowID).append(
                TTXConstants.NEW_LINE);
        results.append("# Task ID: ").append(this.gs_TaskID).append(
                TTXConstants.NEW_LINE);

        // Optional
        results.append("# Exact Match word count: ").append(
                this.gs_ExactMatchWordCount).append(TTXConstants.NEW_LINE);
        results.append("# Fuzzy Match word count: ").append(
                this.gs_FuzzyMatchWordCount).append(TTXConstants.NEW_LINE);
...
//		public static final short 	UNKNOWN_NODE 	14
    ...
    private void elementNodeProcessor(Node p_node, boolean p_isSource)
    {
        ...
        else if (TTXConstants.UT.equalsIgnoreCase(elementName))
        {
            ...
            if (att != null)
            {
                ...
                if (isTuId)
                {
                    ...
                    results.append(TTXConstants.HASH_MARK).append(tuId).append(
                            TTXConstants.NEW_LINE);
        ...
        if (element.nodeCount() > 0)
        {
            ...
            while (nodesIt.hasNext())
            {
                ...
                {
                    Attribute langAtt = 
                        ((Element) node).attribute(TTXConstants.TUV_ATT_LANG);
                    String lang = null;
                    if (langAtt != null)
                    {
        ...
        else
        {
            ...
            {
                Attribute displayTextAtt = 
                    element.attribute(TTXConstants.UT_ATT_DISPLAYTEXT);
                if (displayTextAtt != null)
                {
                    String attValue = displayTextAtt.getValue();
                    if (attValue != null
                    ...
                    {
                        Attribute typeValueAtt = 
                            element.attribute(TTXConstants.UT_ATT_TYPE);
	...
	private boolean isInTargetTuv(Node p_node)
	{
	    ...
	    while (parentNode != null)
	    {
	        ...
	        {
                Attribute langAtt = 
                    parentNode.attribute(TTXConstants.TUV_ATT_LANG);
	            String attName = langAtt.getValue();
                if (langAtt != null
                        && targetLanguage.equalsIgnoreCase(attName))
                {
                    result = true;
                }
