...
public class JobDispatchEngineLocal implements JobDispatchEngine
{
    ...
    private static final String PENDING_AUTOMATIC =
        Job.PENDING + "_" + AUTOMATIC;
    private static final String DISPATCHED = Job.DISPATCHED;
    private static final String 
        READY = Job.READY_TO_BE_DISPATCHED + "_" + Job.MANUAL;

    private static final String PENDING_MANUAL = 
        Job.PENDING + "_" + Job.MANUAL;
        ...
        if (it!=null)
        {
            while (it.hasNext())
            {
                ...
                try {
                    ...
                    m_jobDispatchManager.put(new Long(job.getId()), jobDispatcher);
                }
                catch (Throwable t)
                {
                    String jobName = null;
                    ...
                    c_category.warn("Failed to dispatch pending job " + jobName,t);
                }
            }
        }
    }
    ...
    /**
    * This method is used to create a job dispatcher. When a job is created
    * a job dispatcher must also be created. A one to one relationship exists
    * between JobDispatcher and a Job.
    * @param Job p_job
    * @throws JobException, RemoteException
    */
    public void createDispatcher(Job p_job)
    throws JobException, RemoteException
    {
        ...
        {   
            ...
            {
                c_category.debug("Creating a job dispatcher for the job with Id"
                                 + jobId);
            }
    ...
    */
     ...
     * @param p_reimport - Is this from reimport?
     */
...
//                                         + new Long(tc.getId()).toString() + "/" ;
                                String f = AmbFileStoragePathUtils.getCommentReferenceDirPath()
                                    + File.separator + tc.getId() + File.separator;
    ...
    /**
    * This is a call back method which a Job TimerService will invoke to denote the 
    * completion of a time interval. This method is used solely for jobs marked
    * with the automatic dispatch tag
    * @param Job p_job
    * @throws JobException, RemoteException
    */
    public void timerTriggerEvent(Job p_job)
    throws JobException, RemoteException
    {
        try
        {
    ...
    /**
    * This is a method called by JobAdditionEngine whenever a request is added to a job
    * @param Job p_job. If the word count exceeds the word count in the dispatch criteria
    * the job is dispatched
    * @param Job p_job
    * @throws JobException, RemoteException
    */
    public void wordCountIncreased(Job p_job)
    throws JobException, RemoteException
    {
        JobDispatcher jobDispatcher = getDispatcher(p_job);
    ...
    */
    ...
    throws JobException,RemoteException
    {
        ...
        else
        {
            ...
            try
            {
                ...
                jobImplClone = (JobImpl)session.get(JobImpl.class, new Long(job.getId()));
    ...
    private JobDispatcher getDispatcher(Job p_job)
    {
        ...
        JobDispatcher jd = (JobDispatcher)m_jobDispatchManager.get(getId(p_job));
        // if it can't find one then just create one for the job
        // this may occur on start-up when a BATCH_RESERVED job moves to IMPORT_FAIL and
        // then DISPATCHED.  One was never created for a BATCH_RESERVED job
        if (jd == null)
        {
            jd = new JobDispatcher(p_job);
            // add to the list - because any code that calls this also removes it from the collection
