...
/**
 * This class imports a file that is un-extracted.  It creates
 * the source pages and target pages and copies the file
 * to the appropriate place for user access.
 */
public class UnextractedFileImporter
...
{  
     ...
     * GlobalSightLocale.
     */
    HashMap importFile(Request p_request)
        throws FileImportException
    {
        ...
        try
        {
            ...
            String userId = p_request.getL10nProfile().getProject().getProjectManagerId();

            // copy the file to the internal storage
            UnextractedFile sourceUf = (UnextractedFile)sourcePage.getPrimaryFile();
            storeFile(contentFileName, sourceUf, userId, false);
            
            // persist the target pages
            targetPages = createTargetPages(p_request,
                                            sourcePage);
            ...
            for(Iterator i = targetPages.iterator() ; i.hasNext() ;)
            {
                ...
                String localeName =
                    targetPage.getGlobalSightLocale().toString();
                String alternateTargetPage = (String) alternateTargetPages.get(
                    localeName);
            ...
            // save all the updates to the UnextractedFiles - like file modification and
            // length
            UnextractedFileImportPersistenceHandler uf = new
                UnextractedFileImportPersistenceHandler();
        ...
        try
        {
            ...
            {
                getPageEventObserver().notifyImportSuccessEvent(sourcePage, targetPages);
            }
            else
            {
                c_logger.info("Import failed - updating the state.");
                getPageEventObserver().notifyImportFailEvent(sourcePage, targetPages);
            }
        }
        catch (Exception e)
        {
    ...
    /**
     * Gets a mapping of locale codes to alternate
     * target pages that should be used instead of a copy
     * of the source page.
     * 
     ...
     * @exception Exception
     */
    private HashMap findAlternateTargetPages(Request p_request) throws GeneralException
    {
        try {
        ...
        {
            c_logger.error("Problem parsing EventFlowXml for alternate target pages.", e);
        ...
        throws FileImportException
    {
        ...
        try
        {
            sp = getPageManager().getPageWithUnextractedFileForImport(p_request,
                                                           getSourceLocale(p_request),
                                                           0);
        }
    ...
    /**
     * Creates the target pages associated with the source page and
     * the target locales specified in the L10nProfile as part of the
     * request.
     */
        ...
        throws FileImportException
    {
        ...
        try
        {
            List targetLocales = Arrays.asList(
            		p_request.getTargetLocalesToImport());
        ...
        {     
            ...
            throw new FileImportException(
                FileImportException.MSG_FAIL_TO_CREATE_TARGET_PAGES, args, pe);
        }
        catch (Throwable e)
        {
            c_logger.error("Exception occurred when trying to " +
            ...
            args[1] = Long.toString(p_request.getId());
            throw new FileImportException(
                FileImportException.MSG_FAIL_TO_CREATE_TARGET_PAGES,
        ...
        throws FileImportException
    {
        try
        {
            UnextractedFile uf = getNativeFileManager().copyFileToStorage(p_fileName, p_uf, 
                                                                          p_removeOriginal);
        ...
        catch (NativeFileManagerException e)
        {
            ...
            throw new FileImportException(FileImportException.MSG_FAILED_TO_COPY_FILE_TO_STORAGE,
                                          args, e);
        }
    }
     ...
     */
    private NativeFileManager getNativeFileManager()
        throws FileImportException
    {
