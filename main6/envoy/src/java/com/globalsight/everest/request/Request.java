...
public interface Request
{
    ...
    /**
     * The localization request types that are valid.  They are
     * greater than 0.
     */
    ...
    /**
     * The error requests are negative so they are easier to find.
     * Can just check if the type is < 0 to determine if it is an
     * error.
     */
    ...
    /**
     * Returns the Event Flow XML associated with this request.
     * This information is needed by CXE when the request is
     * finished being localized and must be exported to CXE.
     *
     ...
     *
     * @return String The unique name for the page to be associated/or
     * already associated with this request.
     */
     ...
     *
     * @param p_externalPageId The unique name to give the page.
     * @exception RequestHandlerException Returns an exception if the
     * page has already been set within the request and the names
     * are different.
     */
    ...
    /**
     * Returns the GXML or PRSXML associated with this request.
     * This XML is created in CXE and passed to CAP
     * NOTE: this could either be a temp filename or a string of GXML.
    ...
    /**
     * Sets the GXML or PRSXML associated with this request.
     * NOTE: this could either be a temp filename or a string of GXML/PRSXML.
     * This field is not persisted.
     */
     ...
     *
     * @return BatchInfo Returns the batch information for this
     * request or null if this request isn't part of a batch.
     */
    ...
    /**
     * Gets the encoding (code/char set) of the page (GXML) to be or
     * already associated with this request.
     */
     ...
     *
     * @return The exception associated with this request.  Will be
     * null if there isn't an exception.
     */
     ...
     * @exception RequestHandlerException - couldn't deserialize the
     * exception
     */
    ...
    /**
     * Returns the base href specified by CXE.
     * This may be an empty string if none was specified.
     */
    public String getBaseHref();

    /**
     * Returns the id of the workflow template that is associated with
     * this request and with the target locale specified.
     * The id is the unique identifier for the template.
     *
     * @param GlobalSightLocale The locale to find the template for.
     * @return long The id of the workflow template.
     *
     * @exception An exception is thrown if the template can't be
     * found for the locale specified.
     */
     ...
     *
     * @return SourcePage The page that was created from parsing the
     * GXML in this request.  This can be null if the page hasn't
     * been created/set yet.
     */
    public SourcePage getSourcePage();

    /**
     * Sets the source page.  This is not set on a query for a
     * request, so must be set by the code.
     */
     ...
     *
     * @param p_type The value that maps to a particular datasource
     * (database, filesystem, content management system)
     * @exception RequestHandlerException An exception is thrown if
     * the page has already been set.  At this point the datasource
     * type can't be changed.
     */
    public void setDataSourceType(String p_type)
        throws RequestHandlerException;
     ...
     * @exception Returns an exception if it couldn't retrieve the
     * data source name.
     */
    ...
    /**
     * Sets the request type.  The type can change from one without
     * error to one with error if the GXML fails parsing (import).
     *
     * @param p_type The type to set the request to.
     *
     * @exception Returns an error if the type specified isn't a valid
     * type.
     */
    ...
    /**
     * Sets the original source file content name.  This is a file
     * that contains the original source file content (temp file).
     *
    ...
    /**
     * Gets the original source file content name.  This is a file
     * that contains the original source file content (temp file).
     *
     * This field may not be persisted.
     */
    ...
    /**
     * Returns the list of targets that are already active with the
     * same page that this request is associated with.
     *
     * The list of targets also contains the various jobs they are active in.
     *
     * This list is populated and used during importing.
     * It is not persisted in the database.
     *
     * @return A hashtable where the target locale is the key and the
     * job id is the value.
     */
    public Hashtable getActiveTargets();

    /**
     * Adds a target locale to the request that this page is already
     * actively being worked on and the job it is active in.
     *
     * This list is persisted only in memory not the database.
     *
     * @param p_tLocale The target locale the page is active in (as a
     * GlobalSightLocale object from the cache).
     * @param p_job The job that the target page is active in.
     */
    public void addActiveTarget(GlobalSightLocale p_tLocale, Job p_job);

    /**
     * Returns a set of target locales (GlobalSightLocale) that this
     * particuarl page of the request is NOT active in - so it can be
     * imported.
     *
     * Basically all the target locales from the L10nProfile minus the
     * active target locales.
     */
    public List getInactiveTargetLocales();
    
    /**
     * add GlobalSightLocale to here to make this workflow unimported
     * when creating job
     * @param p_tLocale
     */
