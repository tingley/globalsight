...
public class CompanyRelatedUncacheableFileServlet extends HttpServlet
{
    ...
    static
    {
        ...
        contentTypeMap.put("dotx", "application/vnd.openxmlformats-officedocument.wordprocessingml.template");
        contentTypeMap.put("pptx", "application/vnd.openxmlformats-officedocument.presentationml.presentation");
        contentTypeMap.put("potx", "application/vnd.openxmlformats-officedocument.presentationml.template");
        contentTypeMap.put("xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
        contentTypeMap.put("xltx", "application/vnd.openxmlformats-officedocument.spreadsheetml.template");
    ...
    */
        ...
        throws ServletException, IOException
    {
    	try {
            ...
            WorkObject wo = (WorkObject) TaskHelper.retrieveObject(p_request
					.getSession(), WebAppConstants.WORK_OBJECT);
            ...
            if (wo == null) {
                ...
                {
                    companyName = CompanyWrapper.getCurrentCompanyName();
                }
            } else {
...
//            File file = new File(docHome, fileName);
            ...
            if(SUPER_COMPANY_ID.equals(companyId))
            {
                fileFullPath = new StringBuffer().append(docHome)
                                             ...
                                             .append(p_request.getPathInfo())
                                             .toString(); 
            }
            else
            {
            	fileFullPath = new StringBuffer().append(docHome).append(
						File.separator).append(companyName).append(
						p_request.getServletPath()).append(
						p_request.getPathInfo()).toString();
            }
            ...
            if(zipFile)
            {
                ...
                p_response.setHeader("Content-Disposition", "attachment; filename=" + fileName + ".zip;");
            }
            else
            {
