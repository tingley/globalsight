...
public class TaskListServlet extends HttpServlet
{
    ...
    public void service(HttpServletRequest request, HttpServletResponse response)
    {
        ...
        try
        {
            ...
            if (isLoginSession(session, request))
            {
                ...
                boolean isSuperPm = ServerProxy.getUserManager().containsPermissionGroup(userId,
                        WebAppConstants.SUPER_PM_NAME);
                if (user.getCompanyName().equals(CompanyWrapper.getSuperCompanyName())
                        && !isSuperPm)
                ...
                {
                    result = export(request.getParameter("taskId"), request.getParameter("jobId"),
                            request.getParameter("workflowId"), request.getParameter("exportFrom"),
                            request);
                }
        ...
        try
        {
            ...
            if (params.isSuperUser())
            {
                ArrayList<Company> companies = new ArrayList<Company>(
                        ServerProxy.getJobHandler().getAllCompanies());
    ...
    private String getContextForTab(String taskIdString, PermissionSet perms, HttpSession p_session)
    {
        JSONObject object = new JSONObject();
        ...
        boolean isShowComments = perms.getPermissionFor(Permission.ACTIVITIES_JOB_COMMENTS_VIEW)
                || perms.getPermissionFor(Permission.ACTIVITIES_COMMENTS_VIEW);
        boolean isReviewerCommentsReport = perms
        ...
        try
        {
            ...
            boolean isAssignee = new Boolean(taskImpl.getAllAssignees()
                    .contains((String) p_session.getAttribute(WebAppConstants.USER_NAME)));
            if (!isAssignee)
            {
                disableButtons = true;
            }
            ...
            String scorecardUrl = "";
            boolean isShowScorecard = task.getWorkflow().getScorecardShowType() == -1 ? false
                    : true;
                ...
                if (!disableButtons && workOffline)
                {
                    if (reviewOnly)
                    {
                        if (isReviewerCommentsReport)
                            workOfflineUrl = "/globalsight/ControlServlet?linkName=downloadreport&pageName=TK2&state="
                                    + taskState + "&taskId=" + taskId;
                    }
                    else
                    {
                        workOfflineUrl = "/globalsight/ControlServlet?linkName=download&pageName=TK2&state="
                                + taskState + "&taskId=" + taskId;
                    }
                }
            ...
            boolean isShowSecondTargetFile = !perms
                    .getPermissionFor(Permission.ACTIVITIES_SECONDARYTARGETFILE);
            if (isShowSecondTargetFile)
            {
                ...
                {
                    secondaryTargetFilesUrl = "/globalsight/ControlServlet?linkName=taskSecondaryTargetFiles&pageName=TK2&taskAction=getTask&state="
                            + taskState + "&taskId=" + taskId;
                }
            }

            String commentUrl = "/globalsight/ControlServlet?linkName=comment&pageName=TK2&taskAction=getTask&state="
                    + taskState + "&taskId=" + taskId;
            ...
            boolean isShowQaChecks = (disableButtons == false)
                    ...
                    && (QACheckerHelper.isShowQAChecksTab(task)
                            || DITAQACheckerHelper.isShowDITAChecksTab(task));
    ...
    private String getQAChecksUrl(Task p_task)
    {
        ...
        {
            url.append(
                    "/globalsight/ControlServlet?linkName=downloadQAReport&pageName=QA_downloadQAReport&state=")
                    .append(state).append("&taskId=").append(taskId);
        }
        else if (DITAQACheckerHelper.isShowDITAChecksTab(p_task))
        {
            url.append(
                    "/globalsight/ControlServlet?linkName=downloadDitaReport&pageName=QA_downloadDitaReport&state=")
                    .append(state).append("&taskId=").append(taskId);
        }
        return url.toString();
    }

    private String checkUploadingStatus(String taskId, String jobId, String workflowIds,
            String exportFrom)
    {
            ...
            try
            {
                ...
                for (Workflow workflow : job.getWorkflows())
                {
                    ...
                    for (Iterator<Long> ids = tasks.keySet().iterator(); ids.hasNext();)
                    {
                        ...
                        try
                        {
                            if (ServerProxy.getTaskManager().getTask(theTaskId)
                                    .getIsUploading() == 'Y')
                            {
                                isUploading = true;
                                break;
    ...
    private String export(String taskId, String jobId, String workflowIds, String exportFrom,
        ...
        try
        {
            ...
            for (Workflow workflow : workflows)
            {
                ...
                L10nProfile l10nProfile = LocProfileHandlerHelper
                        .getL10nProfile(job.getL10nProfileId());
                ...
                String exportType = workflow.getState().equals(Workflow.LOCALIZED)
                ...
                long exportBatchId = ExportEventObserverHelper.notifyBeginExportTargetBatch(job,
                        user, pageIds, workflowIdList, null, exportType);

                WorkflowHandlerHelper.exportPage(ep, pageIds, true, exportBatchId);
            }
    ...
    private List<String> getDownloadFiles(String taskId, String jobId, String workflowIds,
            String exportFrom, HttpServletRequest p_request)
    {
        int totalPagesNum = 0;
        int exportPageNum = 0;
        ...
        try
        {
                ...
                for (File file : downloadFiles)
                {
                    ...
                    else if (filePath.indexOf(tempPassoloWebservicePrefix) > 0)
                    {
                        String tempPath = filePath
                                .substring(filePath.indexOf(tempPassoloWebservicePrefix));
                        if (!selectFiles.contains(tempPath))
                        {
                            selectFiles.add(tempPath);
                        }
            ...
            else
            {
                ...
                String result = numberFormat
                        .format((float) exportPageNum / (float) totalPagesNum * 100);
                selectFiles.clear();
    ...
    private void setTargetFiles(File folder, List<File> targetFiles, List<File> xlzFolders)
    {
        ...
        for (File file : fileList)
        {
            if (file.isFile())
            {
                ...
                {
                    xlzFolders.add(
                            new File(file.getPath().substring(0, file.getPath().length() - 4)));
                }
            }
    ...
    private List<TaskVo> getTasks(User user, TaskListParams params)
    {
        ...
        try
        {
            ...
            {
                sp.setSourceLocale(
                        ServerProxy.getLocaleManager().getLocaleById(Long.parseLong(tmp)));
            }
            ...
            if (StringUtil.isNotEmpty(tmp) && !"0".equals(tmp) && !"null".equalsIgnoreCase(tmp))
            {
                sp.setCompanyName(ServerProxy.getJobHandler().getCompanyById(Long.parseLong(tmp))
                        .getCompanyName());
            }
            ...
            for (int i = start; i < end; i++)
            {
                ...
                if (WorkflowConstants.TASK_ALL_STATES == state)
                {
                    ...
                    if (oriState == Task.STATE_FINISHING
                            && taskImpl.getState() != Task.STATE_COMPLETED)
    ...
    private TaskListParams getTaskListParams(HttpServletRequest request,
            SessionManager sessionManager)
    {
        TaskListParams listParams = (TaskListParams) sessionManager
        ...
        if (listParams == null)
        {
            ...
            listParams.setCanManageWorkflows(
            ...
            listParams.setCanExportInProgress(
                    perms.getPermissionFor(Permission.ACTIVITIES_EXPORT_INPROGRESS));
        ...
        if (StringUtil.isNotEmpty(sortColumn) && "true".equals(getFilterFromRequest))
            listParams.setSortColumn(sortColumn);

        String sortTypeString = request.getParameter("sortType");
        if (StringUtil.isNotEmpty(sortTypeString) && "true".equals(getFilterFromRequest))
        {
        ...
        if ("true".equals(getFilterFromRequest))
        {
            try
            {
                setFilterValue(filters, "acceptanceStartFilter",
                        request.getParameter("acceptanceStartFilter"));
                setFilterValue(filters, "acceptanceStartOptionsFilter",
                        request.getParameter("acceptanceStartOptionsFilter"));
                setFilterValue(filters, "acceptanceEndFilter",
                        request.getParameter("acceptanceEndFilter"));
                setFilterValue(filters, "acceptanceEndOptionsFilter",
                        request.getParameter("acceptanceEndOptionsFilter"));
                setFilterValue(filters, "completionStartFilter",
                        request.getParameter("completionStartFilter"));
                setFilterValue(filters, "completionStartOptionsFilter",
                        request.getParameter("completionStartOptionsFilter"));
                setFilterValue(filters, "completionEndFilter",
                        request.getParameter("completionEndFilter"));
                setFilterValue(filters, "completionEndOptionsFilter",
                        request.getParameter("completionEndOptionsFilter"));
                ...
                setFilterValue(filters, "jobNameFilter", new String(
                        request.getParameter("jobNameFilter").getBytes("ISO8859-1"), "UTF-8"));
                setFilterValue(filters, "activityNameFilter",
                        request.getParameter("activityNameFilter"));
                setFilterValue(filters, "assigneesNameFilter",
                        request.getParameter("assigneesNameFilter"));
                setFilterValue(filters, "sourceLocaleFilter",
                        request.getParameter("sourceLocaleFilter"));
                setFilterValue(filters, "targetLocaleFilter",
                        request.getParameter("targetLocaleFilter"));
    ...
    private void setFilterValue(HashMap<String, String> filters, String filterName,
            String filterValue)
    {
    ...
    private String getButtonStatus(int state, PermissionSet perms, User user)
    {
        ...
        boolean isCompleteActivity = perms
                .getPermissionFor(Permission.ACTIVITIES_BATCH_COMPLETE_ACTIVITY);
        boolean isCompleteWorkflow = perms
                .getPermissionFor(Permission.ACTIVITIES_BATCH_COMPLETE_WORKFLOW);
        ...
        boolean isExportInProgress = perms
        ...
        boolean isCanDownloadCombined = perms
                .getPermissionFor(Permission.ACTIVITIES_DOWNLOAD_COMBINED);
        boolean isOfflineUpload = perms
                .getPermissionFor(Permission.ACTIVITIES_OFFLINEUPLOAD_FROMANYACTIVITY);
        ...
        boolean showAssignees = TaskSearchUtil.isProjectManager(user)
                || perms.getPermissionFor(Permission.PROJECTS_MANAGE);
