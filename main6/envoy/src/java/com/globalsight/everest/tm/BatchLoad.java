...
/**
 * Batch utility to upload TMX files to the server and import them into a
 * specific TM.
 */
public class BatchLoad
{
        ...
        try
        {
            ...
            else
            {
                if (!(tmxType.equals(ImportOptions.TYPE_XML)
                        ...
                        || tmxType.equals(ImportOptions.TYPE_TTMX_IL) || tmxType
                            .equals(ImportOptions.TYPE_TTMX_XPTAG)))
                {
            ...
            else
            {
                if (!(syncMode.equals(ImportOptions.SYNC_OVERWRITE)
                        || syncMode.equals(ImportOptions.SYNC_MERGE) || syncMode
                            .equals(ImportOptions.SYNC_DISCARD)))
                {
            ...
            ImportStats r = runImport(System.err, tmName, tmxName, tmxType,
                    syncMode);
    ...
    public static void createTm(String tmName) throws Exception
    {
        ...
        tm.setCompanyId(Long.parseLong(CompanyThreadLocal.getInstance()
                .getValue()));
        tm.setName(tmName);
    ...
    {
        ProjectTM tm = ServerProxy.getProjectHandler().getProjectTMByName(
                tmName, false);
        if (tm != null)
        {
            ...
            throws Exception
    {
        ...
        ImportUtil.createInstance().saveTmFileWithValidation(new File(tmxName),
                validatedFile);
        ...
        IImportManager importer = ServerProxy.getTmManager()
                .getImporter(tmName);
        ...
        importer.attachListener(new IProcessStatusListener()
        {
            public void listen(int entryCount, int percentage, String message)
            {
                if (progressOut != null)
                {
                    progressOut.print("\r" + tmxName + ": " + percentage
    ...
    public static String getTimeString(long duration)
    {
        ...
        else
        {
            ...
            return String.valueOf(hours) + "h " + String.valueOf(minutes)
                    + "m " + String.valueOf(seconds) + "s";
        }
    }
