...
public class Benchmark
{
            ...
            throws Exception
    {
        ...
        out.println(BuildVersion.PRODUCT + " " + BuildVersion.VERSION + " ("
        ...
        out.println(runtime.availableProcessors() + " CPUs, "
        ...
        try
        {
            ...
            new Tm3Enabler(conn, company).enable(new ProgressReporter()
            {
            ...
            for (TmVersion tmVersion : new TmVersion[]
            { TmVersion.TM2, TmVersion.TM3 })
            {
                ...
                {
                    BatchLoad.ImportStats r = BatchLoad.runImport(System.err,
                            "Benchmark TM", f.toString(),
                            ImportOptions.TYPE_TMX2,
                            ImportOptions.SYNC_OVERWRITE);
                    out.println(f + " (" + r.getTuCount() + " TUs): "
                            + BatchLoad.getTimeString(r.getDt()) + ", "
                            + BatchLoad.getTimeString(r.getDtPerDu())
                            + " per TU");
                }
    ...
    public static BatchLoad.ImportStats importUntilStable(PrintStream out,
            String tmxFile) throws Exception
    {
        Long lastDt = null;
        while (true)
        {
            BatchLoad.ImportStats r = BatchLoad.runImport(System.err,
                    "Benchmark TM", tmxFile, ImportOptions.TYPE_TMX2,
                    ImportOptions.SYNC_OVERWRITE);
            out.println(tmxFile + " (" + r.getTuCount() + " TUs): "
                    + BatchLoad.getTimeString(r.getDt()) + ", "
    ...
    public static long getTm2TuCount() throws Exception
    {
        ...
        try
        {
            ...
            ResultSet r = conn.prepareStatement(q).executeQuery();
            r.next();
            return r.getLong(1);
        }
        finally
