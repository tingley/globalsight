...
public class ImportManager extends IImportManagerImpl implements Serializable
{
    ...
    private Tm m_database = null;

    public ImportManager(Tm p_database, SessionInfo p_session)
            throws ImporterException
    {
        ...
        CATEGORY.debug("Default TM import manager created.");
    }

    protected ImportManager(Tm p_database, SessionInfo p_session,
    ...
    protected void runImport() throws ImporterException
    {
        ...
        try
        {
            while (m_reader.hasNext())
            {
                ...
                //for gbs-753:When import tmx,if the "file options" > "import file format" is
                ...
                if ("tmx-worldserver".equalsIgnoreCase(import_file_format)){
                    try {
                        SegmentTmTu stt = (SegmentTmTu) result.getResultObject();
                        ...
                        if (allLocales != null && allLocales.size()>0) {
                            ...
                            while (allLocalesIt.hasNext()) {
                                GlobalSightLocale gsl = (GlobalSightLocale) allLocalesIt.next();
                                //may contains more than one tuv for one locale in one tu
                                Collection tuvList = stt.getTuvList(gsl);
                                if (tuvList != null && tuvList.size()>0) {
                                    Iterator tuvIt = tuvList.iterator();
                                    while (tuvIt.hasNext()) {
                                        BaseTmTuv btt = (BaseTmTuv) tuvIt.next();
                        ...
                        CATEGORY.error("Failed to escape '\\' before { and } for tmx-worldserver file.");
                    }
                }
                ...
                if (entries.size() == BATCHSIZE)
                {
                    ...
                    {
                        ++errCounter;
                        ErrorRecorder.record(new File(getFileName()), e, m_listener, errorTuv);
                        errorTuv += e.getTuvs().size();
                        CATEGORY.error("error adding entries", e);
                    }
                    catch (Throwable ex)
                    {
                        ++errCounter;
            ...
            if (errorTuv > 0)
            {
                ...
                String message = ProcessStatus.getStringFormattedFromResBundle(m_listener,
                        "lb_import_tm_tuv_error", pattern, errorTuv, ErrorRecorder.getStorePath(getFileName()));
                showStatus(counter, expectedEntries, message);
            }

            // PSO request
            CATEGORY.info("Finished successfully import of file "
                    + getFileName());
        }
        catch (IOException ignore)
        {
...
//                    + (errCounter == 1 ? "" : "s");
    ...
    protected IReader createReader(ImportOptions p_options)
            throws ImporterException
    {
        return new TmxReader(p_options, m_database);
    }
            ...
            com.globalsight.everest.tm.importer.ImportOptions p_options)
    {
        ...
        if (mode
                .equals(com.globalsight.everest.tm.importer.ImportOptions.SYNC_OVERWRITE))
        {
    ...
    {
        LingServerProxy.getTmCoreManager().saveToSegmentTm(m_database,
