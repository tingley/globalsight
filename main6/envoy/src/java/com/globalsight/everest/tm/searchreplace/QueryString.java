...
/**
 * QueryString stores a query string for concordance search. The class
 * divedes the query string into words and provides various
 * representation of them such as word token and n-gram token for tm
 * index search, etc.
 */
...
public class QueryString
{
    ...
    public QueryString(
        String p_queryString, GlobalSightLocale p_sourceLocale)
    {
    ...
    /**
     * getNonWildcardTokensForQuery returns tokens for querying TM
     * indexes. The parameter specifies if word or n-gram tokens are
     * returned. The tokens don't contain wildcard characters.
     *
     * @param p_wordTmIndex If true, word tokens are returned. If
     * false, n-gram tokens are returned.
    ...
    /**
     * getNonWildcardWordsForExistenceCheck returns a word list for
     * checking the existence of those words in a segment string. The
     * words don't contain wildcard characters.
     *
     * @param p_preserveCase If true, preserve the case of the
     * words. If false, words are down cased.
     * @return List of words (String)
     */
    ...
    /**
     * getNonWildcardWordForQuery returns word list for querying word
     * based TM index. The words don't contain wildcard characters.
     *
     * @return List of words (String)
     */
    private List getNonWildcardWordsForQuery()
        throws Exception
    {
        ...
        for(Iterator it = m_wordList.iterator(); it.hasNext();)
        {
            ...
            if(!containsWildcard(word))
            {
                ...
                word = Text.substrUtf8LimitLen(
                    word, WordTokenizer.TOKEN_COLUM_LEN);
    ...
    /**
     * getNonWildcardNgramTokens returns n-gram token list for
     * querying n-gram based TM index. The n-gram tokens don't contain
     * wildcard characters.
     *
     * @return List of n-gram tokens (String)
     */
    private List getNonWildcardNgramTokens()
        throws Exception
    {
        ...
        for(Iterator it = m_wordList.iterator(); it.hasNext();)
        {
            ...
            // make n-gram tokens
            List tokens
                = ngramTokenizer.tokenize(word, 0, 0, 0, m_locale, true);
            ...
            for(Iterator itToken = tokens.iterator(); itToken.hasNext();)
            {
                ...
                if(tokenString.indexOf('*') == -1)
                {
                    ...
                    tokenString
                        = Text.replaceString(tokenString, "\uffff", "*");
    ...
    /**
     * getWildcardTokensForQuery returns tokens for querying TM
     * indexes. The parameter specifies if word or n-gram tokens are
     * returned. The tokens contain wildcard characters and they
    ...
    /**
     * getWildcardWordsForExistenceCheck returns a word list for
     * checking the existence of those words in a segment string. The
     * words contain wildcard characters and they are formatted
     * suitable for using in regular expression.
     *
     * @param p_preserveCase If true, preserve the case of the
     * words. If false, words are down cased.
    ...
    /**
     * getWildcardWordForQuery returns word list for querying word
     * based TM index. The words contain wild card characters and they
     * are formatted suitable for using in SQL LIKE clause.
     *
    ...
    /**
     * getWildcardNgramTokens returns n-gram token list for querying
     * n-gram based TM index. The n-gram tokens contain wildcard
     * characters and they are formatted suitable for using in SQL
     * LIKE clause.
     *
     * @return List of n-gram tokens (String). SQL formatted.
     */
    private List getWildcardNgramTokens()
        throws Exception
    {
        NgramTokenizer ngramTokenizer = new NgramTokenizer();
            ...
            for(Iterator itToken = tokens.iterator(); itToken.hasNext();)
            {
                ...
                if((idx = tokenString.indexOf('*')) != -1)
                {
                    ...
                    if(idx == 1)
                    {
                        ...
                        newTokenString
                            = makeWildcardQueryString(newTokenString, '%');
                        newTokenString = Text.replaceString(
                            newTokenString, "\uffff", "*");
                    ...
                    tokenString
                        = makeWildcardQueryString(tokenString, '%');
    ...
    /**
     * Test if a string contains wildcard character '*'. An escaped
     * wildcard character '\*' is a literal asterisk '*'. The method
     * compares the number of instances of '\*' and '*' in a string
     * and if the numbers are different, it reports that the string
     * contains wildcard characters.
     *
     * @param p_string String to be examined.
    ...
    private String makeRegExpPattern(String p_pattern)
    {
        ...
        for(int i = 0; i < specialCharList.length; i++)
        {
            ...
            pattern = Text.replaceString(
                pattern, specialChar, escape + specialChar);
        }
