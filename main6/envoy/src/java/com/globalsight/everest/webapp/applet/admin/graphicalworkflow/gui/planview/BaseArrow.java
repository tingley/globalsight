...
class BaseArrow extends UIObject 
{
    ...
    //////////////////////////////////////////////////////////////////////
    public int getNodeWidth() { return -1; }
    public int getNodeHeight() { return -1;}
    //////////////////////////////////////////////////////////////////////
    ...
    private void setParameters(GraphicalPane _pane) 
    {
       ...
       /* else if (_pane instanceof TNPane)
        {
            aHeadH = 6;
            aHeadC = java.lang.Math.cos (3.1415 * 30 / 180);//overwrite parent class
            aHeadS = java.lang.Math.sin (3.1415 * 30 / 180);//overwrite parent class
        }  */
    }
    ...
    {
        if ( shape.contains(_pos) ) return true;
        ...
        /*  int _w = 0;
          ...
          Rectangle line_area = new Rectangle(_x, _y, _w, _h);
          if ( line_area.contains(_pos) ) */
    ...
    void draw(Graphics2D g2, boolean mode)
    {
        ...
        if (mode)
        {
            ...
            /*
           if(sourceNode instanceof UIDelay ){
               if(ey < sy)
               {
                   if(ey > sy-30 && sx > ex)
                   {                     
               ...
               if(sy < ey){                   
                   if ( (ex - sx) > 0 || (ex - sx) < 0){
                       ...
                       if(dx >= 1)
                       {
                           ...
                           sx = getTailExForDelay(sx,ex,sy,ey); 
                   }                   
               }                
           }
           
           //Target node as Delay
             if(targetNode instanceof UIDelay ){
               if(ey > sy)
               {
                   if(sy > ey-30 && sx > ex)
                   {                        
               ...
               if(sy > ey){
                   if ( (ex - sx) > 0 || (ex - sx) < 0){
                       ...
                       if(dx >= 1)
                       {
                           ...
                           ex = getHeadExForDelay(sx,ex,sy,ey); 
                       }
                   }
        ...
        if (mode)
        {
            ...
            if (gPane instanceof GVPane)
            {
                ...
                labelField.setBounds (((sxz+exz)-getLabelFieldPreferredWidth(g2))/2-2, 
                                      (syz+eyz)/2-10,
                                      getLabelFieldPreferredWidth(g2)+4,
                                      DrawInfo.getPreferredTextSize().height);                
                //labelField.setText (msgCat.getMsg (objectName)); - bug for zooming
                //labelField.repaint();
                ...
                {
                    //ignore this exception at least until graphical planner uses swing
                } 
                */
            }

            // coments this block of code as not using Tumbnailview
            /* else if (gPane instanceof TNPane)
             { 
                 g2.fillPolygon(aHxPoints, aHyPoints, 4);
                 shape = new Polygon(aHxPoints, aHyPoints, 4);
             }     */
        }
        else
        {
    ...
    public void moveArrowRect()
    {
        ...
        headArea.addPoint((int)(-5*Sin + pos.x), (int)( 5*Cos + pos.y));
        headArea.addPoint((int)(-5*Sin + headPos.x), (int)( 5*Cos + headPos.y));
        headArea.addPoint((int)( 5*Sin + headPos.x), (int)(-5*Cos + headPos.y));
    }
    ...
    public void keyTyped(KeyEvent event) 
    {
        ...
        {
            //The following line is commented out by Hamid. Please leave it that
            //way:
    ...
    public void focusLost(FocusEvent event) 
    {
        ...
        if (event.getSource() == labelField)
        {
            ...
            if ( !(modelObject instanceof WorkflowArrow) ) return; //added by Hamid
            WorkflowTask plan_node = ((WorkflowArrow)modelObject).getSourceNode();
           ...
           // if ( (plan_node.getType()==WorkflowConstants.ACTIVITY) || 
             //                  (plan_node.getType()==WorkflowConstants.CONDITION) )
            {
                for (int j = 0; j < _arrows.size(); j++)
                {
                    ...
                    if ( (_arrow != (WorkflowArrow)modelObject) && objectName.equals(strLabel))
                    {
                        dupDetected = true;
                        break;
                    }
                ...
                if ( dupDetected == true )
                {
                    ...
                    //  if ( plan_node instanceof ConditionNode )
                    {
                        JOptionPane.showMessageDialog(null, 
                                AppletHelper.getI18nContent("msg_no_dup_arrow_error"));
                    }             
            ...
            try
            {
                ...
                if ( !arrowNameEmpty() )  ((WorkflowArrow)modelObject).setName(objectName);
            ...
            catch (Exception e)
            {
                ...
                /*WindowUtil.showMsgDlg(WindowUtil.getFrame(this),
                                      ...
                                      msgCat.getMsg("Arrow Name Error!"),
    ...
    private Point getArrowHeadPosition()
    {
        ...
        if (intermediatePoints != null && intermediatePoints.length > 0)
        {
            ...
            /*if(gPane instanceof TNPane){                    
                sx =(int)originalIPoints[originalIPoints.length - 1].x;
                sy=(int)originalIPoints[originalIPoints.length - 1].y;
                //Log.println(Log.LEVEL3, "BaseArrow:::if(gPane instanceof TNPane) sx:" + sx + "sy:" + sy);
            } */
        ...
        {        
          Point p = calculateUISubProcessArrowIntersection(
              targetNode, sx, sy, dx, dy, Sin, Cos);
          x = p.x;
          y = p.y;
        }
        else if (targetNode instanceof BaseConnection)
        {
            ...
            // of all nodes
            // Instead created abstract methods in UIObject for getNodeWidth() & height
    ...
    private Point getArrowTailPosition()
    {
        ...
        {          
          Point p = calculateUISubProcessArrowIntersection(sourceNode, dx, dy, sx, sy,  
                                        Sin, Cos) ;
          x = p.x ;
          y = p.y ;
        }
    ...
    {
        if ((Math.abs((double)(pos.x-x)) + Math.abs((double)(pos.y-y)))
            <= (Math.abs((double)(headPos.x-x)) + Math.abs((double)(headPos.y-y))))
        {
            return false;
        }
        else
    ...
    BaseArrow getDummy()
    {
        ...
        /*if (pos == null)
        {
            //Log.println(Log.LEVEL1, "null pos in getdummy");
        }
        else  */
        if (pos !=null)
        {
        ...
        if (color != null)
        {
            obj.setColor(new Color (color.getRed(), color.getGreen(), color.getBlue()));
        }
        return obj;
    }
    ...
    /**
     * @author:Hamid
     * This new utility method is used in the key listener for the arrow's name
     * text field.
     */
    /*private void validateArrowName()
    {
        ...
        if ( text.length() >= WorkflowConstants.MAX_NODE_NAME_LENGTH )
        {
            ...
            JOptionPane.showMessageDialog(null, 
                                          msgCat.getMsg("msg_arrowMaxLength"));
            labelField.requestFocus(); 
    ...
    /**
     * @author:Hamid
     * This new utility method is to check whether the arrow's name is empty or
     * not. It is used in the method <code>focusLost()</code> so that the arrow
     * is not saved until the user provides a valid name for it.
     */
    ...
    /**
     * @author:Hamid
     * This new utility method is used dynamically increase the size of the 
     * text field for the arrwo's name, while the user is typing it. This method
     * is called in the key listener of the arrow's name text field.
     */
    private void sizeArrowTextField()
    {
        ...
        if ( g == null ) g = (Graphics2D)gPane.getGraphics();
        ...
        {
            labelFieldWidth = 45;     
            labelField.setBounds(fieldBounds.x, fieldBounds.y, 
                                 labelFieldWidth, fieldBounds.height);
        }
        else
        {
    ...
    private Point calculateUISubProcessArrowIntersection(
        UIObject node, int sx, int sy, int dx, int dy, 
        double Sin, double Cos) 
    {
        ...
        if (Math.abs(Sin) > sine60) // always land in the center of the top or bottom
        {
            x = dx + (width/2) ;
            y = dy ;
            if (sy > dy)
        ...
        {
            // always land at a point where a virtual semicircle makes tangential
        ...
        else  // always land in the center of the side nearest the source node
        {
            x = dx ;
            if (sx > dx)
            {
