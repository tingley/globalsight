...
public class GVPane extends GraphicalPane implements KeyListener, Printable
{
	...
	public void populate(Vector p_postData)
	{
		...
		AppletHelper.setI18nContents((Hashtable) m_dialogData.get(EnvoyAppletConstants.I18N_CONTENT));
	...
	private Polygon calculateArrowArea(BaseArrow arrow)
	{
		...
		arrowArea.addPoint(arrow.getHeadPosition().x + 3, arrow
		        .getHeadPosition().y);
		arrowArea.addPoint(arrow.getHeadPosition().x - 3, arrow
		        .getHeadPosition().y);
		if (intermediatePoints != null && intermediatePoints.length > 0)
	...
	protected void createActivityNode(int _x, int _y)
	{
		...
		selObj.setPosition(new Point(_x, _y));
		selObj.setBounds(_x - UIActivity.getStdWidth() / 2, _y
		        - UIActivity.getStdHeight() / 2, UIActivity.getStdWidth(),
		        UIActivity.getStdHeight());
		requestFocus();
	}
		...
		else
		{
			...
			try
			{
				...
				else if (modelPI != null)
				{
					...
					modelPI
					        .removeWorkflowArrowInstance(((WorkflowArrowInstance) _obj
					                .getModelObject()));
				}
	...
	private boolean deleteArrowInstance(WorkflowArrowInstance arrow_instance)
	{
		try
		{
			...
			{
				int _reply = WindowUtil.showMsgDlg(WindowUtil.getFrame(this),
				        AppletHelper.getI18nContent("msg_arrow_with_pending_events"), 
				        AppletHelper.getI18nContent("msg_event_pending_warning"),
				        WindowUtil.OPTION_YESNO, WindowUtil.TYPE_QUESTION);
	        ...
	        Point[] pointArray)
	{
		...
		if (!isFirst(_arrow)) return false;
	...
	void mouseUpThis(MouseEvent ev)
	{
		...
		{
			if (isOnNode(_x, _y) && isButton3(ev)) return;
			...
			while (i > 0)
			{
				        ...
				        .getHeadPosition()))
				{
					...
					// UIStart
					{
						...
						if (addArrow(_arrow, ui_obj, _arrow
		...
		else if (ev.getSource() == this)
		{
			...
			{
				if (isPopupEnabled()) doNodePopup(ev);
			}
		}
		// repaint();
			...
			if (firstClick || drawMultiLine)
			{
				if (uiActivityOffset)
				{
					_arrow.drawMultipleLine(_arrow.getSourceNode()
					        .getLocation().x
					        + _x, _arrow.getSourceNode().getLocation().y + _y,
					        g2);
				}
				else
				{
			...
			if (selArrowHead)
			{
				...
				if (firstClick || drawMultiLine)
				{
					if (uiActivityOffset)
					{
						_arrow.drawMultipleLine(_arrow.getSourceNode()
						        .getLocation().x
						        + _x, _arrow.getSourceNode().getLocation().y
						        + _y, g2);
					}
					else
					{
	...
	private void addActivityArrow(UIActivity ui_node, int _x, int _y)
	{
		...
		{
			UIObject ui_obj = (UIObject) uiObjects.elementAt(--i);
			if (!(ui_obj instanceof UIActivity)) continue;
			if (ui_obj == ui_node)
			{
				addingArrow = true;
				createActionArrow(ui_obj.getLocation().x + _x, ui_obj
				        .getLocation().y + _y);
				int _xx = ui_obj.getLocation().x + _x;
				int _yy = ui_obj.getLocation().y + _y;
	...
	{
		if (baseArrow == null) return false;
		...
		while (i > 0)
		{
			...
			{
				message = AppletHelper.getI18nContent("msg_two_outgoing_arrows");
				showMessageDialog = true;
			}
			...
			{
				message = AppletHelper.getI18nContent("msg_condition_to_condition");
				showMessageDialog = true;
			}
	...
	private boolean doValidation(Vector workflowTasks)
	{
		...
		while (workflowIterator.hasNext())
		{
			...
			if (workflowTask.getStructuralState() != WorkflowConstants.REMOVED)
			{
				...
				{
					JOptionPane.showMessageDialog(null, AppletHelper.getI18nContent("msg_invalid_workflow"));
	        ...
	        int p_pageIndex)
	{
		...
		g2.translate(p_pageFormat.getImageableX(), p_pageFormat
		                .getImageableY());
	...
	private float getScale(int pageWidth, int pageHeight)
	{
         ...
         * whole content(left with whitespace)
         * To trip the whitespace, we assume the percentage is 75%.
         * This value can be adjusted when there is some wrong happened.
         * The safted value is 100%.
         */
