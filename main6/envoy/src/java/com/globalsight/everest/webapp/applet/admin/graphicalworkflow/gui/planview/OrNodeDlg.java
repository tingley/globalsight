...
public class OrNodeDlg extends NodesDlg
{
    ...
    public OrNodeDlg(JFrame p_Frame, WorkflowTaskInstance p_plNode, int p_nTab, WFApp p_wfApp)
    { 
        super(p_Frame, p_plNode, p_nTab, p_wfApp);
        
        m_msgCat = new MessageCatalog (GraphicalPane.localeName);
        NodeModel m_nm = new NodeModel(NodeModel.OR_NODE, p_plNode);
        setModel( m_nm );
        setTitle(AppletHelper.getI18nContent("lb_or_node_properties"));

    }

    public OrNodeDlg(JFrame p_Frame, WorkflowTask p_plNode, int p_nTab, WFApp p_wfApp)
    { 
        super(p_Frame, p_plNode, p_nTab, p_wfApp);
        m_msgCat = new MessageCatalog (GraphicalPane.localeName);
        NodeModel m_nm = new NodeModel(NodeModel.OR_NODE, p_plNode);
    ...
    public OrNodeDlg(JFrame p_Frame, WorkflowTask p_plNode, WFApp p_wfApp)
    { //shailaja. Changed Frame to JFrame
        this( p_Frame, p_plNode, 0, p_wfApp );
    }

    public OrNodeDlg(JFrame p_Frame, WorkflowTaskInstance p_plNode, WFApp p_wfApp)
    { //shailaja. Changed Frame to JFrame
        this( p_Frame, p_plNode, 0, p_wfApp );
