...
class UIActivity extends BaseActivity
{
    ...
    {
        computePreferredSize();     // make sure sizes have been computed based on font
        return WIDTH;
    }
    ...
    {
        if (roleNameField != null) remove (roleNameField);
        if (nameField != null) remove (nameField);
    }   
    ...
    /*public void buttonRoleDown()
    {
        invokeDlg (Constants.ROLETAB);
    }

    public void buttonFormDown()
    {
        invokeDlg (Constants.FORMSTAB);
    }

    

    public void buttonAttributeDown()
    {
        invokeDlg (Constants.USERDEFINEDTAB);
    }    

    public void buttonScriptDown()
    {
        invokeDlg(Constants.SCRIPTSTAB);
    }    

    public void buttonTimerDown()
    {
        invokeDlg(Constants.TIMERTAB);
    }
     
    public void buttonStatusDown()
    {
        invokeDlg(Constants.STATUSTAB);
    }*/    
    ...
    {
        // Note: In netscape-solaris, we're not getting mousePressed/mouseReleased events on
        ...
        /* try
         {
           /*  if (event.getSource() == roleNameField)
             ...
             if (event.getSource() == nameField)
             {
                 ...
                 // if name field is blank, reset to default
    ...
    {
        if ( (event.getSource() == this) && !(gPane.getNodeType() == GVPane.ARROW_TYPE) )
        {
    ...
    public UIActivity(Point p, GraphicalPane inGPane)
    {
        ...
        rolePane.setBounds (hGap, vGap + 1, FIELD_WIDTH, FIELD_HEIGHT+(FIELD_HEIGHT/2));
        namePane.setBounds (hGap, 2*FIELD_HEIGHT-2*vGap, FIELD_WIDTH, FIELD_HEIGHT+(FIELD_HEIGHT/2));
    ...
    private JPopupMenu makePopup()
    {
        ...
        assigneeMenu.addActionListener(this);
        popupMenu.add(assigneeMenu);

        attributeMenu = new JMenuItem(msgCat.getMsg("User-DefinedAttributes"));
        attributeMenu.addActionListener(this);
        popupMenu.add(attributeMenu);
        ...
        formMenu.addActionListener(this);
        popupMenu.add(formMenu);

        timerMenu = new JMenuItem(msgCat.getMsg("Timers"));
        timerMenu.addActionListener(this);
        popupMenu.add(timerMenu);

        if (gPane.modelPI != null )
        {
            statusMenu = new JMenuItem(msgCat.getMsg("TimerStatus"));
    ...
    {
        // do not show the popup for a completed activity.
        if (e.isPopupTrigger() && 
            //showPopup() &&
            ((GVPane)gPane).isPopupEnabled())
        {
    ...
    public void actionPerformed(ActionEvent event)
    {
        ...
        else if (obj == formMenu)
        {
            buttonFormDown();
        }
        else if (obj == assigneeMenu)
        {
            buttonRoleDown(); 
        }
        else if (obj == attributeMenu)
        {
            buttonAttributeDown(); 
        }
        ...
        else if (obj == timerMenu)
        {
            buttonTimerDown();
        }
        else if (obj == statusMenu)
        {
            buttonStatusDown();
        } */
    }
    ...
    public void invokeDlg(int tabNumber)
    {
        ...
        if (gPane.modelPI == null)
        {
            ...
            Vector data = WorkflowTaskDialog.getDialog((GVPane)gPane,
                                                       "", ht);
        }
        else
        {
            ...
            {
                taskInfoBean = ((GVPane)gPane).getTaskInfoBeanMap().
                          get(new Long(wfti.getSequence()));
            }
            else
            {
                taskInfoBean = ((GVPane)gPane).getTaskInfoBeanMap().
                          get(new Long(wfti.getTaskId()));
            }
            ...
            Vector data = WorkflowTaskDialog.getDialog((GVPane)gPane,
                                                       "", ht);
            ((GVPane)gPane).getTaskInfoBeanMap().putAll(((GVPane)gPane).getModifiedTaskInfoMap());
        }      
        ...
        //if ((objectName=  ((WorkflowTask)modelObject).getActivityName()) !=null )
    ...
    {
        if (event.getSource() == nameField || event.getSource() == roleNameField)
    ...
    void draw(Graphics g, boolean mode)
    {
        ...
        //non-dragging painting is not needed since UIActivity is a panel itself (unlike other nodes)
        Graphics2D g2 = (Graphics2D)g;
        if (!mode)
        {
    ...
    // Routine to compute all size/position parameters based on textField's preferred size.
    // This is needed because the UIAcitivty size should grow if larger fonts are used
    private static void computePreferredSize()
    {
        if (bComputePreferredSizeCalled) return;
    ...
    {
        return gPane.modelPI == null ? true :
