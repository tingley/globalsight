...
public class SegmentationRuleFileBasicHandler
    extends PageHandler
{
    /**
        ...
        throws ServletException, IOException, EnvoyServletException
    {
        ...
        try
        {
            ...
            else if (SegmentationRuleConstant.DUPLICATE.equals(action))
            {
                ...
                SegmentationRuleFile segmentationRuleFile = ServerProxy.getSegmentationRuleFilePersistenceManager().
                    readSegmentationRuleFile(Long.parseLong(id));
                
                segmentationRuleFile.setName("new_segmentation_rule");

                sessionMgr.setAttribute(SegmentationRuleConstant.SEGMENTATIONRULE_KEY, segmentationRuleFile);
	...
	private void writeUploadfileToRequest(HttpServletRequest p_request)
	{
		try
		{
			...
			if (ruleStart != -1)
			{
				// Delete BOM mark in unicode encoding file header if any be there.
	...
	private String doValidator(HttpServletRequest p_request, SegmentationRuleFileImpl rule) throws ExtractorException
	{
        ...
        throws RemoteException, NamingException, GeneralException
    {
        ArrayList list = (ArrayList)
            ServerProxy.getSegmentationRuleFilePersistenceManager().getAllSegmentationRuleFiles();
        ...
        if (list != null)
        {
            ...
            {
                SegmentationRuleFileImpl rule = (SegmentationRuleFileImpl)list.get(i);
...
//            (Locale)p_session.getAttribute(WebAppConstants.UILOCALE);
     ...
     * @throws EnvoyServletException
     */
    private void setCompanyNames(SessionManager sessionMgr) throws EnvoyServletException
	{
     ...
     */
    private void getParams(HttpServletRequest p_request, SegmentationRuleFileImpl p_ruleFile)
    {
		...
		if(companyName != null)
		{
			String companyId = com.globalsight.everest.company.CompanyWrapper.getCompanyIdByName(companyName);
			p_ruleFile.setCompanyId(companyId);
		}
    }
}
