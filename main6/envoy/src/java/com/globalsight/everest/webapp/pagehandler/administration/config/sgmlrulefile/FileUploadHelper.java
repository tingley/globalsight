...
public class FileUploadHelper
{
    ...
    static private final int    MAX_LINE_LENGTH = 4096;
//    static private final String FILE_UPLOAD_DIR = Catalog.CATALOG_ROOT;
...
//    private File        m_savedFile = null;
        ...
        throws Exception
    {
        try
        {
            ...
            CATEGORY.debug("Start uploading SGML DTD");
            ...
            //  contruct a correct filepath to save the uploaded file
            //
            if (outFile != null && outFile.exists())
            {
                //  First, define and create SGML DTD upload directory
                //  structure, if not already done so
                //
//                File savedDir = new File(FILE_UPLOAD_DIR);
//                savedDir.mkdirs();
...
//                setSavedFilepath(m_savedFile.getAbsolutePath());
                File savedFile = new File(AmbFileStoragePathUtils.getCatalogDir(), getFilename());
                renameFile(outFile, savedFile);

                setSavedFilepath(savedFile.getAbsolutePath());
            }
        }
