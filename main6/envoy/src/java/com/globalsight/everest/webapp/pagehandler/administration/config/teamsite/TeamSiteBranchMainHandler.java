...
import com.globalsight.cxe.persistence.cms.teamsitedbmgr.TeamSiteDBManager;
...
/**
 * TeamSiteBranchPageHandler, A page handler to produce the entry page(index.jsp) for TeamSite Branch anagement.
 * <p>
...
public class TeamSiteBranchMainHandler extends PageHandler
{
    ...
    throws ServletException, IOException, EnvoyServletException
    {
        ...
        String tl = (String)p_theRequest.getParameter(TEAMSITE_TARGET_LANGUAGE);
        ...
        if ((tl != null)&&(sb != null)&&(tb != null)&&(server != null)&&(store!=null))
        {
            Integer Itmp = new Integer(tl);
            Integer IServer = new Integer(server);
            Integer IStore = new Integer(store);
            TeamSiteBranch TSBUpdate = new TeamSiteBranch(sb, Itmp.intValue(), tb,
                                                          IServer.intValue(),
                                                          IStore.intValue());
            // update the database
            try
        ...
        throws RemoteException, NamingException, GeneralException
    {
        ...
        while (it.hasNext())
        {
            ...
            TeamSiteServer ts = 
                ServerProxy.getTeamSiteServerPersistenceManager()
                           .readTeamSiteServer(tsb.getTeamSiteServerId());
            
            if (!currentCompanyId.equalsIgnoreCase(ts.getCompanyId()))
            {
                it.remove();
            }
        }
        ...
        setTableNavigation(p_request, p_session, new ArrayList(branches),
                       new TeamSiteBranchComparator(uiLocale),
                       10,
                       TS_LIST, TS_KEY);
    ...
    /**
     * Before being able to create a Teamsite Profile, certain objects must exist.
     * Check that here.
     */
