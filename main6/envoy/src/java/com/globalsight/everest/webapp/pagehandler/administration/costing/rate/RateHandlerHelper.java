...
public class RateHandlerHelper
{
     ...
     */
    public static Collection getAllRates()
     ...
     * @return A Rate
     */
    public static Rate getRate(long id)
        throws EnvoyServletException
    {
        try
        {
     ...
     * 
     * @param p_rate Rate object
     * @param p_withRates Specifies 'true' if the collection of rates that
     *                    are associated with the role should be populated, or 'false'
     *                    if they shouldn't be populated.
     *
	...
	try
	{                                    
            return ServerProxy.getUserManager().getContainerRole(p_rate, p_withRates);
        }
	catch (GeneralException ge)
	{
	    throw new EnvoyServletException(ge);
	...
	try
	{
	    ArrayList al = new ArrayList(ServerProxy.getLocaleManager().getSourceTargetLocalePairs());
     ...
     * 
     */
    public static Rate addRate(Rate p_rate)
        throws EnvoyServletException
    {
	try
	{
     ...
     * 
     */
    public static void modifyRate(Rate p_rate)
        throws EnvoyServletException
    {
	try
	{
            ...
            if (sc.getBooleanParameter(SystemConfigParamNames.COSTING_ENABLED) == true)
            {
                ...
                jobs = new ArrayList(
                       WorkflowHandlerHelper.getJobsByRate(rateIdLong.toString()));
                    ...
                    while (it.hasNext())
                    {
                        ...
                        while (sortedTaskIterator.hasNext())
                        {
                            ...
                            if(t != null)
                            {
                                ...
                                if(curRate != null && curRate.getName().equals(p_rate.getName()))
                                {
                                    ...
                                    CostCalculator calculator = new CostCalculator(curJob.getId(), oCurrency, true, Cost.EXPENSE);
                                ...
                                if (sc.getBooleanParameter(SystemConfigParamNames.COSTING_ENABLED) == true)
                                {
                                    ...
                                    if(curRate != null && curRate.getName().equals(p_rate.getName()))
                                    {
                                        ...
                                        String curr = c.getIsoCode();
                                        Currency oCurrency = ServerProxy.getCostingEngine().getCurrency(curr);
                                        CostCalculator calculator = new CostCalculator(curJob.getId(), oCurrency, true, Cost.REVENUE);
                                        calculator.sendToCalculateCost();
                                        nextJob = true;
                                    }
                                }
