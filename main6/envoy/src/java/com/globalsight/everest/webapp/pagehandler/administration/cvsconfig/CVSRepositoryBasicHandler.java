...
public class CVSRepositoryBasicHandler extends PageHandler 
        ...
        try
        {
            if (action.equals(CVSConfigConstants.UPDATE)) {
                ...
                CVSRepository repository = 
                    (CVSRepository)manager.getRepository(Integer.parseInt(id));
                
                sessionMgr.setAttribute(CVSConfigConstants.CVS_REPOSITORY, 
                                        repository);
                
                p_request.setAttribute("edit", "true");
            }
            
            setCVSRepository(p_request);
        }
     ...
     */
    private void setCVSRepository(HttpServletRequest p_request)
        throws RemoteException, NamingException, GeneralException
    {
        CVSServerManagerLocal manager = new CVSServerManagerLocal();
        ArrayList list = (ArrayList)manager.getAllRepository();
        p_request.setAttribute(CVSConfigConstants.CVS_REPOSITORY_LIST, list);
        p_request.setAttribute(CVSConfigConstants.CVS_SERVER_LIST, 
                               manager.getAllServer());
    }
}
