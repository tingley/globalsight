...
public class CVSFileProfileMainHandler extends PageHandler {
    ...
    private void dataForTable(HttpServletRequest p_request, HttpSession p_session, HashMap<String, String> p_params) throws RemoteException, NamingException, GeneralException
	{
    	...
    	//ArrayList<CVSFileProfile> cvsFileProfiles = (ArrayList<CVSFileProfile>)manager.getAllCVSFileProfiles();
    	//sessionMgr.setAttribute(CVSConfigConstants.CVS_FILE_PROFILE_LIST, cvsFileProfiles);

    	Vector data = vectorizedCollection(manager.getAllCVSFileProfiles(p_params));
	    Locale uiLocale = (Locale)p_session.getAttribute(WebAppConstants.UILOCALE);
	
	    setTableNavigation(p_request, p_session, data,
	                   new CVSFileProfileComparator(uiLocale),
	                   10,
	                   CVSConfigConstants.CVS_FILE_PROFILE_LIST, CVSConfigConstants.CVS_FILE_PROFILE_KEY);
	}
     ...
     */
    private void create(HttpServletRequest p_request) throws CVSConfigException, RemoteException {
    	...
    	try {
			...
			project = (ProjectImpl)ServerProxy.getProjectHandler().getProjectById(Long.parseLong(projectId));
			...
			if (indexStr != null && !indexStr.trim().equals("")) {
				...
				for (int i = 0; i < index; i++) {
					...
					if (fp != null && !fp.trim().equals("-1")) {
						...
						cvsfp.setFileProfile((FileProfileImpl)ServerProxy.getFileProfilePersistenceManager().getFileProfileById(Long.parseLong(fp), false));
    ...
    private void update(HttpServletRequest p_request) throws CVSConfigException, RemoteException {
    	CVSFileProfile cvsfp = null;
    	HttpSession session = null;
    	try {
			...
			sessionMgr.setAttribute(CVSConfigConstants.CVS_FILE_PROFILE_KEY, null);
    ...
    private void remove(HttpServletRequest p_request) throws CVSConfigException, RemoteException {
    	try {
    		HttpSession session = p_request.getSession(false);
			SessionManager sessionMgr = (SessionManager) session.getAttribute(WebAppConstants.SESSION_MANAGER);
			ResourceBundle bundle = getBundle(session);
    ...
    private void setSearchInfo(HttpSession p_session, SessionManager p_sessionMgr) throws ProjectHandlerException, RemoteException, GeneralException, NamingException {
        ...
        List projectInfos = ServerProxy.getProjectHandler().getProjectInfosByUser(user.getUserId());
