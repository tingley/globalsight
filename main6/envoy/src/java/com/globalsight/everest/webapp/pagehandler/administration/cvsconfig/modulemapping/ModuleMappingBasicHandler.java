...
public class ModuleMappingBasicHandler extends PageHandler implements ModuleMappingConstants {
    /**
        ...
        throws ServletException, IOException, EnvoyServletException
    {
        ...
        try
        {
        	if (action.equals(UPDATE))
            {
                ...
                ModuleMapping moduleMapping = (ModuleMapping)manager.getModuleMapping(Long.parseLong(id));
            	...
            	String id = (String)p_request.getParameter("id");
            	ModuleMappingRename mmr = manager.getModuleMappingRename(Long.parseLong(id));
            	...
            	ModuleMapping moduleMapping = (ModuleMapping)sessionMgr.getAttribute(MODULE_MAPPING_KEY);
            	ModuleMappingRename mmr = (ModuleMappingRename)sessionMgr.getAttribute(MODULE_MAPPING_RENAME_KEY);
            	if (mmr == null) {
            		mmr = new ModuleMappingRename();
                	mmr.setModuleMapping(moduleMapping);
                	mmr.setSourceName((String)p_request.getParameter(ModuleMappingConstants.SOURCE_NAME));
                	mmr.setTargetName((String)p_request.getParameter(ModuleMappingConstants.TARGET_NAME));
                ...
                p_request.setAttribute("edit", "true");
            	dataForTable(p_request, session, manager.getModuleMapping(moduleMapping.getId()));
            }
    ...
    private void dataForTable(HttpServletRequest p_request, HttpSession p_session, ModuleMapping p_mm) throws RemoteException, NamingException, GeneralException
	{
		...
		sessionMgr.setAttribute(ModuleMappingConstants.MODULE_MAPPING_RENAME_LIST, renames);
	    ...
	    setTableNavigation(p_request, p_session, servers,
	                   new ModuleMappingRenameComparator(uiLocale), 
	                   10,
	                   ModuleMappingConstants.MODULE_MAPPING_RENAME_LIST, ModuleMappingConstants.MODULE_MAPPING_RENAME_KEY);
	}
