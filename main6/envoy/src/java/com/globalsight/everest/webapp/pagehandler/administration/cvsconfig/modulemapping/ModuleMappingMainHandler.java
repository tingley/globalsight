...
public class ModuleMappingMainHandler extends PageHandler {
	...
	public void invokePageHandler(WebPageDescriptor p_pageDescriptor,
		...
		try {
			...
			if (ModuleMappingConstants.CREATE.equals(action)) {
				...
				tmp = (tmp != null && !"-1".equals(tmp.trim())) ? tmp.trim() : "-1";
			...
			session.setAttribute("companyName", CompanyWrapper.getCurrentCompanyName());
			session.setAttribute("mmSearchParam", params == null ? new HashMap<String, String>() : params);
    ...
    private void setModuleMapping(HttpServletRequest p_request) throws RemoteException, NamingException, GeneralException
	{
	    ...
	    p_request.setAttribute(ModuleMappingConstants.SOURCE_LOCALE_PAIRS, sourceLocales);
	    p_request.setAttribute(ModuleMappingConstants.TARGET_LOCALE_PAIRS, targetLocales);
	}

    private void dataForTable(HttpServletRequest p_request, HttpSession p_session, HashMap<String, String> p_params) throws RemoteException, NamingException, GeneralException
	{
    	List list = null;
    	if (p_params == null)
		...
		sessionMgr.setAttribute(ModuleMappingConstants.MODULE_MAPPING_LIST, list);
	    ...
	    setTableNavigation(p_request, p_session, servers,
	                   new ModuleMappingComparator(uiLocale),
	                   10,
	                   ModuleMappingConstants.MODULE_MAPPING_LIST, ModuleMappingConstants.MODULE_MAPPING_KEY);
	}

    private void createModuleMapping(HttpServletRequest p_request) throws ModuleMappingException, RemoteException {
    	...
    	try {
			...
			String srcLocale = ServerProxy.getLocaleManager().getLocaleById(Integer.parseInt(p_request.getParameter(ModuleMappingConstants.SOURCE_LOCALE))).toString();
			String srcLocaleLong = ServerProxy.getLocaleManager().getLocaleByString(srcLocale).getDisplayName();
			String srcModule = p_request.getParameter(ModuleMappingConstants.SOURCE_MODULE);
			...
			ArrayList<String> subFolders = getAllSubFolders(CVSUtil.getBaseDocRoot() + srcModule);
			...
			for (int i = 0; i < count; i++) {
				...
				if (tarLocale == null || tarLocale.equals(""))
					continue;
				tarLocale = ServerProxy.getLocaleManager().getLocaleById(Integer.parseInt(tarLocale)).toString();
				tarLocaleLong = ServerProxy.getLocaleManager().getLocaleByString(tarLocale).getDisplayName();
    ...
    private void updateModuleMapping(HttpServletRequest p_request) throws ModuleMappingException, RemoteException {
    	ArrayList<ModuleMapping> mms = new ArrayList<ModuleMapping>();
    	...
    	try {
			...
			SessionManager sessionMgr = (SessionManager) session.getAttribute(WebAppConstants.SESSION_MANAGER); 
			mm = (ModuleMapping)sessionMgr.getAttribute(ModuleMappingConstants.MODULE_MAPPING_KEY);
			...
			String srcLocale = ServerProxy.getLocaleManager().getLocaleById(Integer.parseInt(p_request.getParameter(ModuleMappingConstants.SOURCE_LOCALE))).toString();
			String srcLocaleLong = ServerProxy.getLocaleManager().getLocaleByString(srcLocale).getDisplayName();
			String tarLocale = ServerProxy.getLocaleManager().getLocaleById(Integer.parseInt(p_request.getParameter(ModuleMappingConstants.TARGET_LOCALE))).toString();
			String tarLocaleLong = ServerProxy.getLocaleManager().getLocaleByString(tarLocale).getDisplayName();
			long serverId = Long.parseLong(p_request.getParameter("cvsServer"));
			String srcModule = p_request.getParameter(ModuleMappingConstants.SOURCE_MODULE);
			String tarModule = p_request.getParameter(ModuleMappingConstants.TARGET_MODULE);
			...
			//if (!mm.getSubFolderMapped().equals(oldMM.getSubFolderMapped())) {
				if (subfolder) {
					//Create multiple module mapping with sub-folders in source module, and create folder under the target module
...
//							existMMs.add(mm);
		...
		try {
			...
			if (list != null) {
				for (int i = 0; i < list.length; i++) {
					tmp = list[i];
					if (tmp.isDirectory() && !"CVS".equals(tmp.getName().toUpperCase())) {
	...
	private void generateTargetFolder(String p_tarModule, ArrayList<String> p_folders, CVSServer p_server) {
		if (p_folders == null || p_folders.size() == 0)
			return;
		String baseTargetPath = CVSUtil.getBaseDocRoot() + p_tarModule + File.separator;
		try {
			File folder = null;
			for (String t : p_folders) {
				...
				if (folder.exists() && folder.isDirectory())
					continue;
				folder.mkdirs();
				String[] cmd = new String[]{"cvs", "-d", p_server.getCVSRootEnv(), "add", t};
