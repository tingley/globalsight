...
import com.globalsight.cxe.persistence.previewurl.PreviewUrlPersistenceManagerLocal;
...
/**
 * DBProfileColumnHandler, A page handler to produce the
 * entry page for DataSources management.
 */
public class DBProfileColumnHandler extends PageHandler
{
    private final int GRID_COLUMN_SIZE = 5;
    ...
    throws ServletException, IOException, EnvoyServletException
    {
        ...
        SessionManager sessionMgr =
            (SessionManager)session.getAttribute(
	...
	if (sessionMgr.getAttribute("DBColumns") == null)
        ...
        if (p_theRequest.getParameter("DBProfileName") != null)
            sessionMgr.setAttribute("DBProfileName", (String)p_theRequest.getParameter("DBProfileName"));
        if (p_theRequest.getParameter("DBProfileDescription") != null)
            sessionMgr.setAttribute("DBProfileDescription", p_theRequest.getParameter("DBProfileDescription"));
        if (p_theRequest.getParameter("locProfile") != null)
            sessionMgr.setAttribute("locProfile", p_theRequest.getParameter("locProfile"));
     ...
     * <p>
     * @param p_isDoGet - Determines whether the request is a get or post.
    ...
    private Vector getData(HttpSession p_session,
                           SessionManager p_sessionMgr)
        throws EnvoyServletException
    {
	ResourceBundle bundle = getBundle(p_session);
	String[] labels = {bundle.getString("lb_new"),
			   bundle.getString("lb_edit"),
			   bundle.getString("lb_remove"),
			   bundle.getString("lb_previous"),
			   bundle.getString("lb_cancel"),
			   bundle.getString("lb_next"),
			   bundle.getString("lb_move_up"),
			   bundle.getString("lb_move_down")};
	String[] header = {bundle.getString("lb_name"),
	...
	m_knownFormatTypePairs = HashtableValueOrderWalkerFactory.createHashtableValueOrderWalker(getKnownFormatTypes());

	m_mode_pairs = getModePairs(bundle);
	DatabaseProfileImpl dbprofile =
            (DatabaseProfileImpl)p_sessionMgr.getAttribute("ModDBProfile");
	long profile_id;
	if (dbprofile != null)
	{
	...
	GridData gridData = getDBColumns(profile_id);
	String[] dialogLabels = {bundle.getString("lb_name") + 
				 ...
				 bundle.getString("lb_globalsight") + bundle.getString("lb_colon") +
	...
	String[] dialogButtons = {bundle.getString("applet_save"),
				  bundle.getString("applet_saveb"),
				  bundle.getString("applet_savex"),
				  bundle.getString("applet_cancel"),
				  bundle.getString("applet_cancelb"),
				  bundle.getString("applet_ok"),
				  bundle.getString("applet_okb"),
				  bundle.getString("applet_okx")};
	String[] imageNames = {bundle.getString("applet_new"),
	...
	// use method in FileProfileMainHandler
	objs.addElement(HashtableValueOrderWalkerFactory.createHashtableValueOrderWalker(getXmlRules(p_session)));
	return objs;
    }

    // create a grid data object
    private GridData getDBColumns(long profileId) throws EnvoyServletException
    {
	Vector dbcolumns = null;

        // if there is no database profile, there are no columns from database to display
        ...
        else
        {
            ...
            try
            {
		dbcolumns = vectorizedCollection(ServerProxy.getDatabaseColumnPersistenceManager().getDatabaseColumns(profileId));
        ...
        GridData gridData = new GridData(rowSize, GRID_COLUMN_SIZE, false, true);
        Integer Itmp;
        for (int i=0; i < rowSize; i++)
        {
            DatabaseColumnImpl dbcolumn = (DatabaseColumnImpl)dbcolumns.elementAt(i);
            gridData.setCellData(1, i+1, dbcolumn);
            gridData.setCellData(2, i+1, dbcolumn.getLabel());
            gridData.setCellData(3, i+1, dbcolumn.getTableName());
            Itmp = new Integer((int)dbcolumn.getFormatType());
            gridData.setCellData(4, i+1, m_knownFormatTypePairs.get(Itmp));
            Itmp = new Integer(dbcolumn.getContentMode());
            gridData.setCellData(5, i+1, m_mode_pairs.get(Itmp));
        }
        return gridData;
    }
    ...
    HashtableValueOrderWalker getModePairs(ResourceBundle bundle)
    {
        ...
        return HashtableValueOrderWalkerFactory.createHashtableValueOrderWalker(mode_pair);
    }

  //process data and save info to session object to pass on to other pages when necessary
    ...
    throws EnvoyServletException, IOException
  {
    try
    {
        ObjectInputStream inputFromApplet = new ObjectInputStream(p_theRequest.getInputStream());
        Vector data = (Vector)inputFromApplet.readObject();
        if (data != null) {
            DatabaseColumnImpl dbcolumn = (DatabaseColumnImpl)data.elementAt(1);
            //HttpSession session = p_theRequest.getSession();
		...
		p_sessionMgr.setAttribute("DBColumns", m_DBColumns);
		m_ModDBColumns = "True";
		p_sessionMgr.setAttribute("ModDBColumns", m_ModDBColumns);
		return data;//break;
	    case TRANSPOSE:
		// the incoming element always has the lowest index
		// ColumnNumber's run 1,...,N and Vector indices 0,..., (N-1)
		int itmp = (int)dbcolumn.getColumnNumber();
		DatabaseColumnImpl dbcolumn1 = (DatabaseColumnImpl)m_DBColumns.set(itmp-1, dbcolumn);
		dbcolumn1.setColumnNumber(dbcolumn1.getColumnNumber()+1);
		...
		m_DBColumns.remove(index1);
                    // decrement the column numbers of all columns with higher column numbers
		DatabaseColumnImpl dbcolumn2;
		for (int i=index1; i < m_DBColumns.size(); i++)
                {
		    dbcolumn2 = (DatabaseColumnImpl)m_DBColumns.elementAt(i);
		    dbcolumn2.setColumnNumber(dbcolumn2.getColumnNumber()-1);
		}
		p_sessionMgr.setAttribute("DBColumns", m_DBColumns);
		break;
	    }
    ...
    {
        throw new EnvoyServletException(EnvoyServletException.EX_GENERAL, ex);
    }
    ...
    private Hashtable getKnownFormatTypes()
        throws EnvoyServletException
    {
        //Call server
        ...
        try
        {
            knownFormatTypes = vectorizedCollection(
        ...
        for (int i = 0; i < rowSize; i++)
        {
            KnownFormatTypeImpl knownformattype =
                (KnownFormatTypeImpl)knownFormatTypes.elementAt(i);
        ...
        throws EnvoyServletException
    {
        ...
        try
        {
            xmlrulefiles = vectorizedCollection(
                ServerProxy.getXmlRuleFilePersistenceManager().
                getAllXmlRuleFiles());
        }
        catch (Exception e)
        {
        ...
        for (int i = 0; i < rowSize; i++)
        {
            XmlRuleFileImpl xmlrulefile =
                (XmlRuleFileImpl)xmlrulefiles.elementAt(i);
