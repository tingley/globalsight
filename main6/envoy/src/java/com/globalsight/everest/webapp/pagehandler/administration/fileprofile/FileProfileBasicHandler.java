...
public class FileProfileBasicHandler extends PageHandler
{
        ...
        throws ServletException, IOException, EnvoyServletException
    {
        ...
        try
        {
        	...
        	if (FileProfileConstants.VERIFY_ACTION.equals(action))
        	{
        		...
        		{
        			message = String.valueOf(FileProfileConstants.FILE_NOT_EXIST); 
        		}
        		else
        		{
            ...
            if (action.equals(FileProfileConstants.EDIT))
            {
    			...
    			if (idString != null) 
    			{
					...
					formatType = ServerProxy.getFileProfilePersistenceManager()
    ...
    private void setDataForComboBoxes(HttpServletRequest p_request, String p_action, String p_format, List p_extension)
        throws GeneralException, NamingException, RemoteException
    {
        ...
        p_request.setAttribute("names", 
                ServerProxy.getFileProfilePersistenceManager().getAllFileProfiles());

        // format types
        List allFormatTypes = new ArrayList(ServerProxy.getFileProfilePersistenceManager().getAllKnownFormatTypes());
        ...
        long companyId = ServerProxy.getJobHandler().getCompany(companyName)
                .getIdAsLong();
        p_request.setAttribute("filters", FilterHelper.getKnownFormatFilterMap(allFormatTypes, companyId));
        
        // Data encodings
        p_request.setAttribute("encodings", ServerProxy.getLocaleManager().getAllCodeSets());

        // XML Rules
        p_request.setAttribute("xmlRules",
        ...
        // File Extensions
        List extensions = new ArrayList(ServerProxy.getFileProfilePersistenceManager().getAllFileExtensions());
        ...
        if (session != null && session.getAttribute(WebAppConstants.UILOCALE) != null) 
        {
            ...
            Collections.sort(extensions, new FileExtensionComparator(locale));
        }
        ...
        /**
        List xlfExts = new ArrayList();
        xlfExts.add("xlf");
        xlfExts.add("xliff");
        ArrayList xlfFps = new ArrayList(ServerProxy
                .getFileProfilePersistenceManager().getFileProfilesByExtension(
                        xlfExts, companyId));
        p_request.setAttribute("xlfFps", xlfFps);
        */
    }
    
    // Hidden some useless file format
    public void  hideSomeFormat(List<?> p_allFormatTypes, String p_action, String p_useFormat)
    {
    	ArrayList<String> ulFormatlist = new ArrayList<String>(FileProfileConstants.useLessFormatList);
