...
public class GSEditionMainHandler extends PageHandler
{
            ...
            if(action != null) {
                ...
                else if (action.equals("remove"))
                {
                    ...
                    StringBuffer fileStorageRoot = new StringBuffer(SystemConfiguration
                            .getInstance().getStringParameter(SystemConfigParamNames.FILE_STORAGE_DIR));
                    StringBuffer fileStorageRootBak = new StringBuffer(fileStorageRoot.toString());
                    ...
                    fileStorageRoot = fileStorageRoot.append(File.separator).append(
                            ...
                            .append(jobName).append(File.separator).append(local);

                    File file = new File(fileStorageRoot.toString() + File.separator + fileName);
                    if (!file.exists() || !file.isFile())
                    {
                    	JobHandler jobHandler = ServerProxy.getJobHandler();
                        Job job = jobHandler.getJobByJobName(jobName);
                        long companyId = new Long(job.getCompanyId()).longValue();
                        String companyName = jobHandler.getCompanyById(companyId).getCompanyName();
                        if (fileStorageRootBak.toString().endsWith(companyName) == false) {
                        	fileStorageRootBak.append(File.separator).append(companyName);
                        	fileStorageRootBak = fileStorageRootBak.append(File.separator).append(
                                    WebAppConstants.VIRTUALDIR_TOPLEVEL).append(File.separator).append(
                                    WebAppConstants.ORIGINAL_SORUCE_FILE).append(File.separator)
                                    .append(jobName).append(File.separator).append(local);
                        }
                        file = new File(fileStorageRootBak.toString() + File.separator + fileName);
                    }
    ...
    {
        Vector activities = 
            vectorizedCollection(gsEditionManager.getAllGSEdition());
        Locale uiLocale = (Locale)p_session.getAttribute(
                                    WebAppConstants.UILOCALE);

        setTableNavigation(p_request, p_session, activities,
                           new GSEditionComparator(uiLocale),
                           10,"GSEditionList", "GSEditonKey");
    }
    ...
    private void createGSEdition(HttpServletRequest p_request) 
        ...
        try {
            ...
            Ambassador ambassador = WebServiceClientHelper.getClientAmbassador(
            		hostName, hostPort, userName, password, gsEdition.getEnableHttps());
            ...
            String permissionStr = ambassador.getAllPermissionsByUser(realAccessToken);
    ...
    private void editGSEdition(HttpServletRequest p_request) 
        ...
        if(p_request.getParameter("gsEditionID") != null) 
        {
            ...
            try {
                ...
                Ambassador ambassador = WebServiceClientHelper.getClientAmbassador(gsEdition.getHostName(), 
                                                                                   gsEdition.getHostPort(),
                                                                                   gsEdition.getUserName(),
                                                                                   gsEdition.getPassword(),
                                                                                   gsEdition.getEnableHttps());
                String fullAccessToken = ambassador.login(gsEdition.getUserName(), gsEdition.getPassword());
                String realAccessToken = WebServiceClientHelper.getRealAccessToken(fullAccessToken);
    ...
    private void removeGSEdition(HttpServletRequest p_request) 
        ...
        if(p_request.getParameter("gsEditionID") != null) {
            ...
            try {
                Collection allProjectTms = ServerProxy.getProjectHandler().getAllProjectTMs();
