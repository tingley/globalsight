...
public class PermissionHelper
{
    ...
    /**
     * Gets all the PermissionGroups that this user can assign.
     * If the user has the permission to assign all permission groups,
     * then he gets all. If he doesn't, then he only gets permissiongroups
     * which are a subset of his own permission set.
     * 
     * @return Collection
     */
    ...
    throws EnvoyServletException
    {
        try
        {
            ...
            if (p_userPermissionSet.getPermissionFor(Permission.USERS_EDIT_ASSIGN_ANY_PERMGROUPS))
        ...
        throws  EnvoyServletException
    {
        try
        {
            Collection pGroups = Permission.getPermissionManager()
                .getAllPermissionGroupsByCompanyId(CompanyWrapper.SUPER_COMPANY_ID);
        ...
        throws  EnvoyServletException
    {
        try
        {
            return Permission.getPermissionManager().getAllPermissionGroupsForUser(p_userId);
        }
        catch (Exception e)
        {
            throw new EnvoyServletException(e);
        ...
        throws  EnvoyServletException
    {
        try
        {
            ArrayList userIds = (ArrayList)
                 Permission.getPermissionManager().getAllUsersForPermissionGroup(p_id);
            ArrayList users = new ArrayList(userIds.size());
            // Loop through and fetch User's because we want the user's name too.
     ...
     */
    public static void saveBasicInfo(PermissionGroup permGroup, HttpServletRequest request)
        throws  EnvoyServletException
    {
        String buf = (String) request.getParameter("nameField");
        if (buf != null) permGroup.setName(buf);
        buf = (String) request.getParameter("descField");
        if (buf != null) permGroup.setDescription(buf);
     ...
     */
    public static void savePermissionSet(PermissionGroup permGroup, HttpServletRequest request)
        throws  EnvoyServletException
    {
        ...
        while (enumeration.hasMoreElements()) {
            ...
            {
                // strip off "perm."
                set.setPermissionFor(name.substring(5), true);
            }
        }
     ...
     */
    public static void saveUsers(PermissionGroup permGroup, HttpServletRequest p_request)
        throws EnvoyServletException
    {
        if (p_request == null)
            return;
    ...
    public static void updateUsers(PermissionGroup permGroup, HttpServletRequest request)
        throws  EnvoyServletException
    {
        ...
        ArrayList changed = (ArrayList) sessionMgr.getAttribute("usersForGroup");
        if (changed == null) return;
        ArrayList existing = (ArrayList)PermissionHelper.getAllUsersForPermissionGroup(permGroup.getId());
            ...
            else
            {
                ...
                for (int i = 0; i < existing.size(); i++)
                {
                    ...
                    for (int j = 0; j < changed.size(); j++)
                    {
                        ...
                        if (user.getUserId() == cuser.getUserId())
                        {
                            found = true;
                            break;
                        }
                    }
                    if (!found) remove.add(user.getUserId());
                }
                if (remove.size() > 0)
                ...
                for (int i = 0; i < changed.size(); i++)
                {
                    ...
                    if (!found) add.add(user.getUserId());
                }
                if (add.size() > 0)
    ...
    /*
     * Parse the xml and create a new xml file and store it in the session.
     * The reason to create a new xml file is that the jsp needs to know 
     * localized labels and if the permission is set or not.  So it'll take
     * something like:
     *
     *  <permission id="localePairs.remove"/>
     * and generate:
     *  <permission id="localePairs.remove" label="Remove" set="false"/>
     */
    public static void setPermissionXmlInSession(SessionManager sessionMgr,
                                 HttpSession session, PermissionSet permSet,
                                 String permissionXml)
    {
        ...
        try
        {
            ...
            for (int i=0; i < perms.getLength(); i++)
            {
                ...
                perm.setAttribute("set", permSet == null ? "false" : String.valueOf(permSet.getPermissionFor(id)));
            }
        }
