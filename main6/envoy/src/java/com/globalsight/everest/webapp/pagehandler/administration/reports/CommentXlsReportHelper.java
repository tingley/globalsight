...
public class CommentXlsReportHelper 
{
    ...
    throws Exception
    {
        ...
        String paramCreateDateStartCount = 
            p_request.getParameter(JobSearchConstants.CREATION_START);
        String paramCreateDateStartOpts = 
            p_request.getParameter(JobSearchConstants.CREATION_START_OPTIONS);
        ...
        String paramCreateDateEndCount = 
            p_request.getParameter(JobSearchConstants.CREATION_END);
        String paramCreateDateEndOpts = 
            p_request.getParameter(JobSearchConstants.CREATION_END_OPTIONS);
     ...
     * @param p_sheet
     */
    private void addJobCommentHeader(WritableSheet p_sheet, ResourceBundle bundle) throws Exception
    {
     ...
     * @param p_sheet
     */
    private void addTaskCommentHeader(WritableSheet p_sheet, ResourceBundle bundle) throws Exception
    {
     ...
     * @param p_sheet
     */
    private void addSegmentCommentHeader(WritableSheet p_sheet, ResourceBundle bundle) throws Exception
    {
     ...
     * @param p_sheet
     */
    private void addHeader(WritableSheet p_sheet, String sheetTitle, ResourceBundle bundle)
    throws Exception
    {
        ...
        p_sheet.addCell(new Label(col++,row, bundle.getString("job_name"),headerFormat));
        p_sheet.setColumnView(col -1,10);        
        p_sheet.addCell(new Label(col++,row, bundle.getString("language"),headerFormat));
        p_sheet.setColumnView(col -1,10);
        p_sheet.addCell(new Label(col++,row, bundle.getString("segment_number"),headerFormat));
        p_sheet.setColumnView(col -1,20); 
        p_sheet.addCell(new Label(col++,row, bundle.getString("by_who"),headerFormat));
        p_sheet.setColumnView(col -1,15);
        p_sheet.addCell(new Label(col++,row, bundle.getString("on_date"),headerFormat));
        p_sheet.setColumnView(col -1,25);
        
        if (showStatus == true)
        {
            p_sheet.addCell(new Label(col++,row, bundle.getString("status"),headerFormat));
        }
        p_sheet.setColumnView(col -1,10);
        if (true == showPriority)
        {
            p_sheet.addCell(new Label(col++,row, bundle.getString("priority"),headerFormat));
        }
        if (showCategory)
        {
            p_sheet.addCell(new Label(col++,row, bundle.getString("category"),headerFormat));
        }
        p_sheet.setColumnView(col -1,10);
        p_sheet.addCell(new Label(col++,row,  bundle.getString("comment_header"),headerFormat));
        p_sheet.setColumnView(col -1,20);
        p_sheet.addCell(new Label(col++,row, bundle.getString("comment_body"),headerFormat));
        p_sheet.setColumnView(col -1,25);
        p_sheet.addCell(new Label(col++,row, bundle.getString("link"),headerFormat));
        p_sheet.setColumnView(col -1,20);             
    }
     ...
     */
     ...
     throws Exception 
     {
         ...
         paramJobId = (paramJobId == null) ? new String[] { "*" } : paramJobId;
     ...
     */
     ...
     {
         String[] paramCommentPriority = 
             p_request.getParameterValues("commentPriority_On");  
    ...
    private void setStatus(HttpServletRequest p_request)
    {
        ...
        String[] paramCommentStatusOpen = 
            p_request.getParameterValues("commenStatus_open");    
        String[] paramCommentStatusQuery = 
            p_request.getParameterValues("commenStatus_query");    
        String[] paramCommentStatusClosed = 
            p_request.getParameterValues("commenStatus_closed");    
    ...
    private HashMap addAllHeader(HttpServletRequest p_request, 
                              WritableSheet jobSheet,
                              WritableSheet taskSheet, 
                              WritableSheet segmentSheet)
    throws Exception
    {
        ...
        segmentSheet = m_workbook.createSheet(bundle.getString("segment_comments"),0);
        ...
        if ((paramCommentTypeJob !=null ) && ("Job".equals(paramCommentTypeJob[0]) == true))
        {
           ...
           {
               jobSheet = m_workbook.createSheet(bundle.getString("job_comments"),1);
        ...
        {            
            ...
            {
                taskSheet = m_workbook.createSheet(bundle.getString("activity_comments"),2);
    ...
    private void addComments(HttpServletRequest p_request) throws Exception
    {
        ...
        HashMap sheetMap = addAllHeader(p_request, jobSheet, taskSheet, segmentSheet);
     ...
     *
     */
    private void addJobComment(HttpServletRequest p_request, Job j, WritableSheet jobSheet, IntHolder row) throws Exception
    {
        if (null == jobSheet)
        {
            return;
     ...
     *
     */
    private void addTaskComment(HttpServletRequest p_request, Job j, WritableSheet taskSheet, IntHolder row) throws Exception
    {
        if (null == taskSheet) return;
     ...
     *
     */
    private void addSegmentCommentFilter(HttpServletRequest p_request, Job j, WritableSheet segmentSheet, IntHolder row, List comments, int flag) throws Exception
    {
        ...
        for (Iterator it = comments.iterator(); it.hasNext(); )
        {
            ...
            {              
                ...
                for (Iterator iterator = list.iterator(); iterator.hasNext();) {
                    ...
                    addCommentHistory(p_request, j, segmentSheet, row, comment, issueHistory, flag);
                }
     ...
     *
     */
    private void addSegmentComment(HttpServletRequest p_request, Job j, WritableSheet segmentSheet, IntHolder row) throws Exception
    {
        ...
        for (Iterator it = c.iterator(); it.hasNext(); )
        {
            ...
            for (Iterator i = targetPages.iterator() ; i.hasNext() ; )
            {
                ...
                List commentList = ServerProxy.getCommentManager().
                                       getIssues(Issue.TYPE_SEGMENT, targPageId+"_");
    ...
    *@exception Exception
    */
    private void addComment(HttpServletRequest p_request,
                            Job j, 
                            WritableSheet sheet,
                            IntHolder row, 
                            Comment comment, 
                            int flag) throws Exception
    {
        ...
        {
            sheet.addCell(new Label(c++,r, this.targetPageVar.getGlobalSightLocale().getDisplayName(uiLocale)));
        }
        else
        {
        ...
        // by who
        sheet.addCell(new Label(c++,r, comment.getCreatorId()));
        
        //2.7   Comment create date: Insert Comment creation date.
        sheet.addCell(new Label(c++,r, dateFormat.format(comment.getCreatedDateAsDate())));
        ...
        {
            sheet.addCell(new Label(c++,r, ""));
            sheet.addCell(new Formula(c++,r, "HYPERLINK(\"" + pageUrl + "\",\"" + externalPageId  + "\")"));
        }
    ...
    /**
     * add Comment for each row, for segment comment usage, each row contains a comment history
     *@exception Exception
     */
     private void addCommentHistory(HttpServletRequest p_request,
                             Job j, 
                             WritableSheet sheet,
                             IntHolder row, 
                             Comment comment,
                             ...
                             int flag) throws Exception
     {
         ...
         // by who
         sheet.addCell(new Label(c++,r, issueHistory.reportedBy()));
         
         //2.7.2    Comment create date: Insert Comment creation date.
         sheet.addCell(new Label(c++,r, dateFormat.format(issueHistory.dateReportedAsDate())));
