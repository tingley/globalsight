...
public class VendorPOXlsReport
{
    ...
    public VendorPOXlsReport(HttpServletRequest request, HttpServletResponse response)
            throws Exception
    {
     ...
     * @exception Exception
     */
    private void generateReport(HttpServletRequest p_request, HttpServletResponse p_response,
            MyData p_data) throws Exception
    {
        ...
        HashMap projectMap = getProjectData(p_request, p_response, recalculateFinishedWorkflow,
                p_data);
    ...
    private void addHeaderForDellMatches(Workbook p_workbook, MyData p_data) throws Exception
    {
        ...
        if (p_data.headers[0] != null)
        {
            Cell cell_InContext = getCell(fourRow, c++);
            cell_InContext
                    .setCellValue(bundle.getString("jobinfo.tmmatches.invoice.incontextmatches"));
            cell_InContext.setCellStyle(getHeaderStyle(p_workbook));
        }

        Cell cell_FuzzyMatches_Invoice = getCell(fourRow, c++);
        cell_FuzzyMatches_Invoice
                .setCellValue(bundle.getString("jobinfo.tmmatches.invoice.fuzzymatches"));
    ...
    private void addHeaderForTradosMatches(Workbook p_workbook, MyData p_data) throws Exception
    {
        ...
        cell_Per100Matches
                .setCellValue(bundle.getString("jobinfo.tradosmatches.invoice.per100matches"));
            ...
            HttpServletRequest p_request) throws Exception
    {
        ...
        {
            cell_A_Header.setCellValue(
                    bundle.getString("lb_selected_projects") + " " + bundle.getString("all"));
        ...
        else
        {
            ...
            while (iter.hasNext())
            {
                ...
                String v = projectName + " (" + bundle.getString("lb_report_id") + "="
                        + pid.toString() + ")";
        ...
        {
            cell_D_Header.setCellValue(
                    bundle.getString("lb_selected_langs") + " " + bundle.getString("all"));
            ...
            boolean p_recalculateFinishedWorkflow, MyData p_data) throws Exception
    {
        ...
        {
            queriedJobs.addAll(
                    ServerProxy.getJobHandler().getJobs(getSearchParams(p_request, p_data)));
        ...
        for (Job j : jobs)
        {
            ...
            if (!(Job.READY_TO_BE_DISPATCHED.equals(j.getState())
                    || Job.DISPATCHED.equals(j.getState()) || Job.EXPORTED.equals(j.getState())
                    || Job.EXPORT_FAIL.equals(j.getState()) || Job.ARCHIVED.equals(j.getState())
            ...
            while (wfIter.hasNext())
            {
                ...
                if (!(Workflow.READY_TO_BE_DISPATCHED.equals(state)
                        || Workflow.EXPORTED.equals(state) || Workflow.DISPATCHED.equals(state)
                        || Workflow.LOCALIZED.equals(state) || Workflow.EXPORT_FAILED.equals(state)
                ...
                if (costByWordCount != null)
                {
                    ...
                    else
                    {
                        ...
                        data.contextMatchWordCountCost = add(
                                new BigDecimal(BIG_DECIMAL_ZERO_STRING),
                                costByWordCount.getContextMatchCost());
                    }
