...
public class ReportFile
{
    ...
    boolean isLazy;
    boolean expand;
    List<ReportFile> children;
    
    public ReportFile(File p_file)
    { 
        this(p_file.getName(), p_file.getPath(), p_file.isDirectory(), p_file.isDirectory(), 
                false, null);
    }
    
    public ReportFile(String p_title, String p_key, boolean p_isFolder, boolean p_isLazy, 
            boolean p_expand, List<ReportFile> p_children)
    {
        title = p_title;
        key = p_key;
        isFolder = p_isFolder;
    ...
    public List<ReportFile> getChildren()
    {
        return children;
    }

    public void setChildren(List<ReportFile> children)
    {
        this.children = children;
    }
    
    public void addChildren(ReportFile p_child)
    {
        if (children == null)
        {
            children = new ArrayList<ReportFile>();
        }
    ...
    public String toJSON()
    {
        ...
        if (title != null && title.trim().length() > 0)
        {
            ...
            if (children != null && children.size() > 0)
            {
                ...
                for (ReportFile reportFile : children)
                {
