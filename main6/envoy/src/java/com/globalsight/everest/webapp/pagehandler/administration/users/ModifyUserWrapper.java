...
/**
 * Helper class used by pagehandlers to contain code used to modify
 * existing users, assign them to the appropriate groups, and generate
 * all their user roles.
 */
    ...
    extends CreateUserWrapper
{
     ...
     * Default constructor.
     */
    public ModifyUserWrapper(UserManager p_userMgr, 
                             User p_userRequestingMod,
                             User p_user)
        throws RemoteException, GeneralException, NamingException
    {
        ...
        ArrayList<UserDefaultRole> tmp = SetDefaultRoleUtil.getDefaultRolesByUser(m_user.getUserId());
        if (tmp != null) {
        	m_defaultRoles = new ArrayList<UserDefaultRole>(tmp);
        	m_tmpDefaultRoles = new ArrayList<UserDefaultRole>(tmp);
        	HashMap<String, UserDefaultRole> hash = SetDefaultRoleUtil.convert(tmp); 
    ...
    /**
     * Called to commit the contents of the wrapper back to LDAP,
     * using the UserManager we've held onto.
     */
     ...
     * Called to commit the contents of the wrapper back to LDAP,
     * using the UserManager we've held onto.
     * Calendar information is not updated.  
     */
    public void commitWrapper()
        throws EnvoyServletException

    {  
        try
        {
            m_userMgr.modifyUser(m_userPerformingAction, m_user, 
                                 m_projects, m_fieldSecurity,
                                 new ArrayList(m_roles));
            SetDefaultRoleUtil.saveDefaultRoles(m_user.getUserId(), m_defaultRoles);
        }
        catch (Exception e)
        {
...
//            }
            ...
            Vector vTargets = (Vector) p_sourceTargetMap.get(sourceLocale + "=" + companyId);
...
//            }
    ...
    /**
     * Generates a batch of user roles from a source/target locale pair,
     * and a Hashtable of activity-cost pairs.
     *
     * @param p_sourceLocale <code>Locale.toString()</code> for the
     * valid source locale.
     * @param p_targetLocale <code>Locale.toString()</code> for the
     * valid target locale.
     * @param p_activityCostMap A Hashtable containing String names of
     * activities as keys, and Float values of costs for those
     * activities.
     */
    public void addUserRoles(String p_sourceLocale, String p_targetLocale,
        Hashtable p_activityCostMap)
        throws EnvoyServletException
    {
        addUserRoles(p_sourceLocale, p_targetLocale, 
                     p_activityCostMap, getCurCompanyId());
    }
        ...
        try
        {
            ...
            while (eKeys.hasMoreElements())
            {
                ...
                if (UserUtil.isJobCostingEnabled())
                {
                    ...
                    Rate expenseRate;
                    expenseRate = (Rate)ServerProxy.getCostingEngine().getRate(expense);
                    if (expenseRate != null)
                    {
                        userRole.setRate((new Long(expenseRate.getId())).toString());
                    }
                    else
                    {
                        userRole.setRate("-1");
            ...
            Vector vTargets = (Vector) p_sourceTargetMap.get(p_sourceLocale + "=" + p_companyId);

            if (vTargets == null)
            {
                vTargets = new Vector();
                p_sourceTargetMap.put(p_sourceLocale + "=" + p_companyId, vTargets);
            }
    ...
    /**
     * Removes all roles for the specified source and target locales,
     * and purges the pairs form the source/target map, too.
     */
        ...
        if(vTargets != null)
        {
            ...
            while (eRoles.hasMoreElements())
            {
                ...
                if (curRole.getSourceLocale().equalsIgnoreCase(p_sourceLocale) &&
                    curRole.getTargetLocale().equalsIgnoreCase(p_targetLocale) &&
                    curRole.getActivity().getCompanyId().equals(p_companyId))
                {
                    if(curRole != null)
                    {
    ...
    public Vector getRoles(String p_sourceLocale, String p_targetLocale)
    {
        ...
        for (int i = 0; i < m_tmpRoles.size(); i++)
        {
            ...
            if (p_sourceLocale.equalsIgnoreCase(curRole.getSourceLocale()) &&
                p_targetLocale.equalsIgnoreCase(curRole.getTargetLocale()))
            {
    ...
    public void addDefaultRole(UserDefaultRole p_role) {
    	...
    	String key = p_role.getSourceLocaleId() + "_" + p_role.getTargetLocaleId();
    ...
    private void resetDefaultRole(UserDefaultRole p_role, UserDefaultRole p_newRole) {
    	if (p_role == null)
    		return;
    	int i;
...
//    	}
    ...
    public void cancelDefaultRoles() {
    	...
    	m_tmpDefaultRolesHash = (HashMap<String, UserDefaultRole>) m_defaultRolesHash.clone();
    }
    ...
    public ArrayList<String> getTargetLocalesOfDefaultRole() {
    	...
    	if (m_tmpDefaultRolesHash == null) 
    		return tls;
    	for (Iterator it = m_tmpDefaultRolesHash.keySet().iterator(); it.hasNext();) {
