...
public class SetDefaultRoleUtil {
	...
	public static ArrayList<UserDefaultRole> getDefaultRolesByUser(String p_userId) {
		...
		long sourceLocaleId, targetLocaleId, id;
		UserDefaultRole role = null;
		UserManagerWLRemote userManager = ServerProxy.getUserManager();
		LocaleManagerWLRemote localeManager = ServerProxy.getLocaleManager();
	...
	public static HashMap<String, UserDefaultRole> convert(ArrayList<UserDefaultRole> p_roles) {
		...
		for (int i = 0; i < p_roles.size(); i++) {
			role = p_roles.get(i);
			result.put(role.getSourceLocaleId() + "_" + role.getTargetLocaleId(), role);
		}
		return result;
	}
	...
	public static void saveDefaultRoles(String p_userId, ArrayList<UserDefaultRole> p_roles) {
		UserDefaultRole role = null;
		if (p_roles == null || p_roles.size() == 0)
			return;
		try {
	...
	private static UserDefaultRole contained(ArrayList<UserDefaultRole> p_list, UserDefaultRole p_role) {
		...
		for (int i=0;i<p_list.size();i++) {
			role = p_list.get(i);
			if (role.getSourceLocaleId() == p_role.getSourceLocaleId() && role.getTargetLocaleId() == p_role.getTargetLocaleId() && role.getUserId().equals(p_role.getUserId()))
				return role;
		}
		return null;
	}
	...
	public static void setUserDefaultRoleToProject(Project p_project) {
		...
		try {
			...
			ArrayList noActivities = new ArrayList();
			...
			Vector acts = UserHandlerHelper.getAllActivities(new Locale("en_US"));
					...
					if (userRoles == null) {
						...
						for (Iterator keys = existActivities.keySet().iterator(); keys.hasNext();) {
							...
							for (int k =0; k<roles.size();k++) {
								...
								if (role.getSourceLocale().equals(sourceLocale) && role.getTargetLocale().equals(targetLocale)) {
	...
	private static void removeProceedRole(ArrayList p_roles, String p_srcLocale, String p_tarLocale) {
		if (p_roles == null || p_roles.size() == 0)
			return;
		...
		for (int i=0;i<p_roles.size();i++) {
			...
			if (role.getSourceLocale().equals(p_srcLocale) && role.getTargetLocale().equals(p_tarLocale))
				p_roles.remove(role);
		}
	}
