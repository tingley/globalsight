            ...
            implements VendorConstants
{
        ...
        EnvoyServletException
    {
        ...
        if (action != null && (action.startsWith("cancel") || action.equals("ok")))
        {
            VendorSearchParameters sp =
                 (VendorSearchParameters) sessionMgr.getAttribute("vendorSearch");
    ...
    throws EnvoyServletException
    {
        ...
        setTableNavigation(request, session, vendors, 
                           new VendorComparator(uiLocale),
                           vendorsPerPage, 
                           VENDOR_LIST,
                           VENDOR_KEY);
     ...
     */
    private void addPermissionGroups(Vendor p_vendor, SessionManager p_sessionMgr)
        throws EnvoyServletException
    {
        ArrayList userPerms = (ArrayList) p_sessionMgr.getAttribute("userPerms");
        if (userPerms == null || userPerms.size() ==0)
        {
            CATEGORY.warn("No permission groups are being mapped to vendor " + p_vendor.getUserId());
            return;
        }
     ...
     * do the update.
     */
    private void updatePermissionGroups(Vendor p_vendor, SessionManager p_sessionMgr)
        throws EnvoyServletException
    {
