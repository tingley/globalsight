...
public class BasicWorkflowTemplateHandler extends PageHandler
        implements WorkflowTemplateConstants
{
            ...
            EnvoyServletException
    {
        ...
        {
            sessionMgr.setAttribute(CHOSEN_NAME, p_request
                                    .getParameter(NAME_FIELD));
            sessionMgr.setAttribute(CHOSEN_DESCRIPTION, p_request
                                    .getParameter(DESCRIPTION_FIELD)); 
            sessionMgr.setAttribute(CHOSEN_PROJECT, p_request
                                    .getParameter(PROJECT_FIELD));
            sessionMgr.setAttribute(CHOSEN_NOTIFICATION, p_request
                                    .getParameter(NOTIFICATION_FIELD));
            sessionMgr.setAttribute(CHOSEN_LOCALE_PAIR, p_request
                                    .getParameter(LOCALE_PAIR_FIELD));
            sessionMgr.setAttribute(CHOSEN_TARGET_ENCODING, p_request
                                    .getParameter(ENCODING_FIELD));
        }
        else
        {
    ...
    /**
     * Set necessary information in the session. This is
     * when they are creating a NEW workflow.
     * 
     * @param p_request
    ...
    throws EnvoyServletException
    {
        ...
        HashMap wfManagers = 
            WorkflowTemplateHandlerHelper.getAllWorkflowManagersInProject();
        List localePairs = 
            WorkflowTemplateHandlerHelper.getAllLocalePairs(uiLocale);
        List targetEncodings = 
            WorkflowTemplateHandlerHelper.getAllCodeSets();
    ...
    /**
     * This method is used during modify workflow template. 
     ...
     * @exception EnvoyServletException
     */
    private void addWorkflowToRequest(HttpServletRequest p_request, String p_wfId)
        throws EnvoyServletException
    {
        long wfId = -1;
        // This should be part of a util class (after adding paging to a util class, i'll add
        // this one too.
        try
        {
        ...
        WorkflowTemplateInfo wfti = storedWfti == null ?
        ...
        p_request.setAttribute(TEMPLATE_ID, new Long(wfti.getWorkflowTemplateId()));

        LocalePair lp = 
            WorkflowTemplateHandlerHelper.getLocalePairBySourceTargetIds(
                wfti.getSourceLocale().getId(), 
    ...
    /**
     * Set the all possible locales available for leveraging
     * based on the available locale pairs. We'll then
     * use these locales in javascript to populate the "Leverage from:"
     * multi-select list.
     * 
     * @param p_request
     * @param localePairs
     * @exception EnvoyServletException
     */
    public void setLeverageLocales(HttpServletRequest p_request, List localePairs)
    throws EnvoyServletException
    {
        ...
        {                                    
            ...
            for (int j = 0; j < numsupportedLocales; j++)
            {
                ...
                {   
                    ...
                    if(!validLeverageLocale(trgLocale,(GlobalSightLocale)
                                            supportedLocales.elementAt(j)) ||
                       (targetLeverageLocalesObjects.contains(supportedLocales.elementAt(j))))
                    {
                        continue;
                    }

                    targetLeverageLocalesObjects.addElement(supportedLocales.elementAt(j));
                    targetLeverageLocalesDisplays.addElement((String)displayNames.elementAt(j));
                }
            }            
        }
        sessionMgr.setAttribute(LEVERAGE_OBJ, (Vector)targetLeverageLocalesObjects);
        sessionMgr.setAttribute(LEVERAGE_DISP, (Vector)targetLeverageLocalesDisplays);
    }
     ...
     * @return 
     */
    private boolean validLeverageLocale(GlobalSightLocale p_defaultTargetLocale,
