...
public class LocProfileNewAndEditHandler extends PageHandler implements LocProfileStateConstants
{
    private static final Logger CATEGORY = Logger.getLogger(LocProfileNewAndEditHandler.class.getName());
    
    public void invokePageHandler(WebPageDescriptor p_pageDescriptor,HttpServletRequest p_request,HttpServletResponse p_response,ServletContext p_context)
        ...
        if("edit".equals(action)){
            ...
            if (id == null || p_request.getMethod().equalsIgnoreCase(REQUEST_METHOD_GET)) 
            {
                p_response.sendRedirect("/globalsight/ControlServlet?activityName=locprofiles");
                return;
            }
            BasicL10nProfile editLocprofile = (BasicL10nProfile) LocProfileHandlerHelper.getL10nProfile(Long.parseLong(id));
            ...
            throws ServletException, IOException, EnvoyServletException
    {
        ...
        p_request.setAttribute("names", LocProfileHandlerHelper.getL10nProfileNames());
    ...
    private String getJSONWorkflows(HttpServletRequest p_request){
        String locProfileProjectId = p_request.getParameter("locProfileProjectId");
        String sourceLocaleId = p_request.getParameter("sourceLocaleId");
        HttpSession session = p_request.getSession();
        Locale uiLocale = (Locale) session.getAttribute(UILOCALE);
        Hashtable targetlocale_pairs = getTargetLocalesByProject(uiLocale, Long.parseLong(sourceLocaleId), locProfileProjectId);
        
        Vector targetLocalesId = (Vector)targetlocale_pairs.get("object");
        Vector targetLocalesDisplayNames = (Vector)targetlocale_pairs.get("display");
        JSONObject jsonObj = new JSONObject();
        JSONArray jsonArr = new JSONArray();
        try{
            for(int i=0; i<targetLocalesId.size();i++){
                long targetLocaleIdLong = Long.parseLong((String) targetLocalesId.elementAt(i));
                long sourceLocaleIdLong = Long.parseLong(sourceLocaleId);
                long locProfileProjectIdLong = Long.parseLong(locProfileProjectId);
                JSONObject jsonTargetLocaleObj = new JSONObject();
                jsonTargetLocaleObj.put("targetLocaleId",targetLocalesId.elementAt(i));
                jsonTargetLocaleObj.put("targetLocaleDisplayName", targetLocalesDisplayNames.elementAt(i));
                jsonTargetLocaleObj.put("targetLocaleWorkflows",getJSONEachWorkflow(targetLocaleIdLong,sourceLocaleIdLong,locProfileProjectIdLong));
    ...
    private JSONArray getJSONEachWorkflow(long targetLocaleId,long sourceLocaleId,long locProfileProjectId) throws JSONException{
        List targetLocaleWorkflows;
        JSONArray jsonWorkflowsArr = new JSONArray();
        try
        {
            targetLocaleWorkflows = new ArrayList(ServerProxy.getProjectHandler()
        ...
        for (int i=0; i<targetLocaleWorkflows.size();i++) {
            WorkflowTemplateInfo wft = (WorkflowTemplateInfo)targetLocaleWorkflows.get(i);
        ...
        throws EnvoyServletException
    {
        ...
        for (int i = 0; i < targetlocales.size(); i++)
        {
            ...
            {
                CATEGORY.debug("LocProfileNewAndEditHandler: locale_id "+id+" lang "+
    ...
    private void setEditDataToRequest(HttpServletRequest p_request, BasicL10nProfile editLocprofile){
        ...
        p_request.setAttribute("LocProfileDescription", editLocprofile.getDescription());
        ...
        p_request.setAttribute("LocProfileProjectId", editLocprofile.getProjectId() + "");
        p_request.setAttribute("JobPriority", editLocprofile.getPriority() + "");
        GlobalSightLocale locale = (GlobalSightLocale)editLocprofile.getSourceLocale();
        ...
        for(int i=0;i<targets.length;i++){
            WorkflowTemplateInfo workflowTemplateInfo = (WorkflowTemplateInfo) editLocprofile.getWorkflowTemplateInfo((GlobalSightLocale) targets[i]);
