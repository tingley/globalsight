...
public class ImportErrorHandler extends JobDetailsHandler
{
    ...
    {
        NavigationBean pendingBean =
            new NavigationBean(JobManagementHandler.PENDING_BEAN, p_thePageDescriptor.getPageName());
        p_request.setAttribute(JobManagementHandler.PENDING_BEAN, pendingBean);
        NavigationBean detailsBean =
            new NavigationBean(JobManagementHandler.DETAILS_BEAN, p_thePageDescriptor.getPageName());
        ...
        String jobId = (String) p_request.getAttribute(JobManagementHandler.JOB_ID);
        Job job = HibernateUtil.get(JobImpl.class, Long.parseLong(jobId));
        p_request.setAttribute("allPageError", Boolean.toString(JobHandlerLocal.containsAllErrors(job)));
	...
	for (int i=0; i<sourcePages.size(); i++)
	{
	    ...
	    sB.append("<TD VALIGN=\"BOTTOM\"><SPAN CLASS=\"" +
		      (state.equals(PageState.IMPORT_FAIL) ? TABLE_ENTRY_RED : TABLE_ENTRY) + "\">" +
		      state + "</SPAN></TD>\n");
	    // Message
	    GeneralException ge = curPage.getRequest().getException();
	    sB.append("<TD VALIGN=\"BOTTOM\"><SPAN CLASS=\"standardText\">" +
		      ((state.equals(PageState.IMPORT_FAIL) || 
                       (curPage.getRequest().getType() < 0 )) ?  
                    		   curPage.getRequest().getException().getTopLevelMessage(uiLocale) + "\n" +
		      ((ge != null && ge.getOriginalException() != null && ge.getOriginalException().getMessage() != null 
		    		  && ge.getOriginalException().getMessage().indexOf("SAXException") != -1)
		    		  ? ge.getOriginalException().getMessage().substring(
		    				  ge.getOriginalException().getMessage().indexOf(":") + 1)
		    				  : ""): "") +
		      "</SPAN></TD>\n");
	    sB.append("</TR>");
	}
