...
/**
 * This page handler displays the Export page where you
 * choose which workflows you want to export. The
 * actual exporting of the selected workflows is handled
...
extends PageHandler
{
    ...
    throws ServletException, IOException, RemoteException, EnvoyServletException
    {
        ...
        NavigationBean baseBean =
        new NavigationBean(JobManagementHandler.EXPORT_BEAN, pageName);
        NavigationBean completeBean =
        new NavigationBean(JobManagementHandler.LOCALIZED_BEAN, pageName);
        NavigationBean exportedBean =
        new NavigationBean(JobManagementHandler.EXPORTED_BEAN, pageName);
        ...
        p_request.setAttribute(JobManagementHandler.LOCALIZED_BEAN, completeBean);
        p_request.setAttribute(JobManagementHandler.EXPORTED_BEAN, exportedBean);

        if (! "true".equals(
            p_request.getParameter(JobManagementHandler.EXPORT_MULTIPLE_ACTIVITIES_PARAM)))
        {
            Job job = getJobDetailsInfo(p_request, uiLocale);
            p_request.setAttribute(ATTR_JOB, job);

            if ("true".equals(
                p_request.getParameter(JobManagementHandler.EXPORT_FOR_UPDATE_PARAM)))
    ...
    public static List activeWorkflows(Job p_job)
    throws EnvoyServletException
    {
        List wfs = new ArrayList();
        Iterator it = WorkflowHandlerHelper.
                      getWorkflowsByJobId(p_job.getJobId()).iterator();
        while (it.hasNext())
        {
    ...
    /**
     * We get here to display Job or Workflow details, so determine which
     * and obtain the job and populate the appropriate values. in the request.
     *
     * @param p_request
     * @param p_uiLocale
     * @return
    ...
    throws EnvoyServletException
    {
        Job job = null;
        String param =
        p_request.getParameter(JobManagementHandler.JOB_ID);
        if (param != null)
        {
        ...
        LocProfileHandlerHelper.getL10nProfile(job.getL10nProfileId());
        p_request.setAttribute(JobManagementHandler.L10NPROFILE_NAME_SCRIPTLET,
                               l10nProfile.getName());
        Project project =
        WorkflowHandlerHelper.getProjectById(l10nProfile.getProjectId());
    ...
    /**
     * Looks up the known format type for the associated file profile
     * and returns true if it's an MS Office document.
     *
     * @param p_targetPage
     * @exception Exception
     */
    ...
    /**
     * Looks up the known format type for the associated file profile
     * and returns true if it's a native RTF document.
     *
     * @param p_targetPage
     * @exception Exception
     */
    ...
    /**
     * Looks up the known format type for the associated
     * file profile and returns true if it's an unextracted file
     *
     * @param p_targetPage
     * @return
     * @exception Exception
    ...
    throws EnvoyServletException
    {
        try
        {
            long fileProfileId = p_targetPage.getSourcePage().getRequest().getDataSourceId();
            KnownFormatType format = SourcePage.getFormatTypeByFpId(fileProfileId);
