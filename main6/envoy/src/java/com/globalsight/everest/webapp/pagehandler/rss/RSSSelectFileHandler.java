...
public class RSSSelectFileHandler extends PageHandler
{
    ...
    private static final Logger CATEGORY = Logger.getLogger(RSSSelectFileHandler.class.getName());
    ...
    {
        return new File(AmbFileStoragePathUtils.getFileStorageDir().getAbsoluteFile()
                + File.separator + "RSS Docs" + File.separator);
    }
            ...
            SessionManager p_sessionMgr) throws EnvoyServletException
    {
        ...
        if (directoryContent != null)
        {
            for (int i = 0; i < directoryContent.length; i++)
            {
                ...
                {
                    getFileList(p_sessionMgr).add(getRelativePath(getFileStorageDir(), doc));
                }
            }
        }
    }
            ...
            throws IOException, EnvoyServletException
    {
        ...
        // 2. job name
        String jobName = EditUtil.utf8ToUnicode(p_request.getParameter(JOB_NAME));
     ...
     *
     * @param p_mappedFiles
     *            A hashtable with all the files to be imported mapped to the
     *            file profile it should be associated with. The key is the file
     *            name and the value is the file profile.
     * @param p_jobName
     *            job name
            ...
            throws EnvoyServletException
    {
        ...
        try
        {
            ...
            for (Iterator i = fileNames.iterator(); i.hasNext();)
            {
                ...
                if (scriptOnImport != null && scriptOnImport.length() > 0)
                {
                    ...
                    if (!scriptedFolder.exists())
                    {
                        ...
                        String filePath = getFileStorageDir().getAbsolutePath() + File.separator
                                + file.getParent();
                    ...
                    else // the corresponding folder was not created by the
                         // script.
                    {
                        files2FpId.put(fileName, Long.toString(fp.getId()));
                        files2ExitValue.put(fileName, new Integer(exitValue));
                ...
                else // there is no script on import for the file profile.
                     // normal case.
                {
                    files2FpId.put(fileName, Long.toString(fp.getId()));
            ...
            {
                sendToCxe2(files2FpId, files2ExitValue, p_jobName, uuid, batchId,
                        p_importInitiatorId, p_targetLocalesList, p_jobType);
            }
        }
        catch (Exception e)
        {
     ...
     *
     * @param p_files2FpId
     *            A hashtable with all the files including scripted files to be
     *            imported. The key is the file name and the value is the file
     *            profile id.
     * @param files2ExitValue
     *            A hashtable with all the files including scripted files to be
     *            imported. The key is the file name and the value is the exit
     *            value of script result.
     * @param p_jobName
     *            job name
     * @param p_batchId
     *            job's batch id
    ...
    private static void sendToCxe2(Hashtable p_files2FpId, Hashtable p_files2ExitValue,
            String p_jobName, String uuid, String p_batchId, String p_initiatorId,
            List p_targetLocalesList, String p_jobType) throws Exception
    {
        ...
        for (Iterator i = fileNames.iterator(); i.hasNext();)
        {
            ...
            CxeProxy.importFromFileSystem(fileName, p_jobName, uuid, p_batchId, fileProfileId,
                    pageCount, new Integer(count), new Integer(1), new Integer(1), isAutoImport,
