...
public class RSSUtil 
{
    ...
    public static Feed getFeedByURL(String rssUrl) 
    {
		...
		try {
			...
			feed = new Feed(rssUrl, encoding, "2.0",
	    		    channelTitle, channelLink, channelDescription,
	    		    channelLanguage, channelCopyRight,
			...
			s_logger.error("Faild to create 'syndFeed' for RSS url : " + rssUrl, e);
		}
    ...
    public static ArrayList<Item> refreshFeedByURL(Feed p_feed) 
    {
		...
		try {
			...
			s_logger.error("Can't create XmlReader for RSS url : " + p_feed.getRssUrl(), e);
		} catch (FeedException e) {
			s_logger.error("Faild to create 'syndFeed' for RSS url : " + p_feed.getRssUrl(), e);
		}
    ...
    public static String saveToFile(String p_folder, Item p_item) {
    	...
    	try {
    		folderName += File.separator + convertTo(p_item.getFeed().getChannelTitle()) + File.separator;
    		File f = new File(folderName);
    		if (!f.exists() || f.isFile()) {
    			f.mkdirs();
    		}
			String filename = folderName + getDateString() + "_" + convertTo(p_item.getTitle()) + ".html";
			
			File file = new File(filename);
			if (file.exists()) {
				s_logger.error("Error::The file [ " + filename + " ]has been exist");
				return null;
			}
    ...
    /**
     * Generate a standard HTML format file
     * Because the content got from RSS xml is part of HTML file, then need to add some tags
     * to make it as a standard HTML format
     * @param p_item Content of item
     * @return
     */
	 ...
	 * @throws IOException
	 */
	public static ArrayList<String> saveData(String p_jobName, String p_srcLocale, String p_projectId,
			String p_projectName, String p_notes, String p_itemId, User p_user)
	...
	public static void refreshFeed(Feed p_feed, ArrayList<String> translatedItems) {
		Set items = p_feed.getItems();
		Object[] allItems = (Object[])items.toArray();
		Item item = null;
		for (Object obj : allItems) {
			item = (Item)obj;
			if (translatedItems.contains(item.getTitle() + "_" + item.getPubDate()))
				item.setStatus(1);
		}
	}
