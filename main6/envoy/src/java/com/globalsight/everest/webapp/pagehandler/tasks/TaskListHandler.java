...
public class TaskListHandler extends PageHandler
{
    private static final GlobalSightCategory log = (GlobalSightCategory) GlobalSightCategory
            .getLogger(TaskListHandler.class);
            ...
            throws GeneralException, NamingException, IOException
    {
        ...
        String consolidate = downloadOfflineFilesOptions.get(7);
        ...
        while (taskIterator.hasNext())
        {
            ...
            //String srcLocale = task.getSourceLocale().getLanguageCode() + "_" + task.getSourceLocale().getCountryCode();
            //String targetLocale = task.getTargetLocale().getLanguage() + "_" + task.getTargetLocale().getCountryCode();
     ...
     * @throws NamingException 
     */
    private void acceptAllTasks(HttpSession p_session, HttpServletRequest p_request, User p_user)
            throws TaskException, RemoteException, GeneralException, NamingException
    {
        TaskSearchParameters params = new TaskSearchParameters();
...

    {
        ...
        if (listType != null)
        {
            ...
            {
                taskState = extractIntegerValue(p_request
        ...
        int sortColumn = extractIntegerValue(p_request
                .getParameter(MYACT_COL_SORT_ID), -1);
        ...
        if (listType == null)
        {
            ...
            {
                tasks = TaskHelper.getTasks(p_session.getId(), p_user
                        .getUserId(), taskState);
            }
        }
        else if (listType.equals("lastSearch"))
        ...
        else if (listType.equals("stateOnly"))
        {
            ...
            /*
            tasks = TaskHelper.getTasks(p_session.getId(), p_user.getUserId(),
                    taskState);
                    */
        ...
        if (tasks.size() == 0)
        {
            ...
            {
                p_request.setAttribute("noresults", bundle
                        .getString("msg_activity_search_no_available")
            ...
            else if (listType.equals("stateOnly"))
            {
                ...
                p_request.setAttribute("noresults", bundle
                        .getString("msg_activity_search_no_state")
                        + " '"
                        + stateStr
                        + "'.\n"
                        + bundle.getString("msg_activity_search_try"));
            ...
            {
                p_request.setAttribute("noresults", bundle
                        .getString("msg_activity_search_no_match"));
            }
        }
        ...
        p_request.setAttribute("taskListStart", p_session
                .getAttribute("taskListStart") == null ? 0 : p_session
            ...
            throws ServletException, IOException, EnvoyServletException
    {
        ...
        TaskHelper.rejectTask(p_session.getId(), p_user.getUserId(), p_user
                .getUserName(), task, comment);
            ...
            throws ServletException, IOException, TaskException, GeneralException, NamingException
    {
        ...
        else 
        {
            ...
            if (task != null)
            {
                TaskHelper.updateTaskInSession(p_session, p_user.getUserId(), task
                        .getId());
                // update the MRU list with the changed state
                TaskHelper.updateMRUtask(p_request, p_session, task, p_response,
                        Task.STATE_ACCEPTED);
            ...
            throws EnvoyServletException
    {
        ...
        try
        {
            ...
            if (buf != null && buf.trim().length() != 0)
            {
                ...
                sp
                        .setAcceptanceStartCondition(request
