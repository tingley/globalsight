...
{
    private static final Logger logger = Logger.getLogger(UpdateLeverageHandler.class);
     ...
     */
    private static Map<Long, Integer> percentageMap = Collections
            ...
            HttpServletResponse p_response, Object form) throws Exception
    {
        ...
        StringBuffer wfsWithMtProfile = new StringBuffer();
        int count = 0;
        for (Iterator<Long> it = wfIds.iterator(); it.hasNext();)
        {
            ...
            MachineTranslationProfile mtProfile = MTProfileHandlerHelper.getMtProfileByL10nProfile(
                    wf.getJob().getL10nProfile(), wf.getTargetLocale());
            if (mtProfile != null && mtProfile.isActive())
            {
                wfsWithMtProfile.append(" ").append(wfId);
            }
        }
        ...
        try
        {
            ...
            json.put("readyWfIds", strReadyWfIds);
            json.put("nonReadyWfs", strNonReadyWfs);
            json.put("wfsWithMtProfile", wfsWithMtProfile.toString().trim());
        ...
        Runnable runnable = new Runnable()
        {
            public void run()
            {
                try
                {
                    ...
                    logger.info("Update Leverage is done for task: " + task.getId());
                }
                catch (Exception ex)
                {
        ...
        Runnable runnable = new Runnable()
        {
            public void run()
            {
                try
                {
                    ...
                    percentageMap.put(jobId, 100);
                    logger.info("Update Leverage is done for job: " + job.getId());
                }
                catch (Exception ex)
                {
                    logger.error(ex.getMessage(), ex);
