...
public class FileUploadHelper
{
        ...
        throws Exception
    {
        try
        {
            ...
            while (bytesRead != -1)
            {
                ...
                if (lineRead.startsWith("Content-Disposition: form-data; name=\""))
                {
                    if (lineRead.indexOf("filename=\"") != -1)
                    {
                        ...
                        //
                        outFile = File.createTempFile("GSTermUpload", null);
            ...
            {
                //  First, define and create upload directory
                //  structure, if not already done so
                //

                if(p_request.getAttribute("filePath") != null) {
                    setSavedFilepath(p_request.getAttribute("filePath").toString() + "/");
                }
                else {
                ...
                //
                if(p_request.getAttribute("fileName") != null) {
                    String newFileName= m_filename.substring(m_filename.lastIndexOf("."), m_filename.length());
                    newFileName = p_request.getAttribute("fileName").toString() + newFileName;
                    m_savedFile = new File(savedDir, newFileName);
                }
                else {
    ...
    public static void renameFile(File p_originalName, 
                                  File p_newName, 
                                  boolean isSaveOriginalFile)
    throws IOException
    {
    ...
    public static void renameFile(File p_originalName, File p_newName)
        throws IOException
    {
        renameFile(p_originalName, p_newName, false);
    }
}
