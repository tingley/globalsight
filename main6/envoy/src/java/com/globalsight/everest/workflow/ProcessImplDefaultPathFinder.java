 ...
 * instance. The nodes are returned in a list and are not in order - 
 * since the workflow is not necessarily linear.
 * 
 * A "default" process impl path includes all activity nodes within an AND/OR path and
 * all the activity nodes within the default branch of a Conditional node.
 * If the process implementation is running then it looks at the conditional branches
 * that have been chosen over the default ones. 
 */
public class ProcessImplDefaultPathFinder
{
     ...
     * @param p_startNodeId - The id of the node considered as the 
     *                        starting node in the path.  If it's less
     *                        than zero, we'll start from the START node.
     * @param p_destinationArrow - The outgoing arrow name for a condition
     *                             node which determines the next node.
     * @param p_nodes - The nodes of the workflow.
     */
    static List activityNodesInDefaultPath(long p_workflowInstanceId,
                                                long p_startNodeId,
                                                ArrorInfo p_destinationArrow,
        ...
        throws WorkflowException, RemoteException
    {
        ...
        try
        {
        	WorkflowTaskInstance startNode = p_startNodeId > 0 ? 
                findStartNode(p_startNodeId, p_nodes) : 
                findStartNode(p_nodes);
        ...
        {
            s_logger.error("Error occurred when finding the default path of workflow " +
                           p_workflowInstanceId, e);
            String args[] = {Long.toString(p_workflowInstanceId)};
            throw new WorkflowException(WorkflowException.MSG_FAILED_TO_FIND_DEFAULT_PATH,
                                         args, e);
    ...
    /**
     * Returns the node in this workflow instance which would be considered
     * as a start point for the rest of the default path.
     *
     * @param p_nodes All the nodes of a particular process instance.
     * @return The node considered a starting point in the workflow's
     * default path.  This node is not necessarily the very first node
     * of the path.
     */
     ...
     * 
     * @param p_ni The node instance that has been processed and
     *             its target node(s) should be processed too.
     * @param p_destinationArrow - The outgoing arrow name for a condition
     *                             node which determines the next node.
     * @param p_nodesInPath  This is a return variable that will
     *                       contain all the activity nodes in the
     *                       default path from the one passed in.
     ...
     *            The outgoing arrow name for a condition node which determines
     *            the next node.
     * @param p_nodesInPath
        ...
        throws WorkflowException
    {
        ...
        case WorkflowConstants.ACTIVITY:
            ...
            break;
        // treated the same since all path(s) could have been taken in the OR
        // case - just like ANDs
        ...
        case WorkflowConstants.CONDITION:
            // find the default way or if the node is active or already been processed find
            // the one that has been chosen and procced (only go down one way)
            
            String destinationArrow = null;
            if (p_arrowInfo != null)
            {
                Vector<WorkflowArrowInstance> arrows = p_ni.getIncomingArrows();
                for (WorkflowArrowInstance arrow : arrows)
                {
                    if (arrow.getSourceNode().getTaskId() == p_arrowInfo.getSourceId())
                    {
            ...
            returnNode = evaluateNode(p_ni, p_arrowInfo, 
                                      p_nodesInPath);
            break;
        default:
            // log error message and keep going
            s_logger.error("The node type " + p_ni.getType() + " is not supported.");
            break;
       }       
       return returnNode; 
    }    
     ...
     * working correctly anymore.  It returned false and the recursion ran rampant.
     * Couldn't figure out why it was failing....it worked in System4 4.4.7, but failed in 5.1
     *
     * Replaced it with this method that tested to see if the node is already in
     * the list and returns "true" if it is.
     */
    private static boolean alreadyInPath(List p_nodesInPath, WorkflowTaskInstance p_ni)
    {
        boolean found = false;
    ...
    /**
     * Find the path to follow from the condition node and return it.
     * Either returns the default node or the node that has been chosen if 
     * this workflow instance is running and passed this condition node.
     *
     * @param p_cni - The condition node.
     * @param p_destinationArrow - The outgoing arrow name for a condition
     *                             node which determines the next node.
     */
			...
			throws WorkflowException
	{
		...
		{
			chosenNode = WorkflowProcessAdapter
					.targetNodeInstanceByArrowLabel(p_cni, p_destinationArrow);
		}
     ...
     * @return
     */
    private static WorkflowTaskInstance getDefaultOneByState(WorkflowTaskInstance p_task) {
    ...
    public static WorkflowTaskInstance getDefaultOne(WorkflowTaskInstance p_task) {
    	...
    	while(e.hasMoreElements()) {
    		...
    		if(p_task.getConditionSpec().getBranchSpec(arrow.getName()).isDefault()){
    			return targetTask;
    		}
    	}
