    ...
    extends PersistenceCommand
{
    ...
    private static final String m_updateJob = 
        "update job set page_count = page_count + 1 where id = ?";

    private static final String m_selectWordCountJobId =
        "select distinct j.id from job j, request r, source_page sp where " +
        "j.id = r.job_id and sp.id = r.page_id and j.state = ? and " +
        "r.l10n_profile_id = ? and sp.data_source_type = ?";

    private static final String m_selectBatchJobId =
        "select distinct j.id from job j, request r where " +
        "j.id = r.job_id and r.batch_id = ?";
    
    private static final String m_selectJobId =
        "select distinct j.id from job j where j.name = ?";
        ...
        throws PersistenceException
    {
        ...
        finally
        {
            try
            {
                if (m_psUpdateRequest != null)  m_psUpdateRequest.close();
                if (m_psUpdateJob != null) m_psUpdateJob.close();
                if (m_psBatchJobId != null) m_psBatchJobId.close();
                if (m_psJobId != null) m_psJobId.close();
                if (m_psWordCountJobId != null) m_psWordCountJobId.close();
            }
            catch (Exception e)
            {
            }
        ...
        throws Exception
    {
        ...
        {
            m_psWordCountJobId = p_connection.prepareStatement(
                m_selectWordCountJobId);
        }
    }
        ...
        throws Exception
    {
        ...
        try
        {
            if (m_psWordCountJobId !=null)
            {
                ...
                m_psWordCountJobId.setLong(2, m_request.getL10nProfile().getId());
...
//                m_psBatchJobId.setString(1, "BATCH_RESERVED");
                m_psBatchJobId.setString(1, m_request.getBatchInfo().getBatchId());
                ...
                {
                    m_psJobId.setString(1, m_request.getBatchInfo().getBatchId());
