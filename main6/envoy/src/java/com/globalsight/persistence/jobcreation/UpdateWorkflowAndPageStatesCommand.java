...
/**
 * This class performs the queries and updates necessary to update 
 * the workflows and source/target pages of a job to IMPORT_FAIL if necessary.
 * It looks at the target pages associated with the workflow and determines
 * if the workflow and/or the source page should fail import.
 * Also if a source page is marked as FAIL_IMPORT then all of its targets
 * are marked too.
 * This is used with the reimport option REIMPORT_NEW_TARGETS
 */
public class UpdateWorkflowAndPageStatesCommand 
    extends PersistenceCommand
{
    private static final String QUERY_FAILED_SOURCE_PAGES = 
        "(select source_page_id from target_page where " +
        "workflow_iflow_instance_id = ? and state = '" + PageState.IMPORT_FAIL +"')";
    // query for all the workflow ids associated with a particular job
    private static final String QUERY_WORKFLOW_IDS = 
        "select iflow_instance_id from workflow wtp where job_id = ?";

    // get the number of target pages that failed import within a workflow
    private static final String QUERY_FAILED_TARGET_PAGE_COUNT = 
        "select count(*) from target_page where state = '" +
        PageState.IMPORT_FAIL + "' and workflow_iflow_instance_id = ?";

    // get the number of target pages total in a workflow
    private static final String QUERY_TARGET_PAGE_COUNT = 
        "select count(*) from target_page where workflow_iflow_instance_id = ?";
    
    // update a workflow with the IMPORT_FAIL state
    private static final String UPDATE_WORKFLOW_STATE_FAIL = 
        "update workflow set state = '" + Workflow.IMPORT_FAILED +
        "', timestamp = ? where iflow_instance_id = ?";

    // update a source page with the IMPORT_FAIL state
    private static final String UPDATE_SOURCE_PAGE_STATE_FAIL = 
        "update source_page set state = '" + PageState.IMPORT_FAIL +
        "', timestamp = ? where id in " + QUERY_FAILED_SOURCE_PAGES;

    // update a target page with the IMPORT_FAIL state
    // only update if not already set to IMPORT_FAIL
    // no need to do it then
    private static final String UPDATE_TARGET_PAGES_STATE_FAIL = 
        "update target_page set state = '" + PageState.IMPORT_FAIL +
        "', timestamp = ? where state != '" + PageState.IMPORT_FAIL + 
        "' and source_page_id in " + QUERY_FAILED_SOURCE_PAGES;
    ...
    private static final String QUERY_REQUEST_ID_BY_FAILED_SOURCE_PAGE = 
        "select id from request where page_id in " + QUERY_FAILED_SOURCE_PAGES +
        " and type = 'EXTRACTED_LOCALIZATION_REQUEST'";
    
    private PreparedStatement m_psQueryWfIds;   // query the ids of the workflows in a job
    private PreparedStatement m_psQueryFailedCount; //query the failed target page count for a workflow
    private PreparedStatement m_psQueryCount;   //query the target page count for a workflow
    ...
    // holds the list of ids of requests that need to 
    ...
    /**
     * Constructor to use when updating a collection of new target pages
     * and the old ones.
     */
        ...
        throws PersistenceException 
    {
        ...
        finally
        {
            try
            {
                if (m_psWorkflowFailedCommand != null) m_psWorkflowFailedCommand.close();
                if (m_psSourcePageFailedCommand != null) m_psSourcePageFailedCommand.close();
                if (m_psTargetPageFailedCommand != null) m_psTargetPageFailedCommand.close();
                if (m_psQueryWfIds != null) m_psQueryWfIds.close();
                if (m_psQueryFailedCount != null) m_psQueryFailedCount.close();
                if (m_psQueryCount != null) m_psQueryCount.close();
                if (m_psQueryFailedRequestIds != null) m_psQueryFailedRequestIds.close();
            }
            catch (Exception e)
            {
            }
        ...
        throws Exception 
    {
        ...
        m_psQueryFailedCount = 
            p_connection.prepareStatement(QUERY_FAILED_TARGET_PAGE_COUNT);
        m_psQueryCount = p_connection.prepareStatement(QUERY_TARGET_PAGE_COUNT);
        m_psQueryFailedRequestIds = 
            p_connection.prepareStatement(QUERY_REQUEST_ID_BY_FAILED_SOURCE_PAGE);
        m_psWorkflowFailedCommand = 
            p_connection.prepareStatement(UPDATE_WORKFLOW_STATE_FAIL);
        m_psSourcePageFailedCommand = 
            p_connection.prepareStatement(UPDATE_SOURCE_PAGE_STATE_FAIL);
        m_psTargetPageFailedCommand = 
            p_connection.prepareStatement(UPDATE_TARGET_PAGES_STATE_FAIL);
    }
    
    /**
     *  Figures out what state to set the workflows in and
     *  possiblyl source page.  Depends on the grouping of target pages 
     *  and their states.
     */
    public void setData() throws Exception 
    {
        ...
        for (Iterator i = wfIds.iterator() ; i.hasNext() ; )
        {
            ...
            // check if all the target pages of the workflow
            // are marked as import fail
    ...
    /**
     * Return an array of all the ids of the requests
     * that should be marked as failed.
     * The ids are of type Long.
     */
    public List getFailedRequestsById()
    {
     ...
     */
    private List getWorkflowIds()
        throws PersistenceException
    {
        ArrayList wfIdsList = new ArrayList();
     ...
     */
    private int numberOfTargetPages(long p_wfId)
        throws PersistenceException
    {
    ...
    /** 
     * Adds the request ids that should be marked as fail to the
     * data member that stores failed request ids.  
     * This method looks at all requests assocaited with a particular workflow.
     */
    private void addToFailedRequestIds(long p_wfId)
        throws PersistenceException
    {
        Connection connection = null;
        try
        {
            ...
            while (rs.next())
            {
                ...
                c_logger.debug("Request " + failedRequestId + " must be marked as failed " +
