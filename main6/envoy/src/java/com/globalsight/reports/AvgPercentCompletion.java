...
/**
* Presents a report on the average percent completion by target locale for all workflows
* for jobs that are DISPATCHED.
*/
public class AvgPercentCompletion extends GlobalSightReplet
{
    ...
    */
    ...
    /**
    * Adds the user ids of the project managers who actually have projects
    * as replet parameters
    * <br>
    */
    protected void addProjectManagerParameter()
    throws Exception
    {
        Connection c = null;
        ...
        try {
            ...
            rs = ps.executeQuery();
            projectManagers.add(ReportsPackage.getMessage(m_bundle,"criteria_allPms"));
            while (rs.next())
            ...
            theParameters.addChoice("projectMgr", defvalue, projectManagers.toArray());
            theParameters.setAlias("projectMgr", ReportsPackage.getMessage(m_bundle,"projectMgr"));
        }
        finally {
    ...
    */
    ...
    throws Exception
    {
        ...
        ArrayList jobs = null;
        if (projectManager.equals(ReportsPackage.getMessage(m_bundle,"criteria_allPms")))
        {
           jobs = new ArrayList(ServerProxy.getJobHandler().getJobsByState(DISPATCHED));
           jobs.addAll(ServerProxy.getJobHandler().getJobsByState(LOCALIZED));
        }
        else
        {
            jobs = new ArrayList(ServerProxy.getJobHandler().getJobsByManagerIdAndState(projectManager, DISPATCHED));
            jobs.addAll(ServerProxy.getJobHandler().getJobsByManagerIdAndState(projectManager, LOCALIZED));
        }
        ...
        //sort the job table model by job_id, priority,project,pm,create_date,jobname
        ...
        int[] sortCols = new int[] {
            ...
            JobTableModel.CREATE_DATE, JobTableModel.JOBNAME, JobTableModel.ID };
        ...
        //now run through the sorted table and add a form at the top for each job
        for (int r=0; r < jtm.getRowCount(); r++)
        {
            Long jobId = (Long) dst.getObject(r+1,0); //ignore header row
            WorkflowTableModel wtm = (WorkflowTableModel) workflowTableModels.get(jobId);
    ...
    /**
    * Displays a simple bar chart showing the number of jobs per activity
    * <br>
    */
    private void showAvgPercentCompletionChart(JobTableModel p_jtm, HashMap p_workflowTableModels)
    {
        //run through the jobs and workflows and create a table containing the target locales
        //and percent completion
        PercentCompletionTable pct = new PercentCompletionTable(p_jtm,p_workflowTableModels);
        TableModelLens tml = new TableModelLens(pct);
        SortFilter sf = new SortFilter(tml, new int[] {0}, true);
        GroupFilter gf = new GroupFilter (sf, 1, new AverageFormula(), new AverageFormula());
    ...
    /**
    * Returns an arraylist containing workflowtablemodels for each job
    * <br>
    * @param p_jobWorkflows -- an arraylist of arraylists for each job containing the workflows
    * @return ArrayList of WorkflowTableModels
    */
    private HashMap makeWorkflowTableModels(JobTableModel p_jtm, ArrayList p_jobWorkflows)
    {
        HashMap wtms = new HashMap();
        for (int i=0; i < p_jobWorkflows.size(); i++)
        {
            ArrayList workflows = (ArrayList) p_jobWorkflows.get(i);
            WorkflowTableModel wtm = new WorkflowTableModel(workflows,theSession,null);
    ...
    /**
    * Returns an array list containing an array list for each job containing its workflows
    * <br>
    * @param p_jobs -- ArrayList of jobs
    */
    private ArrayList getAllWorkflowsForAllJobs(ArrayList p_jobs)
    {
        ArrayList jobWorkflows = new ArrayList(); //contains an array list for each job
        for (int i=0; i < p_jobs.size(); i++)
        {
            Job job = (Job) p_jobs.get(i);
            ArrayList workflows = null; //the workflows for this job
            try {
                workflows = new ArrayList(ServerProxy.getJobReportingManager().getWorkflowsByJobId(job.getId()));
            }
            catch (Exception e)
            {
    ...
    /**
    * Adds a form at the top with the selected PM and workflow state
    * <br>
    ...
    */
    private void addCriteriaFormAtTop(String projectManager)
    {
        ...
        String labels[] = new String[]{
            ...
            ReportsPackage.getMessage(m_bundle,"projectMgr") + LABEL_SUFFIX };
        String wfstate = ReportsPackage.getMessage(m_bundle,DISPATCHED) +","+ ReportsPackage.getMessage(m_bundle,LOCALIZED);
        String fields[] = new String[]{ wfstate, projectManager };
    ...
    */
    private void addJobForm(DefaultSortedTable p_dst, int r)
    {
        ...
        labels.add(newLabel);
        fields.add(p_dst.getObject(r+1,JobTableModel.JOBNAME)); /*ignore the header row*/
    ...
    /**
    * Inner class to handle keeping track of the percent completion by workflow
    * <br>
    */
    class PercentCompletionTable extends AbstractTableModel
    {
        ...
        public PercentCompletionTable (JobTableModel p_jtm, HashMap p_workflows)
        {
            ...
            for (int j=0; j < p_jtm.getRowCount(); j++)
            {
                ...
                WorkflowTableModel wtm = (WorkflowTableModel) p_workflows.get(jobid);
                for (int w=0; w < wtm.getRowCount(); w++)
                {
                    Object targetLocale = wtm.getValueAt(w,WorkflowTableModel.TRGLOCALE);
                    Object percentComplete = wtm.getValueAt(w,WorkflowTableModel.PER_COMPLETE);
                    Object thiscolumn[] = {
        ...
        public int getRowCount() { return m_dataRows.size();}
        public int getColumnCount() { return 2;}
        public String getColumnName(int c) { return m_columnNames[c];}
