...
//    private Font m_fieldFont = GlobalSightReplet.FONT_NORMAL;
        ...
        throws RepletException
    {
        try {
            ...
            m_jobId = (Long) this.theSessionMgr.getAttribute(JobManagementHandler.JOB_ID);
            //added the permission. if internal cost is unchecked then hide the
            //information of internal cost
            PermissionSet userPerms = 
                (PermissionSet)this.theSession.getAttribute(
                                                  WebAppConstants.PERMISSIONS);
            isInternalCostReview = 
                userPerms.getPermissionFor(Permission.COSTING_EXPENSE_VIEW);
        }
     ...
     * Also determines the grouping and chart styles.
     * <br>
     * @param RepletRequest
     * @return ReportSheet
     * @throws RepletException
    ...
    public ReportSheet createReport(RepletRequest req) throws RepletException
    {
        ...
        HttpServletRequest http_request = (HttpServletRequest) req.getParameter(RepletRequest.HTTP_REQUEST);
    ...
    private void bindMessages(RepletRequest req)
    {
        ...
        TextElement txtReportTitle = (TextElement) theReport.getElement("txtReportTitle");
        txtReportTitle.setText(ReportsPackage.getMessage(m_bundle,"txtReportTitle"));
    }
     ...
     * @throws Exception
     */
    private void bindData(RepletRequest req)
        throws Exception
    {
        if (isJobCostingOn())
        {
            String currencyName = (String) theSessionMgr.getAttribute(JobManagementHandler.CURRENCY);
            m_currency = ServerProxy.getCostingEngine().getCurrency(currencyName);
        }
        else
            m_currency = null;
        ...
        WorkflowTableModel2 wtm = new WorkflowTableModel2(workflows, theSession, m_currency);
        ...
        ss.addNewline(2);
        ss.addSeparator(StyleConstants.THIN_LINE);
        String textId = ss.addText(ReportsPackage.getMessage(m_bundle,"breakdownOnNextPage"));
    ...
    /**
     * Adds the given WorkflowTableModel to the report
     * <br>
     * @param WorkflowTableModel2 p_wtm
     */
    private void addWorkflowSummaryTable(WorkflowTableModel2 p_wtm)
    {
        ...
        if (isJobCostingOn())
        {
            ...
            NumberFormat currencyFormat = CurrencyFormat.getCurrencyFormat(m_currency);

            //now run through the table and replace the cost with a formatted cost
            for (int r=0; r < p_wtm.getRowCount(); r++)
            {
                //Internal Cost Review permission based
                if(!isInternalCostReview && (r == WorkflowTableModel2.COST || 
                                 r == WorkflowTableModel2.ACTUAL_REVENUE)) {
                    continue;
                }
                if(isInternalCostReview) {
                    double cost = ((Float)p_wtm.getValueAt(r,WorkflowTableModel2.COST)).doubleValue();
                    double actualCost = ((Float)p_wtm.getValueAt(r,WorkflowTableModel2.ACTUALCOST)).doubleValue();
                    String formattedExpensesCurrency = currencyFormat.format(cost);
                    String formattedExpensesActualCurrency = currencyFormat.format(actualCost);
                    p_wtm.setValueAt(formattedExpensesCurrency,r,WorkflowTableModel2.COST);
                    p_wtm.setValueAt(formattedExpensesActualCurrency,r,WorkflowTableModel2.ACTUALCOST);
                }

                if(isJobRevenueOn())
                {
                    //double revenue = ((Float)p_wtm.getValueAt(r,WorkflowTableModel2.REVENUE)).doubleValue();
                    double actualRevenue = ((Float)p_wtm.getValueAt(r,WorkflowTableModel2.ACTUAL_REVENUE)).doubleValue();
                    //String formattedRevenueCurrency = currencyFormat.format(revenue);
                    String formattedRevenueActualCurrency = currencyFormat.format(actualRevenue);
                    //p_wtm.setValueAt(formattedRevenueCurrency,r,WorkflowTableModel2.REVENUE);
                    p_wtm.setValueAt(formattedRevenueActualCurrency,r,WorkflowTableModel2.ACTUAL_REVENUE);
                }
        ...
        String textId = ss.addText(ReportsPackage.getMessage(m_bundle,"allWorkflows"));
        ss.getElement(textId).setFont(GlobalSightReplet.FONT_LABEL);
        ss.addNewline(1);
    ...
    /**
     * Adds a form to the report containing the job information
     * <br>
     * @param p_table -- a table model
    ...
    private void addJobForm(JobTableModel2 p_table)
    {
        ...
        ss.addSeparator(StyleConstants.THIN_LINE);
        /**
         * Discription for the "sequence" array. 
         * It is defined a sequence about some data of what you want to show in the job form 
         */         
        int sequence[] = new int[] {  JobTableModel2.JOBNAME,
	        						  JobTableModel2.JOBSTATUS,
	        						  JobTableModel2.CURRENCY,
	        						  JobTableModel2.PRIORITY,
	        						  JobTableModel2.PROJECT,
	        						  JobTableModel2.LOCPROFILE,
	        						  JobTableModel2.PM,
	        						  JobTableModel2.SRCLOCALE,
	        						  JobTableModel2.DATASRC,
	        						  JobTableModel2.WORDCNT,
	        						  JobTableModel2.CREATE_DATE,
	        						  JobTableModel2.COST,
	        						  JobTableModel2.ACTUALCOST,
	        						  JobTableModel2.TOTALCOST,
	        						  JobTableModel2.ACTUAL_REVENUE,
	        						  JobTableModel2.TOTAL_REVENUE
        							};
        //start to get the table name and failed data in the loop
        for (int c = 0; c < sequence.length; c++)
        {
      	
            if(!isInternalCostReview && (sequence[c] == JobTableModel2.COST || 
            							 sequence[c] == JobTableModel2.ACTUALCOST ||
            							 sequence[c] == JobTableModel2.TOTALCOST)) 
            {
                //do not want internal cost if has not the privilege of Internal Cost Review 
                continue;
            }
            ...
            {
                //this should handle estimated, actual and total costs and billing.
                double cost = ((Float) p_table.getValueAt(sequence[c])).doubleValue();
                NumberFormat numberFormat = CurrencyFormat.getCurrencyFormat(m_currency);
            ...
            maxLabelLen = (label.length() > maxLabelLen) ? label.length() : maxLabelLen;
            labels.add(label);
            fields.add(field);
        }
        ...
        DefaultTableModel dtm = new DefaultTableModel(tableData, new Vector(labels));
    ...
    private boolean isCostingRelated(int c)
    {
        if (c == JobTableModel2.COST || c == JobTableModel2.ACTUALCOST ||
            ...
            || c== JobTableModel2.ACTUAL_REVENUE || c == JobTableModel2.TOTAL_REVENUE )
            return true;
        return false;
    }

    /**
     * Prints out a two column table containing the pagename and the
     * source word count.
     * <br>
     * @param p_job -- the Job
     */
    private void addTableWithPageNames(Job p_job)
    {
        ...
        TableModel tm = new AbstractTableModel() {
                ...
                public Object getValueAt(int r, int i) {return data[r][i];}
                public void setValueAt(Object o, int r, int i) {data[r][i]=o;}
            };
        int row = 0;
        while (iter.hasNext())
        {
        ...
        String textId = ss.addText(ReportsPackage.getMessage(m_bundle,"pagesInJob"));
        ss.getElement(textId).setFont(GlobalSightReplet.FONT_LABEL);
        ss.addNewline(1);
        ss.addTable(prof);
    }
    ...
    /**
     * Prints out a two column table containing the surcharge names and
     * their values. Use the JSP as a model for the code.
     * <br>
     * @param p_job -- the Job
     * @param m_currency - the currency
     */
    private void addTableWithSurcharges(Job p_job, int p_costType)
    {
        Cost cost = GlobalSightReplet.calculateJobCost(p_job, m_currency, p_costType);
        ...
        if (numRows > 0)
        {
            ...
            TableModel tm = new AbstractTableModel() {
                    ...
                    String[] columnNames = {ReportsPackage.getMessage(m_bundle,"surchargeName"),ReportsPackage.getMessage(m_bundle,"surchargeType"),ReportsPackage.getMessage(m_bundle,"surchargeAmount")};
                    public int getRowCount() {return data.length;}
            ...
            while (iter.hasNext())
            {
                ...
                if (surcharge.getType().equals("FlatSurcharge"))
                {
                    ...
                    amount = CurrencyFormat.getCurrencyFormat(m_currency).format(surchargeAmount);
                    ...
                    totalFlatSurchage = totalFlatSurchage.add(flatSurcharge.getAmount());
                }
            }
            ...
            while (iter.hasNext())
            {
                ...
                if (surcharge.getType().equals("PercentageSurcharge"))
                {
                    ...
                    float percentageAmount = percentageSurcharge.surchargeAmount(cost.getActualCost().add(totalFlatSurchage)).getAmount();
                    amount = CurrencyFormat.getCurrencyFormat(m_currency).format(percentageAmount);
                    type= ReportsPackage.getMessage(m_bundle, "percentageSurcharge") + " (" + percentage + "%)";
            ...
            {
                textId = ss.addText(ReportsPackage.getMessage(m_bundle,"expensesSurchargesInJob"));
            }
            else
            {
                textId = ss.addText(ReportsPackage.getMessage(m_bundle,"revenueSurchargesInJob"));
            }
            ss.getElement(textId).setFont(GlobalSightReplet.FONT_LABEL);
            ss.addNewline(1);
            ss.addTable(prof);
    ...
    /**
     * Adds a note whether the final cost is an override or actual with/without surcharges.
     * Also introduces the surcharge table
     */
    private void addTotalCostNote(Job p_job, int p_costType)
        throws Exception
    {
        Cost finalCost = GlobalSightReplet.calculateJobCost(p_job, m_currency, p_costType);
        String label;
        if (finalCost.isOverriden())
        {
            if(p_costType == Cost.EXPENSE)
            {
                label = ss.addText(ReportsPackage.getMessage(m_bundle,"overrideExpensesCostNote"));
            }
            else
            {
                label = ss.addText(ReportsPackage.getMessage(m_bundle,"overrideRevenueCostNote"));
            }
        }
        else
        {
            if (finalCost.getSurcharges().size() > 0)
            {
                if(p_costType == Cost.EXPENSE)
                {
                    label = ss.addText(ReportsPackage.getMessage(m_bundle,"expensesSurchargeCostNote"));
                }
                else
                {
                    label = ss.addText(ReportsPackage.getMessage(m_bundle,"revenueSurchargeCostNote"));
                }
            }
            else
            {
                if(p_costType == Cost.EXPENSE)
                {
                    label = ss.addText(ReportsPackage.getMessage(m_bundle,"noExpensesSurchargeCostNote"));
                }
                else
                {
                    label = ss.addText(ReportsPackage.getMessage(m_bundle,"noRevenueSurchargeCostNote"));
                }
            }
        }
    ...
    /**
     * Outputs a table containing the activities for each workflow
     * <br>
     * @param ArrayList p_workflows -- the workflows for this job
     */
    
    private void addDetailedWorkflowBreakdownTables(long p_jobId, ArrayList p_workflows)
        throws Exception
    {
        ...
        String textId = ss.addText(ReportsPackage.getMessage(m_bundle,"activityBreakdown"));
        //set the label font after adding the table so it doesn't affect the table font
        ss.getElement(textId).setFont(GlobalSightReplet.FONT_LABEL);
        ss.addNewline(2);

        //query out all the completed activities for all the workflows for this job
        ...
        for (int i = 0; i < workflowCounts; i++)
        {
            ...
            if(isJobRevenueOn())
            {
                table = new DefaultTableModel(tasks.size(),allColumnNames.length);
                table.setColumnIdentifiers(allColumnNames);
            }
            else
            {
                table = new DefaultTableModel(tasks.size(),noRevenueColumnNames.length);
            ...
            List defaultTaskIds = findDefaultTaskIds(w.getId()); //discover this statement is too slow when debuging
            int taskCounts = wfiTasks.size();
            //below is generating the activity table for per workflow in the loop 
            for (int j = 0; j < taskCounts; j++)
            {
                WorkflowTaskInstance wfTask = (WorkflowTaskInstance)wfiTasks.get(j);
                ...
                String accepter = ReportsPackage.getMessage(m_bundle, "notAcceptedYet");
                if (wfTask.getTaskState() == Task.STATE_ACTIVE)
                {
                    ...
                    if (task != null)
                    {
                        accepter = task.getAcceptor();
                    }

                    if (accepter == null || accepter.length() == 0)
                    {
                        accepter = ReportsPackage.getMessage(m_bundle, "notAcceptedYet");
                    }
                ...
                else
                {
                    ...
                    {
                        Object[] values = (Object[]) completedActivities.get(taskId);
                ...
                {
                    completedDate = DateHelper.getFormattedDateAndTime(task.getCompletedDate(), theUiLocale);
                    isCompleted = true;
                }
                ...
                {
                    // get expense details
                    Cost cost = GlobalSightReplet.calculateTaskCost(task,m_currency, Cost.EXPENSE);
                    if (cost.getEstimatedCost() != null)
                        estimatedCost = Float.toString(cost.getEstimatedCost().getAmount());
                    if (cost.getActualCost() != null)
                        actualCost = Float.toString(cost.getActualCost().getAmount());


                    if(isJobRevenueOn())
                    {
                        // get revenue details
                        Cost revenue = GlobalSightReplet.calculateTaskCost(task,m_currency, Cost.REVENUE);
                        if (revenue.getEstimatedCost() != null)
                            estimatedRevenue = Float.toString(revenue.getEstimatedCost().getAmount());
                        if (revenue.getActualCost() != null)
                            actualRevenue = Float.toString(revenue.getActualCost().getAmount());
                    }

                    //Rate r = task.getExpenseRate();
                    AmountOfWork aow = task.getAmountOfWork(Rate.UnitOfWork.HOURLY);
                    //Estimated Hours
                    if (aow != null)
                    {
                        estimatedHours = Double.toString(aow.getEstimatedAmount());
                ...
                if (timeInDays > 0)
                {
                    duration = DateHelper.daysHoursMinutes(
                        timeInDays,
            ...
            Collections.sort(uncompletedTasks,new MyUnCompletedTaskComparator(defaultTaskIds));
            ...
            for (int k=0; k < completedTasks.size(); k++)
            {
                ...
                {
                    //put all the values into the table except for the actual completed date
            ...
            idx=0;
            int currentRow = -1;
            for (int k=completedTasks.size(); k < completedTasks.size() + uncompletedTasks.size(); k++)
            {
           ...
           TableModelLens tml = new TableModelLens (table);
            ...
            SubTableLens stl = new SubTableLens(tml,0,1,tml.getRowCount(),tml.getColCount()-1);
            ...
            //now set all the uncompleted rows to use italics except for default path
            for (int u=0; u < uncompletedTasks.size(); u++)
            {
                ...
                if (defaultTaskIds.contains(taskId))
                    prof.setRowFont(completedTasks.size() + u +1, GlobalSightReplet.FONT_NORMAL);
                else
                    prof.setRowFont(completedTasks.size() + u +1, GlobalSightReplet.FONT_ITALIC);
            }

            //now set the current row as green
            if (currentRow > -1)
            {
                prof.setRowForeground(currentRow + 1,
                    new Color(0x009966));
            }
    ...
    /**
     * Returns a List of Longs for the IDs of tasks
     * in the default path.
     *
     * @param p_wfId -- the particular workflow
     * @return
     */
    ...
    /**
     * Returns a HashMap filled with an Object array containing the taskid, taskname, and responsible user
     * for each completed activity for all the workflows for the specified job
     * <br>
     * @param p_jobid -- the ID of the job
        ...
        throws Exception
    {
        if (m_completedActivitiesQuery == null)
        {
            ...
            String twfuser = sc.getStringParameter(SystemConfigParamNames.IFLOW_DB_USERNAME);
