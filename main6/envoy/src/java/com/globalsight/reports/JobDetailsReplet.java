...
public class JobDetailsReplet extends GlobalSightReplet
{
    ...
    {
        String companyName = (String)theSession.getAttribute(UserLdapHelper.LDAP_ATTR_COMPANY);
        CompanyThreadLocal.getInstance().setValue(companyName);
        c_category.debug("companyId:" + CompanyThreadLocal.getInstance().getValue());
        
        StringBuffer sb = new StringBuffer("select job.id, job.name, job.state from job ");
    ...
    /**
     * Queries for all job ids,names,and states for (D,L,E,A) jobs
     * <br>
     * @throws Exception
     */
    private void queryAllJobs() throws Exception
    {
        ...
        try {
            ...
            while (rs.next())
            {
                ...
                displayJobName.append(" (").append(jobid.toString()).append(")");
                LabeledValueHolder lvh = new LabeledValueHolder(jobid,displayJobName.toString());
    ...
    /**
     * Adds a radio button parameter for job status to the replet parameters
     * so that we know which job list they selected from
     * <br>
     */
    protected void addJobStatusParameter()
    {
        Object dispatched = new LabeledValueHolder(DISPATCHED,ReportsPackage.getMessage(m_bundle,DISPATCHED));
        Object localized = new LabeledValueHolder(LOCALIZED,ReportsPackage.getMessage(m_bundle,LOCALIZED));
        Object exported = new LabeledValueHolder(EXPORTED,ReportsPackage.getMessage(m_bundle,EXPORTED));
        Object archived = new LabeledValueHolder(ARCHIVED,ReportsPackage.getMessage(m_bundle,ARCHIVED));
        ...
        if (states.size() > 0)
        {
            theParameters.addRadio("jobstatus", states.get(0), states.toArray());
            theParameters.setAlias("jobstatus", ReportsPackage.getMessage(m_bundle,"jobstatus"));
        }
    }

    /**
     * Adds a choice list of all the jobs in the given state as
     * a replet parameter. Adds four lists as replet parameters (dispatched,localized,exported,archived)
     */
    private void addJobListParameters()
    {
        ...
        if (m_dispatchedJobs.size() > 0)
        {
            theParameters.addChoice("dispatchedJobs", m_dispatchedJobs.get(0), m_dispatchedJobs.toArray());
            theParameters.setAlias("dispatchedJobs", ReportsPackage.getMessage(m_bundle,"dispatchedJobs"));
        }

        if (m_localizedJobs.size() > 0)
        {
            theParameters.addChoice("localizedJobs", m_localizedJobs.get(0), m_localizedJobs.toArray());
            theParameters.setAlias("localizedJobs", ReportsPackage.getMessage(m_bundle,"localizedJobs"));
        }

        if (m_exportedJobs.size() > 0)
        {
            theParameters.addChoice("exportedJobs", m_exportedJobs.get(0), m_exportedJobs.toArray());
            theParameters.setAlias("exportedJobs", ReportsPackage.getMessage(m_bundle,"exportedJobs"));
        }

        if (m_archivedJobs.size() > 0)
        {
            theParameters.addChoice("archivedJobs", m_archivedJobs.get(0), m_archivedJobs.toArray());
            theParameters.setAlias("archivedJobs", ReportsPackage.getMessage(m_bundle,"archivedJobs"));
        }
    }
        ...
        throws Exception
    {
        ...
        HttpServletRequest http_request = (HttpServletRequest) req.getParameter(RepletRequest.HTTP_REQUEST);
        theSession = http_request.getSession(false);
        //add a form at the top to show the criteria that was selected for the report
        LabeledValueHolder chosenStateHolder = (LabeledValueHolder) req.getParameter("jobstatus");
        ...
        if (isJobCostingOn())
        {
            LabeledValueHolder chosenCurrency = (LabeledValueHolder) req.getParameter("currency");
        ...
        if (chosenState.equals(DISPATCHED))
            chosenJobHolder = (LabeledValueHolder) req.getParameter("dispatchedJobs");
        else if (chosenState.equals(LOCALIZED))
            chosenJobHolder = (LabeledValueHolder) req.getParameter("localizedJobs");
        else if (chosenState.equals(EXPORTED))
            chosenJobHolder = (LabeledValueHolder) req.getParameter("exportedJobs");
        else if (chosenState.equals(ARCHIVED))
            chosenJobHolder = (LabeledValueHolder) req.getParameter("archivedJobs");
        ...
        Collection c = ServerProxy.getJobReportingManager().getWorkflowsByJobId(job.getId());
        ArrayList workflows = new ArrayList(c);
        WorkflowTableModel wtm = new WorkflowTableModel(workflows, theSession, m_currency, false);
        boolean isInContextMatch = PageHandler.isInContextMatch(job, wtm.isUseInContext());
        if(!isInContextMatch){
            String[] columns = {
     ...
     * <br>
     * @param WorkflowTableModel p_wtm
     */
    private void addWorkflowSummaryTable(WorkflowTableModel p_wtm, boolean isInContextMatch)
    {
        int[] subcols;
        int i=0;
        if (isJobCostingOn())
...
//        subcols[i++] = WorkflowTableModel.CONTEXT_WC;
        ...
        if (isJobCostingOn())
        {
            ...
            {
                double cost = ((Float)p_wtm.getValueAt(r,WorkflowTableModel.COST)).doubleValue();
    ...
    /**
     * Adds the given WorkflowTableModel to the report as a chart
     * <br>
     * @param WorkflowTableModel p_wtm
     * @param p_chosenState -- the job state
     */
    private void addWorkflowSummaryChart(WorkflowTableModel p_wtm)
    {
        ...
        {
            labels[r]= (String)model.getObject(r+1,WorkflowTableModel.TRGLOCALE);
            subrows[r] = r+1;
        }
    ...
    /**
     * Adds a form at the top with the selected job state and job name
     * <br>
     * @param projectmgr -- the selected PM
     * @param wfstate -- the selected WF state
     */
    private void addCriteriaFormAtTop(String p_jobstate, String p_jobname)
    {
        ArrayList labels = new ArrayList();
        labels.add(ReportsPackage.getMessage(m_bundle,"jobstatus") + LABEL_SUFFIX);
        labels.add(ReportsPackage.getMessage(m_bundle,"jobname") + LABEL_SUFFIX);
        ...
        if (isJobCostingOn())
        {
            labels.add(ReportsPackage.getMessage(m_bundle,"currency") + LABEL_SUFFIX);
            fields.add(m_currency.getDisplayName());
        }

        DefaultFormLens dfl = new DefaultFormLens(labels.toArray(),fields.toArray());
    ...
    private void addJobForm(TableModel p_table, int r, Job p_job)
    {
        ...
        for (int c=1; c < p_table.getColumnCount(); c++)
        {
            ...
            if (c == JobTableModel.COST && isJobCostingOn())
            {
                ...
                String formattedCost = CurrencyFormat.getCurrencyFormat(m_currency).format(cost);
                field = formattedCost;
            }
            else
            {
        ...
        labels.add(ReportsPackage.getMessage(m_bundle,"locProfile") + LABEL_SUFFIX);
     ...
     */
    private void addDetailedWorkflowBreakdownTables(long p_jobId, WorkflowTableModel p_wtm, ArrayList p_workflows)
        throws Exception
    {
        ...
        for (int i=0; i < p_workflows.size(); i++)
        {
            ...
            int activeRow = -1;
            DefaultTableModel table = new DefaultTableModel(tasks.size(),columnNames.length);
    ...
    class MyTaskComparator implements Comparator
    {
        ...
        public MyTaskComparator(List p_defaultTaskIds,HashMap p_completedActivities)
        {
