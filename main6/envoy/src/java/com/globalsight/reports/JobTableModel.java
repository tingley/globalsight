...
public class JobTableModel extends AbstractTableModel
{
    ...
    public static final String[] COLUMN_NAMES = {
        "ID", "PRIORITY", "PROJECT", "PM", "JOBNAME", "SRCLOCALE", 
        "DATASRC", "WORDCNT", "CREATE_DATE", "COST", "REVENUE"
    };
    ...
    /**
    * Creates a new JobTableModel holding the relevant data 
    * about a job excluding workflow information
    * <br>
    * @param p_job -- the job to get data from
    * @param p_uilocale -- the UI locale
    * @return JobTableModel
    */
    public JobTableModel(List<Job> p_jobs, 
                          Locale p_uilocale, Currency p_currency )
    {
        ...
        //don't use job costing values even if it's turned on because the particular report
    ...
    */
    private void fillAllData(List<Job> p_jobs)
    {
        for (Job job : p_jobs)
        {
            ...
            //add the job itself to the table so that sorting does not lose the job
    ...
    */
    ...
    /**
    * Set the value at the desired cell
    * <br>
    * @param r -- row
    * @param c -- column
...
/**
    * Sets the value for the desired column
    * <br>
    * @param j --a Job
    * @param dataRow --the desired DataRowWrap
    */
    ...
    private void setJobValue(DataRowWrap dataRow, Job j)
    {
        ...
        try
        {
            ...
            l10nProfile = LocProfileHandlerHelper.getL10nProfile(
                                                    j.getL10nProfileId() );
    ...
    /**
    * Gets the value for the desired column
    * <br>
    * @param j --a Job
    ...
    */
    private Object getJobValue(Job j, int c)
    {
        ...
        try
        {
            ...
            case PRIORITY:
                ...
                break;
            case PROJECT:
                l10nProfile = LocProfileHandlerHelper.getL10nProfile(j.getL10nProfileId());
                o= l10nProfile.getProject().getName();
                break;
            case PM:
                l10nProfile = LocProfileHandlerHelper.getL10nProfile(j.getL10nProfileId());
                o= l10nProfile.getProject().getProjectManagerId();
                break;
            case JOBNAME:
                o= j.getJobName();
                break;
            ...
            case WORDCNT:
                ...
                break;
            case CREATE_DATE:
                o= DateHelper.getFormattedDateAndTime(j.getCreateDate(),m_uilocale);
                break;
            case COST:
                if (m_jobCosting)
                {
                    ...
                    boolean isInContextMatch = PageHandler.isInContextMatch(j, isUseInContext);
                    boolean isUseDefaultContextMatch = PageHandler.isDefaultContextMatch(j);
                    Cost cost1 = getJobCost(j, m_currency, Cost.EXPENSE);
                    o = (isInContextMatch) ? ReportsPackage.getEstimatedCost(cost1) : (isUseDefaultContextMatch) ? ReportsPackage.getUseDefaultContextEstimatedCost(cost1) : ReportsPackage.getNoUseEstimatCost(cost1);
                }
                else
                {
                    o = ReportsPackage.ZERO_COST;
            ...
            case REVENUE:
                if (m_jobCosting && GlobalSightReplet.isJobRevenueOn())
                {
                    ...
                    Cost revenue1 = getJobCost(j, m_currency, Cost.REVENUE);
                    o = (isInContextMatch) ? ReportsPackage.getEstimatedCost(revenue1) : 
                        (isDefaultContextMatch) ? ReportsPackage.getUseDefaultContextEstimatedCost(revenue1) : ReportsPackage.getNoUseEstimatCost(revenue1);
                }
                else
                {
                    o = ReportsPackage.ZERO_COST;
    ...
    private Cost getJobCost(Job p_job, Currency p_currency, int p_costType)
    {
        ...
        {
            cost = BasicReportHandler.calculateJobCost(
                                           p_job, p_currency, p_costType );
        }
        else
        {
            cost = BasicReportHandler.calculateJobCost(
    ...
    class DataRowWrap implements Comparable<Object>
    {
        ...
        public int compareTo(Object object)
        {
           ...
           return new CompareToBuilder()
                ...
                .append(this.jobName, myClass.jobName)
                .toComparison();
        }
