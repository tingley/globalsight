...
public class WorkflowTableModel extends AbstractTableModel
{
    ...
    public String[] COLUMN_NAMES = { "TRGLOCALE", "WFSTATE", "CURACTIVITY",
            "ACCEPTER", "SEGMENT_TM_WC", "IN_CONTEXT_WC", "CONTEXT_WC", "FUZZY_LOW_WC",
            "FUZZY_MED_WC", "FUZZY_MED_HI_WC", "FUZZY_HI_WC", "NO_MATCH",
            "REP_WC", "PER_COMPLETE", "DURATION", "COST", "REVENUE" };
     ...
     */
    public WorkflowTableModel(ArrayList<Workflow> p_workflows, String p_wfstate,
            HttpSession p_session, Currency p_currency)
    {
        this(p_workflows, p_wfstate, p_session, p_currency, true);
    }

    public WorkflowTableModel(ArrayList<Workflow> p_workflows, String p_wfstate, HttpSession p_session, Currency p_currency, boolean fillAllData)
    {
     ...
     *            the currency to use if job costing is on (otherwise null)
     */
    public WorkflowTableModel(ArrayList<Workflow> p_workflows, HttpSession p_session,
            Currency p_currency)
    {
        this(p_workflows, null, p_session, p_currency);
    }

    public WorkflowTableModel(ArrayList<Workflow> p_workflows, HttpSession p_session, Currency p_currency, boolean fillAllData)
    {
    ...
    public void fillAllData(String p_wfstate, ArrayList<Workflow> p_workflows)
    {
        for (Workflow w : p_workflows)
        {
                ...
                !Workflow.CANCELLED.equals(p_wfstate))
            {
                ...
                {
                    datacolumns.put(CURACTIVITY, getWorkflowValue(w,CURACTIVITY));
                ...
                datacolumns.put(SEGMENT_TM_WC, getWorkflowValue(w, SEGMENT_TM_WC));
                if(useInContext){
                    datacolumns.put(IN_CONTEXT_WC, getWorkflowValue(w,IN_CONTEXT_WC));
                }
                if(useDefaultContext)
                {
                    datacolumns.put(CONTEXT_WC, getWorkflowValue(w,CONTEXT_WC));
                }
                datacolumns.put(FUZZY_HI_WC, getWorkflowValue(w, FUZZY_HI_WC));
                datacolumns.put(FUZZY_MED_HI_WC, getWorkflowValue(w, FUZZY_MED_HI_WC));
                datacolumns.put(FUZZY_MED_WC, getWorkflowValue(w, FUZZY_MED_WC));
                datacolumns.put(FUZZY_LOW_WC, getWorkflowValue(w, FUZZY_LOW_WC));
                datacolumns.put(NO_MATCH, getWorkflowValue(w, NO_MATCH));
                datacolumns.put(REP_WC, getWorkflowValue(w, REP_WC));
                datacolumns.put(PER_COMPLETE, getWorkflowValue(w, PER_COMPLETE));
                ...
                if (m_jobCosting)
                {
                    TranslationMemoryProfile tmProfile = w.getJob().getL10nProfile().getTranslationMemoryProfile();
                    boolean isUseInContext = tmProfile.getIsContextMatchLeveraging();
                    boolean isInContextMatch = false;
                    try
                    {
                        isInContextMatch = PageHandler.isInContextMatch(w.getJob(), isUseInContext);
                    }
                    catch (Exception e)
                    {
                        e.printStackTrace();
                    }
                    boolean isDefaultContext = PageHandler.isDefaultContextMatch(w.getJob());
                    ...
                    datacolumns.put(COST, (isInContextMatch) ? ReportsPackage.getEstimatedCost(cost) : (isDefaultContext)
                    ...
                    if(BasicReportHandler.isJobRevenueOn())
                    {
                        ...
                        Cost revenue = BasicReportHandler.calculateWorkflowCost(w,m_currency,Cost.REVENUE);
                        datacolumns.put(REVENUE, ReportsPackage.getActualCost(revenue));
                    }
                }
                else
                {
    ...
    private Object getWorkflowValue(Workflow w, int c)
    {
        ...
        try
        {
            ...
            case SEGMENT_TM_WC:
                ...
                break;
            case CONTEXT_WC:
                o = getTotalWordCount(w, CONTEXT_MATCH_TYPE) == -1? "N/A" : getTotalWordCount(w, CONTEXT_MATCH_TYPE);
                break;
            case IN_CONTEXT_WC:
                o = getTotalWordCount(w, IN_CONTEXT_MATCH_TYPE) == -1? "N/A" : getTotalWordCount(w, IN_CONTEXT_MATCH_TYPE);
                break;
            case FUZZY_HI_WC:
    ...
    private static Integer getTotalWordCount(Workflow p_workflow, int p_type)
    {
        ...
        while (pages.hasNext())
        {
            ...
            case SEGMENT_TM_MATCH_TYPE:
                count += (PageHandler.isInContextMatch(p_workflow.getJob())) 
                    ? wordCounts.getSegmentTmWordCount()
                            : (PageHandler.isDefaultContextMatch(p_workflow.getJob())) ? 
                                    wordCounts.getNoUseExactMatchWordCount() - wordCounts.getContextMatchWordCount()
                                    : wordCounts.getNoUseExactMatchWordCount();
                break;
            case IN_CONTEXT_MATCH_TYPE:
            {
                count += (PageHandler.isInContextMatch(p_workflow.getJob()))?wordCounts.getInContextWordCount():-1;
                break;
            }
            case CONTEXT_MATCH_TYPE:
            {
                count += (PageHandler.isDefaultContextMatch(p_workflow.getJob()) ? wordCounts.getContextMatchWordCount() : -1);
                break;
            }
