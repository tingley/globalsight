package com.globalsight.reports.ter;
/*
...
{
    static private final Logger logger = Logger.getLogger(TERtest.class);

    @SuppressWarnings({ "rawtypes" })
    public double calculateTER(String[] args, LinkedHashMap<String, ArrayList<String>> hypsegs,
            LinkedHashMap<String, ArrayList<String>> refsegs, boolean writeToFile)
    {
        // 1. process arguments
...
//        LinkedHashMap<String, ArrayList<String>> hypsegs = null;
//        LinkedHashMap<String, ArrayList<String>> refsegs = null;
        LinkedHashMap<String, ArrayList<String>> reflensegs = null;
...
//        else
//        {
//            hypsegs = new LinkedHashMap<String, ArrayList<String>>();
//            if (hypdoc == null)
//                logger.info("hyp doc is null");
...
//        else
//        {
//            refsegs = new LinkedHashMap<String, ArrayList<String>>();
//            refsids = new HashMap();
//            if (refdoc == null)
//                logger.info("ref doc is null");
...
//        }
        ...
        if (reflen_fn != "")
        {
            ...
            {
                reflensegs = new LinkedHashMap<String, ArrayList<String>>();
...
//            System.exit(1);
        ...
        BufferedWriter pra_out = null;
        BufferedWriter prm_out = null;
        BufferedWriter ter_out = null;
        BufferedWriter sum_out = null;
        BufferedWriter nbt_out = null;
        if (writeToFile)
        {
            xml_out = openFile(formats, "xml", out_pfx, hyp_fn, ref_fn, reflen_fn, caseon);
            pra_out = openFile(formats, "pra", out_pfx, hyp_fn, ref_fn, reflen_fn, caseon);
            prm_out = openFile(formats, "pra_more", out_pfx, hyp_fn, ref_fn, reflen_fn, caseon);
            ter_out = openFile(formats, "ter", out_pfx, hyp_fn, ref_fn, reflen_fn, caseon);
            sum_out = openFile(formats, "sum", out_pfx, hyp_fn, ref_fn, reflen_fn, caseon);
            nbt_out = openFile(formats, "sum_nbest", out_pfx, hyp_fn, ref_fn, reflen_fn, caseon);
        }
            ...
            if (refsegs.containsKey(id_nrank))
            {
                ...
                if (reflensegs != null)
                {
                    ...
                    if (reflenseglist == null)
                        logger.warn("NO reference length can be found for hyp: " + refids);
                }
                ...
                TERalignment result = score_all_refs((String) hyps.get(0),
                        (ArrayList) refsegs.get(id_nrank), reflenseglist, refids, refspan, hypspan,
                        costfunc);
...
//                  System.exit(1);
            ...
            {
                logger.warn("No reference for segment: " + id_nrank);
//                System.exit(1);
                continue;
            }
        }
        ...
        if (logger.isDebugEnabled())
        {
            logger.info("Total TER: " + (TOTAL_EDITS / TOTAL_WORDS) + " (" + TOTAL_EDITS + "/"
                    + TOTAL_WORDS + ")");
            logger.info("Number of calls to beam search: " + TERcalc.numBeamCalls());
            logger.info("Number of segments scored: " + TERcalc.numSegsScored());
            logger.info("Number of shifts tried: " + TERcalc.numShiftsTried());            
        }

        return TOTAL_EDITS / TOTAL_WORDS;
    }

    public static BufferedWriter openFile(ArrayList formats, String type, String out_pfx,
            String hyp_fn, String ref_fn, String reflen_fn, boolean caseon)
    {
            ...
            try
            {
                ...
                else if (type.equals("sum"))
                {
                    bw.write("Hypothesis File: " + hyp_fn + "\nReference File: " + ref_fn + "\n"
                            ...
                            + "\n");
                    bw.write(String
                            .format("%1$-19s | %2$-4s | %3$-4s | %4$-4s | %5$-4s | %6$-4s | %7$-6s | %8$-8s | %9$-8s\n",
                ...
                else if (type.equals("sum"))
                {
                    ...
                    bw.write(String
                            .format("%1$-19s | %2$-4d | %3$-4d | %4$-4d | %5$-4d | %6$-4d | %7$-6.1f | %8$-8.3f | %9$-8.3f\n",
                                    "TOTAL", tot_ins, tot_del, tot_sub, tot_sft, tot_wsf, tot_err,
                                    tot_wds, tot_err * 100.0 / tot_wds));
                }
                bw.close();
            }
            catch (IOException ioe)
        ...
        try
        {
            sum.write(String
                    .format("%1$-19s | %2$4d | %3$4d | %4$4d | %5$4d | %6$4d | %7$6.1f | %8$8.3f | %9$8.3f\n",
                            ...
                            result.numWsf, result.numEdits, result.numWords, result.score() * 100.0));
    ...
    public static void writeNbestSum(BufferedWriter nbt, TERalignment result, String id)
    {
        try
        {
            nbt.write(String.format(
                    ...
                    result.numIns, result.numDel, result.numSub, result.numSft, result.numWsf,
                    result.numEdits, result.numWords, result.score() * 100.0));
        }
        catch (IOException ioe)
        {
            logger.error(ioe);
    ...
    public static boolean verifyFormats(int in_ref_format, int in_hyp_format, ArrayList out_formats)
    {
        ...
        {
            logger.info("** Error: Both hypothesis and reference have to be in the SAME format");
            return false;
        }
        else if (in_ref_format == 1 && out_formats.indexOf("xml") > -1)
        {
            logger.info("** Warning: XML ouput may not have correct doc id for Trans format inputs");
            return true;
        }
        else
            return true;
    ...
    public static TERalignment score_all_refs(String hyp, List refs, List reflens, List refids,
            String refspan, String hypspan, TERcost costfunc)
    {
        double totwords = 0;
        String ref;
        String refid = "";
    ...
    public static LinkedHashMap<String, ArrayList<String>> load_segs(String fn)
    {
        LinkedHashMap<String, ArrayList<String>> segs = new LinkedHashMap<String, ArrayList<String>>();
        ...
        try
        {
            ...
            while ((line = stream.readLine()) != null)
            {
                ...
                {
                    logger.warn("Warning, Invalid line: " + line);
                }
            }
        }
        catch (IOException ioe)
        ...
        try
        {
            ...
            while ((line = stream.readLine()) != null)
            {
                ...
                if (m.matches())
                {
                    ...
                    {
                        logger.error("Error, translation spans should only be used with SINGLE reference");
                        return null;
                    }
                }
                else
