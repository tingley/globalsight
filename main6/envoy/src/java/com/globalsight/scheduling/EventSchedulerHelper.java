...
/**
 * EventSchedulerHelper is a helper class with static methods which can be accessed
 * and used from any scheduling related class.
 */
...
public class EventSchedulerHelper
{
    ...
    private static boolean m_systemNotificationEnabled = 
     ...
     * @param p_eventType - The event type (i.e. accept, complete, or etc.).
     * @param p_domainObjType - The domain object type.
     * @param p_domainObjId - The id of the domain object.
     */
    public static FluxEventMap findFluxEventMap(Integer p_eventType,
                                                Integer p_domainObjType,
                                                Long p_domainObjId)
        throws Exception
    {
    ...
    public static FluxEventMap findFluxEventMap(HashMap p_eventInfo)
        throws Exception
    {
        Long objectId = (Long) p_eventInfo.get(
            SchedulerConstants.SCHEDULE_DOMAIN_OBJ_ID);
        Integer objectType = (Integer) p_eventInfo.get(
            SchedulerConstants.DOMAIN_OBJ_TYPE);
        Integer eventTypeKey = (Integer) p_eventInfo.get(
            SchedulerConstants.SCHEDULE_EVENT_TYPE);
        FluxEventMap fem = EventSchedulerHelper.findFluxEventMap(
            eventTypeKey, objectType, objectId);
        return fem;
    }
     ...
     * @param p_localeAsString - locale as a string (i.e. en_US).
    ...
    /**
     * Create a new FluxEventMap object which contains information about
     * a newly created event.  This info is used for unscheduling the event.
     * @param p_fluxEventMap - The object to be persisted.
     */
     ...
     * @param p_timeInMilliSec The base date in milliseconds.
     * @param p_seconds The duration to be added to that date.
    ...
    static Date determineDate(long p_timeInMilliSec, long p_seconds)
    {
        ...
        else if ((p_seconds = p_seconds / 60l) <= 
                 Integer.MAX_VALUE)
        {
     ...
     */
    static String localePair(Locale p_sourceLocale,
    ...
    /**
     * Remove an existing FluxEventMap object. This happens when an
     * event is unscheduled.
     * @param p_fluxEventMap - The object to be removed.
     */
    ...
    static void notifyProjectManager(Map p_emailInfo, 
                                     String p_subjectKey,
                                     String p_messageKey)
        throws Exception
    {
        ...
        Long wfId = (Long)p_emailInfo.get(
            SchedulerConstants.WF_ID);
        ...
        List wfManagerIds = wf.getWorkflowOwnerIdsByType(
        ...
        throws Exception 
   {
	...
	private static String[] getArguments(Map p_emailInfo) 
	                              throws PersistenceException, RemoteException, 
	                                     ProjectHandlerException, GeneralException, 
	                                     NamingException {
		Long wfId = (Long) p_emailInfo.get(SchedulerConstants.WF_ID);
		...
		messageArguments[7] = assigneesName;
	...
	private static User[] toUsers(String assigneesName) throws RemoteException, 
		...
		{
			users[i] = ServerProxy.getUserManager().getUser(
					allAssignee[i]);		   
		}
		return users;
	}
     ...
     * @param p_date - The date to be formatted.
     * @param p_displayLocale - The locale used for formatting the date.
     * @return A formatted date based on the given locale.
     */
    private static String getConvertedTime(Date p_date, 
                                           Locale p_displayLocale,
                                           TimeZone p_timeZone)
    {
