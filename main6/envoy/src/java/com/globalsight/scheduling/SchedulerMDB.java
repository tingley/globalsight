...
/**
 * This MessageDrivenBean is responsible
 * for performing the event scheduling as an asynchronous process.
 */
public class SchedulerMDB extends GenericQueueMDB
{
    ...
    /**
    * Start the scheduling related process as a separate thread.  This method
    * is not a public API and is ONLY invoked by it's consumer for
    * scheduling purposes (i.e. scheduling/unscheduling an event).  Upon an
    * exception, a notification will be sent to the Project Manager.
    *
    ...
    */
    public void onMessage(Message p_message)
    {
        ...
        catch (Exception e)
        {
            ...
            if (map != null)
            {
                try
                {                
                    HashMap emailInfo = (HashMap)map.get(SchedulerConstants.EMAIL_INFO);
                ...
                {
                    s_logger.error(
                        "Failed to notify PM about scheduling failure.",
                        e);
                }
            }            
        }
    ...
    //TomyD -- each of these actions will have a different set of info stored in the HashMap.
    // Part of this info is stored within WorkflowServerLocal class since it's the one that calls
    // the EventSchedulerLocal for performing the action (i.e. email info, accept/complete duration,
    // %threshold, and so on).
    private void performSchedulingAction(HashMap p_map)
    throws Exception
    {
        ...
        switch (actionType)
        {
            ...
            case SchedulerConstants.ACCEPT_ACTIVITY:
                //stop the accept timer if still running (also remove from our map table)
                unscheduleEvent(p_map);
                //schedule completed by event (also persist in our map table)
                scheduleEvent(p_map);
                break;
            case SchedulerConstants.SKIP_ACTIVITY:
                //stop all the remaining timers for the active task (remove from map table).
                unscheduleEvent(p_map);
                break;
            ...
            case SchedulerConstants.FINISH_ACTIVITY:
                //stop all the remaining timers for the finished task (remove from db)
                unscheduleEvent(p_map);
                // create timers for the next task (if any)
                scheduleEvent(p_map);
                break;
            case SchedulerConstants.MODIFY_WORKFLOW: //activate task (modify wf)
    ...
    private void unscheduleEvent(HashMap p_map)
        throws Exception
    {
        // based on domainObjId, domainObjType, and/or event type get JobId
        Long objId = (Long)p_map.get(SchedulerConstants.UNSCHEDULE_DOMAIN_OBJ_ID);
        Integer objType = (Integer)p_map.get(SchedulerConstants.DOMAIN_OBJ_TYPE);
        Integer eventType = (Integer)p_map.get(SchedulerConstants.UNSCHEDULE_EVENT_TYPE);
    ...
    // unschedule events by first removing them from schedule queue and then from
    // our flux_gs_map table. This method is invoked during workflow cancellation
    // as the state of a work item is not known (it's more expensive to find a work item
    // based on the node instance id).
    private void unscheduleEvent(Long p_objectId, Integer p_objectType)
        throws Exception
    {
        
        Iterator iterator = EventSchedulerHelper
            .findFluxEventMaps(p_objectType, p_objectId);
    ...
    private void scheduleEvent(HashMap p_map)
        throws Exception
    {
        //get schedule event type (whether it's for scheduling accept or complete event)
        Integer eventType = (Integer)p_map.get(SchedulerConstants.SCHEDULE_EVENT_TYPE);

        // it's possible to get here when an activity has been finished.  If there's
        ...
        BaseFluxCalendar calendar = 
                (BaseFluxCalendar)p_map.get(SchedulerConstants.BIZ_CALENDAR);
        
        //This base calendar is used for the activity due issue, this calendar is the 
        //default calendar of GlobalSight.
        String companyId = (String) p_map.get(SchedulerConstants.CURRENT_COMPANY_ID);
        FluxCalendar defaultCalendar = ServerProxy.getCalendarManager()
                                       .findDefaultCalendar(companyId);
        //end of the base calendar.
        
        //threshold
        Float thresholdAsFloat = (Float)p_map.get(SchedulerConstants.WARNING_THRESHOLD);
        //creation date
        Long creationDate = (Long)p_map.get(SchedulerConstants.CREATED_DATE);
        // repeat count
        Integer repeatCount = (Integer)p_map.get(SchedulerConstants.REPEAT_COUNT);
        ...
        Integer acceptType = (Integer)SchedulerConstants
                .s_eventTypes.get(SchedulerConstants.ACCEPT_TYPE);

        boolean isAcceptType = eventType.intValue() == acceptType.intValue();

        String eventTypePrefix = 
                isAcceptType ? 
                SchedulerConstants.ACCEPTANCE : 
            SchedulerConstants.COMPLETION ;
        
        // now using the estimated acceptance/completion date, determine the warning date
        ...
        //Here add two parameters to the emailInfo to show how many days be overdue.
        ...
        String jobId = scheduleEvent(deadline, warningDate, repeatCount,
                               listener, calendar, emailInfo,
                               eventTypePrefix);
        emailInfo.put(SchedulerConstants.OVERDUE, Boolean.TRUE); 
        String jobIdOverdue = scheduleEvent(deadlinePM, deadlineUser, repeatCount,
                                      listener, calendar, emailInfo, 
                                      eventTypePrefix);
    ...
    throws Exception
    {
        try
        {
            Long objId = (Long)p_map.get(SchedulerConstants.SCHEDULE_DOMAIN_OBJ_ID);
            Integer objType = (Integer)p_map.get(SchedulerConstants.DOMAIN_OBJ_TYPE);
    ...
    private String scheduleEvent(Date deadline, Date warningDate, 
                                 Integer repeatCount, Class listener, 
                                 BaseFluxCalendar calendar, 
                                 HashMap emailInfo, String eventTypePrefix)
        throws Exception
    {
    ...
    private void unschedule(FluxEventMap p_fluxEventMap)
        throws Exception
    {
        //unschedule event
