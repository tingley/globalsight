...
public class ImportUtil
{
    ...
    //TODO
    public static boolean isEmptyOrIllegalRow(Cell[] p_rowCells, List p_colDescriptors)
    {
    	...
    	if (p_rowCells.length > 1)
    	{
        	...
        	{
                ColumnDescriptor col = (ColumnDescriptor)p_colDescriptors.get(i);
    ...
    public static int findHeaderRowNumber(Sheet p_readSheet, int p_fromRow, Termbase p_termbase)
    {
    	if (p_fromRow < 0)
    	{
    		return -1;
    	...
    	for (int i = p_fromRow; i < p_readSheet.getRows(); i++)
    	{
    		...
    		for (int j = 0; j < cells.length; j++)
    		{
    			if (!isEmptyCell(cells[j]) && 
    					isLanguageDefined(p_termbase, cells[j].getContents().trim()))
    			{
    				languageCell++;
    			}
    			if (languageCell > 1)
    ...
    public static boolean isLanguageDefined(Termbase p_termbase, String p_language)
    {
    ...
    public static int getAssociatedColumn(String p_columnName, List p_columnDescriptors)
    {
    	int ac = -1;
    	if (p_columnName != null)
    	{
    				...
    				p_columnName.startsWith("Term"))
    		{
    			...
    			{
    				ColumnDescriptor firstColumn = (ColumnDescriptor)p_columnDescriptors.get(0);
    ...
    public static String mapTermbaseDefinedLang(Termbase p_termbase, String p_columnName)
    {
    	ArrayList langs = p_termbase.getLanguages();
    	Language lang = null;
    	for (int i = 0; i < langs.size(); i++)
