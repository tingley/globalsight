...
import com.globalsight.cxe.util.mail.MailSenderUtil;
...
/**
 * This class is the concrete implementation of Mailer interface and is
 * responsible for sending emails (which it delegates to MailSender)
 */

public class MailerLocal implements Mailer
{
    // the resource name for retrieving email message subject and text
    public static final String DEFAULT_RESOURCE_NAME = "com/globalsight/resources/messages/EmailMessageResource";

    private UserManagerWLRemote m_userManager = null;

    private static Logger s_logger = Logger.getLogger(MailerLocal.class.getName());
     ...
     */
    public List getEmailAddresses(String[] p_userIds) throws MailerException, RemoteException
    {
        List returnVal = null;
        try
        {
            returnVal = getUserManager().getEmailInformationForUsers(p_userIds);
        }
        catch (Exception e)
        {
            String[] arg =
            { p_userIds.toString() };
            throw new MailerException(MailerException.MSG_FAILED_TO_GET_EMAIL_ADDRESSES, arg, e);
        }
        return returnVal;
    }

    /**
     * @see Mailer.sendMail(EmailInformation, EmailInformation, String, String,
     *      String[], String)
     */
    public void sendMail(EmailInformation p_sendFromEmailInfo,
            EmailInformation p_recipientEmailInfo, String p_subjectKey, String p_messageKey,
            String[] p_messageArguments, String p_companyIdStr)
            throws MailerException, RemoteException
    {
        if (!m_systemNotificationEnabled
                || !isNotificationEnabled(p_subjectKey, p_recipientEmailInfo.getUserId()))
        {
            return;
        }
        ...
        boolean jobCommentsPermission = perSet.getPermissionFor(Permission.ACTIVITIES_COMMENTS_JOB);
        ...
        String message = MessageFormat.format(bundle.getString(p_messageKey), p_messageArguments);
        // send the email
        ...
        {
            sendMail(from, email, ccEmail, bccEmail, subject, message, attachment.split(","));
    ...
    public void sendMail(EmailInformation p_sendFromEmailInfo,
            EmailInformation p_recipientEmailInfo, String p_subjectKey, String p_messageKey,
            String[] p_messageArguments, String[] p_attachments, long p_companyId)
            throws MailerException, RemoteException
    {
        if (!m_systemNotificationEnabled
                || !isNotificationEnabled(p_subjectKey, p_recipientEmailInfo.getUserId()))
        ...
        // Email address
        String fromEmail = MailerHelper.getSendFrom(String.valueOf(p_companyId),
                p_sendFromEmailInfo);
    ...
    /**
     * Modify on Mailer.sendMail(EmailInformation, EmailInformation, String,
     * String, String[], String)
     */
    public void sendMail(String p_sendFromUserId, EmailInformation recipientEmailInfo,
            String p_subjectKey, String p_messageKey, String[] p_messageArguments,
            String p_companyIdStr) throws MailerException, RemoteException
    {
        EmailInformation fromEmailInfo = ServerProxy.getUserManager()
                .getEmailInformationForUser(p_sendFromUserId);
        sendMail(fromEmailInfo, recipientEmailInfo, p_subjectKey, p_messageKey, p_messageArguments,
                p_companyIdStr);
    }
    ...
    public void sendMailFromAdmin(String p_recipient, String[] p_messageArguments,
            String p_subjectKey, String p_messageKey, String p_companyIdStr)
            throws MailerException, RemoteException
    {
        if (!m_systemNotificationEnabled)
        {
        ...
        sendMailFromAdmin(Locale.getDefault(), recipient, p_subjectKey, p_messageKey,
                p_messageArguments, null, p_companyIdStr);
    }

    /**
     * Send an email to a recipient from the system admin. This method is used
     * for system notifications (i.e. during an import/export).
     *
     */
    public void sendMailFromAdmin(User p_recipient, String[] p_messageArguments,
            String p_subjectKey, String p_messageKey, String p_companyIdStr)
            throws MailerException, RemoteException
    {
        if (!m_systemNotificationEnabled
        ...
        sendMailFromAdmin(LocaleWrapper.getLocale(userLocale), p_recipient, p_subjectKey,
                p_messageKey, p_messageArguments, null, p_companyIdStr);
    }

    /**
     * Send an email to the system administrator. This method is used for system
     * notifications (i.e. during an import/export) where the project manager
     * could not be identified. Attachments can be added to the email message.
     * They are strings that are converted into file attachments.
     */
    public void sendMailToAdmin(String[] p_messageArguments, String p_subjectKey,
    ...
    private UserManagerWLRemote getUserManager() throws Exception
    {
    ...
    /*
     * Determines whether the notification preference for this user is enabled,
     * and whether the user even has permission to get the notification.
     */
    private static boolean isNotificationEnabled(String p_subjectKey, String p_userId)
    {
        ...
        try
        {
            ...
            isNotificationEnabled = MailerConstants
                    .hasPermissionForThisCategoryOfNotification(perms, p_subjectKey);
            if (isNotificationEnabled == false)
            {
                s_logger.warn("User " + p_userId
                        + " does not have permission to receive notifications for " + p_subjectKey);
                return isNotificationEnabled; // we know at this point there is
        ...
        {
            s_logger.error("Failed to get permissions for user " + p_userId
                    + " to determine if he/she could receive notifications for " + p_subjectKey, e);
            return false;
        }
        ...
        try
        {
            ...
            UserParameter up = ServerProxy.getUserParameterManager().getUserParameter(p_userId,
                    UserParamNames.NOTIFICATION_ENABLED);

            String category = MailerConstants.getNotificationParamName(p_subjectKey);
            if (category != null)
            {
                categoryParam = ServerProxy.getUserParameterManager().getUserParameter(p_userId,
                        category);
            }

            isNotificationEnabled = (up == null || up.getBooleanValue())
                    && (categoryParam == null || categoryParam.getBooleanValue());
        }
        catch (Exception e)
        {
            s_logger.error("Failed to get notification preference for user: " + p_userId, e);
        }
     ...
     */
    private void sendMailFromAdmin(Locale p_userLocale, User p_user, String p_subjectKey,
            String p_messageKey, String[] p_messageArguments, String[] p_attachments,
            String p_companyIdStr) throws MailerException
    {
        if (CONTAINS_DURATION_IN_EMAIL.contains(p_messageKey))
        {
            p_messageArguments = buildMessageArguments(p_messageArguments);
        ...
        String companyName = CompanyWrapper.getCompanyNameById(p_companyIdStr);
        ResourceBundle bundle = SystemResourceBundle.getInstance()
                .getEmailResourceBundle(DEFAULT_RESOURCE_NAME, p_userLocale, companyName);
        String from = null;
        try
        {
            from = MailerHelper.getSendFrom(p_companyIdStr);
     ...
     */
    private void sendMail(String p_sendFrom, String p_sendTo, String p_sendCc, String p_sendBcc,
            String p_subject, String p_message, String[] p_attachments) throws MailerException
    {
        if (!m_systemNotificationEnabled)
        {
            return;
        ...
        try
        {
            ...
            data.put("from", p_sendFrom);
            data.put("to", p_sendTo);
            data.put("cc", p_sendCc);
            data.put("bcc", p_sendBcc);
            data.put("subject", p_subject);
            data.put("text", p_message);
            data.put("attachments", p_attachments);
            // GBS-4400
            MailSenderUtil.sendMailWithThread(data);
        }
        catch (Exception e)
        {
            ...
            args[1] = sendTo;
            args[2] = p_subject;
            throw new MailerException(MailerException.MSG_FAILED_TO_SEND_EMAIL, args, e);
        }
    }
    ...
    private String[] handleMessageArgments(String[] p_args)
    {
        ...
        {
            s_logger.error("Failed to handleMessageArgments: " + p_args, e);
        }
