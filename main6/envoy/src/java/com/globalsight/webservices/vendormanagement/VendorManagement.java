...
public class VendorManagement extends AbstractWebService
{
    ...
    // -------------------- member variables ----------------------------------
    // used by checkIfInstalled() to remember whether the web service is installed
    ...
    static
    {
        ...
        {
            s_logger.error("Failed to create the Vendor Management web service.",ne);
        }
    }
        ...
        throws WebServiceException
    {
        ...
        try
        {
            ...
            responseXml.append("\t<vendorId>").append(v.getIdAsLong().toString()).
                        append("</vendorId>\r\n");
            responseXml.append("\t<customVendorId>").append(v.getCustomVendorId()).
                        append("</customVendorId>\r\n");
            responseXml.append("\t<status>").append(SUCCESSFUL).append("</status>\r\n");
            responseXml.append("</addVendorResult>");
        } catch (VendorException vme)
        {
            String message = null;
            // the user couldn't be created - but the vendor was.
            if (vme.getMessageKey().equals(VendorException.MSG_FAILED_TO_CREATE_USER))
            {
            ...
            message = makeErrorXml("addVendor", message + vme.getLocalizedMessage());
        ...
        {
            s_logger.error("Failed to add a vendor with the following information: " +
                           p_xmlVendorInfo, e);
            String message = "Failed to add a vendor.  " + e.getLocalizedMessage();
     ...
     */
    public String modifyVendor(String p_accessToken, 
                               String p_xmlVendorInfo)
        throws WebServiceException
    {
        StringBuffer responseXml = null;
        ...
        } catch (VendorException vme)
        {
            ...
            else if (vme.getMessageKey().equals(VendorException.MSG_FAILED_TO_MODIFY_USER))
            {
                message = "Vendor " + v.getCustomVendorId() + 
                          " was modified successfully, however " +
            ...
            s_logger.error(message, vme);
            message = makeErrorXml("modifyVendor", message + vme.getLocalizedMessage());
            throw new WebServiceException(message);
        }
        catch (WebServiceException wse)
        {
        ...
        {
            s_logger.error("Failed to modify vendor with info: " + p_xmlVendorInfo, e);
            String message = "Failed to modify a vendor.  " + e.getLocalizedMessage(); 
        ...
        throws WebServiceException
    {
        ...
        try
        {
            ...
            for (Iterator vi = vendors.iterator() ; vi.hasNext() ; )
            {
                ...
                responseXml.append("\t<vendorId>").append(v.getId()).append("</vendorId>\r\n");
                responseXml.append("\t<customVendorId>").append(
                    getSecureValue(fs, VendorSecureFields.CUSTOM_ID, v.getCustomVendorId())).
                            append("</customVendorId>\r\n");
                responseXml.append("\t<firstName>").append(
                    getSecureValue(fs, VendorSecureFields.FIRST_NAME, v.getFirstName())).
                            append("</firstName>\r\n");
                responseXml.append("\t<lastName>").append(
                    getSecureValue(fs, VendorSecureFields.LAST_NAME, v.getLastName())).
                            append("</lastName>\r\n");
                responseXml.append("\t<pseudonym>").append(v.getPseudonym()).
                            append("</pseudonym>\r\n");
                responseXml.append("\t<status>").append(
                    getSecureValue(fs, VendorSecureFields.STATUS, v.getStatus())).
                            append("</status>\r\n");
                responseXml.append("</vendor>\r\n");
            }
            responseXml.append("</vendorInformation>\n");
        ...
        {
            s_logger.error( "Failed to get the basic information for all vendors.", e);
        ...
        throws WebServiceException
    {
        ...
        try
        {
            ...
            if (v != null)
            {
                ...
                {
                    responseXml.append("\t<title>").append(
                        getSecureValue(fs, VendorSecureFields.TITLE, v.getTitle())).
                                append("</title>\r\n");
                }
                String isInternalVendor = v.isInternalVendor() ? "true" : "false";
                responseXml.append("\t<isInternalVendor>").append(
                    getSecureValue(fs, VendorSecureFields.IS_INTERNAL, isInternalVendor)).
                        append("</isInternalVendor>\r\n");
                if (v.getCompanyName() != null && v.getCompanyName().length() > 0)
                {
                    responseXml.append("\t<companyName>").append(
                        getSecureValue(fs, VendorSecureFields.COMPANY, v.getCompanyName())).
                            append("</companyName>\r\n");
                }
                if (v.getAddress() != null && v.getAddress().length() > 0) 
                {
                    responseXml.append("\t<address>").append(
                        getSecureValue(fs, VendorSecureFields.ADDRESS, v.getAddress())).
                        append("</address>\r\n");
                }
                if (v.getNationalities() != null && v.getNationalities().length() > 0)
                {
                    responseXml.append("\t<nationality>").append(
                        getSecureValue(fs, VendorSecureFields.CITIZENSHIP, v.getNationalities())).
                        append("</nationality>\r\n");
                }
                if (v.getDateOfBirth() != null && v.getDateOfBirth().length() > 0)
                {
                    responseXml.append("\t<dob>").append(
                        getSecureValue(fs, VendorSecureFields.DOB, v.getDateOfBirth())).
                        append("</dob>\r\n");
                }
                if (v.getEmail() != null && v.getEmail().length() > 0)
                {
                    responseXml.append("\t<communication>\r\n");
                    responseXml.append("\t\t<communicationType>EMAIL</communicationType>\r\n");
                    responseXml.append("\t\t<communicationValue>").append(
                        getSecureValue(fs, VendorSecureFields.EMAIL, v.getEmail())).
                                append("</communicationValue>\r\n");
                    responseXml.append("\t</communication>\r\n");
                }
                responseXml.append(getPhoneNumbersXml(v, fs));

                responseXml.append("\t<communicationLocale>").append(
                    getSecureValue(fs, VendorSecureFields.EMAIL_LANGUAGE, v.getDefaultUILocale())).
                    append("</communicationLocale>\r\n");

                if (v.getResume() != null &&
                    v.getResume().length() > 0)
                {
                    responseXml.append("\t<resume>");
                    responseXml.append(
                        getSecureValue(fs, VendorSecureFields.RESUME, v.getResume())).
                        append("</resume>\r\n");
                }
                ...
                {
                    responseXml.append("\t<notes>").append(
                        getSecureValue(fs, VendorSecureFields.NOTES, v.getNotes())).
                            append("</notes>\r\n");
                }                                     
                ...
                responseXml.append("\t<user isAmbassadorUser=\"").append(isAmbassadorUser ? "true\">" : "false\"/>");
                responseXml.append("\r\n");
                if (isAmbassadorUser)
                {
                    if (v.getUser() != null)
                    {
                        responseXml.append("\t\t<username>").append(
                            getSecureValue(fs, VendorSecureFields.USERNAME, v.getUser().getUserId())).
                            append("</username>");
                    }
    ...
    /**
     * This checks whether the web service is installed.
     * The websvc.installKey system parameter value is
     * checked against the expected value. If not installed,
    ...
    /**
     * Parses through the XML sent in and creates a vendor or modified
     * an existing vendor from the vendor information specified.
     * It also only allows updates to the fields that are SHARED with
     * the user requesting the modification or add.
     *
     * @param p_xmlVendorInfo An XML string describing the vendor.
     * @param p_createNewVendor  Specifies if the vendodr is new or an existing
     *                           one to be modified.  'true' means create a new one,
     *                           'false' means modify an existing one.
     *
        ...
        throws WebServiceException
    {
        ...
        else
        {
            ...
            {
                String errMessage = "Failed to find vendor with custom id " + customVendorId + 
                               " for modification.";
                s_logger.error(errMessage, e);
                throw new WebServiceException(errMessage);
            }
            // couldn't find the vendor so throw an error - they must specifically
        ...
        try
        {
            fs = ServerProxy.getSecurityManager().getFieldSecurity(p_userRequesting, 
                                                                   v,
                                                                      true);
        } 
        catch (Exception e)
        {
            String errMessage = "Failed to add/modify the vendor " + v.getCustomVendorId() +
        ...
        if(canFieldBeUpdated(fs, VendorSecureFields.IS_INTERNAL))
        {
                ...
                if (isInternal != null &&
                        isInternal.toLowerCase().equals("true"))
                {
        ...
        if (canFieldBeUpdated(fs, VendorSecureFields.AMBASSADOR_ACCESS))
        {
            ...
            // if an existing user then find the user and set in the vendor object.
            if (isExistingUser.toLowerCase().equals("true"))
            {
                ...
                {
                    s_logger.error("Couldn't find the existing user the vendor should be assigned to.", e);
                    u = null;
                }
                if (u != null)
                {
            ...
            else    // not set to be used in ambassador
            {
    ...
    private void setCustomFields(Vendor p_v, NodeList p_sections)
    {
        ...
        if (p_sections != null) 
        {
            for (int i = 0 ; i < p_sections.getLength() ; i++)
            {
                ...
                String section = 
                      VendorXmlParser.getValue(elem, VendorXmlParser.CUSTOM_NAME);
                ...
                {
                    // just work with an empty hashtable.  none of the fields will
                    // be valid since there isn't a custom form but each invalid field
                    // will be logged out and written to the vendor's notes field
                    validFields = new Hashtable();
                }

                // get fields in the section
                NodeList fields = elem.getElementsByTagName(VendorXmlParser.CUSTOM_FIELD);
                for (int fi = 0 ; fi <fields.getLength() ; fi++)
                {
                    Element fElem = (Element)fields.item(fi);
                    // get the field name and value
                    String fieldName = 
                                VendorXmlParser.getValue(fElem, VendorXmlParser.CUSTOM_NAME);
                    String value = 
                            VendorXmlParser.getValue(fElem, VendorXmlParser.CUSTOM_FIELD_VALUE);
                    if (value != null)
                    {
                        value.toLowerCase();
                    }
                    ...
                    if (validFields.containsKey(key))
                    {
                        ...
                        // if a checkbox change the value to be the exact as the fieldName
                        if (CustomFormParser.FIELD_TYPE_CHECKBOX.equals(type))
                        {
                            // assumes the type is checkbox if the value is true/false
                            ...
                            {
                                // if the value equals "false" then no need to store the value
                                continue;
                            }
                        }
                        try
                        ...
                        {
                            // if an error add custom fields to the notes area and keep going
                            p_v.appendNotes("\n\rCustom field " + key + " could not be added or updated with the value " +
                                          value + " to the vendor.\n\r");
                        }
                    }
                    else
                    {
                        s_logger.error("Custom field " + key + " is not a valid custom field.  It wasn't added to the vendor.");
                        p_v.appendNotes("\n\rCustom field " + key + " could not be added or updated with the value.  It isn't a valid field or section.");
                    }
                } // for
            } // for
        } // if sections exist
    ...
    private void setRoles(Vendor p_vendor, NodeList p_roles)
    {
        ...
        for (int i = 0 ; i < p_roles.getLength() ; i++)
        {
            ...
            String sLocaleName = 
                  VendorXmlParser.getValue(elem, VendorXmlParser.SOURCE_LOCALE);
            String tLocaleName = 
                    VendorXmlParser.getValue(elem, VendorXmlParser.TARGET_LOCALE);
            String activityName = 
                    VendorXmlParser.getValue(elem, VendorXmlParser.ACTIVITY);

            try
            {
                Activity a = ServerProxy.getJobHandler().getActivity(activityName);
                ...
                // activity and locale pair.  If so copy the old one over (retain rates and ID).
            ...
            catch (Exception e)
            {
                ...
                p_vendor.appendNotes("\n\rRole with source locale " + 
                                      sLocaleName + " and target locale " +
                                    tLocaleName + " and activity " + activityName +
    ...
    private void setProjects(Vendor p_vendor, boolean inAllProjects, List p_projects)
        throws WebServiceException
    {
        try
        {
            ...
            else                                                
            {
                ...
                if (p_projects != null)
                {
                    for (Iterator pi = p_projects.iterator() ; pi.hasNext() ; ) 
                    {
                        ...
                        Project p  = getProjectHandler().getProjectById(projectId.intValue());
                        p_vendor.addToProject(p);
                    }
                }
            }
        ...
        throws WebServiceException
    {
        ...
        {
            String errMessage = "Failed to get the vendor's resume " + p_filename +
    ...
    private String getRolesXml(Vendor p_v, FieldSecurity p_fs)
    {
        ...
        if (getSecureValue(p_fs, VendorSecureFields.ROLES,
        ...
        else
        {
            ...
            for (Iterator ri = roles.iterator() ; ri.hasNext() ; )
            {
                ...
                sb.append("\t<role>\r\n");
                sb.append("\t\t<sourceLocale>").append(lp.getSource().toString()). 
                            append("</sourceLocale>\r\n");
                sb.append("\t\t<targetLocale>").append(lp.getTarget().toString()).
                            append("</targetLocale>\r\n");
                sb.append("\t\t<activityType>").append(vr.getActivity().getName()).
                            append("</activityType>\r\n");
                sb.append("\t</role>\r\n");
            }
        }
    ...
    private String getProjectsXml(Vendor p_v, FieldSecurity p_fs)
    {
        ...
        if (getSecureValue(p_fs, VendorSecureFields.PROJECTS,
                                   NOT_CONFIDENTIAL).equals(CONFIDENTIAL))
        {
            sb.append("\t<projects>");
            sb.append(CONFIDENTIAL);
        ...
        else
        {
            ...
            sb.append("\t<projects all=\"").append(inAllProjects ? "true\"/>" : "false\">");
            sb.append("\r\n");
            if (!inAllProjects)
            {
    ...
    private String getPhoneNumbersXml(Vendor p_v, FieldSecurity p_fs)
    {
        ...
        {
            String homeNumber = p_v.getPhoneNumber(CommunicationInfo.CommunicationType.HOME);
            if (homeNumber != null && homeNumber.length() > 0) 
            {
                sb.append("\t<communication>\r\n");
                sb.append("\t\t<communicationType>HOME</communicationType>\r\n");
                sb.append("\t\t<communicationValue>").append(
                        getSecureValue(p_fs, VendorSecureFields.HOME_PHONE,
                                       homeNumber)).append("</communicationValue>\r\n");
                sb.append("\t</communication>\r\n");
            }
        }
        if (p_v.getPhoneNumber(CommunicationInfo.CommunicationType.WORK) != null)
        {
            String workNumber = p_v.getPhoneNumber(CommunicationInfo.CommunicationType.WORK);
            if (workNumber != null && workNumber.length() > 0)
            {
                sb.append("\t<communication>\r\n");
                sb.append("\t\t<communicationType>WORK</communicationType>\r\n");
                sb.append("\t\t<communicationValue>").append(
                       getSecureValue(p_fs, VendorSecureFields.WORK_PHONE,
                            workNumber)).append("</communicationValue>\r\n");
                sb.append("\t</communication>\r\n");
            }
        }
        if (p_v.getPhoneNumber(CommunicationInfo.CommunicationType.CELL) != null)
        {
            String cellNumber = p_v.getPhoneNumber(CommunicationInfo.CommunicationType.CELL);
            if (cellNumber != null && cellNumber.length() > 0 )
            {
                sb.append("\t<communication>\r\n");
                sb.append("\t\t<communicationType>CELL</communicationType>\r\n");
                sb.append("\t\t<communicationValue>").append(
                        getSecureValue(p_fs, VendorSecureFields.CELL_PHONE,
                        cellNumber)).append("</communicationValue>\r\n");
                sb.append("\t</communication>\r\n");
            }
        }
        if (p_v.getPhoneNumber(CommunicationInfo.CommunicationType.FAX) != null)
        {
            String faxNumber = p_v.getPhoneNumber(CommunicationInfo.CommunicationType.FAX);
            if (faxNumber != null && faxNumber.length() > 0)
            {
                sb.append("\t<communication>\r\n");
                sb.append("\t\t<communicationType>FAX</communicationType>\r\n");
                sb.append("\t\t<communicationValue>").append(
                       getSecureValue(p_fs, VendorSecureFields.FAX,
            ...
            if (cfs != null && cfs.size() > 0)
            {
                ...
                for (Iterator ki2 = keys2.iterator() ; ki2.hasNext() ; )
                {
                    ...
                    for (Iterator fi = l.iterator() ; fi.hasNext() ; )
                    {
                        ...
                        // tbd - probably should check the type to ensure a checkbox
    ...
    /**
     * Return the value for the specified field according to the field
     * security.
     */
