...
public class NamedNodeMapImpl
     ...
     */
    protected int findNamePoint2(String name, int start) {
    	...
    	if(nodes != null) {
    		...
    		while (first <= last) {
    			...
    			if(test == 0) {
    				System.out.println("name = " + name + " elementAt " + i);
    				return i; // Name found
                }
    			else if (test < 0) {
    				last = i - 1;
    	...
    	System.out.println("new position " + (-1 - i));
    	return -1 - i; // not-found has to be encoded.

    } // findNamePoint(String):int

    /**
     * The method implemented a sequential search, if the node was found by the given name,
     * return the index of node, if not, return a new index
     * which is the length of attribute list adding 1.
     */
    protected int findNamePoint(String name, int start) {
    	...
    	if(nodes != null)
    	{
    		...
    		{
    			if(name.equals(((Node)nodes.elementAt(j)).getNodeName()))
    			{
        				return j ;    					
    			}
    		}
