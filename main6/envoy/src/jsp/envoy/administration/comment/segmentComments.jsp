...
//    String backPage = (String)request.getParameter(WebAppConstants.PAGE_NAME);
    String backPage = (String) session.getAttribute("segmentCommentsBackPage");
    ...
    {
        backURL = taskCommentsURL;
    }
%>
<HTML>
<!-- This JSP is envoy\administration\comment\segmentComments.jsp -->
<HEAD>
<TITLE><%=title%></TITLE>
<style>
.comment {
...
function openReviewEditor(key)
{
    ...
    else
    {
        ...
        var url = "&<%=WebAppConstants.SOURCE_PAGE_ID%>=<%=sourcePageId%>" +
                  ...
                  "&curTuvId=" + ids[2] +
                  "&curSubId=" + ids[3];
        if ("taskComments" == "<%= backPage %>")
        {
           url += "&<%=WebAppConstants.TASK_ID%>=<%=taskId%>"
        }
...
r<%@ include file="/envoy/common/header.jspIncl" %>
<%@ include file="/envoy/common/navigation.jspIncl" %>
<%@ include file="/envoy/wizards/guides.jspIncl" %>
<div ID="contentLayer" STYLE=" POSITION: ABSOLUTE; Z-INDEX: 9; TOP: 108; LEFT: 20px; RIGHT: 20px;">
    <SPAN CLASS="mainHeading">
    <%=bundle.getString("lb_segment")%> <%=bundle.getString("lb_comments")%>
    </SPAN>

<div style="padding-top:10px" class="standardText">
<%=bundle.getString("lb_file")%>: <b><%=pageName%></b>
&nbsp;&nbsp;&nbsp;&nbsp;
<%=bundle.getString("lb_target_locale")%>: <b><%=targLocale%></b>
</div>
...
<!-- Comments data table -->
    <table cellpadding=0 cellspacing=0 border=0 class="standardText">
        <tr>
            <td style="padding-bottom: 3px;padding-top:5px"><%=bundle.getString("lb_show")%>:
            ...
            <%
                ...
                {
                    String status = (String)statusList.get(j);
                    out.print("<option value=" + status);
                    if (status.equals(selectedStatus))
                        out.print(" selected ");
                    out.println(">" + bundle.getString("issue.status." + status) + "</option>");
                }
            %>
            </select>
        </tr>
        ...
        <% } %>
          <amb:table bean="segmentCommentList" id="issue"
            ...
            <amb:column label="lb_status" sortBy="<%=IssueComparator.STATE%>">
                <%=bundle.getString("issue.status." + issue.getStatus())%>
            </amb:column>
...
<% } %>
    ...
    <tr>
        <td style="padding-top:5px">
            <input type="button" value="<%=bundle.getString("lb_back_to_comments")%>"
        onclick="submitForm();">
        </td>
    </tr>
</table>
