<%@ page
	...
	com.globalsight.everest.webapp.pagehandler.PageHandler, 
        ...
        session="true"
%>
<jsp:useBean id="new3" scope="request"
 class="com.globalsight.everest.webapp.javabean.NavigationBean" />
<%
    ResourceBundle bundle = PageHandler.getBundle(session);

    String newURL = new3.getPageURL();
    ...
    String subTitle = bundle.getString("lb_teamsite_branches") +
                      bundle.getString("lb_colon") + " " +
                      bundle.getString("lb_new_branches");
...
function submitTargetForm(stringSent) {
  if (document.layers) {
      menu.document.fileListForm.<%= TeamSiteBranchMainHandler.TEAMSITE_TARGET_BRANCH %>.value = stringSent;
      if (confirmForm(menu.document.fileListForm)) menu.document.fileListForm.submit();
    }
  else {
      menu.fileListForm.<%= TeamSiteBranchMainHandler.TEAMSITE_TARGET_BRANCH %>.value = stringSent;
      if (confirmForm(menu.fileListForm)) menu.fileListForm.submit();
    }
  }


function confirmForm(formSent) {
	if (!isSelectionMade(formSent.<%= TeamSiteBranchMainHandler.TEAMSITE_TARGET_LANGUAGE %>)) {
		alert("<%= bundle.getString("jsmsg_teamsite_branches_target") %>");
		return false;
	}
	return true;
}
...
if (self.name == 'menu') {
	...
	self.document.writeln('<frameset rows="138,*" frameborder="no" border="0" framespacing="0">');
		self.document.writeln('<frame src="/globalsight/envoy/administration/config/teamsite/new2header.jsp" name="header" scrolling="no" noresize marginheight="0" marginwidth="0">');
	    self.document.writeln('<frame src="/globalsight/envoy/administration/config/teamsite/menu2.jsp" name="menu" noresize scrolling="auto" marginheight="0" marginwidth="0">');
