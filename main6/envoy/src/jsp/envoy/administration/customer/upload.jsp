...
function loadUserPreferences()
{
    try
    {
        idPreferences.load('MY_LOCALE_PREFERENCES');

        try
        {
           setSourceLocale(idPreferences.getAttribute('sourceLanguage'));
        }       
        catch (ex)
        {           
        }
        ...
        try
        {
           setTargetLocale(idPreferences.getAttribute('targetLanguage'));
        }       
        catch (ex)
        {           
        }
    ...
    {
        // Preferences may not be available on some machines
    }
}

function setSourceLocale(srcLocale)
{
   var options = customerForm.srcLocales.options;
   for (var i = 0; i < options.length; ++i)
   {
      if (options(i).text == srcLocale)
       {
           customerForm.srcLocales.selectedIndex = i;
           getTargets(i);
           break;
       }
   }
...
function setTargetLocale(targLocale)
{
    var options = customerForm.targLocales.options;
    for (var i = 0; i < options.length; ++i)
    {
       if (targLocale != null && targLocale.indexOf(options(i).text) > -1)
        {
   ...
   try
    {
        ...
        idPreferences.save('MY_LOCALE_PREFERENCES');
    }
    catch (ex)
    {
...
function getTargetLanguage()
{
   ...
   { 
      ...
      {
         selectedLocales[i] = options(i).text;
      }
   }
   return selectedLocales.join(",");
}

function getSourceLanguage()
{
    return customerForm.srcLocales.options(customerForm.srcLocales.selectedIndex).text;
}
...
<table cellspacing="0" cellpadding="4" border="0" class="standardText">
  <tr>
    <td>
        <%=bundle.getString("lb_job_name")%> <span class="asterisk">*</span>:
    </td>
    <td>
        <input type="text" name="jobName" size=30 value="<%=jobName%>" maxlength="150">
        <input type="hidden" name="jobField" size=30 value="<%=jobName%>" maxlength="150">
    </td>
  </tr>
...
%>
  ...
  <tr>
    <td valign=top>
        <%=bundle.getString("lb_job")%> <%=bundle.getString("lb_notes")%>:
    </td>
    <td>
