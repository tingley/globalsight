...
<%
   ...
   int chosenConn = (int)ModDBProfile.getCheckInConnectionProfileId();
   String chosenInsertSQL = (String)ModDBProfile.getCheckInInsertSql();
   String chosenUpdateSQL = (String)ModDBProfile.getCheckInUpdateSql();
   ...
   String subTitle = "Database Profiles: New Profile:";
   String title = bundle.getString("lb_db_write_sql");
   String labelWriteConnection = bundle.getString("lb_final_db_connection");
...
{
	if (needWarning) return confirm("<%= bundle.getString("jsmsg_jump_warning_1") %> <%= bundle.getString("lb_db_profile") %>\n\n<%= bundle.getString("jsmsg_jump_warning_2") %>");
}
...
<TR>
<TD WIDTH=500>
<%=bundle.getString("helper_text_db_edit_write")%>
</TD>
</TR>
</TABLE>
<P>
...
<%= labelWriteConnection %><SPAN CLASS="asterisk">*</SPAN>:
</SPAN>
...
<%
            ...
            if (DBConnectionPairs != null)
            {
               ...
               for (int i=0; i < DBConnectionPairs.size(); i++)
               {
                    ...
                    selected = num.equals(ltmp) ? "SELECTED" : "";
%>
                        <OPTION value ="<%= num %>" <%=selected%>><%= dbcp %></OPTION>                        
<%                    
                }
            }
%>
...
<BR>
<textarea name="finalInsertSQL" cols="70" rows="5" wrap="virtual" CLASS="standardText">
<%
...
<BR>
 <textarea name="finalUpdateSQL" cols="70" rows="5" wrap="virtual" CLASS="standardText"><%
