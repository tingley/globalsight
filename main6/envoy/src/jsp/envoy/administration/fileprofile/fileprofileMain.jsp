<%@page import="com.globalsight.everest.cvsconfig.CVSFileProfile"%><%@ taglib
	uri="/WEB-INF/tlds/globalsight.tld" prefix="amb"%>
	...
	<DIV ID="contentLayer"
		...
		<form name="fpForm" id="fpForm" method="post">
			...
			<table cellpadding=0 cellspacing=0 border=0 class="standardText"
				width="100%" align="left" style="min-width: 1024px;">
				<tr valign="top">
					<td align="right"><amb:tableNav bean="fileprofiles"
							key="<%=FileProfileConstants.FILEPROFILE_KEY%>" pageUrl="self" />
					</td>
				</tr>
				<tr>
					<td><amb:table bean="fileprofiles" id="fp" hasFilter="true"
							key="<%=FileProfileConstants.FILEPROFILE_KEY%>"
							dataClass="com.globalsight.cxe.entity.fileprofile.FileprofileVo"
							pageUrl="self" emptyTableMsg="<%=emptyMsg%>">
							<amb:column label="checkbox" width="2%">
								<input type="checkbox" name="radioBtn" id="<%=fp.getId()%>"
									value="<%=fp.getId()%>,<%=existCVSFPs.contains(String.valueOf(fp.getId())) ? "1" : "0"%>"
									onclick="buttonManagement()">
							</amb:column>
							<amb:column label="lb_name"
								sortBy="<%=FileProfileComparator.NAME%>" filter="uNameFilter"
								filterValue="<%=uNameFilter%>" width="150px">
								<amb:permission name="<%=Permission.FILE_PROFILES_EDIT%>">
									<a href='javascript:void(0)' title='Edit FileProfile'
										onclick="modifyuser('<%=fp.getId()%>')">
								</amb:permission>
								<%=fp.getName()%>
								<amb:permission name="<%=Permission.FILE_PROFILES_EDIT%>">
									</a>
							...
							<amb:column label="lb_description"
								sortBy="<%=FileProfileComparator.DESC%>" width="260px">
								<%
								    out.print(fp.getDescription() == null ? "" : fp.getDescription());
								%>
							</amb:column>
							<amb:column label="lb_loc_profile"
								sortBy="<%=FileProfileComparator.LP%>" filter="uLPFilter"
								filterValue="<%=uLPFilter%>" width="20%">
								<amb:permission name="<%=Permission.LOCPROFILES_EDIT%>">
									<a href='javascript:void(0)' title='Edit L10nProfile'
										onclick="editL10nprofile('<%=fp.getL10nprofileId()%>')">
								</amb:permission>
								<%
							...
							<amb:column label="lb_source_file_format"
								sortBy="<%=FileProfileComparator.FORMATTYPES_NAME%>"
								filter="uSourceFileFormatFilter"
								filterValue="<%=uSourceFileFormatFilter%>" width="15%">
								<%
								    String formatTypesName = fp.getFormatName();
								            out.print(formatTypesName);
								%>
							</amb:column>
							<amb:column label="lb_loc_filter_name"
								sortBy="<%=FileProfileComparator.FILTER_NAME%>"
								filter="uFNFilter" filterValue="<%=uFNFilter%>" width="20%">
								<amb:permission name="<%=Permission.FILTER_CONFIGURATION_VIEW%>">
									<a href='javascript:void(0)' title='Edit FilterConfiguration'
										onclick="editfc(<%=fp.getSpecialFilterId()%>)">
								</amb:permission>
								<%
							...
							<amb:column label="lb_source_file_encoding"
								sortBy="<%=FileProfileComparator.CODE_NAME%>" width="120px">
								<%
								    String codeName = fp.getCodeSet();
								            out.print(codeName);
								%>
							</amb:column>
							<amb:column label="lb_file_extensions"
								sortBy="<%=FileProfileComparator.EXTENSIONS_NAME%>"
								width="120px">
								<%
							...
							<amb:column label="lb_company_name"
								sortBy="<%=FileProfileComparator.ASC_COMPANY%>"
								filter="uCompanyFilter" filterValue="<%=uCompanyFilter%>">
								<%=fp.getCompanyName()%>
							</amb:column>
							<%
							    }
				...
				<td><amb:tableNav bean="fileprofiles"
						key="<%=FileProfileConstants.FILEPROFILE_KEY%>" pageUrl="self"
						scope="10,20,50,All" showTotalCount="false" /></td>
				<TR>
					</DIV>
				<TR>
					<TD>&nbsp;</TD>
