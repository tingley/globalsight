...
%>
<html>
<!-- This JSP is:/envoy/administration/reports/ImplementedCommentsCheckReport.jsp-->
<head>
...
function setDisableTRWrapper(trid)
{
    ...
    {
    	ImplementedChkForm.submitButton.disabled=false;
...
function dataSelectAll()
{
	...
	var endVal=ImplementedChkForm.<%=creationEnd%>.value;
	if(endVal)
	{
		defautSelect();
        return ""; 
	...
	$.ajax({
		...
		success: function(data) {
					if (data != null && data.status == inProgressStatus) {
				    	...
				    	$("form[name='ImplementedChkForm']").submit();
				    }
    			 },
		dataType: 'json'
	});
...
function filterJob2()
{
	ImplementedChkForm.jobNameList.options.length = 0;

    //selected job status
    var currSelectValueJobStatus = new Array();
    for(i = 0; i < ImplementedChkForm.jobStatus.length; i++)
    {
        var op = ImplementedChkForm.jobStatus.options[i];
        if(op.selected)
        {
    ...
    for(i = 0; i < ImplementedChkForm.targetLocalesList.length; i++)
    {
        var op = ImplementedChkForm.targetLocalesList.options[i];
        if(op.selected)
        {
    ...
    for(i = 0; i < ImplementedChkForm.projectNameList.length; i++)
    {
        var op = ImplementedChkForm.projectNameList.options[i];
        if(op.selected)
        {
    ...
    if(ImplementedChkForm.jobNameList.options.length==0)
    {
    	ImplementedChkForm.submitButton.disabled=true;
    }
    else
...
<span class="mainHeading"><B><%=bundle.getString("implemented_comments_check_report_web_form")%></B></span>
<BR><BR>
<TABLE WIDTH="80%">
    <TR><TD><SPAN CLASS="smallText"><%=bundle.getString("optionally_submit_generate")%></SPAN></TD></TR>
</TABLE>
<p/><p/>

<form name="ImplementedChkForm" method="post" action="<%=formAction%>">
<input type="hidden" name="<%=ReportConstants.REPORT_TYPE%>" value="<%=ReportConstants.IMPLEMENTED_COMMENTS_CHECK_REPORT%>">
