<%@ page contentType="text/html; charset=UTF-8" errorPage="/envoy/common/error.jsp"
         import="java.awt.Color,
                 ...
                 com.globalsight.reports.datawrap.CostsByLocaleReportDataWrap,
...
<HTML>
  <HEAD>
	...
	<TITLE><%=bundle.getString("costs_by_locale_report")%></TITLE>
  </HEAD>
  ...
  <BODY bgcolor="white" leftmargin="0" topmargin="0" marginwidth="0" marginheight="0" onload="onAfterAction()">
        ...
        String url = (String)request.getAttribute("image");
          ...
          HashMap map = (HashMap)sessionManager.getReportAttribute(Constants.COSTS_BY_LOCALE_REPORT_KEY);
          CostsByLocaleReportDataWrap reportDataObject = (CostsByLocaleReportDataWrap)map.get(Constants.COSTS_BY_LOCALE_REPORT_KEY+Constants.REPORT_DATA_WRAP);
          ArrayList tableHeadList = (ArrayList)reportDataObject.getTableHeadList();
          ArrayList pageDataList = (ArrayList)request.getAttribute(Constants.COSTS_BY_LOCALE_REPORT_CURRENT_PAGE_LIST);
        %>
  ...
  <DIV ID="divToolBar" aligh=left>
	<table><tr height=40px>
        <td>
		<form name=FirstForm Action="/globalsight/TranswareReports?reportPageName=CostsByLocaleReport&act=turnpage" method=post>
			<input type=hidden name=costsByLocaleReportPageNum value="1">
			<input type=Submit name=First value=<%=bundle.getString("lb_first")%>>
		</form>
	</td>
        <td>
		<form name=PreviousForm Action="/globalsight/TranswareReports?reportPageName=CostsByLocaleReport&act=turnpage" method=post>
			<input type=hidden name=costsByLocaleReportPageNum value=<%=((Integer)request.getAttribute("costsByLocaleReportPageNum")).intValue()-1%> style="width:30px">
			<input type=Submit name=Previous value=<%=bundle.getString("lb_Prev")%>>
		</form>
	</td>
        <td>
		<form name=CustomForm Action="/globalsight/TranswareReports?reportPageName=CostsByLocaleReport&act=turnpage" method=post>
			<input type=text name=costsByLocaleReportPageNum size=4 value=<%=(Integer)request.getAttribute("costsByLocaleReportPageNum")%>>
		</form>
	</td>
        <td>
		<form name=NextForm Action="/globalsight/TranswareReports?reportPageName=CostsByLocaleReport&act=turnpage" method=post>
			<input type=hidden name=costsByLocaleReportPageNum value=<%=((Integer)request.getAttribute("costsByLocaleReportPageNum")).intValue()+1%> style="width:30px">
			<input type=Submit name=Next value=<%=bundle.getString("lb_next")%>>
		</form>
	</td>
        <td>
	 	<form name=LastForm Action="/globalsight/TranswareReports?reportPageName=CostsByLocaleReport&act=turnpage" method=post>
			<input type=hidden name=costsByLocaleReportPageNum value=<%=reportDataObject.getTotalPageNum()%> style="width:30px">
  ...
  <div id="divEntire">
    <TABLE valign="top">
	    ...
	    <TR>
	        <TD>
	          <DIV ID="divArea0_4" align=center>
	            <img src='<%=url %>'>
	          </DIV>
	        </TD>
        </TR>
        ...
        <TD>
	      ...
	      <TABLE id="TmRecordTable" CELLSPACING=0 CELLPADDING=0 WIDTH=629.1>
	      <TR>
	      <%
	        boolean isShowHead = (pageDataList.size() > 0) ? true : false;
	        for(int tableHeadColumn = 0; isShowHead && (tableHeadColumn < tableHeadList.size()); tableHeadColumn++)
	        {
	      %>
            <TD  Style="border-top-style: solid ; border-top-color: #000000;  border-top-width: 1;border-left-style: solid ; border-left-color: #000000;  border-left-width: 1;border-bottom-style: solid ; border-bottom-color: #000000;  border-bottom-width: 1;border-right-style: solid ; border-right-color: #000000;  border-right-width: 1;padding-top: 1;padding-bottom: 1;padding-left: 1;padding-right: 1;" class=size9 BGCOLOR="#0c1476" VALIGN=MIDDLE align=center HEIGHT=30>
		  ...
		  <TR>
		    <%
		      ...
		      {
		        String firstColumnString = rowDataList.get(0).toString();
		        if(!firstColumnString.equalsIgnoreCase("&nbsp"))
		        {
		    %>
		    <TD  Style="border-left-style: solid ; border-left-color: #000000;  border-left-width: 1;border-bottom-style: solid ; border-bottom-color: #000000;  border-bottom-width: 1;border-right-style: solid ; border-right-color: #000000;  border-right-width: 1;padding-top: 1;padding-bottom: 1;padding-left: 1;padding-right: 1;" class=size9 VALIGN=MIDDLE WIDTH="97.2" HEIGHT=30>
			  <span class=left><FONT COLOR="#000000" FACE="Arial"><B><%=(rowDataList.get(colDataIndex)== null ? "&nbsp" : rowDataList.get(colDataIndex))%></B></FONT></span>
			</TD>
		     <%
                        }
                        else
                     ...
                     %>
                     <TD  Style="border-left-style: solid ; border-left-color: #000000;  border-left-width: 1;border-bottom-style: solid ; border-bottom-color: #000000;  border-bottom-width: 1;border-right-style: solid ; border-right-color: #000000;  border-right-width: 1;padding-top: 1;padding-bottom: 1;padding-left: 1;padding-right: 1;" class=size9 VALIGN=MIDDLE WIDTH="97.2" HEIGHT=30>
			  <span class=left><FONT COLOR="#000000" FACE="Arial"><%=(rowDataList.get(colDataIndex)== null ? "&nbsp" : rowDataList.get(colDataIndex))%></FONT></span>
			</TD>
			<%}%>
                    <%}%>
		  </TR>
