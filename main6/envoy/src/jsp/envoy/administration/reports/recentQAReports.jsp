...
%>
<html>
<!-- This JSP is: /envoy/administration/reports/recentQAReports.jsp -->
<head>
<title><%=bundle.getString("lb_recent_qa_reports")%></title>
<style type="text/css">
table td,table td * {
	vertical-align: top;
}
...
$(document).ready(function () {
    ...
    $("#treeDIV").dynatree({
        ...
        selectMode: 3,
        initAjax: {
            url: "/globalsight/ControlServlet?activityName=QAReports&taskId=<%=taskId%>&action=<%=ReportConstants.ACTION_VIEW%>"
        },
...
function fnDownload(){
	...
	recentReportsForm.action = "/globalsight/ControlServlet?activityName=QAReports&taskId=<%=taskId%>&action=<%=ReportConstants.ACTION_DOWNLOAD%>";
...
function fnDelete(){
	...
	$("#selReports").val(selectedReports);
	recentReportsForm.action = "/globalsight/ControlServlet?activityName=QAReports&taskId=<%=taskId%>&action=<%=ReportConstants.ACTION_DELETE%>";
	recentReportsForm.submit();
}
...
<TR><TD><%=bundle.getString("helper_text_recent_qa_reports_instruction")%></TD></TR>
</TABLE>
<br/>
...
<div>&nbsp;</div>
<div align="center">
    <input type="BUTTON" CLASS="standardText" VALUE="<%=bundle.getString("lb_download")%>" onClick="fnDownload();">&nbsp;&nbsp;
    <input type="BUTTON" CLASS="standardText" VALUE="<%=bundle.getString("lb_close")%>" onClick="window.close();">
</div>
<body>
</HTML>
