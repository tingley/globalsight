<%@ page contentType="text/html; charset=UTF-8" errorPage="/envoy/common/error.jsp"
         import="java.sql.*,
                 ...
                 com.globalsight.reports.datawrap.TmReportDataWrap,
...
<HTML>
  <HEAD>
    ...
    <STYLE type="text/css">
      ...
      #divArea0_4 {position: absolute; top: 176px; left: 99px;  width: 628px;  Font-Family: Times; Font-Size: 9pt; z-index: 1}
	...
	<TITLE><%=bundle.getString("tm_report")%></TITLE>
  </HEAD>
  ...
  <BODY bgcolor="white" leftmargin="0" topmargin="0" marginwidth="0" marginheight="0" onload="onAfterAction()">
        <% 
          ...
          HashMap map = (HashMap)sessionManager.getReportAttribute(Constants.TM_REPORT_KEY);
          TmReportDataWrap reportDataObject = (TmReportDataWrap)map.get(Constants.TM_REPORT_KEY+Constants.REPORT_DATA_WRAP);
          ArrayList tableHeadList = (ArrayList)reportDataObject.getTableHeadList();
          ArrayList pageDataList = (ArrayList)request.getAttribute(Constants.TM_REPORT_CURRENT_PAGE_LIST);
        %>
  ...
  <DIV ID="divToolBar" aligh=left>
	<table><tr height=40px>
        <td>
		<form name=FirstForm Action="/globalsight/TranswareReports?reportPageName=TmReport&act=turnpage" method=post>
			<input type=hidden name=tmReportPageNum value="1">
			<input type=Submit name=First value=<%=bundle.getString("lb_first")%>>
		</form>
	</td>
        <td>
		<form name=PreviousForm Action="/globalsight/TranswareReports?reportPageName=TmReport&act=turnpage" method=post>
			<input type=hidden name=tmReportPageNum value=<%=((Integer)request.getAttribute("tmReportPageNum")).intValue()-1%> style="width:30px">
			<input type=Submit name=Previous value=<%=bundle.getString("lb_Prev")%>>
		</form>
	</td>
        <td>
		<form name=CustomForm Action="/globalsight/TranswareReports?reportPageName=TmReport&act=turnpage" method=post>
			<input type=text name=tmReportPageNum size=4 value=<%=(Integer)request.getAttribute("tmReportPageNum")%>>
		</form>
	</td>
        <td>
		<form name=NextForm Action="/globalsight/TranswareReports?reportPageName=TmReport&act=turnpage" method=post>
			<input type=hidden name=tmReportPageNum value=<%=((Integer)request.getAttribute("tmReportPageNum")).intValue()+1%> style="width:30px">
			<input type=Submit name=Next value=<%=bundle.getString("lb_next")%>>
		</form>
	</td>
        <td>
	 	<form name=LastForm Action="/globalsight/TranswareReports?reportPageName=TmReport&act=turnpage" method=post>
			<input type=hidden name=tmReportPageNum value=<%=reportDataObject.getTotalPageNum()%> style="width:30px">
		  ...
		  <TR>
		    ...
		    <TD  Style="border-left-style: solid ; border-left-color: #000000;  border-left-width: 1;border-bottom-style: solid ; border-bottom-color: #000000;  border-bottom-width: 1;border-right-style: solid ; border-right-color: #000000;  border-right-width: 1;padding-top: 1;padding-bottom: 1;padding-left: 1;padding-right: 1;" class=size9 VALIGN=MIDDLE WIDTH="97.2" HEIGHT=30>
		     ...
		     <%
                     ...
                     %>
                     <TD  Style="border-left-style: solid ; border-left-color: #000000;  border-left-width: 1;border-bottom-style: solid ; border-bottom-color: #000000;  border-bottom-width: 1;border-right-style: solid ; border-right-color: #000000;  border-right-width: 1;padding-top: 1;padding-bottom: 1;padding-left: 1;padding-right: 1;" class=size9 VALIGN=MIDDLE WIDTH="97.2" HEIGHT=30>
			  <span class=left><FONT COLOR="#000000" FACE="Arial"><%=rowDataList.get(colDataIndex)%></FONT></span>
			</TD>
			<%}%>
                    <%}%>
		  </TR>
