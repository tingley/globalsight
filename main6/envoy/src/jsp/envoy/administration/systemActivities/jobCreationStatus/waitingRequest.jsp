  ...
  cursor:pointer
	...
	<DIV ID="contentLayer"
			...
			<table cellpadding="0" cellspacing="0" border="0">
				<tr>
					<td class="tableHeadingListOn"><img
						...
						class="sortHREFWhite" href="<%=selfUrl%>"> <%=bundle.getString("lb_job_creation_status")%></a>
					...
					<td class="tableHeadingListOff"><img
						src="/globalsight/images/tab_left_gray.gif" border="0" /> <a
						class="sortHREFWhite" href="<%=loginBlockConfigUrl%>"> <%=bundle.getString("lb_login_block_status")%></a>
						<img src="/globalsight/images/tab_right_gray.gif" border="0" /></td>
				</tr>
			</table>
		</div>
		<div
			...
			<table cellpadding="0" cellspacing="0" border="0">
				<tr>
					<td id="jobWorkflowsTab" class="tableHeadingListOn"><img
						src="/globalsight/images/tab_left_blue.gif" border="0" /> <a
						class="sortHREFWhite" href="<%=selfUrl%>"> <%=bundle.getString("lb_job_create_wait")%></a>
						<img src="/globalsight/images/tab_right_blue.gif" border="0" /></td>
					<td width="2"></td>
					<td id="jobWorkflowsTab" class="tableHeadingListOff"><img
						src="/globalsight/images/tab_left_gray.gif" border="0" /> <a
						class="sortHREFWhite" href="<%=importingUrl%>"> <%=bundle.getString("lb_job_create_import")%></a>
						<img src="/globalsight/images/tab_right_gray.gif" border="0" /></td>
				</tr>
			</table>
		</div>
		<div class="standardText">
			<amb:tableNav bean="waitingRequestDefine"
				key="waitingRequestDefineKey" pageUrl="self" />
			...
			<FORM NAME="MyForm" METHOD="POST">
				<table width="100%">
					<tr>
						...
						<td valign="top" id="contentTd"><amb:table
								bean="waitingRequestDefine" id="requestVo"
								key="waitingRequestDefineKey"
								...
								<amb:column label="lb_company"
									sortBy="<%=RequestFileComparator.Company%>">
		                            ${requestVo.company}
		                        </amb:column>
								<amb:column label="lb_job_id"
									sortBy="<%=RequestFileComparator.JOB_ID%>">
		                            ${requestVo.jobId}
		                        </amb:column>
								<amb:column label="lb_job_name"
									sortBy="<%=RequestFileComparator.JOB_NAME%>">
		                            ${requestVo.jobName}
		                        </amb:column>
								<amb:column label="lb_file"
									sortBy="<%=RequestFileComparator.FILE_NAME%>" width="40%"
									style="text-align: left; word-break: break-all; word-wrap: break-word;">
		                            ${requestVo.file}
		                        </amb:column>
								<amb:column label="lb_size"
									sortBy="<%=RequestFileComparator.FILE_SIZE%>">
		                            ${requestVo.size}
		                        </amb:column>
								<amb:column label="lb_project"
									sortBy="<%=RequestFileComparator.FILE_PROFILE%>">
		                            ${requestVo.project}
		                        </amb:column>
								<amb:column label="lb_file_profile"
									sortBy="<%=RequestFileComparator.FILE_PROFILE%>">
		                            ${requestVo.fileProfile}
		                        </amb:column>
								<amb:column label="jobinfo.jobdetail.jobpriority"
									sortBy="<%=RequestFileComparator.PRIORITY%>">
		                            ${requestVo.priority}
		                        </amb:column>
								<amb:column label="lb_date_request"
									sortBy="<%=RequestFileComparator.REQUEST_TIME%>">
				...
				<amb:tableNav bean="waitingRequestDefine"
					key="waitingRequestDefineKey" pageUrl="self" scope="10,20,50" />
			</div>	
							</td>
					</tr>
				</table>
			...
			<DIV ID="ButtonLayer" ALIGN="LEFT">
				<INPUT TYPE="BUTTON" NAME="Error"
					VALUE="<%=bundle.getString("lb_remove")%>" onClick="cancelFiles();" class="pointer">
			</DIV>
		</DIV>
</BODY>
</HTML>
