...
function showTargetList(){
    try {
        content.target.content.buildData(localData.target, true);
    } catch (ignore) {
    	try{
        content.content.buildData(localData.target, true);
    	}catch(ignore2) {}
    }
}
...
function showSourceList(){
    try {
	    content.source.content.buildData(localData.source, true);
    } catch (ignore) {
    	try{
    	content.content.buildData(localData.source, true);
    	}catch(ignore2) {}
    }
}
