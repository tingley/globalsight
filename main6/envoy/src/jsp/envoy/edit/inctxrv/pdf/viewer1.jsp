...
function load()
{
	<% if(pdfUrl != null)  { %>
	$("#outerContainer").hide();
	var nurl =  "/globalsight/envoy/edit/inctxrv/pdf/viewer1.jsp?file=<%=pdfUrl %>";
...
//2
var pageContent = buildPageContent(loPn, window.parent.parent.parent.localDataForInCtxRv, false);
...
while (pageContent.segments.length == 0 && pageContent.content.length == 0)
{
	...
	if (loPn1 > 0)
	{
		pageContent = buildPageContent(loPn1, window.parent.parent.parent.localDataForInCtxRv, false);
	}
	else
	{
		break;
...
if (divArr && divArr.length > 0)
{
	...
	for(var i = 0; i < divArr.length; i++)
	{
		...
		if (o == divChild)
		{
			...
			var segment = getSegment(false, pageContent, o, i, divArr, divLeft, divWidth, clickX);
