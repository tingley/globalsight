...
{  
	...
	if (el.id)
    {
        ...
        var tempval = "<html dir='" + el.dir + "'>\n" +
            ...
            "  font-size: 10pt; word-wrap: break-word; margin: 1px; background-color: #FFFF99;}\n" +
            "</style></head>" +
       ...
       el.supportsXHTML = (document.getElementById(el.id).contentWindow.document.documentElement) &&
        ...
        el.getHTML = function () {
            ...
            if(el.frameWindow.document)
			{
            	vBody = el.frameWindow.document.body;
			}
			else
			{
				vBody = document.getElementById(el.id).contentWindow.document.body;
			}
            
            removeNoUsedBr(vBody);
        ...
        document.getElementById(el.id).contentWindow.document.onkeydown = function () {
            ...
            if (event.ctrlKey && !event.altKey)
            {
              ...
              else if (key == 66 || key == 73 || key == 85) // "B/I/U"
              {
                if (!el.formattingKeys)
                {
                        event.cancelBubble = true;
                        event.returnValue = false;
                        return false;
            ...
            else if (!event.ctrlKey && event.altKey)
            {
            	...
            	else if (key == 220) // Alt-\
                {
            	  event.cancelBubble = true;
                  event.returnValue = false;
            	  if (parent.save()) parent.openNext(true);
                  return;
                }
            }
            
            // keys with ctrl alt modifier
            else if (event.ctrlKey && event.altKey){
            	if (key == 107) // Alt-Ctrl-Num+
                {
            	  event.cancelBubble = true;
                  event.returnValue = false;
            	  if (parent.save()) parent.openNext(true);
                ...
                else if (key == 33) // Alt-Ctrl-PgUp
                {
                    event.cancelBubble = true;
                    event.returnValue = false;
                    parent.splitSegments();
                }
                else if (key == 34) // Alt-Ctrl-PgDn
                {
                    event.cancelBubble = true;
                    event.returnValue = false;
                    parent.mergeSegments();
                }
            }
        };
...
function removeNoUsedBr(root)
{
	if (root != null)
	{
		var allNodes = root.childNodes;
	    if (allNodes && allNodes.length && allNodes.length > 0)
	    {
	    	for (var i = allNodes.length - 1; i >= 0; i--)
	        {
	        	...
	        	if ("br" == nod.localName)
	    		{
	    			var att = nod.attributes;
	    			if (att)
	        		{
	            		var dirty = att.getNamedItem("_moz_dirty");
...
{
	window.addEventListener("onload", initAllRichEdits, false);
}
