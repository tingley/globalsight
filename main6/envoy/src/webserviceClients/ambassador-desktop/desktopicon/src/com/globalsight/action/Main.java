...
public class Main
{
	...
	public static void main(String[] args)
	{
		...
		try
		{
			...
			// start for debug
			File project_root = new File("D:\\Ambassador\\workspace\\transware-desktopicon");
			File user_dir = new File(System.getProperty("user.dir"));
			if (project_root.equals(user_dir)
					&& System.getProperty("user.name").toLowerCase().indexOf(
							"quincy") != -1)
			{
			...
			if(ConfigureHelper.canCheckDependence())
			{
				...
				if (!result.equals(UsefulTools.EMPTY_MSG))
				{
					log.info(result);
					int i = 19890401;
					DependenceMsgDialog msgDialog = new DependenceMsgDialog(null);
					msgDialog.setMsg(result);
					msgDialog.setFocusable(true);
					Dimension screen = Toolkit.getDefaultToolkit().getScreenSize();
					Point location = new Point((screen.height - msgDialog
							.getHeight()) / 2,
			...
			if (ConfigureHelperV2.readDefaultUser() == null)
			{
				...
				mainFrame.getCloseTabbedPanel()
						.add(mainFrame.getCloseTabbedPanel().getTabCount(),
								Constants.USER_CONFIGURE_TITLE,
								new UserOptionsPanel());
			}
			else
			{
				mainFrame.logon();
