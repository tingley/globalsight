		...
		try
		{
			...
			else if (isWindowsOS())
			{
				if (checkFileSuffix(p_fileName, "txt"))
					exe.execute(new String[] { "notepad", p_fileName });
				else
					exe.execute(new String[] { "cmd", "/c", "start",
							"Opening file...", p_fileName });
			}
			else if (isMacOS())
			{
				exe.execute(new String[] { "open", p_fileName });
			}
			else if (isLinux())
			{
			    if (new File(p_fileName).isDirectory())
			    {
			        exe.execute(new String[] { "nautilus", p_fileName });
			    }
			    else if (p_fileName.startsWith("http") || checkFileSuffix(p_fileName, "html"))
			    {
			        exe.execute(new String[] { "firefox", p_fileName });
			    }
			    else if (checkFileSuffix(p_fileName, "pdf"))
			    {
			        exe.execute(new String[] { "evince", p_fileName });
			    }
                else
                {
                    exe.execute(new String[] { "gedit", p_fileName});
                }
			}
		}
		catch (Exception e1)
