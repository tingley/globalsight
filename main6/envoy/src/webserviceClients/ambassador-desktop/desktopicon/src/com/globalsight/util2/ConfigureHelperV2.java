...
public class ConfigureHelperV2
{
	...
	static
	{
		try
		{
			...
			if (oldDoc == null)
			{
				...
				root.addAttribute(a_version, Constants.APP_VERSION_DOUBLE);
	...
	private static Job elementToJob(Element p_jobElement) throws Exception
	{
		...
		String username = p_jobElement.getParent().getParent().attributeValue(
				a_username);
		...
		j.setCVSJob(new Boolean(p_jobElement.attribute(a_cvsjob).getText()).booleanValue());
		j.setSourceLocale(p_jobElement.attribute(a_cvssourcelocale) == null ? "" : p_jobElement.attribute(a_cvssourcelocale).getText());
		return j;
	}
	...
	{
		writeFormattedXML(p_doc, new File(Constants.CONFIGURE_XML), true, false);
	}
	...
	private static Document readConfigureFile()
	{
		...
		if (m_doc == null || time != m_lasttime)
		{
			...
			{
				log.error("can not read configure file "
