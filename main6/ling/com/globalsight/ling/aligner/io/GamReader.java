...
/**
 * GamReader reads GlobalSight Alignment Map (GAM) file and set
 * the alignment info in AlignmentResult object.
 */

public class GamReader
    implements GamConstants
{
     ...
     *
     * @param p_reader Reader object from which the GAM file is read
     * @param p_sourceTuvs List of source BaseTmTuv objects
     * @param p_targetTuvs List of target BaseTmTuv objects
     * @param p_fileName GAM file name (for error message)
     * @return AlignmentResult object
     */
    public AlignmentResult read(
        Reader p_reader, List p_sourceTuvs, List p_targetTuvs,
        String p_fileName)
        throws Exception
    {
    ...
    // ErrorHandler interface methods
        ...
        public AlignmentResult getAlignmentResult()
        {
            ...
            while(it.hasNext())
            {
                IdAssembler.IdListPair idListPair
                    = (IdAssembler.IdListPair)it.next();
                
                List sourceTuvs
                    = getSourceTuvsByIds(idListPair.getSourceList());
                List targetTuvs
                    = getTargetTuvsByIds(idListPair.getTargetList());
        ...
        private void processGam(Attributes p_attributes)
            throws Exception
        {
        ...
        private void processAlign(Attributes p_attributes)
            throws SAXException
        {
    ...
    /*
     * This class is created in order to support the following <align>
     * elements.
     *
     * <align source="1" target="1" approved="Y" />
     * <align source="1" target="2" approved="Y" />
     * <align source="2" target="3" approved="Y" />
     *
    ...
    private class IdAssembler
    {
        ...
        public List createIdListPairs()
        {
            ...
            for(Iterator itSourceKey = m_sourceToTarget.keySet().iterator();
                itSourceKey.hasNext();
                itSourceKey = m_sourceToTarget.keySet().iterator())
            {
        ...
        public class IdListPair
        {
            ...
            {
                return "soruce list: " + m_sourceList
                    + " target list: " + m_targetList;
            }
