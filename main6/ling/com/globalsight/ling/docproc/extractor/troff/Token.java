...
/**
 * <p>Defines a Token and token IDs which are returned from the troff
 * parsers getNextToken() method.</p>
 */
public class Token
{
    /**
     * ID for an empty token = EOF.
     */
     ...
     */
    public Token(int p_nType, String p_sContent)
    {
        m_nType = p_nType;
    ...
    private Hashtable mapCharToType()
    {
        ...
        h.put(new Character('\u000c'), new Integer(Token.FF));
        h.put(new Character('\n'),     new Integer(Token.LINEBREAK)) ;
        h.put(new Character('\r'),     new Integer(Token.LF));
        h.put(new Character('\t'),     new Integer(Token.TAB));
        h.put(new Character('\u00a0'), new Integer(Token.NBSPACE));
        return h;
    }
}
