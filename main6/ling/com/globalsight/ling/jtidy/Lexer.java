 ...
 *
 * <p>Given a file stream fp it returns a sequence of tokens.
 * GetToken(fp) gets the next token.  UngetToken(fp) provides one
 * level undo.  The tags include an attribute list: linked list of
 * attribute/value nodes - each node has 2 null-terminated strings.
 * Entities are replaced in attribute values white space is compacted
 * if not in preformatted mode. If not in preformatted mode then
 * leading white space is discarded and subsequent white space
 * sequences compacted to single space chars.  If XmlTags is no then
 * Tag names are folded to upper case and attribute names to lower
 * case.
 *
...
public class Lexer
{
    ...
    private static final Lexer.W3CVersionInfo[] W3CVERSION = {
        ...
        new W3CVersionInfo("HTML 4.01 Transitional", "XHTML 1.0 Transitional",
            VOYAGER_LOOSE, Dict.VERS_HTML40_LOOSE),
        new W3CVersionInfo("HTML 4.01 Frameset", "XHTML 1.0 Frameset",
            VOYAGER_FRAMESET, Dict.VERS_FRAMESET),
        new W3CVersionInfo("HTML 4.0", "XHTML 1.0 Strict",
            VOYAGER_STRICT, Dict.VERS_HTML40_STRICT),
        new W3CVersionInfo("HTML 4.0 Transitional", "XHTML 1.0 Transitional",
            VOYAGER_LOOSE, Dict.VERS_HTML40_LOOSE),
        ...
        new W3CVersionInfo("HTML 3.2 Final", "XHTML 1.0 Transitional",
            VOYAGER_LOOSE, Dict.VERS_HTML32),
        new W3CVersionInfo("HTML 3.2 Draft", "XHTML 1.0 Transitional",
            VOYAGER_LOOSE, Dict.VERS_HTML32),
        new W3CVersionInfo("HTML 2.0", "XHTML 1.0 Strict",
            VOYAGER_STRICT, Dict.VERS_HTML20),
        new W3CVersionInfo("HTML 4.01", "XHTML 1.1",
            VOYAGER_STRICT, Dict.VERS_XHTML11)};
    ...
    /**
     * Lexer character buffer parse tree nodes span onto this buffer
     * which contains the concatenated text contents of all of the
     * elements. Lexsize must be reset for each file. Byte buffer of
     * UTF-8 chars.
     */
    ...
    /**
     * Creates a new node and add it to nodelist.
     * @param type node type: Node.ROOT_NODE | Node.DOCTYPE_TAG |
     * Node.COMMENT_TAG | Node.PROC_INS_TAG | Node.TEXT_NODE |
     * Node.START_TAG | Node.END_TAG | Node.START_END_TAG |
     * Node.CDATA_TAG | Node.SECTION_TAG | Node. ASP_TAG |
    ...
    public void parseEntity(short mode)
    {
        ...
        if ("&apos".equals(str) && !configuration.xmlOut && !this.isvoyager &&
            !configuration.xHTML)
        {
        ...
        // drops invalid numeric entities from XML mode. Fix by Pablo
        // Mayrgundter 17-08-2004
        ...
        if (ch <= 0 || (ch >= 256 && c != ';'))
        {
            ...
            if (this.lexsize > start + 1)
            {
                if (ch >= 128 && ch <= 159)
                {
                    ...
                    // "or" DISCARDED_CHAR with the other errors if
                    // discarding char; otherwise default is replacing

                    int replaceMode = c1 != 0 ?
                        Report.REPLACED_CHAR : Report.DISCARDED_CHAR;
                    ...
                    report.encodingError(this,
                        (short) (Report.INVALID_NCR | replaceMode), ch);
    ...
    /**
     * Add meta element for Tidy. If the meta tag is already present,
     * update release date.
    ...
    public boolean addGenerator(Node root)
    {
        ...
        if (head != null)
        {
            String meta = "HTML Tidy for Java (vers. " +
                Report.RELEASE_DATE_STRING + "), adapted for GlobalSight";
    ...
    public short findGivenVersion(Node doctype)
    {
        ...
        if ("SYSTEM ".equalsIgnoreCase(str1))
        {
            ...
            {
                System.arraycopy(TidyUtils.getBytes("SYSTEM"),
                    0, this.lexbuf, doctype.start + 5, 6);
            }
        ...
        if ("PUBLIC ".equalsIgnoreCase(str1))
        {
            ...
            {
                System.arraycopy(TidyUtils.getBytes("PUBLIC "),
                    0, this.lexbuf, doctype.start + 5, 6);
            }
        }
        else
    ...
    public void fixHTMLNameSpace(Node root, String profile)
    {
        ...
        if (node != null)
        {
            ...
            else
            {
                ...
                attr.dict = AttributeTable.getDefaultAttributeTable().
                    findAttribute(attr);
    ...
    /**
     * Put DOCTYPE declaration between the &lt:?xml version "1.0"
     * ... ?&gt; declaration, if any, and the <code>html</code> tag.
     * Should also work for any comments, etc. that may precede the
     * <code>html</code> tag.
     * @param root root node
    ...
    public boolean setXHTMLDocType(Node root)
    {
        ...
        if (this.configuration.docTypeMode == Configuration.DOCTYPE_AUTO)
        {
            ...
            else
            {
                ...
                fpi = null;
                sysid = "";
                // #473490 - fix by Bjšrn Hšhrmann 10 Oct 01
                if (doctype != null)
                {
                    Node.discardElement(doctype);
                }
    ...
    public boolean fixDocType(Node root)
    {
        ...
        if (this.configuration.xmlOut || this.configuration.xmlTags ||
            this.isvoyager)
        {
            if (doctype != null)
            {
    ...
    /**
     * Ensure XML document starts with <code>&lt;?XML version="1.0"?&gt;</code>.
    ...
    public boolean fixXmlDecl(Node root)
    {
        ...
        // We need to insert a check if declared encoding and output
        // encoding mismatch and fix the Xml declaration accordingly!!!
        if (encoding == null &&
            this.configuration.getOutCharEncoding() != Configuration.UTF8)
        {
    ...
    public Node inferredTag(String name)
    {
        ...
        node = newNode(Node.START_TAG, this.lexbuf, this.txtstart,
            this.txtend, name);
        node.implicit = true;
        return node;
    }
    ...
    /**
     * Create a text node for the contents of a CDATA element like
     * style or script which ends with &lt;/foo> for some foo.
    ...
    public Node getCDATA(Node container)
    {
        ...
        while ((c = this.in.readChar()) != StreamIn.END_OF_STREAM)
        {
            ...
            if (qt > 0)
            {
                // #598860 script parsing fails with quote chars.  A
                // quoted string is ended by the quotation character,
                // or end of line.
                if ((c == '\r' || c == '\n' || c == qt) &&
            ...
            else if (c == '>' && start >= 0) // End of begin or end tag
            {
                ...
                if (endtag && ((len = this.lexsize - start) ==
    ...
    public Node getToken(short mode)
    {
        ...
        if (this.pushed)
        {
            // Duplicate inlines in preference to pushed text nodes
            // when appropriate.
            if (this.token.type != Node.TEXT_NODE ||
        ...
        while ((c = this.in.readChar()) != StreamIn.END_OF_STREAM)
        {
            ...
            switch (this.state)
            {
                case LEX_CONTENT :
                    ...
                    // Discard white space if appropriate.  It's
                    // cheaper to do this here rather than in parser
                    // methods for elements that don't have mixed
                    // content.
                ...
                case LEX_GT :
                    ...
                    if (c == '/')
                    {
                        ...
                        if (TidyUtils.isLetter((char) c))
                        {
                            ...
                            if (this.txtend > this.txtstart)
                            {
                                ...
                                this.token = newNode(Node.TEXT_NODE,
                                    this.lexbuf, this.txtstart, this.txtend);
                                return this.token;
                            }
                    ...
                    // look out for comments, doctype or marked
                    // sections this isn't quite right, but its
                    // getting there
                    if (c == '!')
                    {
                        c = this.in.readChar();
                ...
                case LEX_ENDTAG :
                    ...
                    // CvdL: now mask the GS tag as something else.
                    if (this.token.tag == this.configuration.tt.tagGS)
                    {
                        this.token.setType(Node.GS_END_TAG);
                    }
                ...
                case LEX_STARTTAG :
                    ...
                    else if (!this.configuration.xmlTags)
                    {
                        ...
                        if (TidyUtils.toBoolean(
                            this.token.tag.versions & Dict.VERS_PROPRIETARY))
                        {
                            ...
                            if (this.configuration.makeClean &&
                                (this.token.tag != this.configuration.tt.tagNobr &&
                                 ...
                                 this.token.tag != this.configuration.tt.tagGS))
                            {
                    ...
                    if (this.token.tag == this.configuration.tt.tagGS)
                    {
                        this.token.setType(this.token.type == Node.START_TAG ?
                            Node.GS_TAG : Node.GS_START_END_TAG);
                    }
                ...
                case LEX_COMMENT :
                    ...
                    end_comment : while (true)
                    {
                        ...
                        if (c == '>')
                        {
                            ...
                            this.state = LEX_CONTENT;
                            this.waswhite = false;
                            this.token = newNode(Node.COMMENT_TAG,
                                this.lexbuf, this.txtstart, this.txtend);
                ...
                case LEX_DOCTYPE :
                    ...
                    this.state = LEX_CONTENT;
                    this.waswhite = false;
                    this.token = newNode(Node.DOCTYPE_TAG,
                        this.lexbuf, this.txtstart, this.txtend);
                ...
                case LEX_PROCINSTR :
                    ...
                    this.state = LEX_CONTENT;
                    this.waswhite = false;
                    this.token = newNode(Node.PROC_INS_TAG,
                        this.lexbuf, this.txtstart, this.txtend);
                    return this.token;
                ...
                case LEX_ASP :
                    ...
                    this.state = LEX_CONTENT;
                    this.waswhite = false;
                    this.token = newNode(Node.ASP_TAG,
                        this.lexbuf, this.txtstart, this.txtend);
                    return this.token;
                ...
                case LEX_JSTE :
                    ...
                    this.state = LEX_CONTENT;
                    this.waswhite = false;
                    this.token = newNode(Node.JSTE_TAG,
                        this.lexbuf, this.txtstart, this.txtend);
                    return this.token;
                ...
                case LEX_PHP :
                    ...
                    this.state = LEX_CONTENT;
                    this.waswhite = false;
                    this.token = newNode(Node.PHP_TAG,
                        this.lexbuf, this.txtstart, this.txtend);
                    return this.token;
                ...
                case LEX_XMLDECL : // seen "<?xml" so look for "?>"
                    ...
                    this.state = LEX_CONTENT;
                    this.waswhite = false;
                    this.token = newNode(Node.XML_DECL,
                        this.lexbuf, this.txtstart, this.txtend);
                    this.token.attributes = attributes;
                    return this.token;
                ...
                case LEX_SECTION :
                    ...
                    this.state = LEX_CONTENT;
                    this.waswhite = false;
                    this.token = newNode(Node.SECTION_TAG,
                        this.lexbuf, this.txtstart, this.txtend);
                    return this.token;
                ...
                case LEX_CDATA :
                    ...
                    this.state = LEX_CONTENT;
                    this.waswhite = false;
                    this.token = newNode(Node.CDATA_TAG,
                        this.lexbuf, this.txtstart, this.txtend);
                    return this.token;
    ...
    /**
     * Parser for ASP within start tags.  Some people use ASP for to
     * customize attributes.  Tidy isn't really well suited to dealing
     * with ASP.  This is a workaround for attributes, but won't deal
     * with the case where the ASP is used to tailor the attribute
     * value.
     *
     * Here is an example of a work around for using ASP in attribute
     * values: <code>href='<%=rsSchool.Fields("ID").Value%>'</code>
     * where the ASP that generates the attribute value is masked from
     * Tidy by the quotemarks.
     * @return parsed Node
     */
    ...
    public String parseAttribute(boolean[] isempty, Node[] asp, Node[] php)
    {
        ...
        for (;;)
        {
            ...
            // ++len; #427672 - handle attribute names with multibyte
            // chars - fix by Randy Waki - 10 Aug 00
            addCharToLexer(c);
        ...
        // #427672 - handle attribute names with multibyte chars - fix
        // by Randy Waki - 10 Aug 00
    ...
    /**
     * Invoked when &lt; is seen in place of attribute value but
     * terminates on whitespace if not ASP, PHP or Tango this routine
     * recognizes ' and " quoted strings.
     * @return delimiter
     */
     ...
     * @param isempty is attribute empty? Passed as an array reference
     * to allow modification
     * @param pdelim delimiter, passed as an array reference to allow
     * modification
     * @return parsed value
     */
    public String parseValue(String name, boolean foldCase,
        boolean[] isempty, int[] pdelim)
    {
        ...
        // Henry Zrepa reports that some folk are using the embed
        // element with script attributes where newlines are
        ...
        // c should be '=' if there is a value other legal
        // possibilities are white space, '/' and '>'
        ...
        while (true)
        {
            ...
            if (delim == (char) 0)
            {
                ...
                // For cases like <br clear=all/> need to avoid treating />
                // as part of the attribute value, however care is needed
                // to avoid so treating <a href=http://www.acme.com />
                // in this way, which would map the <a> tag to
            ...
            // kludge for JavaScript attribute values with line
            // continuations in string literals
        ...
        if (quotewarning > 10 && seenGt && munge)
        {
            ...
            // javascript URL scheme which may legitimately include <
            // and >, and for attributes starting with "<xml " as
            ...
            if (!AttributeTable.getDefaultAttributeTable().isScript(name) &&
                !(AttributeTable.getDefaultAttributeTable().isUrl(name) &&
                    "javascript:".equals(TidyUtils.getString(
                        this.lexbuf, start, 11))) &&
                !"<xml ".equals(TidyUtils.getString(this.lexbuf, start, 5)))
            {
        ...
        if (len > 0 || delim != 0)
        {
            // ignore leading and trailing white space for all but
            // title, alt, value and prompts attributes unless
            // --literal-attributes is set to yes
            // #994841 - Whitespace is removed from value attributes

            if (munge && !TidyUtils.isInValuesIgnoreCase(
     ...
     * and Unicode characters 161-255, plus dash (-); they cannot
     * start with a dash or a digit; they can also contain escaped
     * characters and any Unicode character as a numeric code (see
     * next item). The backslash followed by at most four hexadecimal
     * digits (0..9A..F) stands for the Unicode character with that
     * number. Any character except a hexadecimal digit can be escaped
     * to remove its special meaning, by putting a backslash in front.
     * @param buf css selector name
     * @return <code>true</code> if the given string is a valid css1
     * selector name
     */
     ...
     * from the istack) One issue arises with pushing inlines when the
    ...
    /**
     * This has the effect of inserting "missing" inline elements
     * around the contents of blocklevel elements such as P, TD, TH,
     * DIV, PRE etc. This procedure is called at the start of
     * ParseBlock. When the inline stack is not empty, as will be the
     * case in:
     *
     * <code>&lt;i>&lt;h1>italic heading&lt;/h1>&lt;/i></code>
     *
     * which is then treated as equivalent to
     *
     * <code>&lt;h1>&lt;i>italic heading&lt;/i>&lt;/h1></code>
     *
     * This is implemented by setting the lexer into a mode where it
     * gets tokens from the inline stack rather than from the input
     * stream.
    ...
    public Node insertedToken()
    {
        ...
        // is this is the "latest" node then update the position,
        ...
        // GLP: Bugfix 126261. Remove when this change is fixed in
        // istack.c in the original Tidy
        node.implicit = true;
    ...
    /**
     * Defer duplicates when entering a table or other element where
     * the inlines shouldn't be duplicated.
     */
     ...
     * Everything is allowed in proprietary versions of HTML.  This is
     * handled here rather than in the tag/attr dicts.
    ...
    private static class W3CVersionInfo
    {
         ...
         */
        public W3CVersionInfo(String name, String voyagerName,
            String profile, short code)
        {
            this.name = name;
