...
/**
 * Used for elements and text nodes element name is null for text
 * nodes start and end are offsets into lexbuf which contains the
 * textual content of all elements in the parse tree. Parent and
 * content allow traversal of the parse tree in any direction.
 * Attributes are represented as a linked list of AttVal nodes which
 * hold the strings for attribute/value pairs.
...
public class Node implements Cloneable
{
     ...
     * node type: gs tag.
     */
    public static final short GS_TAG = 14;
    public static final short GS_END_TAG = 15;
    public static final short GS_START_END_TAG = 16;
     ...
     * @return AttVal instance or null if no attribute with the given
     * name is found
     */
    ...
    /**
     * The same attribute name can't be used more than once in each
     * element. Discard or join attributes according to configuration.
     * @param lexer Lexer
     */
    ...
    /**
     * Insert node into markup tree in pace of element which is moved
     * to become the child of the node.
    ...
    /**
     * This maps <em> hello </em> <strong>world </strong> to <em>
     * hello </em> <strong>world </strong>. If last child of element
     * is a text node then trim trailing white space character moving
     * it to after element's end tag.
     * @param lexer Lexer
     * @param element node
        ...
        if (last != null && last.type == Node.TEXT_NODE)
        {
...

            {
                ...
                if (c == 160 || c == (byte) ' ')
                {
                    ...
                    // fix for [435920]
                    if (c == 160 &&
                        (element.tag == tt.tagTd || element.tag == tt.tagTh))
                    {
     ...
     * <code>&lt;p> hello &lt;em> world &lt;/em></code>.  Trims
     * initial space, by moving it before the start tag, or if this
     * element is the first in parent's content, then by discarding
     * the space.
     * @param lexer Lexer
    ...
    /**
     * Move initial and trailing space out. This routine maps: hello
     * <em> world </em> to hello <em> world </em> and <em> hello </em>
     * <strong>world </strong> to <em> hello </em> <strong>world
     * </strong>.
     * @param lexer Lexer
     * @param element Node
     */
    ...
    /**
     * The doctype has been found after other tags, and needs moving
     * to before the html element.
     * @param lexer Lexer
    ...
    /**
     * Unexpected content in table row is moved to just before the
     * table in accordance with Netscape and IE. This code assumes
     * that node hasn't been inserted into the row.
     * @param row Row node
    ...
    /**
     * If a table row is empty then insert an empty cell. This practice
     * is consistent with browser behavior and avoids potential
     * problems with row spanning cells.
     * @param lexer Lexer
     * @param row row node
     */
    ...
    public static boolean insertMisc(Node element, Node node)
    {
        if (node.type == COMMENT_TAG ||
            ...
            node.type == XML_DECL ||
            node.type == GS_TAG ||
            node.type == GS_START_END_TAG ||
            node.type == GS_END_TAG)
        {
    ...
    /**
     * Is this a new (user defined) node? Used to determine how
     * attributes without values should be printed. This was
    ...
    public Node findHTML(TagTable tt)
    {
        ...
        for (node = this.content; node != null && node.tag != tt.tagHtml;
             node = node.next)
        {
            //
        }
    ...
    /**
     * Add a css class to the node. If a class attribute already
     * exists adds the value to the existing attribute.
    ...
    public void addClass(String classname)
    {
        ...
        // if there already is a class attribute then append class
        // name after a space
        if (classattr != null)
        {
    ...
    public String toString()
    {
        ...
        while (n != null)
        {
            ...
            if (n.type == TEXT_NODE || n.type == COMMENT_TAG ||
                n.type == PROC_INS_TAG)
            {
                s += ",text=";
    ...
    /**
     * Clone this node.
     * @param deep if true deep clone the node (also clones all the
     * contained nodes)
     * @return cloned node
     */
    ...
    public boolean isJavaScript()
    {
        ...
        for (attr = this.attributes; attr != null; attr = attr.next)
        {
            // CvdL: todo: this should match <script language="jscript|
            // javascript|javascript1.x"> or <script type="text/javascript">,
            ...
            if (("language".equalsIgnoreCase(attr.attribute) &&
                ...
                "text/javascript".equalsIgnoreCase(attr.value))
            {
                result = true;
            }
        }
