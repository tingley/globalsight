...
/**
 * Pretty print parse tree. Block-level and unknown elements are
 * printed on new lines and their contents indented 2 spaces Inline
 * elements are printed inline. Inline content is wrapped on spaces
 * (except in attribute values or preformatted text, after start tags
 * and before end tags.
...
public class PPrint
{
    ...
    int cWrapLen(int ind)
    {
        ...
        if ("zh".equals(this.configuration.language))
        {
            ...
            // It would be more accurate to keep a parallel linelen
            // and wraphere incremented by 2 for Chinese characters
    ...
    /**
     * Return one less than the number of bytes used by the UTF-8 byte
     * sequence. The Unicode char is returned in ch.
    ...
    private void printChar(int c, short mode)
    {
        ...
        if (c == ' ' && !TidyUtils.toBoolean(mode &
            (PREFORMATTED | COMMENT | ATTRIBVALUE | CDATA)))
        {
        ...
        // Filters from Word and PowerPoint often use smart quotes
        // resulting in character codes between 128 and 159.

        // Unfortunately, the corresponding HTML 4.0 entities for
        // these are not widely supported.

        // The following converts dashes and quotation marks to the
        // nearest ASCII equivalent.  My thanks to Andrzej Novosiolov
        // for his help with this code.
        ...
        if (this.configuration.getOutCharEncoding() == Configuration.ASCII &&
            (c > 126 || (c < ' ' && c != '\t')))
        {
            if (!this.configuration.numEntities)
            {
     ...
     * routine called to flush the line buffer.
     * @param fout
     * @param mode
     * @param indent
     * @param textarray
     ...
     * @param wrappable
     */
    private void printAttrValue(Out fout, int indent, String value,
        int delim, boolean wrappable)
    {
        int c;
        int[] ci = new int[1];
        ...
        if (value != null)
        {
            ...
            while (i < valueChars.length)
            {
                ...
                if (wrappable && c == ' ' &&
                    indent + linelen < this.configuration.wraplen)
                {
                    wraphere = linelen;
                    wasinstring = inString;
                }

                if (wrappable && wraphere > 0 &&
                    indent + linelen >= this.configuration.wraplen)
                {
    ...
    private void printAttribute(Out fout, int indent, Node node, AttVal attr)
    {
        ...
        if (attr.value == null)
        {
            if (this.configuration.xmlTags || this.configuration.xmlOut)
            {
                printAttrValue(fout, indent,
                    (attr.isBoolAttribute() ? attr.attribute : ""), attr.delim,
                    true);
            }
        ...
        if (attr != null)
        {
            ...
            if (attr.attribute != null)
            {
                ...
                if (!this.configuration.dropProprietaryAttributes ||
                    !(attribute == null ||
                    TidyUtils.toBoolean(attribute.getVersions() & Dict.VERS_PROPRIETARY)))
                {
    ...
    private void printTag(Lexer lexer, Out fout, short mode, int indent, Node node)
    {
        ...
        {
            addC(TidyUtils.foldCase(p.charAt(i), this.configuration.upperCaseTags,
            ...
            !TidyUtils.toBoolean(mode & PREFORMATTED))
        {
            ...
            if (indent + linelen < this.configuration.wraplen)
            {
                ...
                if (!TidyUtils.toBoolean(mode & NOWRAP) &&
                    (!TidyUtils.toBoolean(node.tag.model & Dict.CM_INLINE) ||
                        (node.tag == tt.tagBr)) && afterSpace(node))
                {
                    wraphere = linelen;
                }
    ...
    private void printEndTag(short mode, int indent, Node node)
    {
        ...
        // Netscape ignores SGML standard by not ignoring a line break
        // before </A> or </U> etc.

        // To avoid rendering this as an underlined space, I disable
        // line wrapping before inline end tags
    ...
    private void printPI(Out fout, int indent, Node node)
    {
        ...
        if (node.end <= 0 || node.textarray[node.end - 1] != '?')
        {
            addC('?', linelen++);
        }
    ...
    private void printAsp(Out fout, int indent, Node node)
    {
        ...
        printText(fout, (this.configuration.wrapAsp ? CDATA : COMMENT),
            indent, node.textarray, node.start, node.end);
    ...
    private void printJste(Out fout, int indent, Node node)
    {
        ...
        printText(fout, (this.configuration.wrapJste ? CDATA : COMMENT),
            indent, node.textarray, node.start, node.end);
    ...
    private void printPhp(Out fout, int indent, Node node)
    {
        ...
        printText(fout, (this.configuration.wrapPhp ? CDATA : COMMENT),
            indent, node.textarray, node.start, node.end);
    ...
    /**
     * GS tags are based on XML elements.
     * @param fout
     * @param indent
     * @param node
     */
    private void printGsTag(Out fout, int indent, Node node)
    {
        String p;
        TagTable tt = this.configuration.tt;
        ...
        if (node.type == Node.GS_START_END_TAG)
        {
            addC(' ', linelen++); // Space is NS compatibility hack <br />
            addC('/', linelen++); // Required end tag marker
        }
        ...
        if (node.type == Node.GS_START_END_TAG)
        {
            // The ADD positions and ADDED elements print on single line.
            flushLine(fout, indent);
        }
        else
        {
    ...
    private void printGsEndTag(Out fout, int indent, Node node)
    {
        String p;
    ...
    public void printTree(Out fout, short mode, int indent, Lexer lexer, Node node)
    {
        ...
        else if (node.type == Node.GS_TAG ||
            node.type == Node.GS_START_END_TAG)
        {
            printGsTag(fout, indent, node);
        }
        else if (node.type == Node.GS_END_TAG)
        {
            printGsEndTag(fout, indent, node);
        }
        ...
        else
        {
            ...
            if (node.tag != null && node.tag.getParser() == ParserImpl.PRE)
            {
                ...
                for (content = node.content; content != null;
                     content = content.next)
                {
                    printTree(fout, (short) (mode | PREFORMATTED | NOWRAP),
                ...
                if (this.configuration.makeClean)
                {
                    ...
                    if (node.tag == tt.tagNobr)
                    {
                        for (content = node.content; content != null;
                             content = content.next)
                        {
                            printTree(fout, (short) (mode | NOWRAP),
                                indent, lexer, content);
                        }
                        return;
                    }
            ...
            else
            {
                ...
                if (!this.configuration.hideEndTags ||
                    !(node.tag != null &&
                        TidyUtils.toBoolean(node.tag.model & Dict.CM_OMITST)) ||
    ...
    public void printXMLTree(Out fout, short mode, int indent, Lexer lexer, Node node)
    {
        ...
        if (node.type == Node.TEXT_NODE ||
            (node.type == Node.CDATA_TAG && lexer.configuration.escapeCdata))
        {
            printText(fout, mode, indent, node.textarray, node.start, node.end);
        }
        else if (node.type == Node.COMMENT_TAG)
