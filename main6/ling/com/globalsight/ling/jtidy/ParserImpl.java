...
public final class ParserImpl
{
    ...
    /**
     * Move node to the head, where element is used as starting point
     * in hunt for head. Normally called during parsing.
     * @param lexer
     * @param element
     * @param node
     */
    ...
    {
        /**
         * @see com.globalsight.ling.jtidy.Parser#parse(
         com.globalsight.ling.jtidy.Lexer, com.globalsight.ling.jtidy.Node, short)
         */
        public void parse(Lexer lexer, Node html, short mode)
        {
            ...
            while (true)
            {
                ...
                if (node.type == Node.START_TAG || node.type == Node.START_END_TAG)
                {
                    ...
                    // #427675 - discard illegal frame element
                    // following a frameset - fix by Randy Waki 11 Oct 00
    ...
    public static class ParseHead implements Parser
    {
        ...
        public void parse(Lexer lexer, Node head, short mode)
        {
            ...
            while ((node = lexer.getToken(Lexer.IGNORE_WHITESPACE)) != null)
            {
                ...
                {
                    // #545067 Implicit closing of head broken - warn
                    // only for XHTML input
                    if (lexer.isvoyager)
                    {
                        lexer.report.warning(lexer, head, node,
                            Report.TAG_NOT_ALLOWED_IN);
    ...
    public static class ParseScript implements Parser
    {
        ...
        {
            // CvdL TODO!!! Must parse <script> until </script> or else.
            // Also preserve whitespace and newlines as you go.

            // This isn't quite right for CDATA content as it
            // recognises tags within the content and parses them
            // accordingly. This will unfortunately screw up scripts
            // which include < + letter, < + !, < + ? or < + / +
            // letter.
    ...
    public static class ParseBody implements Parser
    {
        /**
         * @see com.globalsight.ling.jtidy.Parser#parse(
         * com.globalsight.ling.jtidy.Lexer,
         * com.globalsight.ling.jtidy.Node, short)
         */
        public void parse(Lexer lexer, Node body, short mode)
        {
            ...
            while ((node = lexer.getToken(mode)) != null)
            {
                ...
                {
                    if (node.type == Node.START_TAG ||
                        node.type == Node.START_END_TAG || lexer.seenEndHtml)
                    {
                ...
                if (node.type == Node.TEXT_NODE)
                {
                    ...
                    lexer.constrainVersion(
                        ~(Dict.VERS_HTML40_STRICT | Dict.VERS_HTML20));
                ...
                // Netscape allows LI and DD directly in BODY We infer
                // UL or DL respectively and use this boolean to
                // exclude block-level elements so as to match
                // Netscape's observed behaviour.
                    ...
                    || node.tag == tt.tagInput)
                {
                    ...
                    else if ((node.tag.model &
                        (Dict.CM_TABLE | Dict.CM_ROWGRP | Dict.CM_ROW)) != 0)
                    {
                        lexer.ungetToken();
    ...
    public static class ParseFrameSet implements Parser
    {
        ...
        public void parse(Lexer lexer, Node frameset, short mode)
        {
            ...
            while ((node = lexer.getToken(Lexer.IGNORE_WHITESPACE)) != null)
            {
                ...
                if (node.type == Node.START_TAG &&
                    (node.tag.model & Dict.CM_FRAMES) != 0)
                {
                ...
                else if (node.type == Node.START_END_TAG &&
                    (node.tag.model & Dict.CM_FRAMES) != 0)
                {
                    frameset.insertNodeAtEnd(node);
                    continue;
    ...
    public static class ParseInline implements Parser
    {
        ...
        public void parse(Lexer lexer, Node element, short mode)
        {
            ...
            // ParseInline is used for some block level elements like
            // H1 to H6 For such elements we need to insert inline
            // emphasis tags currently on the inline stack. For Inline
            // elements, we normally push them onto the inline stack
            // provided they aren't implicit or OBJECT/APPLET. This
            // test is carried out in PushInline and PopInline, see
            // istack.c We don't push SPAN to replicate current
            // browser behavior
            ...
            {
                // && element.tag != tt.tagSpan #540571 Inconsistent
                // behaviour with span inline element
            ...
            while ((node = lexer.getToken(mode)) != null)
            {
                ...
                if (node.tag == element.tag && node.type == Node.END_TAG)
                {
                    ...
                    // if a font element wraps an anchor and nothing
                    // else then move the font element inside the
                    // anchor since otherwise it won't alter the
                    // anchor text color
                        ...
                        element.content == element.last)
                    {
                        ...
                        if (child.tag == tt.tagA)
                        {
                            ...
                            child.last = element;
                            for (child = element.content; child != null;
                                 child = child.next)
                            {
                ...
                if (node.type == Node.TEXT_NODE)
                {
                    ...
                    if (element.content == null &&
                        !TidyUtils.toBoolean(mode & Lexer.PREFORMATTED))
                    {
                        Node.trimSpaces(lexer, element);
                    }
                ...
                if (node.type == Node.END_TAG)
                {
                    for (parent = element.parent; parent != null;
                ...
                if (!((node.tag.model & Dict.CM_INLINE) != 0))
                {
                    ...
                    // prevent anchors from propagating into block
                    // tags except for headings h1 to h6
        ...
        public void parse(Lexer lexer, Node list, short mode)
        {
            ...
            while ((node = lexer.getToken(Lexer.IGNORE_WHITESPACE)) != null)
            {
                ...
                if (node.type == Node.END_TAG)
                {
                    ...
                    for (parent = list.parent; parent != null;
                         parent = parent.parent)
                    {
                        if (node.tag == parent.tag)
                        {
    ...
    public static class ParseBlock implements Parser
    {
        ...
        public void parse(Lexer lexer, Node element, short mode)
        {
            ...
            // InlineDup() asks the lexer to insert inline emphasis
            // tags currently pushed on the istack, but take care to
            // avoid propagating inline emphasis inside OBJECT or
            // APPLET. For these elements a fresh inline stack context
            // is created and disposed of upon reaching the end of the
            // element. They thus behave like table cells in this
            // respect.
            ...
            while ((node = lexer.getToken(mode)) != null)
            {
                ...
                if (node.tag == tt.tagHtml || node.tag == tt.tagHead ||
                    node.tag == tt.tagBody)
                {
                    if (node.type == Node.START_TAG ||
                        node.type == Node.START_END_TAG)
                ...
                if (node.type == Node.END_TAG)
                {
                    ...
                    {
                        // if this is the end tag for an ancestor
                        // element then infer end tag for this element
                ...
                // Allow Dict.CM_INLINE elements here. Allow
                // Dict.CM_BLOCK elements here unless
                // lexer.excludeBlocks is yes. LI and DD are special
                // cased. Otherwise infer end tag for this element.
                ...
                if (!((node.tag.model & Dict.CM_INLINE) != 0))
                {
                    ...
                    // If an LI contains an illegal FRAME, FRAMESET,
                    // OPTGROUP, or OPTION start tag, discard the
                    // start tag and let the subsequent content get
                    // parsed as content of the enclosing LI. This
                    // seems to mimic IE and Netscape, and avoids an
                    // infinite loop: without this check, ParseBlock
                    // (which is parsing the LI's content) and
                    // ParseList (which is parsing the LI's parent's
                    // content) repeatedly defer to each other to
                    // parse the illegal start tag, each time
                    // inferring a missing </li> or <li> respectively.
                    // NOTE: This check is a bit fragile.  It
                    // specifically checks for the four tags that
                    // happen to weave their way through the current
                    // series of tests performed by ParseBlock and
                    ...
                    {
                        // if parent is a table cell, avoid inferring
                        // the end of the cell
                    ...
                    else
                    {
                        ...
                        // special case where a form start tag occurs
                        // in a tr and is followed by td or th
                ...
                if (node.type == Node.START_TAG || node.type == Node.START_END_TAG)
                {
                    ...
                    {
                        // DSR - 27Apr02 ensure we wrap anchors and
                        ...
                        if (checkstack && !node.implicit)
                        {
                            ...
                            if (!TidyUtils.toBoolean(
                                element.tag.model & Dict.CM_MIXED))
                            {
                                if (lexer.inlineDup(node) > 0)
                                {
                                    continue;
        ...
        public void parse(Lexer lexer, Node table, short mode)
        {
            ...
            while ((node = lexer.getToken(Lexer.IGNORE_WHITESPACE)) != null)
            {
                ...
                if (node.type == Node.END_TAG)
                {
                    ...
                    if ((node.tag != null &&
                        (node.tag.model & (Dict.CM_TABLE | Dict.CM_ROW)) != 0) ||
                        (node.tag != null &&
                         (node.tag.model & (Dict.CM_BLOCK | Dict.CM_INLINE)) != 0))
                    {
                        lexer.report.warning(lexer, table, node,
                    ...
                    for (parent = table.parent; parent != null;
                         parent = parent.parent)
                    {
                        if (node.tag == parent.tag)
                        {
        ...
        public void parse(Lexer lexer, Node colgroup, short mode)
        {
            ...
            while ((node = lexer.getToken(Lexer.IGNORE_WHITESPACE)) != null)
            {
                ...
                if (node.type == Node.END_TAG)
                {
                    ...
                    for (parent = colgroup.parent; parent != null;
                         parent = parent.parent)
                    {
    ...
    public static class ParseRowGroup implements Parser
    {
        ...
        public void parse(Lexer lexer, Node rowgroup, short mode)
        {
            ...
            while ((node = lexer.getToken(Lexer.IGNORE_WHITESPACE)) != null)
            {
                ...
                // if TD or TH then infer <TR> if text or inline or
                // block move before table if head content move to
                // head
                ...
                // if this is the end tag for ancestor element then
                // infer end tag for this element
                ...
                if (node.type == Node.END_TAG)
                {
                    ...
                    if (node.tag == tt.tagTr || node.tag == tt.tagTd ||
                        node.tag == tt.tagTh)
                    {
                        lexer.report.warning(lexer, rowgroup, node,
                            Report.DISCARDING_UNEXPECTED);
                    ...
                    for (parent = rowgroup.parent; parent != null;
                         parent = parent.parent)
                    {
                        if (node.tag == parent.tag)
                        {
        ...
        public void parse(Lexer lexer, Node row, short mode)
        {
            ...
            while ((node = lexer.getToken(Lexer.IGNORE_WHITESPACE)) != null)
            {
                ...
                if (node.type == Node.END_TAG)
                {
                    ...
                    for (parent = row.parent; parent != null;
                         parent = parent.parent)
                    {
                        if (node.tag == parent.tag)
                        {
                ...
                // if text or inline or block move before table if
                // head content move to head
    ...
    public static class ParseNoFrames implements Parser
    {
        ...
        public void parse(Lexer lexer, Node noframes, short mode)
        {
            ...
            while ((node = lexer.getToken(mode)) != null)
            {
                ...
                if (node.type == Node.TEXT_NODE ||
                    (node.tag != null && node.type != Node.END_TAG))
                {
                    if (lexer.seenEndBody)
                    {
    ...
    public static class ParseText implements Parser
    {
        ...
        public void parse(Lexer lexer, Node field, short mode)
        {
            ...
            while ((node = lexer.getToken(mode)) != null)
            {
                ...
                if (node.tag != null &&
                    ((node.tag.model & Dict.CM_INLINE) != 0) &&
                    (node.tag.model & Dict.CM_FIELD) == 0)
                {
                    lexer.report.warning(lexer, field, node,
                        Report.DISCARDING_UNEXPECTED);
                    continue;
    ...
    /**
     * Indicates whether or not whitespace should be preserved for
     * this element. If an <code>xml:space</code> attribute is found,
     * then if the attribute value is <code>preserve</code>, returns
     * <code>true</code>. For any other value, returns
     * <code>false</code>. If an <code>xml:space</code> attribute was
     * <em>not</em> found, then the following element names result in
     * a return value of <code>true: pre, script, style,</code> and
     * <code>xsl:text</code>. Finally, if a <code>TagTable</code> was
     * passed in and the element appears as the "pre" element in the
     * <code>TagTable</code>, then <code>true</code> will be
     * returned. Otherwise, <code>false</code> is returned.
     * @param element The <code>Node</code> to test to see if
    ...
    public static boolean XMLPreserveWhiteSpace(Node element, TagTable tt)
    {
        ...
        for (attr = element.attributes; attr != null; attr = attr.next)
        {
            if (attr.attribute.equals("xml:space"))
            {
                if (attr.value.equals("preserve"))
                {
                    return true;
                }
        ...
        // Debian Bug #137124. Fix based on suggestion by Cesar
        // Eduardo Barros 06 Mar 02
    ...
    public static void parseXMLElement(Lexer lexer, Node element, short mode)
    {
        ...
        // if first child is text then trim initial space and delete
        // text node if it is empty.

        node = element.content;

        if (node != null && node.type == Node.TEXT_NODE &&
            mode != Lexer.PREFORMATTED)
        {
            if (node.textarray[node.start] == (byte) ' ')
            {
        ...
        // if last child is text then trim final space and delete the
        // text node if it is empty
    ...
    public static Node parseXMLDocument(Lexer lexer)
    {
        ...
        {
            lexer.report.warning(lexer, doctype, null,
                Report.DTYPE_NOT_UPPER_CASE);
        }
    ...
    /**
     * Errors in positioning of form start or end tags generally
