...
public final class TagTable
{
    ...
    public static final Dict XML_TAGS =
        new Dict(null, Dict.VERS_ALL, Dict.CM_BLOCK, null, null);
    ...
    private static final Dict[] TAGS = {
        ...
        // GlobalSight addition: GS tags allows local content. Todo:
        // add a GS tag parser that checks the allowed attributes.
        new Dict("gs", Dict.VERS_PROPRIETARY, (Dict.CM_BLOCK), ParserImpl.BLOCK, TagCheckImpl.GS),
    };
     ...
     * gs tag.
     */
    protected Dict tagGS;

    /**
     * a proprietary tag added by Tidy, along with tag_nobr, tag_wbr (and tag_gs).
     */
    ...
    /**
     * Finds a tag by name.
     * @param node Node to find. If the element is found the tag
     * property of node will be set.
    ...
    /**
     * Defines a new tag.
     * @param tagType tag type. Can be TAGTYPE_BLOCK | TAGTYPE_EMPTY |
     * TAGTYPE_PRE | TAGTYPE_INLINE
     * @param name tag name
     */
     ...
     */
    ArrayList findAllDefinedTag(short tagType)
    {
        ArrayList tagNames = new ArrayList();

        Iterator it = tagHashtable.values().iterator();
        while (it.hasNext())
        {
            Dict curDictEntry = (Dict)it.next();
