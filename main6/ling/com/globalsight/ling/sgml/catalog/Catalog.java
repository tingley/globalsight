...
{
    // Wed Mar 09 01:28:02 2005 CvdL. In Windows, serializing file
    // urls to string and constructing new urls from them works only
    // if they contain 3 slashes. On Solaris, there must be 2 or 4.
    // If there are 3, the first directory gets interpreted as the
    // host name.  Vice versa, if Windows sees 2 or 4, it interprets
    // the disk name /:C/ as host name. To each his own then...
    static /*package*/ String FILEPROTOCOL = "file:///";
    static
    {
        try
        {
    ...
    static public String DEFAULT_CATALOG = "catalog";
//    static public String CATALOG_ROOT = ".";
//
//    static
//    {
//        try
...
//            }
//
//            // <file_storage_dir>/catalog/
//            CATALOG_ROOT = root + "catalog";
//
//            new File(CATALOG_ROOT + "/").mkdirs();
//        }
//        catch (Throwable e)
//        {
//            System.err.println("cannot create directory " + CATALOG_ROOT);
//        }
//    }
...
//    }
    ...
    /**
     * <p>Saves a catalog file back to disk</p>
     *
     * @param fileName The filename of the catalog file to write to
     *
     * @throws MalformedURLException The fileName cannot be turned into
     * a valid URL.
     * @throws IOException Error writing catalog file.
     */
    public synchronized void writeCatalog(String fileName)
...
//            String userdir = fixSlashes(CATALOG_ROOT);
            String userdir = fixSlashes(AmbFileStoragePathUtils.getCatalogDir().toString());
            catalogCwd = new URL(FILEPROTOCOL + userdir + "/basename");
        }
...
//            String userdir = fixSlashes(CATALOG_ROOT);
        ...
        catch (MalformedURLException e)
        {
            try
            {
                base = new URL(FILEPROTOCOL + fixSlashes(fileName));
            }
        ...
        debug(1, "Writing catalog", fileName);
        debug(3, "Default BASE", base.toString());

        fileName = base.toString();

        CatalogWriter catfile = new CatalogWriter();
        catfile.writeCatalog(fileName, catalogEntries);
    }
        ...
        throws MalformedURLException, IOException
    {
        ...
        {
            // tack on a basename because URLs point to files not dirs
//            String userdir = fixSlashes(getBaseDirectory());
            String userdir = fixSlashes(AmbFileStoragePathUtils.getCatalogDir().toString());
            catalogCwd = new URL(FILEPROTOCOL + userdir + "/basename");
        }
        catch (MalformedURLException e)
    ...
    /**
     * GlobalSight API: add a new DTD by passing in the PUBLIC
     * identifier and the file name (relative to CATALOG_ROOT).
     */
    public synchronized void addPublicId(String p_public, String p_system)
        throws CatalogException, Exception
    {
        if (resolvePublic(p_public, p_system) != null)
        {
            throw new CatalogException(
                "PUBLIC ID " + p_public + " already exists");
        }

        CatalogEntry entry =
            new CatalogEntry(CatalogEntry.PUBLIC, p_public, p_system);

        addEntry(entry);
    }

    /**
     * Ambassador API: remove a DTD's PUBLIC identifier.
     */
    public synchronized void removePublicId(String p_public)
        throws Exception
    {
        for (int i = 0, max = catalogEntries.size(); i < max; i++)
        {
            CatalogEntry entry = (CatalogEntry)catalogEntries.get(i);

            if (entry.entryType() == CatalogEntry.PUBLIC)
            {
                if (entry.publicId().equals(p_public))
                {
                    // unlink catalog entry
                    catalogEntries.remove(i);
                    return;
                }
            }
        }
    ...
    private void addEntry(CatalogEntry entry)
    {
        ...
        case CatalogEntry.BASE:
        {
            ...
            catch (MalformedURLException e)
            {
                try
                {
                    newbase = new URL(FILEPROTOCOL + value);
                }
                catch (MalformedURLException e2)
                {
        ...
        throws MalformedURLException, IOException
    {
        ...
        for (Enumeration en = catalogEntries.elements();en.hasMoreElements();)
        {
     ...
     * identifier specified for the given system or public
     * identifiers. If no appropriate PUBLIC or SYSTEM entry is found
     * in the Catalog, null is returned.</p>
     *
     ...
     *
     * <p>If a SYSTEM entry exists in the Catalog for the system ID
     * specified, return the mapped value.</p>
     *
        ...
        for (Enumeration en = catalogEntries.elements();en.hasMoreElements();)
        {
            ...
            if (e.entryType() == CatalogEntry.SYSTEM &&
                (e.systemId().equals(systemId) ||
                    (windows && e.systemId().equalsIgnoreCase(systemId))))
            {
                return e.formalSystemIdentifier();
            }
        }
