...
class Scanner
{
    ...
    public Scanner(Reader p_inReader, EntityExpansion p_expander)
    {
        this(p_inReader, false, p_expander);
    }

    public Scanner(Reader p_inReader, boolean p_doTrace,
        EntityExpansion p_expander)
    {
        in = new StreamInfo("", p_inReader);
        atEOF = false;
        trace = p_doTrace;
        expandBuffer = null;
        ...
        throws IOException
    {
        ...
        if (realEntity != null)
        {
            String publicID = realEntity.getPublicID();
            String systemID = realEntity.getSystemID();
            String url = expander.resolveExternalEntity(publicID, systemID);

            Reader entityIn = realEntity.getReader(url);
