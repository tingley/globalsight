 ...
 * of the system. It does leverage, TM population, etc.
 */
public interface TmCoreManager
{
     ...
     */
    TuvMappingHolder populatePageForAllLocales(
        SourcePage p_page, LeverageOptions p_options)
        throws RemoteException, LingManagerException;

    /**
     * Save source and a specified target segments that belong to the
     * page. Segments are saved in Page Tm and Segment Tm.
     *
     * @param p_page source page that has been exported.
     * @param p_options Tm options. It has information which Project
    ...
    /**
     * create LeverageDataCenter with original source segments from a
     * source page
     *
     * @param p_sourcePage SourcePage
     * @param p_leverageOptions LeverageOptions
     ...
     *        True:default
     *        If there is remote tm in tm profile,will leverage from it.
     *        False:
     *        Will not leverage from remote tm however there is remote tm in tm profile.
     *        
     ...
     *
     * @param p_tmId Tm id in which segments are saved
     * @param p_segments Collection of SegmentTmTu objects.
    ...
    public TuvMappingHolder saveToSegmentTm(
            Tm p_tm, Collection p_segments, int p_mode)
        throws RemoteException, LingManagerException;
    
    public TuvMappingHolder saveToSegmentTm(
            Tm p_tm, Collection p_segments, int p_mode, String p_sourceTmName)
            throws RemoteException, LingManagerException, BatchException;
     ...
     *
     * @param p_tmId Tm id in which segments are updated
     * @param p_tuv the SegmentTmTuv object to update.
     *
     * @throws LingManagerException
     */
     ...
     *
     * @param p_tuvs Tuvs (SegmentTmTuv) to be deleted.
     */
     ...
     *
     * @param p_tus Tus (SegmentTmTu) to be deleted.
     */
    public void deleteSegmentTmTus(Tm p_tm, Collection<SegmentTmTu> p_tus)
        throws RemoteException, LingManagerException;

    /**
     * Remove all data for the specified TM, including corpus data and
     * the TM itself.
     * 
     * Interface extracted from com.globalsight.everest.tm.TmRemover.
     * This operation should be completed in the current thread.  The
     * InterruptMonitor may be used to tell if the job should be cancelled.
     * 
     * The implementation should update the provided ProgressReporter with 
     * appropriate status as the operation is performed.
     * 
     * @param p_tm TM to remove data from
     * @param p_reporter process monitor
     * @param p_monitor monitor to check for interrupts
     * 
    ...
    public boolean removeTmData(Tm p_tm, ProgressReporter p_reporter, 
                                InterruptMonitor p_monitor)
        throws RemoteException, LingManagerException;
    
    /**
     * Remove all data in a given locale from the specified TM, including 
     * corpus data. Interface extracted from 
     *      com.globalsight.everest.tm.TmRemover.
     * This operation should be completed in the current thread.  The
     * InterruptMonitor may be used to tell if the job should be cancelled.
     * 
     * @param p_tm TM to remove data from
     * @param p_locale locale to remove data for
     * @param p_interruptMonitor monitor to check for interrupts
     * 
     * @return true if all the data was removed, false if there was an interrupt
     */
     ...
     *  
     *  @param p_tm TM to get statistics for
     *  @param p_uiLocale Locale of the UI.  Used when rendering language names. 
     *  @param p_includeProjects true if project data should be included
    ...
    public StatisticsInfo getTmStatistics(Tm p_tm, Locale p_uiLocale, 
        boolean p_includeProjects) throws RemoteException, LingManagerException;
    
    /**
     * Single-segment leveraging.  This is extracted from
     *      com.global.everest.tm.inprogresstm.InProgressTmManagerLocal
     * It is designed to present 'fallback' leveraging against a gold tm 
     * for the in-progress TM.
     * @param p_tuv tuv to leverage
    ...
    /**
     * Simplified signature to leverage one or more segments against a 
     * set of TMs.
     * @param p_tuvs list of TUVs to leverage
     * @param p_srcLocale
     * @param p_tgtLocales
     * @param p_options
    ...
    /**
     * Return an Iterator that will return all segments in a TM,
     * constrained by date ranges.
     * 
     * NOTE: The date parameters are passed as Strings in DD/MM/YYYY
     * form.  This is due to legacy code in 
     *    com.globalsight.tm.export.ExportOptions
     * It would be nice to fix it.
     *  
     * WARNING: The caller MUST clean up the SegmentResultSet object
     * by calling its finish() method to avoid leaking a Hibernate session
     * (and its underlying DB connection).
     *  
     * @param tm TM to export from
     * @param createdBefore date lower bound (inclusive), in DD/MM/YYYY form 
     * @param createdAfter date upper bounad (inclusive), in DD/MM/YYYY form
    ...
    /**
     * Return the count of the number of segments that would be returned 
     * to a corresponding call to getAllSegments.  
     *
     * NOTE: The date parameters are passed as Strings in DD/MM/YYYY
     * form.  This is due to legacy code in 
     *    com.globalsight.tm.export.ExportOptions
    ...
    /**
     * Return an Iterator that will return all the segments
     * in a TM that have a TUV in the specified locale, constrained by date 
     * ranges.
     * 
     ...
     *  
     * @param tm TM to export from
     * @param locale locale name (ie, "en_US")
     * @param createdBefore date lower bound (inclusive), in DD/MM/YYYY form 
     * @param createdAfter date upper bounad (inclusive), in DD/MM/YYYY form
     * @return Iterator of segments
     */
    public SegmentResultSet  getSegmentsByLocale(Tm tm, 
        String locale, String createdBefore, String createdAfter)
        throws RemoteException, LingManagerException;
    
    /**
     * Return the count of the number of segments that would be returned 
     * to a corresponding call to getSegmentsByLocale.  
     *
     * NOTE: The locale parameter is also a String (ie "en_US").
     * 
     * NOTE: The date parameters are passed as Strings in DD/MM/YYYY
    ...
    /**
     * Return an Iterator that returns all the segments
     * in a TM that were added or modified by the specified project, 
     * constrained by date ranges.
     * 
     * NOTE: The project is also specified by name, again due to legacy
     * behavior in the ExportOptions code.
     * 
     * NOTE: The date parameters are passed as Strings in DD/MM/YYYY
     * form.  This is due to legacy code in 
     *    com.globalsight.tm.export.ExportOptions
    ...
    /**
     * Return the count of the number of segments that would be returned 
     * to a corresponding call to getSegmentsByProjectName.  
     *
     * NOTE: The project is also specified by name, again due to legacy
     * behavior in the ExportOptions code.
     * 
     ...
     * @param sourceLocale
     * @param targetLocale
     * @param tmPriority Order of TMs for purposes of sorting results
    ...
    public List<TMidTUid> tmConcordanceQuery(List<Tm> tms, String query, 
            ...
            Map<Tm, Integer> tmPriority)
            throws RemoteException, LingManagerException;
    
    /**
     * Get a list of all GlobalSightLocales for which there are TUVs in
     * this TM.
     * @param tm tm
    ...
    /**
     * Now for the biggest hack of them all: fetch a Segment TU by ID
     * from a specified TM (also by ID).
     * This code exists primarily for SID support, but the SID implementation
     * is a mess.  Rather than supporting SIDs all the way through the app, 
     * the original author had various high level pieces of code query the 
     * database directly to fetch SIDs at the last minute.  This worked "ok"
     * until TM3 came along, at which point it was a problem.  The code that
     * previously used those hacks now uses -this- hack instead, which will
     * return the TUV regardless of whether TM2 or TM3 is in use.  
     * Note that the OfflinePageData uses similar hacks to load other pieces
     * of non-SID data such as creating user.
     * @deprecated Hopefully we will be able to clean this up someday.   
     */
    public String getSidByTuvId(long tmId, long tuvId)
        throws RemoteException, LingManagerException;
    /**
     * @deprecated
     */
