...
public class LeverageMatchSaver
{
    private static final GlobalSightCategory c_logger =
        (GlobalSightCategory) GlobalSightCategory.getLogger(
            LeverageMatchSaver.class.getName());

    // insert statement
    private static final String INSERT_LEVERAGE_MATCH_NON_CLOB
        ...
        + "matched_tuv_id, matched_table_type, project_tm_index, tm_id, tm_profile_id) VALUES(?, ?, ?, ?, "
        + "?, ?, ?, ?, ?, ?, ?, ?, ?)";
...
//        + "?, ?, ?, ?, ?, ?)";
    ...
    static private final String SELECT_CLOB
        = "SELECT matched_text_clob FROM leverage_match "
        + "WHERE original_source_tuv_id = ? AND sub_id = ? AND "
        + "target_locale_id = ? AND order_num = ? FOR UPDATE";
    
    static private final String SELECT_EXISTING_RECORD
    	= "SELECT original_source_tuv_id FROM leverage_match "
    	+ "WHERE original_source_tuv_id = ? AND sub_id = ? AND "
    ...
    // constructor
    public LeverageMatchSaver(Connection p_connection)
        throws Exception
    {
     ...
     * @param p_leverageDataCenter Repository of matches of a page
     */
...
//        Collection clobMatches = new ArrayList();
        ...
        Iterator itLeverageMatches
            = p_leverageDataCenter.leverageResultIterator();
        while(itLeverageMatches.hasNext())
        {
            ...
            while(itLocales.hasNext())
            {
                ...
                while(itMatch.hasNext())
                {
                    ...
                    int projectTmIndex = Leverager.getProjectTmIndex(leverageOptions, tmId);
                    ...
                    if (segNoTopTag != null && !segNoTopTag.equals(""))
                    {
                        MatchInfo matchInfo = new MatchInfo(
                                ...
                                targetLocale, matchedTuv, projectTmIndex, tmId, tmProfileId);
...
//        saveClobMatches(clobMatches, p_sourcePage);
    ...
    private void saveNonClobMatches(
        ...
        throws Exception
    {
        ...
        try
        {
            ps = m_connection.prepareStatement(
                INSERT_LEVERAGE_MATCH_NON_CLOB);
...
//    }
    ...
    private class MatchInfo
    {
        ...
        MatchInfo(SegmentTmTuv p_originalSourceTuv,
            GlobalSightLocale p_targetLocale, LeveragedTuv p_matchedTuv, int tmIndex, long tmId, long tmProfileId)
        {
        ...
        int getMatchTableType()
        {
            ...
            if(((LeveragedTu)m_matchedTuv.getTu()).getMatchTableType()
                == LeveragedTu.PAGE_TM)
            {
