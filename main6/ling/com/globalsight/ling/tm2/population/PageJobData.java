...
/**
 * A repository of job data for a page. This class holds all source
 * and target job data segments (from translation_unit_variant and
 * translation_unit table) that belong to a page.
 */
...
public class PageJobData
{
     ...
     *
     * @param p_options LeverageOptions object
     */
     ...
     */
    public Collection getCompleteTus()
        throws Exception
    {
    ...
    /**
     * Returns a Collection of Tus in this object that have Tuvs that
     * satisfies the condition specified by the parameters. If
     * p_excludeState is true, Tuvs with the state not the same as
     * p_state are returned. If p_excludeState is false, Tuvs with the
     * same state as p_state are returned.
     *
     * @param p_state state of Tuv
     * @param p_excludeState indicates whether Tuvs returned have
     * p_state or not have p_state
     */
    ...
    private void populateMergedTus()
        throws Exception
    {
        if(m_mergedTus == null)
        {
            ...
            while(it.hasNext())
            {
                ...
                while(itLocale.hasNext())
                {
                    ...
                    if(!mergeState.equals(Tuv.NOT_MERGED))
                    {
                        ...
                        PageTmTuv srcTuv
                            = (PageTmTuv)tu.getFirstTuv(m_sourceLocale);
    ...
    private PageTmTu createMergedTu(List p_tuvs)
        throws Exception
    {
        // get target segment
    ...
    /**
     * This method tries to renumber the sub ids that are not
     * consecutive due to the segment merge. The sub ids can be 1, 2,
     * 101, 102 when two segments are merged. This method renumbers
     * them to 1, 2, 3, 4.
     *
     * This method should be called on matches from Page TM.
     */
    private void renumberSubIds()
        throws Exception
    {
        ...
        try
        {
            ...
            for(Iterator itTu = m_mergedTus.iterator(); itTu.hasNext();)
            {
                ...
                for(Iterator itLocale = tu.getAllTuvLocales().iterator();
                    itLocale.hasNext();)
                {
                    GlobalSightLocale locale
                        = (GlobalSightLocale)itLocale.next();
                
                    for(Iterator itTuv = tu.getTuvList(locale).iterator();
                        itTuv.hasNext();)
                    {
    ...
    private void doRenumberSubIds(
        PageTmTuv p_tuv, GxmlFragmentReader p_gxmlReader)
        throws Exception
    {
        String segment = p_tuv.getSegment();
        GxmlElement gxmlElement = p_gxmlReader.parseFragment(segment);
        List subs = gxmlElement.getDescendantElements(
