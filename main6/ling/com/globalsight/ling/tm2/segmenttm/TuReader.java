 ...
 * retrieve a list of TU ids to read and then use this class to read a
 * subset of TUs into memory. Any data paging should be implemented
 * using the ID list (maye later we implement paging in this class).
 *
 * Caller must call methods in the following sequence:
 *
 * 1) batchReadTus()
 * 2) getNextTu()     repeatedly
 * 3) batchReadDone()
 *
 * The class is reusable in that the above 3 steps can be repeated
 * without allocating a new object.
 *
 * This class should be moved to the ling.tm2 package after that
 * package, and integration, is complete.
 */
public class TuReader
{
    ...
    /**
     * Legacy signature still called from TuQueryResult by the 
     * UI threads that are building tables.  
     */
    @Deprecated
    public TuReader(Connection p_conn) {
        m_conn = p_conn;
        m_session = HibernateUtil.getSession();
    }
    
    public TuReader(Session p_session) {
        m_session = p_session;
        m_conn = m_session.connection();
    }
    ...
    /**
     * Reads TUs and TUVs from the TM. The result sets are kept as
     * member variables of this object while data is read.
     *
     * Caller must call batchReadDone() at the end to free allocated
     * resources.
     *
     * @param p_tuIds is a list of TU ids as Long objects.
     * @param p_min is the start index to retrieve (inclusive)
     * @param p_max is the end index to retrieve (exclusive)
     * @param p_locales is a set of locale ids as Long objects for
     * which TUVs should be retrieved. This argument can be null, or
     * empty, which means all TUVs will be retrieved.
     *
     * @see #batchReadDone()
     */
    public void batchReadTus(List<Long> p_tuIds,
        int p_min, int p_max, ArrayList p_locales)
        throws SQLException
    {
        ...
        m_rsetTus = m_stmtTus.executeQuery(
            "SELECT id, tm_id, format, type, source_locale_id, source_tm_name " +
        ...
        m_rsetTuvs = m_stmtTuvs.executeQuery(
            "SELECT id, tu_id, segment_string, segment_clob, " +
        ...
        throws SQLException, Exception
    {
        ...
        if (m_rsetTus.next())
        {
            result = new SegmentTmTu(
                m_rsetTus.getLong(1),
                m_rsetTus.getLong(2),
                m_rsetTus.getString(3),
                m_rsetTus.getString(4),
                true,                             // translatable
                ExportUtil.getLocaleById(m_session, m_rsetTus.getLong(5)));
            ...
            while (m_rsetTuvs.next())
            {
                ...
                if (tu_id == result.getId())
                {
                    ...
                    tuv.setLocale(ExportUtil.getLocaleById(m_session,
                        m_rsetTuvs.getLong(6)));
