...
class BilingualTm<T extends TM3Data> extends BaseTm<T> 
                                    implements TM3BilingualTm<T> {
    ...
    BilingualTm(TM3DataFactory<T> factory, 
    ...
    private TM3Locale loadLocale(long id) {
        ...
        return getDataFactory().getLocaleById(getSession(), id);
    }
    ...
    public TM3Tu<T> save(TM3Locale srcLocale, T source, 
        ...
        if (!this.getSrcLocale().equals(srcLocale)) {
            throw new IllegalArgumentException("Bilingual TM expected srcLocale=" + 
                    this.getSrcLocale() + ", found=" + srcLocale);
        }
        if (!this.getTgtLocale().equals(tgtLocale)) {
            throw new IllegalArgumentException("Bilingual TM expected tgtLocale=" + 
                    this.getTgtLocale() + ", found=" + tgtLocale);
        }
    ...
    public TM3LeverageResults<T> findMatches(T matchKey,
        ...
        return super.findMatches(matchKey, getSrcLocale(), 
                Collections.singleton(getTgtLocale()), attributes, matchType, lookupTarget);
    }
    ...
    @Override
    public TM3Tu<T> save(T source, Map<TM3Attribute, Object> attributes, T target,
            TM3SaveMode mode, TM3Event event) {
