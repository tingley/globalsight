...
public abstract class TM3Command {
    ...
    protected abstract void handle(Session session, CommandLine command) 
                    throws Exception;
    ...
    /**
     * Commit the new transaction and start a new one.
     * Note that the transaction will be committed automatically
     * as part of normal behavior.  This method is only necessary
     * if the command needs to span multiple transactions.
     * @return newly opened transaction
     */
    protected Transaction commitAndRestartTransaction() {
        transaction.commit();
        transaction = session.beginTransaction();
        return transaction;
    }
    ...
    static final Option PROPERTIES_OPT = OptionBuilder
        ...
        .hasArg()
        .withDescription("database properties file")
        .create(PROPERTIES);
    static final String DEBUG = "debug";
    static final Option DEBUG_OPT = OptionBuilder
        .withDescription("enable sql debugging")
        .create(DEBUG);
    ...
    protected SessionFactory getSessionFactory(CommandLine command) {
        ...
        if (command.hasOption(PROPERTIES)) {
            loadPropertiesFromFile(properties, command.getOptionValue(PROPERTIES), 
                                   true);
        }
        else {
        ...
        if (username == null || password == null || connStr == null) {
            usage("Must specify " + TM3_USER_PROPERTY + ", " +
                  TM3_PASSWORD_PROPERTY + ", and " + TM3_CONNECTION_PROPERTY +
        ...
        hibernateProps.put("hibernate.cglib.use_reflection_optimizer", "false"); // this is default in hibernate 3.2
    ...
    private ClassLoader getClassLoader(Properties props) {
        ...
        try {
            ...
            for (String cp : cps) {
                File f = new File(cp);
                if (!f.exists()) {
                    System.err.println("Skipping non-existent classpath element " + f);
                    continue;
                }
