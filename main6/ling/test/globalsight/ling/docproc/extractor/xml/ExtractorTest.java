...
public class ExtractorTest
{
        ...
        throws ExtractorException, FileNotFoundException, IOException
    {
        if(args.length < 3)
        {
            System.err.println("USAGE: ExtractTest xml_file rule_file encoding");
            System.exit(1);
        }
        
        EFInputData input = new EFInputData();
        input.setCodeset(args[2]);
        java.util.Locale locale = new java.util.Locale("en", "US");
        input.setLocale(locale);
        input.setURL("file:" + args[0]);
        input.setRules(readRuleFile(args[1]));
        ...
        writer.write(DiplomatWriter.WriteXML(output));
        writer.flush();
        writer.close();
        
    }

    private static String readRuleFile(String fileName)
