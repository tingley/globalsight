...
public class LeverageMatchLingManagerLocalTest
{
    ...
    public void testGetNonClobMatches()
    {
        ...
        try {
            ...
            ps = session.connection().prepareStatement(MAX_ORDER_NUM);            
        ...
        GlobalSightLocale targetLocale = new GlobalSightLocale("zh", "CN", false);
        
        // levMatches :: sourceTuv
        String text = "This is my book.";
        long tuId = 1000;
        String type = "text";
        BaseTmTuv sourceTuv = TmUtil.createTmSegment(text, tuId, sourceLocale, type, true);
        sourceTuv.setId(1001);
        assertEquals("Segment is different!", "<segment>This is my book.</segment>", sourceTuv.getSegment());
        ...
        tmProfile.setMultipleExactMatches(TranslationMemoryProfile.LATEST_EXACT_MATCH);
        ...
        LeverageOptions leverageOptions = new LeverageOptions( tmProfile, levLocales);
        // levMatches
        LeverageMatches levMatches = new LeverageMatches(sourceTuv, leverageOptions);

        // Create an in progress match 
        long srcId = 1003;
        long jobId = 1004; // src.job_id
        String srcType = "text"; // src.type
        LeveragedInProgressTu ipMatchTu = new LeveragedInProgressTu(srcId, jobId,
                srcType, true, sourceLocale);
...
//        ipMatchTu.addTuv(srcIpMatchTuv);
        ...
        LeveragedInProgressTuv trgIpMatchTuv = new LeveragedInProgressTuv(trgId, trgSegment, targetLocale);
        ...
        LeveragedSegmentTu staticMatchTu = new LeveragedSegmentTu(1007, 1, "html", "text", true, sourceLocale);
        ...
        LeveragedSegmentTuv ltuv = new LeveragedSegmentTuv(1008, segmentData, targetLocale);
        ...
        Collection<LeverageMatch> results = (Collection) ClassUtil.testMethod(lmlm,
                "getNonClobMatches", ps, levMatches, targetLocale, leverageOptions,
                sourcePageId, null);
        ...
        for (LeverageMatch lm : results)
        {
            ...
            if (projectTmIndex == Leverager.IN_PROGRESS_TM_PRIORITY)
            {
                ...
                assertEquals("MatchType error!", "IN_PROGRESS_TM_EXACT_MATCH", lm.getMatchType());
                assertTrue("TmId error", lm.getTmId()==1004);
                assertTrue("MatchedTableType error", lm.getMatchedTableType()==5);
            ...
            else if (projectTmIndex == Leverager.HIGHEST_PRIORTIY)
            {
                ...
                assertEquals("MatchType error!", "SEGMENT_TM_EXACT_MATCH", lm.getMatchType());
                assertTrue("TmId error", lm.getTmId()==1);
                assertTrue("MatchedTableType error", lm.getMatchedTableType()==1);
            }
        }
