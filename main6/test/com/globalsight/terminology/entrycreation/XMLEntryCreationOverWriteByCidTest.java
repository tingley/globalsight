 ...
 * "Synchronize on concept id" --> "Overwrite existing concepts"
 * 
 * @see EntryCreationOverWriteByCid
 */
public class XMLEntryCreationOverWriteByCidTest extends EntryCreationBasic
{
    @Test
    public void testGetTbConceptByEntry()
    {
        ...
        TbConcept tbc = ((EntryCreationOverWriteByCid) m_instance).getTbConceptByEntry(m_tbid, newEntry, m_session);
    ...
    {        
        setFileType();
        m_instance = new EntryCreationOverWriteByCid(m_fileType);
        m_instance.setFileType(m_fileType);
        
        m_options = new SyncOptions();
        m_options.setSyncMode(SyncOptions.SYNC_BY_CONCEPTID);
