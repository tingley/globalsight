/*
The following code is the core of \fIalign\fR.
It is a C language program which takes as input
texts in two files with two embedded special markers, one for ``hard'' regions
(such as paragraphs), and one for ``soft'' regions (such as sentences).
The hard regions may not be changed, and there must be equal numbers of 
them in the two languages, although there may be just one hard region.
The soft regions may be deleted or inserted (1-0 and 0-1), 
substituted (1-1), contracted or expanded (2-1 and 1-2),
or merged (2-2).
The output is two files with equal numbers of soft regions, each on a line,
with the hard region markers separating groups of lines.
If the -v command line option is included, each soft region is preceded by its probability score.
*/
     ...
     char **argv;
{
  ...
  for( ; hard_regions1 < hard_end1 ; hard_regions1++, hard_regions2++) {
    ...
    n = seq_align(len1, len2, number_of_soft_regions1, number_of_soft_regions2, two_side_distance, &align);
    ...
    for(i = 0; i < n; i++) {
      ...
      if(debug) {fprintf(out1,"n=%d i=%d x1=%d y1=%d x2=%d y2=%d\n",n,i,a->x1,a->y1,a->x2,a->y2);
		 fprintf(out2,"n=%d i=%d x1=%d y1=%d x2=%d y2=%d\n",n,i,a->x1,a->y1,a->x2,a->y2);}
