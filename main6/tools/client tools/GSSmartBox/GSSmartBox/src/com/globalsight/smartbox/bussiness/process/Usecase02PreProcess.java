...
public class Usecase02PreProcess implements PreProcess
{
    ...
    public JobInfo process(String originFilePath, CompanyConfiguration cpConfig)
    {
        ...
        {
            LogUtil.FAILEDLOG
                    .error("File format error: should be csv or txt file");
        ...
        String fileProfileId = determineFileProfileId(sourceFile, sourceLocale,
                cpConfig.getExtension2fp());
     ...
     * @return
     */
    private String determineFileProfileId(String sourceFile,
            String sourceLocale, Map<String, String> extension2fp)
    {
        ...
        if (mapFPName == null)
        {
            String message = "No file profile config for this extension("
                    + extension + ") in GSSmartBox.conf : " + sourceFile;
        ...
        for (FileProfile fp : fileProfileInfo)
        {
            if (fp.getName().equals(mapFPName))
            {
                ...
                {
                    String message = "The file profile(" + mapFPName
                            + ") does not have the file extension(" + extension
                            + ") in GlobalSight Server: " + sourceFile;
                    LogUtil.FAILEDLOG.error(message);
                    return null;
                }
                fpId = fp.getId();
    ...
    private String convertToXML(String format, File originFile)
    {
        ...
        try
        {
            ...
            if ("csv".equals(format))
            {
                while ((str = br.readLine()) != null)
                {
                    ...
                    values[10] = values[10].substring(0,
                            values[10].length() - 1);
        ...
        {
            String message = "Failed to convert to XML, File Name: "
                    + originFile.getName();
            LogUtil.fail(message, e);
            return null;
        }
