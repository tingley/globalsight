...
public class GitUtil
{
    ...
    private static String FROM_DATE = null;
    ...
    private static String OLD_GLOBALSIGHT_JAR = BuildUtil.ROOT
            + "/main6/tools/build/capclasses/globalsight.ear/lib/globalsight.jar";
    ...
    private static Set<String> getChangedFiles() throws Exception
    {
        ...
        if (Main.isInLinux())
        {
            ...
            // "./script/linux/changedfiles.sh"), "UTF-8"));
            // out.write("cd $1\n");
            // out.write("git log --since='" + getFromDate() +
            // "' --name-only --pretty=format:'' > changedfiles.log");
            //
            // out.flush();
            // out.close();

            String[] cmd =
            { "sh", "./script/linux/changedfiles.sh", BuildUtil.ROOT,
                    getFromTag() };

            CmdUtil.run(cmd);

            File f = new File(PATH + "/changedfiles.log");
            ...
            while (!f.exists())
            {
                ...
                if (i > 120)
                {
                    throw new Exception("Can not find changedfiles.log");
                }
            }
        }
        else
        ...
        File file = new File(PATH + "/changedfiles.log");
    ...
    private static String getFromTag() throws Exception
    {
        if (FROM_DATE == null)
        {
            String previousVersion = PropertyUtil.get(new File(
                    "release.properties"), "previousVersion");
            String tag = PropertyUtil.get(new File("releaseTag.properties"),
                    previousVersion);

            if (tag == null)
                throw new Exception("Can not get the release tag of version: "
                        + previousVersion
                        + ", please add it to releaseTag.properties");
    ...
    private static void copyClass(Set<String> files)
    {
        if (!BuildUtil.VERSION.endsWith(".0")
                && !"8.1.1".equals(BuildUtil.VERSION))
        {
            for (String f : files)
            {
                copyClass(f);
    ...
    private static void copySsoJars(Set<String> files)
    {
        for (String f : files)
        {
            ...
            try
            {
                FileUtil.copyFile(src, new File(NEW_EAR_PATH + "/lib/sso/"
                        + realPath));
                log("Add: " + realPath);
            }
            catch (Exception e)
    ...
    private static void copyCommonJars(Set<String> files)
    {
        for (String f : files)
        {
            ...
            try
            {
                FileUtil.copyFile(src, new File(NEW_EAR_PATH + "/lib/"
                        + realPath));
                log("Add: " + realPath);
            }
            catch (Exception e)
