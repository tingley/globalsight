...
public class JBossUpdaterWildfly extends JBossUpdater
{
    ...
    protected boolean isUpdate()
    {
        if (isJbossWildfly == null)
        {
            File f = new File(ServerUtil.getPath()
                    + "/jboss/server/modules/system/layers/base/org/wildfly/security");
            File f2 = new File(UpgradeUtil.newInstance().getPath()
                    + "/jboss/server/modules/system/layers/base/org/wildfly/security");
    ...
    private void backupProperties() throws Exception
    {
        ...
        List<File> pfiles = FileUtil.getAllFiles(pRoot, new FileFilter()
        {
            ...
            public boolean accept(File pathname)
            {
                ...
                if (path.endsWith("/AdobeAdapter.properties")
                        || path.endsWith("/IdmlAdapter.properties"))
                {
    ...
    private void backupJVMSettings()
    {
        backupJVMSetting(STANDALONE_CONF_BAT_PATH, STANDALONE_CONF_BAT_PATH);
        backupJVMSetting(STANDALONE_CONF_PATH, STANDALONE_CONF_PATH);
    }

    private void backupJVMSetting(String p1, String p2)
    {
        String path = ServerUtil.getPath() + p1;
        String newPath = Upgrade.UPGRADE_UTIL.getPath() + p2;
