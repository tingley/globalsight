...
public class DiplomatBasicParser
{
    ...
    public void parse(String p_tmx) throws DiplomatBasicParserException
    {
        ...
        try
        {
            ...
            for (i = 0; i < m_maxLen; i++)
            {
                ...
                else if (ch == '<')
                {
                    ...
                    if (m_start < m_end)
                    {
                        ...
                        if (isInPh)
                        {
                            if (m_inSub && !isMifPlaceholder(subString))
                            {
            ...
            if (i > m_start)
            {
                if (bInTag)
                {
                    throw new DiplomatBasicParserException("Invalid GXML string `" + p_tmx + "'");
                }
                else
                {
                    m_end = m_maxLen;
        ...
        {
            throw new DiplomatBasicParserException("Error parsing GXML: " + e.toString());
        }
     ...
     * including &lt; and &gt;.
     */
    private void processTag(String p_tmx, int p_min, int p_max) throws IndexOutOfBoundsException,
            DiplomatBasicParserException
    {
        ...
        else if (bEmptyTag)
        {
            ...
            m_handler.handleStartTag(tag, m_attributes, p_tmx.substring(p_min, i - 1) + ">");

            m_handler.handleEndTag(tag, "</" + tag + ">");
        }
        else
        {
            m_handler.handleStartTag(tag, m_attributes, p_tmx.substring(p_min, p_max));
