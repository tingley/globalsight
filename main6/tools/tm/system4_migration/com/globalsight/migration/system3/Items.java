    ...
    extends DataTable
{
    ...
    private static final String SOURCE_QUERY
        = "SELECT ITEMNUM, DFID, ITEMTYPE, DATATYPE, "
        + "TEXT FROM ITEMS WHERE MAXFLAG = 'Y' AND LANGID = ?";
    // STATUS = 1 means uptodate
    private static final String TARGET_QUERY
        = "SELECT TEXT, LANGID FROM ITEMS WHERE MAXFLAG = 'Y' AND "
        + "STATUS = 1 AND LANGID <> ? AND ITEMNUM = ? AND DFID = ?";
    private static final String SUBFLOW_QUERY
        = "SELECT TEXT FROM ITEMS WHERE MAXFLAG = 'Y' AND "
        ...
        throws Exception
    {
        ...
        segment.setTranslatable(TRANSLATABLE_TYPES
                                .contains(segment.getItemType())
                                ? true : false);
        ...
        throws Exception
    {
        PreparedStatement statement 
            = m_connection.prepareStatement(SUBFLOW_QUERY);
    ...
    private void addTargetSegments(System3Segment p_segment,
                                   int p_itemnum, int p_dfid)
        throws Exception
    {
        PreparedStatement statement 
            = m_connection.prepareStatement(TARGET_QUERY);
